...
(an excerpt from a legacy model — the file now lives as a fixture in
[`flang/test/fixtures/fts/examples/utilities/discount.fts`](flang/test/fixtures/fts/examples/utilities/discount.fts)):
...
emitted code. Printing that model to Python used to produce, verbatim:
...
the older project's TypeScript core; the project left the repository, and the bridge lost its other
side. The refusal is explicit and says where the removed part now lives:
...
```bash
npm test              # the whole language: parser, types, totality, eight backends, self-hosting
```
...
[Описание языка](docs/overview.ru.md) · [Раскладка проекта](docs/project-layout.ru.md) ·
