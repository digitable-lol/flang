...
есть 95 программ с 511 функциями и 1291 примерами. Для каждой функции строится сетка входов из
...
только программист (фрагмент модели наследия — файл лежит фикстурой в
...
этой модели в Python давала, дословно:
...
```bash
npm test              # весь язык: парсер, типы, тотальность, восемь целей печати, самоприменение
```
...
- Сторож с постоянного шага СНИМАЕТСЯ, если параметр объявлен точным натуральным (`нат` — целое
  ...
  на корпусе: 16 функций несли обещание постоянным шагом со сторожем, осталось 2; мест сторожа
  100 вместо 115, добавлено НОЛЬ — переполнение ловится расширением типа (`нат плюс нат` это
  `число`), а не проверкой в напечатанном коде. Образец —
...
на ходу — в эталоне и в цели C, но не на BEAM, — и имя порождённому даёт родитель, потому что описанное действие
