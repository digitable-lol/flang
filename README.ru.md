...
# → доля-проигрыванием = 517 / 665 = 77.74 %      (8 сентября 2026, это дерево)
# → порог Г4 = 95 %; добрала ли доля порога: НЕТ
sh scripts/доказуемость.sh          # → НЕ ДОКАЗУЕМ, код 1
```

Дробь читается как дробь. Числитель 517 — то, что чекер на C установил, не спрашивая ядро: 390
обязательств проиграны по записанным ходам, 93 пересчитаны на месте там, где запись говорит «по
примеру» или «по свойству», и 34 узла завершаемости пройдены заново структурно. Знаменатель 665 —
все обязательства этого набора записей, поэтому недостающие 148 и есть честная цена: 109 посылок и утверждений
и 17 шагов чекер берёт на слово ядра, 8 сняты вычислением тождества, а не игрой, и 14 принадлежат
той одной записи из 86, которую чекер отверг целиком. Потолок задаёт знаменатель: в нём сидят
подделки, которые набор держит нарочно, а подделка проиграться и не должна, — поэтому 100 % тут
не цель, порог гейта Г4 — 95 %.

Вторая команда — вердикт одним словом: сегодня язык формально **не** доказуем, и код возврата 1.
Причина не в доле. В чекере ещё стоит шаг, который доказывает равенство вычислением, а не
проигрыванием записанных шагов (`перепиской` встречается в `сверщик.c` дважды), и
`scripts/доказуемость.sh` останавливается на нём одном: пока чекер умеет считать, доля, которую он
называет, — не независимая мера, и остальное меряется впустую. Оба числа печатает прогон, и они
будут двигаться; команды — способ снять их заново. Подробный разбор — на сайте: [Что доказано, а
что нет](https://digitable-lol.github.io/flang/what-is-proved.html) — и в
[`docs/what-blocks-1-0.md`](docs/what-blocks-1-0.md).

То, что компилятор подаёт чекеру, выросло 8 сентября 2026: ядро теперь печатает ходы своего
доказательства для двух семейств вместо голого вывода — «тождество после переписки», 18 мест, и
«разбор цели по условию», 33 места, — а чекер их проигрывает. Оттуда и взялась бо́льшая часть тех
108 обязательств, на которые прибавился числитель.
...
[`digitable-lol/asdf-flang`](https://github.com/digitable-lol/asdf-flang). Оба опубликованных
репозитория хранятся здесь сабмодулями — `packaging/homebrew-tap` и `packaging/asdf-plugin`, — и
каждый сверяется с источником в этом дереве перед каждым выпуском. Оба ставят релизный
...
| `flang lock`, `flang package`, `flang new` | замок, несущий сами зависимости; пакет с именем, версией и манифестом — `package` сегодня не собирает ни одного, отвечает `FLANG_TYPE` и кодом 1 (задача 3401, открыта); новый пакет с нуля |
...
`./ярлык задачник:доска` печатает доску. Задача может кончиться и не сделанной: в
`tasks/rejected/` лежат те, что рассмотрели и отклонили, — чтобы причина пережила решение. Правила дерева, которых из кода не видно, — что ломается
