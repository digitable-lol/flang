...
```bash
tar -xzf flang-*-c.tar.gz   # внутри — исходники на C99 и Makefile, больше ничего
make                        # cc -std=c99 -Wall -Wextra -Werror -pedantic -O2
./flang_cli                 # JSON на входе, JSON на выходе, по запросу на строку
```
...
парсер, типы, завершаемость, печать в C. Вычислителя среди них нет, поэтому запустить программу,
прогнать её примеры или открыть оболочку можно только полным инструментарием — он ниже.

**Полному инструментарию Node.js 20 или новее нужен**, и вот почему именно: интерпретатор,
оболочка, языковой сервер, сервер MCP и семь бэкендов из восьми существуют только на JavaScript.
...
известны до запуска. С функциями первого класса такой проверки не бывает — композиция была бы
вычислением, а равенство двух вычислений неразрешимо.
...
устроить. Не хватало языку слова, называющего одновременность. Функции не являются значениями
первого класса, поэтому породить процесс от функции нельзя — и **процесс здесь объявление**: имя,
тип состояния, начальная функция и обработчик.
...
которых **сделаны первый и второй**: поверхность, проверки и планировщик эталона. Надзор
разбирается и проверяется, но стратегий планировщик не применяет — упавший процесс остаётся
остановленным. Процессы не печатаются ни в одну цель: печать такой программы даёт обработчики
как обычные функции и больше ничего. Примеры лежат в [`flang/conc/examples/`](flang/conc/examples).
...
шесть файлов, — то есть компилятор понимает язык так же, как эталон, и никакой набор тестов этого
...
- Функции не являются значениями первого класса. Это осознанный выбор: без экспоненциалов язык
  по-прежнему печатается в цели, где замыканий нет вовсе, а анализ завершаемости остаётся
  разрешимым. Всё, ради чего обычно берут высший порядок, закрывают `отобразить` /
  `отфильтровать` / `свёртка` — они принимают *тело*, а не функцию. Цена — экспоненциальные
  объекты невыразимы, а вместе с ними часть категорных конструкций.
- Эффекты описываются, а не выполняются. Язык чистый; работа с сетью или файлами задумана как
  построение описания действия, которое исполняет хозяин. Монада ввода-вывода не реализована,
  поэтому сегодня эту работу делает хозяин — как в `examples/library-api/host/`.
...
**Конкурентность.** Сделаны первый и второй шаги из семи: поверхность, проверки и планировщик
эталона. Надзор разбирается и проверяется, но не действует; действия `породить` нет; адресат
сообщения обязан быть литералом. Печать программы с процессами даёт обработчики как обычные
функции и больше ничего — ни процессов, ни планировщика, ни в одной цели. Перебора семян нет,
распределённости нет. Числа о том, сколько процессов помещается и сколько стоит отправка, не
называются, потому что седьмой шаг — измерение — не сделан.
