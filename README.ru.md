...
притворяется, что занимает, — а цена пула потоков измерена и названа: пробег обработчика стоит
**1,1 мкс**, передача этого пробега другому потоку — **15,9 мкс**. Пока обработчик дешевле
пятнадцати микросекунд, пул отнимает больше, чем даёт. Остальные шесть целей процессов не печатают
...
**1,145 мкс + 0,003 нс на процесс**: наклона в пределах измерения нет вовсе, а журнал при этом
совпадает с эталоном побайтово — быстрее стал способ получить очередь, а не очередь.
...
потоков нет, и цена его названа числом (пробег 1,1 мкс против 15,9 мкс на передачу его другому
потоку). Процессы печатаются только в Elixir и C, а остальные шесть целей дают из программы с
