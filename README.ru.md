...
14 каталогов, и часть имён повторяется. Есть `src/` и `flang/src/`, есть `test/` и
...
есть 94 программ с 488 функциями и 1231 примерами. Для каждой функции строится сетка входов из
...
`numbers`, `numtree`, `optional`, `result`, `sets`, `strings`, `tree`) написана так же — 12 модулей, 185 функций, из них
181 доказана тотальной. `higher-order` — та, что
