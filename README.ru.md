...
есть 36 программ с 224 функциями и 455 примерами. Для каждой функции строится сетка входов из
...
- Список наращивается с двух концов: `добавить … к …` дописывает в конец, `приписать … к …`
  приписывает в начало. Обе формы обещают ЗНАЧЕНИЕ, а не стоимость, и стоимость названа
  поимённо в `flang/SPEC.md`: у C постоянны обе (запас в арене считается с двух концов), у
  Elixir постоянно приписывание (список BEAM односвязный), у остальных шести — одна копия на
  вызов. Одна, а не по одной на элемент: до появления `приписать` приписывание в начало
  выражалось только свёрткой с `добавить`, и это давало КУБ от длины там, где нужен один проход.
