...
- Функции первого класса есть в языке, но ещё не печатаются. Ограничение снято
  ...
  доказуемость завершения не страдают (`flang/cat/HOF.md`). Разбор, проверка типов, анализ
  завершаемости и интерпретатор это понимают; **восемь бэкендов — нет**, применение даёт
  `FLANG_PARSE: неизвестный вид выражения «apply»`. Пока печать не сделана, `отобразить` /
  `отфильтровать` / `свёртка`, принимающие *тело*, остаются единственным высшим порядком,
  который печатается.
