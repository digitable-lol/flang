...
есть 37 программ с 239 функциями и 505 примерами. Для каждой функции строится сетка входов из
...
`numtree`, `optional`, `result`, `sets`, `strings`, `tree`) написана так же — 11 модулей,
161 функций, из них 157 доказана тотальной. `higher-order` — та, что
...
четырёх до четырнадцати пробегов, смотря по машине). Процессы печатаются в три цели из восьми — C, Elixir и JavaScript, — а остальные пять на программе с
`процесс` честно ОТКАЗЫВАЮТ кодом `FLANG_NO_SCHEDULER`: печатать процессы им нечем, а печатать
программу без них значило бы собрать не то, что написано. Что у какой цели есть, сказано не прозой:
таблица возможностей (`конкурентность`, `параллелизм`, `причина`, `проверка`) уезжает полем
`возможности` в вывод `emit`. Параллелизма у JavaScript не будет никогда — один поток; обещан не он,
а одинаковый НАБОР возможных исходов, и он проверен побайтовой сверкой журнала доставок с эталоном
на сетке семян (2800 совпадений, 146 различных чередований). Действия `породить` нет, поэтому набор процессов
