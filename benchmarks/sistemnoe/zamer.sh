...
# Повторяется целиком, из корня дерева:  sh benchmarks/sistemnoe/zamer.sh
set -e
...
cp "$ZDES/../bez-libc/sloy.c" "$ZDES/../bez-libc/sloy-teksta.c" "$ZDES/fstab-bez-libc.c" "$KORAB/"
cd "$KORAB"
...
cc $OBSHCHIE -c -o drv.o fstab-bez-libc.c
cc -std=gnu99 -O2 -DTOLKO_VHOD -fno-stack-protector -c -o vhod.o sloy.c
...
echo "слои: sloy.c $(sed 's://.*::' sloy.c | perl -0777 -pe 's{/\*.*?\*/}{}gs' | grep -cvE '^[[:space:]]*$') строк + sloy-teksta.c $(sed 's://.*::' sloy-teksta.c | perl -0777 -pe 's{/\*.*?\*/}{}gs' | grep -cvE '^[[:space:]]*$') строк"
