...
echo "--- python"
/usr/bin/time -v python3 "$DIR/tasks.py" "$ZAD" "$N" 2>&1 | grep -E '^[0-9]|Elapsed \(wall|Maximum resident'
echo "--- node"
LC_ALL=C.UTF-8 /usr/bin/time -v node "$DIR/tasks.mjs" "$ZAD" "$N" 2>&1 | grep -E '^[0-9]|Elapsed \(wall|Maximum resident'
