 ...
 *      monoids, monads, isomorphisms, embeddings, intersections. ЗАПАС, а не
 *      необходимость на этом пути: см. В ниже;
 ...
 *
 *   В. ЗАКОНЫ-ПЕРЕСЕЧЕНИЯ (`intersections`) ДОБАВЛЕНЫ. Ч183 хешировала четыре
 *      поля законов из пяти; пятое читает «Есть закон» (proofterm.flang:243).
 *      ОГОВОРКА, и её обязан прочесть тот, кто напишет хранилище: «Есть закон»
 *      зовётся из ОДНОГО места — «По закону» (proofterm.flang:1430), — а это
 *      обработчик АВТОРСКОГО шага `по закону «имя»` внутри теоремы.
 *      Обязательства БЕЗ теоремы, которые здесь и закрываются, туда не
 *      попадают. Значит на этом пути ⑦ — запас: он не покупает ничего и стоит
 *      промахов на коммите, трогающем объявление закона. Оставлен потому, что
 *      кеш, расширенный на обязательства С теоремами, без него солжёт, а цена
 *      запаса сегодня ровно ноль: объявленных законов в замыкании компилятора
 *      нет ни одного.
 *
