 ...
 *
 * Предел берётся ТОТ, ЧТО ВПЕЧАТАН (`--max-depth` при печати), а не тот, что
 * приедет в запросе: стек заводится один раз на процесс, а запросов в трубе
 * сколько угодно. Запрос, попросивший больше впечатанного, упрётся в сторож
 * `fl_enter` и получит объявленный отказ с честным текстом про хозяина — не
 * молчаливую смерть.
 *
...
int main(int argc, char **argv) {
  ...
  run.argv = argv;
  run.status = 0;
  if (!fl_call_deep(fl_stack_wanted(FL_MAX_DEPTH), cli_body, &run)) {
    cli_body(&run);
  }
  return run.status;
}
