...
#define EMIT_TARGETS_MISSING \
  ...
  "его уважает, что она ассоциативна и что квадрат преобразования коммутирует.\n"\
  "Это счёт, а не доказательство, и отчёт говорит об этом словами.\n"\
  "\n"\
  "ЧЕГО НЕТ — УСТРОЙСТВО категорных объявлений (замкнутость категории под\n"\
  "композицией, единицы, сходимость концов у композиций, вид объявленного\n"\
  "равенства) и целиком связь модулей, бифунктор, изоморфизм, моноид, монада,\n"\
  "множества, объявленные свойства, требования, процесс, надзор, прогон.\n"\
