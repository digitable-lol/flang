...
# → доля-проигрыванием = 629 / 651 = 96.62 %      (13 сентября 2026, коммит 1218aa186, flang 0.7.19)
sh scripts/доказуемость.sh          # → НЕ ДОКАЗУЕМ, код 1                       (17 сентября 2026, порог 100 %)
# → проверка 2 — доля проигрыванием не ниже 100 %? НЕТ (97.38 %: 633 из 650; недостижимых мест вынесено 22)
```

Дробь читается как дробь. Числитель 629 — то, что чекер на C установил, не спрашивая ядро: 397
обязательств проиграны по записанным ходам, 93 пересчитаны на месте там, где запись говорит «по
примеру» или «по свойству», 85 узлов завершаемости пройдены заново структурно и 54 вывода факта о
типе проиграны правило за правилом. Знаменатель 651 — все обязательства 89 записей набора за
вычетом 22, которые принадлежат четырём записям, отвергнутым чекером целиком: это нарочные
подделки, и подделке проиграться не положено. Остаток 22 и есть честная цена: 12 посылок и
утверждений и 4 шага чекер берёт на слово ядра, 6 сняты вычислением тождества, а не игрой. Где
стоит каждое из этих мест и чего стоит его закрыть —
[`docs/road-to-one-hundred-measured.md`](road-to-one-hundred-measured.md).

**Этот выпуск долю не сдвинул.** 625 → 629 она выросла в 0.7.18, а 0.7.19 оставил её на месте:
перепечатка семени дала ядру новые умения, а не прирост доли.

Вторая команда — вердикт одним словом. С 17 сентября 2026 порог доли — 100 %: слово владельца
(задача 3348), поднято с прежнего, более низкого порога, который доля брала с запасом, — и вердикт
честно **НЕ ДОКАЗУЕМ**, код 1: проиграно 633 обязательства из 650, 17 чекер ещё берёт на слово
ядра или закрывает вычислением, каждое названо с ценой в
[`docs/road-to-one-hundred-measured.md`](road-to-one-hundred-measured.md). Слово перевернул порог,
а не доля. Слово выводится из четырёх проверок, у каждой число: в чекере нет шага, который доказывает
вычислением вместо переигрывания (мерится ловушкой из трёх ∀-целей, а не грепом по имени функции);
доля 97,38 % — ниже порога 100 %; набор проб пройден целиком — 529 проб на подлог отвергнуты, 244
честные записи приняты; набор проб не ослаб против своего храповика. Правила вывода, которыми пользуется
ядро, проверены и вторым, чужим судьёй — ядром Lean 4; одно правило Lean отверг как
несостоятельное, и оно починено. Тот прогон датирован: он судил 88 правил по 85 леммам
11 сентября. С тех пор список правил вырос — правил в `flang/proof/ПРАВИЛА-ВЫВОДА.tsv` 97, лемм
написано 105, — и заново Lean не гоняли: его на этой машине нет
...
`vimtutor`: двенадцать уроков, каждый ответ проверяет настоящий компилятор. Подробности и что ставит каждый путь —
...
Десятую цель справка наконец называет. `flang --help` и строка вызова в `flang emit --help`
перечисляют `c|cpp|go|rust|java|js|ts|elixir|python|csharp` — это привезла перепечатка семени в
0.7.19. Два текста в том же двоичном всё ещё отстают: проза `flang emit --help` говорит «во все
восемь целей», а отказ на неизвестную цель отвечает «целей здесь ДЕВЯТЬ» и перечисляет девять,
без `ts`. Оба вшиты в двоичный и сдвинутся только следующей перепечаткой семени, поэтому названы
здесь, а не замазаны.
