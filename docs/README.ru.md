...
# → доля-проигрыванием = 650 / 650 = 100.00 %     (19 сентября 2026, коммит a5609e322, flang 0.7.20)
...
Дробь читается как дробь. Числитель 650 — то, что чекер на C установил, не спрашивая ядро: 397
обязательств проиграны по записанным ходам, 97 пересчитаны на месте там, где запись говорит «по
примеру» или «по свойству», 85 узлов завершаемости пройдены заново структурно и 71 вывод факта о
типе проигран правило за правилом. Знаменатель 650 — все 677 обязательств набора за вычетом 27,
которые принадлежат записям, отвергнутым чекером целиком: это нарочные подделки, и подделке
проиграться не положено. Остатка нет: мест, которые чекер берёт на слово ядра или закрывает
вычислением тождества вместо игры, **ноль** — прежде их было 12 посылок и утверждений, 4 шага и 6
вычислений.

**Порог взят 18 сентября 2026.** Порог доли — 100 %, слово владельца (задача 3348), поднято
17 сентября с прежнего, более низкого; доля дошла до него за день: 633 → 650 из 650. Карта мест,
которые пришлось закрыть, с ценой каждого —
[`docs/road-to-one-hundred-measured.md`](road-to-one-hundred-measured.md).

Вторая команда — вердикт одним словом, и он выводится из четырёх проверок, у каждой число: в чекере
нет шага, который доказывает вычислением вместо переигрывания (мерится ловушкой из трёх ∀-целей, а
не грепом по имени функции); доля 100,00 % — не ниже порога; набор проб пройден целиком — 533 пробы
на подлог отвергнуты, 245 честных записей приняты; набор проб не ослаб против своего храповика.
Правила вывода, которыми пользуется ядро, проверены и вторым, чужим судьёй — ядром Lean 4: все 109
правил переведены в леммы и приняты, расхождений вердиктов 0, ловушек 52 и все 52 отвергнуты, ни
одного `sorry`, `axiom` или `native_decide` (прогон на коммите `a5609e322`). Подробный отчёт
([`docs/lean-checks-the-inference-rules.md`](lean-checks-the-inference-rules.md)) отстал: числа в
нём сняты 11 сентября.
