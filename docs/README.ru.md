...
примеру» или «по свойству», и 34 узла завершаемости пройдены заново структурно. Знаменатель 651 —
обязательства набора записей за вычетом 14 недостижимых: они принадлежат той одной записи из 86,
которую чекер отверг целиком, и проиграться не могут в принципе. Остаток 134 и есть честная цена:
109 посылок и утверждений и 17 шагов чекер берёт на слово ядра, 8 сняты вычислением тождества, а не
игрой.

Вторая команда — вердикт одним словом: сегодня язык формально **не** доказуем, и код возврата 1.
Прибор гоняет четыре проверки и печатает все четыре числами. Три из них сошлись: калькулятор из
чекера снят, набор проб на подлог пройден целиком, набор подделок не ослаб. Останавливает вердикт
одна — доля 79,42 % против порога 95 %. Числа печатает прогон, и они будут двигаться; команды —
способ снять их заново. Подробный разбор — на сайте: [Что доказано, а что
