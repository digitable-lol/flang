...
| `flang/` | **язык**: `self/` компилятор, `core/`, `stdlib/`, `proof/`, `conc/`, `ct/`, `src/emit/` рантаймы, `test/`, `bin/` (проводник `flangtutor`); внутри — `flang/scripts/` (53 файла оснастки языка, 271 ссылка) | 955 | 647 (`self/`), 417 (`stdlib/`), 408 (`src/`) | не переезжает | 48 из 55 строк отпечатка — пути `flang/…`; двоичный ищет библиотеку по `<каталог двоичного>/../flang/stdlib` |
