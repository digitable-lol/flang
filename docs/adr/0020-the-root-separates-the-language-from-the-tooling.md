...
| `docs/tasks/` | задачник | 371 (107 открытых) | 53 | не переезжает | `task-numbers-guard.sh`, `flang/scripts/tasks.fscript` |
...
1. **Три четверти корня — язык или то, без чего он не собирается** (`bootstrap/`,
   `flang/`, `docs/examples/`, `fspec/`, `ярлык`). Их не переносить.
...
| Планы на flang | `scripts/guards/tree-inventory.fscript` (правило «замеряемый материал»), `file-extensions.fscript`, `name-splicing-guard.fscript`, `flang/scripts/code-guard.fscript`, `memory-guard.fscript`, `flang/test/glob.flang`, `flang/concurrency/bench/gen.flang` | строки-образцы и примеры |
...
| Релизный архив (`release.yml`, `scripts/release/release-in-c.fscript`) | состав: семь файлов `bootstrap/`, рантаймы `flang/src/emit/`, `flang.1`, `LICENSE` — без `benchmarks/` и `web/` | — |
...
(`scripts/guards/tree-inventory.fscript:231`): у пути, начинающегося с `packaging/`, стоит
