...
#                    fl_add не подставляется на место. В настоящем компиляторе
#                    это правка двух строк (emit/c.mjs и self/emit-c.flang),
#                    поэтому мерить её обязательно.
#   lto-bez-tipov/   lto плюс выключенные проверки тегов (fl_numbers, fl_order):
#                    ту же работу тайпчекер уже сделал на исходнике. Показывает,
#                    сколько стоит повторная проверка, а сколько — сама ширина
#                    значения.
#   etalon           те же пять задач, написанные на C руками
#
# Макросы ставятся в файле МОДУЛЯ, а не в рантайме: сам рантайм обязан
# собираться обычным, иначе в нём нечего было бы звать.
#
# Использование: assemble.sh [КАТАЛОГ-СБОРКИ]
#
# Без довода собирает в «.sborka» рядом с собой. Это не украшение: замер пиковой
# памяти написан планом на flang (memory.flang), а у плана доводов нет — их нет
# и у команды `flang io`. Каталог поэтому назван одинаково в двух местах, и
# менять его надо в обоих.
set -eu
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
OUT="${1:-$ROOT/docs/benchmarks/speed/.sborka}"
PROG="$ROOT/docs/benchmarks/speed/programs"
...
"$ROOT/bootstrap/flang" emit "$PROG/tasks.flang" --target c --out "$OUT/base" \
...
cc -std=c99 -Wall -Wextra -O2 -o "$OUT/etalon" "$PROG/reference.c" -lm
