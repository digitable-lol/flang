...
первого класса (`docs/archive/hof.md`): объект `«Б»^«А»` пишут `функция из «А» в
...
функции, а не сахар над цепочкой одноместных, и это решение (`docs/archive/hof.md`, «Чего
...
моноида (`docs/archive/monad.md`). Предусловием преобразования оказалось не то,
...
упущение. Замкнутое множество видов (`docs/flang/concurrency/SPEC.md`, сегодня их десять) —
...
`flang/ct/order-shipment.flang`; сверявшая её
проверка снята вместе с реализацией на JavaScript 20 августа 2026.
...
фазой 3 в `docs/archive/poly.md` и законов не касается.
...
себе не отвечает. Подробный контракт — `docs/ct/sets.md`; здесь только место
в этой картине.
...
других, почему третьего слова не завелось, — в `docs/ct/sets.md`.
...
`flang/ct/natural-square.flang` это видно на настоящей задаче: два
...
`docs/archive/poly.md`, и она не сделана. Сделанное работает над функторами,
...
`docs/examples/rosetta/towers-of-hanoi.flang`, и ключевое слово сломало бы этот
...
`docs/examples/io/link-report.flang`.

### Словарь поручений — 23 действия, и набор закрыт
...
тип «Отклик»               // что хозяин принёс обратно
  ...
  вариант «Экран» содержит ширина: число, высота: число
...
**Поручений 23, откликов 21, продолжений 4, и все три набора закрыты** — это
<!-- СНЯТО 2026-09-22 список flang/self/parser.flang:6620 = 23 --><!-- СНЯТО 2026-09-22 список flang/self/parser.flang:6629 = 21 --><!-- СНЯТО 2026-09-22 список flang/self/parser.flang:6638 = 4 -->
...
«символ по коду» нет (`docs/flang/SPEC.md`, раздел 5), — значит программа не может
...
прогон — `docs/examples/web/browser-app/`. **Почему каталог и процесс стали поручениями.** Поручений
...
- **служба не умела ЖДАТЬ.** `«Запросить»` — клиентское, а сервер по адресу не
  ходит, он сидит на порту. Служба на flang (`docs/examples/web/shortener`)
...
`docs/examples/rosetta/fibonacci.flang`) — ключевое слово запретило бы их и
...
`docs/examples/wal/append-plan.flang` по всем восьми целям, по одной цели за
...
**Сверка списка целей с поведением — `scripts/targets/plan-across-targets.fscript`.** Различает
...
```
bootstrap/flang io scripts/targets/plan-across-targets.fscript
...
таблица «цель → исход» объявлена данными в `scripts/targets/plan-across-targets.fscript` и
...
Параметрических типов в языке В ТОТ ДЕНЬ не было, а ждать их здесь было нельзя:
без ввода-вывода язык не читал файл СЕГОДНЯ. Сегодня они есть, и форма `в монаде`
тоже; машина продолжений на неё не переехала — задача
`docs/tasks/5555-the-io-continuation-machine-moves-onto-the-monad-form.md`, разбор — в
...
которым язык печатает функции первого класса (`docs/archive/hof.md`). Что из-за этого
...
`в монаде` сделана (`docs/archive/monad.md`) — а машина продолжений на неё **не
...
`docs/archive/hof.md`): без захвата продолжение не становится значением, а без
...
`--no-clock`, `--no-random`, `--no-spawn`, `--no-screen`, `--in-dir` сужают
полномочия хозяина. Запрещённое поручение возвращается откликом `«Сбой»` с кодом
`FLANG_IO_DENIED`.

Одного у двоичного хозяина нет, и он называет это сам: шифрования (`https`
отвечает `FLANG_IO_NO_TLS`). Экран у него есть с 19 сентября 2026 — управляющий
терминал `/dev/tty` с единственным местом по имени `«экран»`; там, где терминала
нет (вывод в трубу или в файл, CI, `nohup`), `«Показать»` и `«Ждать событие»`
отвечают `FLANG_IO_NO_SCREEN`, как отвечали всегда.
...
| множество (`Множество`) | членство и объединение | моноид, коммутативный — **слово «коммутативный» здесь не обещание, а объявление**: `свойство «коммутативность»` рядом с моноидом, и его строка законов называет третий (`docs/ct/zakony.md`) |
...
- **устройство отношений множеств** (`docs/ct/sets.md`) — у вложения оба
...
- **пять объявляемых свойств** — дистрибутивность, идемпотентность, частичный
  порядок, монотонность, коммутативность — **сделано** (`docs/ct/zakony.md`).
...
- инъективность вложения и непустота общей части — **проверялись**
  ...
  проверок разная, и это записано в `docs/ct/sets.md`: склейка у вложения —
...
4. 🟡 **Монада и форма `в монаде` — СДЕЛАНЫ**; естественное преобразование —
   ...
   даром:
   `docs/archive/monad.md`. Осталось преобразование — «из функтора «Список» в
   функтор «Возможно»» говорит про одну и ту же «А» в двух местах, и это фаза 3
   в `docs/archive/poly.md`.
...
9. ✅ **Связь модулей** — интерфейс модуля объявляется его
   ...
   разделом, поимённо.
10. ✅ **Отношения множеств — подобъект и предел** (`docs/ct/sets.md`).
