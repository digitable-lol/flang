...
что обязаны одинаково делать все цели печати.

**Кому эта страница НЕ нужна.** Она написана для того, кто язык переносит,
проверяет или развивает, и читается как контракт: правило, его границы и код
отказа. Замеры, которыми правила добывались, вынесены отсюда в
`docs/archive/spec-measurements.md` — там они с датами и не выдаются за
сегодняшние. Тому, кто пришёл писать на flang, отвечают короче и по делу:
...
- **предел вызовов воспроизведён во ВСЕХ {{цели.изСловом}} целях.**
  Незавершающаяся функция упирается в предел и даёт `FLANG_RECURSION_LIMIT`.
  Шагом везде считается вход в функцию, оборот цикла хвостового самовызова и
  отскок батута; шаг вычислителя мельче, поэтому при одинаковом пределе он
  упирается первым, и напечатанный код не объявит исчерпанным то, что
  вычислитель досчитал. Предел глубины есть везде, включая C, потому что
  переполнение стека в C — падение процесса, а не исключение. Код отказа один и
  тот же у всех целей; совпадение самого ТЕКСТА отказа между целями сегодня не
  мерит ни одна проверка дерева, и обещать его поэтому нечем;
- **счётчик глубины держит обещание только пока предел НИЖЕ стека хозяина.**
  Счётчик считает кадры, а несёт их стек, и сколько кадров влезет, зависит от
  толщины кадра, то есть от программы: между двумя программами разница
  шестнадцатикратная. Поэтому предел держат два средства сразу, а не счётчик
  один. Первое — пробег на потоке с ЯВНО ЗАДАННЫМ стеком, размер которого
  считается ИЗ объявленного предела: `fl_stack_wanted` в C, `rt::stack_wanted` в
  Rust, `rt.call_with_deep_stack` в Python, `resourceLimits.stackSizeMb` у
  JavaScript. Второе — проверка ОСТАТКА стека на входе в функцию (`fl_enter`,
  `Ctx::enter`): никакой размер стека не покроет любую толщину кадра, поэтому
  вход сверяет запас по самому толстому кадру, который эта программа уже
  показала, и исчерпание переводит в объявленный `FLANG_RECURSION_LIMIT` с
  текстом про хозяина. Цена второго — доли процента самого дешёвого входа;
- **у Go рычага нет, и это измерено, а не предположено.** У горутины нет явного
  размера стека: он растёт сам, ПЕРЕЕЗЖАЯ на новое место, поэтому отметка стека,
  снятая на входе в расчёт, после первого же переезда не значит ничего —
  проверку по адресам в Go завести нельзя. Растущий стек зато снимает вопрос по
  умолчанию: до объявленного предела горутина доходит с запасом и отказывает
  объявленным текстом. Выше потолка горутины Go остаётся смертельным —
  `fatal error: stack overflow`, — и поднять потолок нечем. Значит про Go честно
  ровно это: **предел ПО УМОЛЧАНИЮ несётся стеком и отказ объявленный; предел,
  поднятый выше того, что несёт горутина, — не несётся, и там Go умирает**;
- **у JavaScript рычаг работает через прогонщик, и границы названы.** Прямой
  вызов экспортированной функции считает на стеке того, кто позвал: рычаг
  работает либо через прогонщик (`src/emit/js/flang_cli.js`, печатается по
  умолчанию, снимается `--no-cli`), либо через `$callDeep`. Ровно так же устроен
  C — библиотека, вызванная мимо `fl_call_deep`, считает на стеке вызывающего. В
  браузере рычага нет вовсе: `worker_threads` там не существует, а стек Worker'а
  не настраивается; прогонщик туда и не едет — он соседний файл, а не часть
  модуля, и модуль без него полон;
- **где рычага нет, обещание держит проверка остатка.** Переполнение стека
  переводится в объявленный `FLANG_RECURSION_LIMIT` — код из закрытого набора, —
  а текст честно называет хозяина, а не предел, до которого не добрались.
  **Отказ всегда объявленный, и он никогда не зависание, не RangeError и не
  смерть вкладки.** Что долг был не про выдуманную программу, видно на самой
  близкой: точка раскрутки `bootstrap/` — компилятор flang, напечатанный в C, —
  умирала по SIGSEGV на исходнике из вложенных скобок, а после починки отвечает
  `FLANG_RECURSION_LIMIT` и кодом возврата 1;
...
Чем всё это добыто — кадры стека в байтах, наносекунды на вход, замеры до
починки и после — записано отдельно: `docs/archive/spec-measurements.md`. Числа
там датированы днём съёмки; здесь их нет нарочно, потому что приметы к ним
поставить нечем: приборов у проверки чисел в прозе пять, и кадра стека среди
них нет.
...
те же double, и {{цели.словом}} исполнителей о нём не знают ничего.

## 3. Типы

```
Тип := строка | число | признак | ничто
     | неотрицательное | целое | вес | сотых | тысячных   уточнения числа
     | деньги | дата         поверхность наследия: сводятся к числу и строке
     | список Тип
     | «Имя объекта»          запись
     | «Имя типа»             сумма
...
Уточнения числа разобраны ниже поимённо. `деньги` и `дата` пришли с поверхности
наследия (раздел 9) и нового вида значения не заводят: первое — число, второе —
строка.
...
вычислением (сверка бесконечности, раздел УЛИКА). **Собранный двоичный этого
отказа не выносит**: на `вес минус вес` он отвечает «замечаний нет» — зазор
назван в `docs/DESCRIPTION.md` и в таблице долга двоичного.
...
неё и ложно для всякого числа (`docs/examples/paths/shortest-path.flang`).
...
Масштаб ведёт себя как ЕДИНИЦА ИЗМЕРЕНИЯ, а не как границы. **Собранный
двоичный `FLANG_SCALE` не выносит** — тем же зазором, что и `FLANG_INFINITY`
выше:
...
| | что делает | цена |
|---|---|---|
| **выбрано: деление ВЫВОДИТ из точного типа** | результат `делить` перестаёт быть целым, а вместе с целостью теряет масштаб: тип становится `число` | денежная функция с делением возвращает `число`, и сложить её результат с деньгами можно только через функцию с объявленным возвратом. Три функции из девяти в `docs/examples/money/exact-decimal.flang` именно такие |
...
| округлять к сетке масштаба | `делить` на `сотых` округляет результат к целой копейке | меняет СЕМАНТИКУ `делить` — то есть поведение рантаймов {{цели.изСловом}} целей, `flang/core`, моста наследия и сверки самоприменения байт в байт. Ровно то, чего уточнение типа избегает по построению |
...
- **печать в {{цели.словом}} целей.** У пяти из них нет замыканий в том виде,
  что есть
  в JS. Тег и `switch` есть у всех {{цели.изСловом}};
...
отказа, и почему иначе не выходит — в `docs/archive/hof.md`. Без самого правила
...
вычисление — и печать во все {{цели.словом}} целей: снятие функций-значений
сделано одним проходом перед печатью (`self/defunc.flang`), после которого
программа снова первопорядковая, а бэкенды печатают её теми же узлами, что и
всегда. Понимает форму и самоприменение: `self/parser.flang` разбирает тип
функции и `функция «Имя»`, `self/types.flang` знает вид типа «функция».
Поэтому ограничений на то, кто формой пользуется, нет: библиотека
`flang/stdlib/higher-order.flang` написана на ней и проходит `flang check`
целиком.

Захват части аргументов сделан: тег — это пара «имя и захваченное», и печати он
не стоил ни строки. Осталось одно, и это названо выше огрублением: анализ
завершаемости не прослеживает, какой именно тег придёт в данное применение.
Порядок и цена — `docs/archive/hof.md`.
...
обоснование — `docs/archive/poly.md`.
...
проверяются на конечной сетке. Граница названа прямо: **собранный двоичный
объявление `монада` не судит вовсе** — `flang check` на такой программе отвечает
кодом 2 и словами «проверено НЕ ВСЁ: в программе объявлено то, чего бинарник не
судит». Разбор и типы форму понимают, законы не проверяет никто.
Подробности, счёт занятых слов и границы — `docs/archive/monad.md`.
...
берутся из одной таблицы (она же «Куски таблицы» в лексере языка), поэтому
парсер не знает, на каком языке написан исходник; знает это только диагностика,
чтобы цитировать слова той поверхности, на которой файл написан.

Поверхность не обязана быть полной. Из {{словарь.понятий}} понятий таблицы на
всех четырёх поверхностях открыты {{словарь.наЧетырёх}}; у остальных на какой-то
из поверхностей слова нет, и это записано в самой таблице поимённо с причиной:
выбор слова в языке — решение владельца, а не подстановка из словаря. Файл на
такой поверхности пишет недостающее понятие словом другой
(`docs/examples/surfaces`). Считает это `node docs/site/surfaces-run.mjs`
(ярлык `поверхности:проверка`).
...
Разделить пришлось именно запятую, потому что она дешевле остальных путей:
цена мерялась разбором всех программ репозитория до и после, и ни одна не сменила
смысл молча — обе спорные записи отказали вслух. Числа переписи и разбор трёх
отвергнутых путей — `docs/archive/spec-measurements.md`.
...
{{цели.изСловом}}. И по самому Unicode суррогат символом не является: скалярное значение
...
строках пишется НА ЯЗЫКЕ и доказывается тотальным**. Сравнения
`больше`/`меньше` по-прежнему допустимы только для чисел, и своей реализации
`compare` в рантайме цели не завелось: лексикографический порядок это рекурсия
по хвосту строки, и живёт он в библиотеке, где его видно и можно проверить
примерами (`flang/stdlib/tree.flang`, «Строка раньше»). На нём же стоит первый в
языке словарь с логарифмическим доступом — дерево поиска с приоритетом по хешу
ключа, замер — в сверке библиотеки.

Хеш строки писался на языке тем же приёмом и по той же причине. С тех пор у
языка есть встроенная форма `хеш256`: она принимает строку и отдаёт строку, и
рантайм у неё общий на все цели.
...
**Чего одна мера НЕ обещает: одинакового числа знаков у {{цели.изСловом}} целей на строке,
...
такая форма стоила бы {{цели.изСловом}} рантаймов и не давала бы ничего, что нельзя
...
Форму знает и лексер, и разбор на flang, поэтому ограничений на то, кто ею
пользуется, нет: `stdlib` и `docs/examples` вправе, и набор сверки самоприменения от
неё не расходится.
...
элемент хвоста в свежий накопитель. Цена той записи не в разах, а в КЛАССЕ
СЛОЖНОСТИ: `добавить` копирует список у большинства целей, поэтому построение
списка спереди назад выходило кубом от длины там, где нужен один проход, — и
класс сложности зависел от ЦЕЛИ печати, чего не видела ни одна проверка. Замер,
которым это снято, — `docs/archive/spec-measurements.md`.

ЧТО ФОРМА ОБЕЩАЕТ. Значение, а не стоимость, — как и `элемент`. Стоимость
названа поимённо в таблице «Стоимость встроенных форм» ниже: у вычислителя и C
постоянная, у Elixir тоже, у остальных — одна копия на вызов. Одна копия вместо
N — это и есть снятый класс сложности. Проверкой стоит «приписывание в начало
линейно» в сверке вычислителя.
...
недостачей в `docs/examples/leetcode/index.json`.
...
вычислитель, ни {{цели.словом}} целей печати о новой записи не знают ничего. Накопитель
...
| `код символа` | одна кодовая точка, у всех целей одинаково | она же |
| `символ по коду` | одна кодовая точка, у всех целей одинаково — включая оба текста отказа | она же |
...
**Почему стоимость вообще стоит в контракте языка.** Шаг напечатанного кода —
это вход в функцию, виток хвостового цикла и отскок батута; если ОДИН такой шаг
стоит O(длины), то предел шагов перестаёт ограничивать РАБОТУ, и объявленный
бюджет кончается не за секунду. Поэтому продление списка за постоянное время —
не оптимизация, а условие, при котором предел шагов что-то значит; сделано оно
в каждой цели своим приёмом, а у Elixir его заменяет очередь Окасаки.

Приёмы, их цена в каждой цели и замеры, которыми она снята, — в
`docs/archive/spec-measurements.md`. Числа там датированы днём съёмки и заново
не снимались; выдавать их за нынешние нельзя.

Строку про `хвост` стережёт отдельная проверка таблицы стоимостей
(`scripts/guards/emit-promises-guard.fscript`): у каждой цели левого столбца и у
вычислителя она держит точный кусок рантайма и класс — срез или копия, — и
требует, чтобы скобка называла ровно те цели, что берут срез. До неё скобка
называла только C и Rust: таблица занижала цель молча, а по ней выбирают цель
для горячего пути.
...
самоприменения (`docs/archive/hof.md`).
...
Контракт модели — `docs/flang/concurrency/SPEC.md`. В AST она добавляет три необязательных
...
Контракт — `docs/ct/spec.md`, раздел «Эффекты и HTTP». Язык остаётся чистым:
`вариант «Прочитать файл» с путь равным …` строит ОПИСАНИЕ действия — значение,
— а исполняет его хозяин. Поручений 23.
...
`call` и `match` (`docs/archive/monad.md`).
...
свой (`FTS_UTILITY_PROPERTY`), и рантаймы {{цели.изСловом}} целей обязаны выдать именно его, не
...
словом `обеспечивает` (`docs/flang/proof/SPEC.md`). Форма в AST **та же**: ни одного
...
Слово `требует` (`docs/flang/proof/SPEC.md`). Поле появляется **только** у функции с
...
вызова, а результата до вызова не существует. Поля `forall` нет по той же причине,
по которой оно есть у постусловия: квантор ПЕРЕД именем требования называл бы
параметр для индукции, а предусловие индукцией не доказывается — оно снимается у
каждого вызова.

**Квантор по элементам В САМОМ утверждении есть** (ADR-0037 §4.2, задача 3123):
`требует «имя» для всех э из Л: П` / `requires «name» for all e from L: P` —
та же форма, что у цели постусловия (ADR-0026 §11 п.14), тем же узлом `forallIn`
в `expr`, с вложенными кванторами (п.15). `Л` обязан быть списком, имя элемента
связывается типом элемента, `П` — признак; `результат` внутри по-прежнему не
связан. Выражение `требует` типизируется в среде параметров, как цель
постусловия (до этой задачи оно не типизировалось вовсе, а только сужало среду
тела). Внутри тела такое предусловие — допущение о каждом элементе списка: цель
`для всех п из Л: П` при нём закрывается правилом «цель есть допущение» (Т1, а с
другим именем элемента — Т2); у места вызова оно снимается по построению
подставленного списка семейством Э. Печать квантора в цели — задача 7098.
...
(`docs/flang/proof/SPEC.md`, «Две формы под одним словом»). Старая — из прежней
...
командой `flang` — перечень команд печатает `flang --help`. Ниже — устройство
реализации, и только здесь названы её файлы.
...
```
flang/self/            компилятор, написанный на flang: слои и их роли —
                       в docs/flang/self/SPEC.md, состав каталога там же
flang/proof/kernel.flang   ядро доказательств
flang/src/emit/<цель>/     рантайм цели: то, что печать кладёт рядом
                           с напечатанным
bootstrap/             компилятор, напечатанный в C99 и закоммиченный:
                       семя, из которого собирается двоичный
```

Состав `flang/self/` здесь не перечисляется: он живёт в `docs/flang/self/SPEC.md`
одним списком, сверенным с шапками файлов, и второго списка быть не должно —
разойдётся молча.
...
обещание кончалось на границе команды: `flang/concurrency/examples/supervision.flang`
...
ответ, а не заводят по одному на цель.
...
**Таблица ниже — не весь набор кодов, и делать вид, что весь, нельзя.** Она
называет коды ЯДРА языка: разбор, типы, завершаемость, встроенные формы,
процессы и ввод-вывод. За её краем лежат ещё три семьи, у каждой свой контракт и
своя страница: доказательства (`FLANG_PROOF_*`, `docs/flang/proof/SPEC.md`), ввоз
модулей (`FLANG_IMPORT_*`) и категорная поверхность (`FLANG_MONOID_*`,
`FLANG_MONAD_ASSOC`, `FLANG_ORDER_NOT_TRANSITIVE` и соседи, `docs/ct/spec.md`).
Полный список того, что компилятор действительно печатает, берётся из
исходников, а не отсюда; сверяет прозу с ними `flang/scripts/code-guard.fscript` (ярлык `коды:проверка`; прежний `scripts/error-code-guard.flang` снят 11 сентября 2026) —
код, названный в любом документе дерева и не найденный в исходниках, красит
проверку.

**Три кода этой таблицы объявлены контрактом, но собранный двоичный их НЕ
выносит** — `FLANG_INFINITY`, `FLANG_SCALE`, `FLANG_NOT_A_NUMBER`. Зазор назван
числом и разобран в `docs/DESCRIPTION.md`; на `вес минус вес` двоичный отвечает
«замечаний нет».
...
неизвестный план) ЛИБО программа сказала «Не проверено», то есть смотреть ей было
нечем. Поле `код` у `«Провал»` — строка, поэтому число берётся не из
...
(команда `flang io`; у цели JavaScript — `src/emit/js/flang_host_node.js` и
`src/emit/js/flang_host_browser.js`), а факт-чекингу хозяин не выдаётся вовсе.
Именно ради этого свойства эффекты и описываются, а не выполняются.
