...
(`scripts/guards/emit-promises-guard.flang`): у каждой цели левого столбца и у
...
исходников, а не отсюда; сверяет прозу с ними `flang/scripts/code-guard.flang` (ярлык `коды:проверка`; прежний `scripts/error-code-guard.flang` снят 11 сентября 2026) —
