...
`КАТАЛОГИ` lists four directories, and the tree holds **770** `.flang` files. A directory left off the
...
So the coverage is printed on every run and pinned by a test: **190 files out of 770**. The other 580
...
| hand-written code outside the four directories (`flang/proof/examples`, `flang/conc/examples`, `examples/library-api`, `fspec`, `web/wasm`) | **66** | worth covering, but their price has not been measured |
