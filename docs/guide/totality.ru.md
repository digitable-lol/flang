...
## Процессы: правило записано, проверка есть, но код отказа другой
...
обработчик без запаса ловится как непокрытый отказ (`FLANG_UNCOVERED_FAILURE`),
а не под именем из спецификации. Здесь стояло «двоичный объявления `процесс` не
судит вовсе, код 2» — так отвечало семя до перепечатки `0ce948bfd`.
