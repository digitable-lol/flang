...
зависнуть на нём права не имеет. Нетотальную функцию он не вычисляет вовсе — вот
функция, у которой рекурсия идёт по СВОЕМУ ЖЕ результату, а не по части
значения, и доказывать завершение тут нечем:
