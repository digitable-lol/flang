...
> выводе нет. Причина одна на все четыре: эталонный генератор C
> (`flang/src/emit/c.mjs`) оптимизировали, а близнец (`flang/self/emit-c.flang`)
> за ним не пошёл. Это чинится догоном близнеца, но пока не починено, раздел 4.3
