...
> - `docs/flang/self/SPEC.md` **протух снова**: строки 291–293 обещают 1 970 функций,
...
| разбивка корпуса из 43 | 10 stdlib + 1 example + 26 leetcode + 4 core + 2 self | `ls flang/stdlib/*.flang docs/examples/*.flang docs/examples/leetcode/*.flang \| wc -l` |
...
`docs/flang/self/SPEC.md:73-80`, **устарели**: их печатал более ранний компилятор.
...
| где именно объявлена мера | `docs/examples/measure/euclid.flang`, `docs/examples/measure/binary-search.flang` | `grep -rln '^[[:space:]]*убывает ' --include='*.flang' .` |
...
| три условия сторожа меры | строго убыла, не ниже нуля, целая | `docs/flang/SPEC.md:283-287` |
...
| остальные шесть выбрасывают процессы молча, код возврата 0 | `for t in go rust python java csharp js; do bootstrap/flang emit flang/concurrency/examples/budget.flang --target $t >/dev/null; echo "$t код=$?"; done` — число «шесть» устарело вместе со строкой выше |
