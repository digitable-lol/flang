...
#
# Нужно: node ≥ 20 и `npm run build`. Разделу «восемь целей» дополнительно нужны
...
printf 'файлов .flang в репозитории              %s\n' \
  "$(find . -name '*.flang' -not -path './node_modules/*' | wc -l)"
printf 'объявлений «тотальная функция»           %s\n' \
  "$(grep -rh '^тотальная функция ' --include='*.flang' . | wc -l)"
printf 'объявлений «функция» (обычных)           %s\n' \
  "$(grep -rh '^функция ' --include='*.flang' . | wc -l)"
printf 'объявлений «убывает» (мера)              %s\n' \
  "$(grep -rh '^[[:space:]]*убывает ' --include='*.flang' . | wc -l)"
echo 'где именно объявлена мера:'
grep -rln '^[[:space:]]*убывает ' --include='*.flang' . | sed 's/^/  /'
