...
**Это по-прежнему самый крупный рычаг в дереве.**
...
Разгадка в том, ЧЕМ занят счётчик. Из ответа `flang io` видно: поручений
**135**, из них 77 — запуски компилятора (20 разборов, 20 ведомостей, 20
ведомостей без примеров, 17 ведомостей с подменённым телом). Каждый запуск стоит
порядка 0,15 с, то есть **около 11,5 с из 13,85 — это чужая работа, а не работа
языка.** Собственный счёт flang здесь тонет в ожидании потомка. По памяти
разрыв девятикратный (484 МиБ против 51), и его называть надо тоже: у общей
машины место у ворот берут по памяти, а не по времени.

**Правило, которое из этого следует, важнее самих чисел.** Цена переписывания
делится надвое, и делить надо ДО того, как браться:

* **файл, который жуёт строки сам** (сторож жаргона: 270 000 знаков страницы,
  восемнадцать образцов) — платит те самые сотни раз;
* **файл, который гоняет процессы** (счётчик доказательств, замер скорости,
  замер памяти) — платит единицами процентов, потому что и он, и его двойник
  ждут одного и того же потомка.
