...
| `flang/src/emit/c/flang_runtime.c` | 2 874 |
| `flang/src/emit/c/flang_conc.c` | 2 049 |
| `flang/src/emit/c/flang_cli.c` | 1 027 |
| `flang/src/emit/c/flang_runtime.h` | 765 |
| `flang/src/emit/c/flang_conc.h` | 229 |
| **итого** | **11 146** |
...
*измеренная, но не принятая* правка (`docs/zamer-skorosti.md`: 2 строки, даёт
1.03–1.60×, удваивает время сборки). Чистая сборка на этой машине:
...
Опора: печатники, которые уже есть. `flang/self/emit-c.flang` — **3620 строк**
...
| **Сверка поведением** (§8) | опора: `scripts/wasm-sverka.mjs` = 250 строк, `flang/test/corpus-grid.mjs` = 253 | **300–600** |
| **ИТОГО** | | **5 350 – 9 200** |
...
Дефункционализация (`flang/src/defunc.mjs`, 831 строка, приём Рейнольдса 1972)
...
`flang/src/emit/c/flang_conc.c` — 2049 строк кооперативного однопоточного
планировщика (плюс `flang/src/conc.mjs`, 1666 строк на стороне эталона). Печать
...
**Что сверяется побайтово сегодня** (`flang/test/self-bootstrap.test.mjs`, 1327
...
Корпус для третьего уже есть: `flang/test/corpus-grid.mjs` (253 строки) —
...
`scripts/wasm-sverka.mjs` (250 строк), и это не главная статья. Главная — то,
