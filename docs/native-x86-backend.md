...
Замер снят 18 августа 2026 (`bb11b752`), а реализация на JavaScript снята из
дерева 20 августа 2026 (`fe8e8a37`): имена файлов ниже — про дерево того дня.

> **Сверка с живым деревом, 9 сентября 2026 (задача 6201).** Коэффициент 1,80×
> (строки печатника C на flang к строкам его на JavaScript) переснять нечем:
> знаменателя нет, а числитель вырос — `flang/self/emit-c.flang` 5 760 строк.
> Дефункционализация — `flang/self/defunc.flang` (2 458 строк), планировщик —
> `flang/self/conc.flang` (2 977) и живая исполняющая часть
...
| файл | строк |
|---|---:|
| `flang/src/emit/c/flang_repl.c` | 11 183 |
...
`docs/benchmark-speed.md`: даёт 1.03–1.60×, удваивает время сборки). Чистая
...
нём.** `docs/zamer/13-even.flang` — запись случая, когда «Чётное» от −4
...
молча разойтись со свидетелем, и она разойдётся, если писать перевод в десятичное
самостоятельно.**
...
**Что сверяется побайтово сегодня** (`flang/test/self-bootstrap.test.mjs`, 3019
...
**Это сверка печати с печатью, а не с чужим свидетелем** — и она для x86
сохраняется полностью. Неподвижная точка самоприменения работает с байтами ELF
ровно так же, как с текстом C: `x86 от свидетеля == x86 от flang₁ == x86 от
...
`flang/stdlib/*.flang` (13 файлов) плюс `docs/examples/leetcode/*.flang`
