...
Внутри `flang/`: [`flang/self/`](../flang/self) — компилятор, 68 файлов на flang —
...
`scripts/seed-fingerprint`, по хешированной строке на файл — 48 строк во входной половине; вместе со второй
половиной, телом семени, в файле 65 строк.
<!-- СНЯТО 2026-09-13 строк scripts/seed-fingerprint = 65 --> Сегодня семя отстаёт от
исходников — на три файла и 77 функций: `sh scripts/seed/chto-otstalo-ot-semeni.sh` перечисляет, какие
файлы и функции новее
семени, а перепечатка (`sh scripts/bootstrap-reprint.sh`, часы на одном ядре) — единственный путь, которым
