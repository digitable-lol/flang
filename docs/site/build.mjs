...
for (const с of страницы) {
  const { html, оглавление, ссылки } = разобрать(с.исходник);
  с.html = html;
...
for (const с of страницы) {
  for (const ссылка of с.ссылки) {
    ...
    // Ссылка на файл репозитория: проверяем от корня и от каталога документа.
    const варианты = [ссылка.цель, join('docs', ссылка.цель), join('docs', 'zettel', ссылка.цель)];
