...
`flang/examples/web/shortener/` — **2 153 строк**: 1 702 на flang в семи файлах
плюс 421 строка трёх хозяев на Node, которые читают байты из связи и отдают байты
обратно. Между входом и выходом нет ни одной строки не на flang.
...
**Семнадцать утверждений из двадцати четырёх легли сеткой, а не
доказательством,** и отчёт говорит это дословно: «сетка 1 значение (примеры
...
прошло, 0 упало**. Три примера с процессами установленный файл не гоняет вовсе;
с ними примеров 243, и столько же проходит у компилятора из репозитория.
