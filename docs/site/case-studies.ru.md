...
`flang/examples/web/shortener/` — **2 167 строки**: 1 743 на flang в семи файлах
плюс 419 строк трёх хозяев на Node, которые читают байты из связи и отдают байты
...
| `plan-network.flang` | 198 | та же служба через настоящий сокет |
...
**Два утверждения легли сеткой, а не доказательством,** и отчёт говорит это
