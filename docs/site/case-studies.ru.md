...
написанный для сайта, а код в дереве. Листинги и отчёты ниже сняты с этого же
дерева двоичным 0.7.17; модули примеров с 29 августа (коммит
`03f0359ab`) названы по-английски.
...
| точным шагом | спуск по `неотрицательное` ровно на единицу | ноль |
...
Задача, у которой есть не только доказанное завершение, но и **утверждения о
результате** — шесть постусловий `обеспечивает` у «Значения цифры» (два о
...
у `«Значение цифры»` три; постусловий шесть, и все закрыты **не примерами**.
Два о границах:
...
Так отвечает двоичный 0.7.17 11 сентября 2026: отчёта о доказательствах у этого
файла **нет** — прогон `--proof` обрывается нарушением постусловия самого
компилятора (`flang/self/zapis.flang`, «чужой заголовок не признаётся своим»),
код 1. Это ошибка компилятора, а не файла: `flang check` без `--proof` тот же
файл проходит кодом 0 и печатает `без доказанного завершения: «Шаг счастья»
«Счастливое»`. Замер 29 августа тем же ключом печатал отчёт о доказательствах: у двух обычных
функций стояло «обещания нет: функция обычная, о завершении не сказано ничего».
