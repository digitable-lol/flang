...
**Чего не доказано, и это видно из той же таблицы.** Постусловий на все
восемьдесят две задачи **два**, оба в задаче 13 «римские цифры в число»:
...
и есть спецификация, и восемьдесят две задачи показывают его размер числом.
...
`flang/examples/web/shortener/` — **1 732 строки на flang в семи файлах**.
Между разбором запроса и печатью ответа нет ни одной строки не на flang: байты из
связи носит исполнитель плана.

| Файл | Строк | Что там |
| --- | --- | --- |
| `service.flang` | 580 | исход, теоремы, маршрутизация, разбор и печать HTTP |
| `plan-durable.flang` | 381 | та же служба поверх журнала упреждающей записи |
| `server.flang` | 229 | процессы, надзор, три прогона |
| `plan-network.flang` | 201 | та же служба через настоящий сокет |
...
**Сегодня этот отчёт не печатается, и вот почему.** На прогоне 21 августа 2026
команда отвечает кодом 1: три примера функции `«Решить»` не сходятся — те, что
про путь, присланный браузером в процентах (`%D0%BB…`). Разбор процентов
возвращает «нет такого пути» там, где ожидался маршрут. Ведомость у программы с
замечаниями не печатается вовсе, поэтому числа выше — с того прогона, когда
примеры были зелёными, а не с сегодняшнего.

**Что здесь ценно.** Веб-служба, у которой **все 88 функций тотальны**, — это
...
**Семнадцать утверждений из двадцати пяти легли сеткой, а не доказательством,** и отчёт говорит это дословно: «сетка 1 значение (примеры
функции) … Это не доказательство — теоремы при утверждении нет». Про «тело
ответа не длиннее заявленного предела» известно ровно то, что на написанных
примерах нарушений не нашли, — и это и есть та граница, ради которой отчёт
разделяет «доказано» и «сетка».

Прогон примеров службы: `flang test …/server.flang` — **255 примеров, прошло
252, не прошло 3**, код возврата 1. Не проходят те же три примера про путь в
процентах. Это настоящий красный в дереве, а не оговорка страницы: пока он не
починен, служба разбирает не всякий путь, который присылает браузер.
...
- [Что доказано, а что нет](what-is-proved.html) — граница, проведённая числами
