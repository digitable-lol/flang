...
Тот же файл ниже объявляет ещё и `изоморфизм`, а его двоичный не судит
(см. следующие разделы), поэтому команда целиком отвечает кодом 2 и строкой
«проверено НЕ ДО КОНЦА» — прогон `bootstrap/flang` 0.7.17, коммит `2c40752d0`,
11 сентября 2026.
...
`docs/examples/monad/order-total.flang`. Замечания на этом файле сегодня есть,
...
компилятор не проверяет. Рядом — разборы отдельных кусков: `docs/archive/hof.md`, `docs/archive/poly.md`,
`docs/archive/monad.md`, `docs/ct/sets.md`, `docs/ct/zakony.md`.
