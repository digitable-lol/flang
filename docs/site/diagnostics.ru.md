...
Коды возврата: `0` — проверено, `1` — есть замечания, `2` — кривой вызов, `3` —
с `--proof`: утверждение объявлено, а доказательства при нём нет, `4` — часть
проверок не запускалась (`--быстро`) или всё доказано с опорой на сетку
(`--proof --строго`). Все выводы ниже сняты с двоичного 0.7.17 (11 сентября 2026).
...
«Двойку», которой в ввозящем нет. Столбец 56 указывает внутрь постусловия —
...
молча. Постусловие «не меньше двойки» из примера выше не доказано ничем —
без `--proof` файл зелёный, код 0; с `--proof` тот же файл уходит кодом 3.
