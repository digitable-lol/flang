...
Ни одной команды и ни одного числа здесь нет «по памяти»: всё прогнано на этом
дереве компилятором из репозитория (`node flang/bin/flang.mjs`) под Node v26.7.0,
`cc` 15.2.0, Python 3.14.4, Linux 7.0.0.
...
Всё на этой странице напечатано компилятором из репозитория — тем, что ставится
через npm. Отдельный двоичный `flang` — тот, что кладёт Homebrew и собирает
`bootstrap/` — умеет теперь **все {{цели.изСловом}}**, и говорит об этом сам. Прогон собранного из `bootstrap/`
двоичного:
...
Каталог `--out` двоичный, в отличие от компилятора из репозитория, не создаёт: не
...
[замер](wasm.html) сделан на всех программах репозитория.
...
браузере и там, где поток не завёлся, расчёт идёт как прежде, и сообщение о
пределе говорит правду.
...
`flang/src/compat.mjs`). Ответы совпадают БАЙТ В БАЙТ: сценарий выше прогнан
...
- **`flang/src/*.mjs` — не библиотека.** Внутренние модули компилятора
...
- **Установленный двоичный `flang` печатает только в C** и оставляет пустой
  таблицу типов на границе входа. Все {{цели.словом}} целей — у компилятора,
  который ставится через npm.
