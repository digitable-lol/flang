...
`flang` — тот, что ставит Homebrew и собирает `bootstrap/` — умеет **одну цель
из {{цели.изСловом}}**, и говорит об этом сам. Прогон собранного из `bootstrap/`
двоичного:
...
У него же цель `c` требует каталог с ИСХОДНИКАМИ рантайма — он ищет их сам в
