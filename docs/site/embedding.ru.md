...
Ни одной команды здесь нет «по памяти»: все прогнаны на этом дереве двоичным
`flang` — тем самым, что ставится из архива выпуска, — под `cc` 15.2.0,
Python 3.14.4, Linux 7.0.0. Числа рядом с командами сняты тем же прогоном; там,
где их нет, сказано, почему их не обещают.
...
Без файла и без цели команда отказывает и показывает форму вызова:
...
FLANG_TYPE в файле broken.flang, строка 6, столбец 48: левый операнд сравнения
 «gte» имеет тип строка: сравнения порядка допустимы только для чисел
…
...
### Где компилятор берёт исходники рантайма

Рядом с напечатанным модулем печать кладёт рантайм цели — значения, арифметику,
диагностики. Исходники его лежат в дереве (`flang/src/emit/<цель>/`), и печать
читает их с диска: сама она их не сочиняет. Ищет она их в `$FLANG_RUNTIME_DIR`,
затем в `../flang/src/emit/<цель>` и `../share/flang/<цель>`; собранному в
стороннем каталоге двоичному путь называют ключом `--runtime`.

Каталог `--out` печать не создаёт: не найдя его, она отказывает именем файла.

Про границу входа печать предупреждает отдельно, и это не мелочь:
...
То есть напечатанное соберётся и заработает, а вот аргументы прогонщика с
объявленными типами сверяться не будут. Полный разбор — ниже, в разделе про
прогонщик через трубу.
...
**У каждой цели печати представление значения своё**, и это не деталь: именно
оно и есть граница, через которую вы разговариваете с программой. Ниже — по
разделу на цель.
...
**Оговорка про соседний файл.** Рядом лежит `serve.mjs` — тот же сценарий, и
ходит он НЕ через напечатанный модуль, а через вычислитель второй реализации
языка. Реализации этой в дереве больше нет, и сценарий по этой дороге сегодня не
поднимается. Читайте `serve.mjs` как образец УКЛАДА хозяина — состояние
значением, цикл снаружи, — а не как рабочий путь: рабочий путь встраивания один,
и это `emit`.
...
- **Внутренности компилятора библиотекой не обещаны.** Дорога встраивания одна
  и обещана только она — `emit`. Всё остальное меняется без предупреждения; что
  именно [не обещано](../what-blocks-1-0.md), перечислено отдельно.
...
- **Таблица объявленных типов на границе входа не заполняется.** Печать говорит
  об этом сама и печатать не отказывается: напечатанное соберётся и заработает,
  а сверки аргументов прогонщика с объявленными типами не будет.
