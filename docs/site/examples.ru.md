...
Все примеры-программы на flang лежат в одном каталоге — `examples/`: 190 файлов в
...
`git ls-files examples | grep -c '\.flang$'` → 190 (замер 29 августа 2026).
...
| [`io/`](examples/io) | 8 | поручения ввода-вывода: двоичный файл туда и обратно, октеты по проводу, потомок, запрос по HTTPS, временный каталог, разбор пакета |
...
bootstrap/flang test examples/           # весь каталог, 190 файлов
```
