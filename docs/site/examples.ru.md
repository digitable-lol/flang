...
Все примеры-программы на flang лежат в одном каталоге — `docs/examples/`: <!-- СНЯТО 2026-09-17 файлов docs/examples/*.flang = 205 --> 205 файлов в
...
`git -c core.quotepath=false ls-files docs/examples | grep -c '\.flang$'` → <!-- СНЯТО 2026-09-17 файлов docs/examples/*.flang = 205 --> 205 (замер 6 сентября 2026).
...
| [`io/`](docs/examples/io) | <!-- СНЯТО 2026-09-19 файлов docs/examples/io/*.flang = 10 --> 10 | поручения ввода-вывода: двоичный файл туда и обратно, октеты по проводу, потомок, запрос по HTTPS, временный каталог, разбор пакета, полоса на экране терминала |
