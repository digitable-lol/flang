...
Всё, что стоит ниже в блоках вывода, — ответ настоящего прогона: двоичный 0.7.17,
собранный из `bootstrap/` на коммите `2c40752d0` (11 сентября 2026). Команды
копируются подряд.
...
Вход объявлен `неотрицательное`, а не `число`, и это не мелочь. Объявите `принимает н:
...
`неотрицательное`. Вторая — предусловие `требует «н не меньше нуля» н не меньше 0`, за него
платит вызывающий.
...
Типы аргументов `flang run` сверяет. Функция `«Факториал»` объявлена на `неотрицательное`, и
на −3 отвечает так:
...
что высказано и чем это несётся:
  постусловие «удвоенное не меньше исходного» функции «Удвоить» — доказано по объявленным
  типам аргументов: цель сведена правилом «порядок по построению» — утверждение обо ВСЕХ
  входах, а не о написанных; теоремы при нём нет и не нужно
```

Это постусловие ядро доказывает само, без теоремы: `н` неотрицательное, значит
`н плюс н` не меньше `н`. Последняя строка отчёта — `код возврата 0`. Так бывает
не всегда, и отчёт различает три слова:
...
напечатано файлов 6, байт 423153, в ./вывод
...
Первая строка — следствие отчёта выше: доказанное постусловие в C не
печатается, проверять его во время работы незачем. Строка про «версию для
Node» — устаревшее сообщение самого двоичного: реализации на Node нет с
20 августа 2026 (коммит `fe8e8a37`), полной проверки аргументов напечатанной
программы сегодня нет нигде.
...
```
напечатано файлов 7, байт 139047, в ./вывод-rust
…
