...
`io`, `lock`, `package`, `repl`, — и границы у него всё-таки есть. Хорошо в них
то, что двоичный называет их сам, а не молчит:
...
- **цель печати одна** — `c`. Целей у языка {{цели.словом}}
  ({{цели.список}}), остальные остались у реализации-эталона;
...
остальные цели — например, в Rust:
