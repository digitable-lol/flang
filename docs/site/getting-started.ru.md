...
- **категорной части языка и процессов двоичный не судит вовсе** — ни моноида,
  монады, изоморфизма, категории, множеств и пяти объявленных свойств (их законы
  считаются вычислением на сетке), ни связи двух модулей, бифунктора,
  преобразования, требования, процесса, надзора и прогона (их правила
  доказываются сличением объявлений). Ни одного из этих слоёв в нём нет.
  Программа с такими объявлениями получает отказ с поимённым списком того, чего
  двоичный не проверил, и код возврата 2 — и у `check`, и у `--proof`. Ответ
  «замечаний нет» на такой программе читался бы как «проверено», а это неправда;
...
- **`--args` берёт только плоский объект скаляров** — `[…]` и `{…}` двоичный не
  ...
  плоский объект скаляров, вроде '{"н":10}'` с кодом 2. Реализация-эталон берёт
