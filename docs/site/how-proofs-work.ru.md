...
flowchart TD
  ...
  F -->|не доказано| H[сторож: проверка во время работы]
...
**«Сторожа нет»** — ключевые слова. Сторож это проверка, которую компилятор
дописывает в собранную программу, когда доказательство не сошлось до конца. Тут
она не нужна, и её в коде не будет.
...
Причина сторожа названа прямо: `число` — это binary64, и при `н` больше 2⁵³
...
25 проверок во время работы = 24 недоказанных постусловия + 1 сторож
завершаемости. **42 проверки, которые в Python или Go пришлось бы либо писать
