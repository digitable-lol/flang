...
- **память не отдаётся до конца вызова**. Утечек нет вовсе — valgrind даёт ноль
  байт в ноль блоков, — но арена держит всё, что нажила: сортировка слиянием
  4000 чисел берёт **1655 МиБ** пика при живом результате в **125 КиБ**, это
  **13 558×** ([замер памяти](memory.html)).
