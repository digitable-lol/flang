...
отвечает компилятор: **6807 функций из 8775** его несут.
...
Цифры печатает ведомость доказательства (`flang check --proof`) по всему
корпусу, а не оценка. Каждая из них сверяется с этим прогоном сторожем чисел —
тем же, которым сверяется вся проза дерева, — поэтому протухнуть молча они
больше не могут: страница краснеет вместе с деревом.

| | |
|---|---:|
| Функций в корпусе | 8775 |
| Из них тотальных (завершаемость доказана) | 6807 |
| Утверждений о поведении высказано | 298 |
| Из них **доказано ядром** — про все входы | 170 |
| Аксиом в ядре | **0** |
...
- **язык медленнее Python в 1,4 раза, Node в 3,3, а написанный руками C быстрее
  ...
  **1–9 %, неотличимо от нуля** на фоне разброса между прогонами. Сторож
  остаётся редко: его несут **70 функций из 6807 тотальных — 1,0 %**
  (6 постоянным шагом, 64 объявленной мерой, 104 места; счёт печатает
  `flang check --proof`). Там он и правда дорог — **втрое дороже самой
