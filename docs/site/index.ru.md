...
отвечает компилятор: **6650 функций из 8595** его несут.
...
| Из них тотальных (завершаемость доказана) | 6650 |
| Утверждений о поведении высказано | 297 |
| Из них **доказано ядром** — про все входы | 168 |
| Аксиом в ядре | **0** |
...
- **язык медленнее Python в 1,4 раза, Node в 3,3, а написанный руками C быстрее
  ...
  **1–9 %, неотличимо от нуля** на фоне разброса между прогонами. Сторож
  остаётся редко: его несут **70 функций из 6650 тотальных — 1,1 %**
