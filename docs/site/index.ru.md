...
**Одна программа печатается в {{цели.словом}} языков, и поведение сверено байт в байт.**
...
- **язык медленнее Python в 1,4 раза, Node в 3,3, а написанный руками C быстрее
  ...
  где доказательство не оставляет проверки в работающей программе, оно стоит
  **1–9 %, неотличимо от нуля** на фоне разброса между прогонами. Проверка
  остаётся редко: её несут **{{сторож.функций}} функций из
  {{корпус.тотальных}} тотальных** — {{носители.постоянныйШаг}} постоянным
  шагом, {{носители.мера}} объявленной мерой, {{сторож.мест}} места. Там она и
  правда дорога — **втрое дороже самой
