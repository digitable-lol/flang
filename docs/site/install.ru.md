...
**Сегодня 0.5.1 этим путём не встаёт.** Опубликованный плагин ищет файл по
старому имени `flang_cli`, а сборка 0.5.1 кладёт `flang`: сборка проходит,
установка падает последним шагом. До выкладки починки через asdf встаёт 0.5.0.
