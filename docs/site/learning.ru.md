...
Читать — [Установка](install.html). Путей три — Homebrew, asdf, из исходников
(путь через npm убран 3 сентября 2026, коммит `5d876b0e2`); самый короткий —
одна команда Homebrew, без сборки исходников. Node не нужен ни на одном.
...
- [Как это доказывается](how-proofs-work.html) — механика на коде: чем несётся
  завершаемость, где снимается предусловие, что исчезает из собранного кода,
  когда постусловие доказано;
- [Зачем и как](proofs.html) — правила ядра, каждое читается целиком;
...
flang --help                   # тринадцать команд и что каждая делает
```
