...
проверены прогоном по сетке значений. На выборке из двадцати функций библиотеки
ядро доказывает хоть что-нибудь у 14, а содержательно — у 10: остальное
доказывается и при теле-заглушке, то есть говорит не про эту функцию, а про
любую с такой подписью. Число получают прогоном `./ярлык proof:20`.
