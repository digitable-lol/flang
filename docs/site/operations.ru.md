...
| «Высшего порядка» | `flang/stdlib/higher-order.flang` | 72 | 492 |
| «Словарь» | `flang/stdlib/dictionary.flang` | 9 | 206 |
| «Хеш-таблица» | `flang/stdlib/hashmap.flang` | 24 | 802 |
...
Нумерация с единицы — единственное место, где здесь легко ошибиться, поэтому
пример назван так, чтобы ошибка бросалась в глаза:
...
Сумма без повторов — две функции подряд, и обе тотальные:
...
Строка — не список: у неё свой модуль. Разобранная `«Разбить по символу»` даёт
**список строк**, и дальше работают функции модуля «Списки строк», а не
«Списки»:
...
не написать, и в этом смысл. `"/с/абв"` → `"абв"`.
...
Округление вверх пишется через целочисленное деление и сторожа на ноль — иначе
функция не была бы тотальной:
