...
**Запись и список записей** — объект с именами полей. Записи в
`operations.flang` нет, поэтому пример взят из другого файла дерева:
...
### Двоичный берёт только скаляры

Команда `flang` — это две разные программы под одним именем: полный
инструментарий из npm (`@digitable-lol/flang`, работает на Node) и двоичный файл
из `brew` и `asdf` (Node не нужен). Вывод выше снят с полного инструментария.

Двоичный берёт у `--args` **только плоский объект скаляров**: число, строку,
`true`, `false`, `null`. Массив и объект он не разбирает вовсе:
...
Отказ уходит в поток ошибок, код возврата 2.

Скаляры двоичный берёт и считает то же самое. Печатает он одно значение, без
обёртки:
...
Составное значение двоичному передают оболочкой: там оно пишется словами языка,
