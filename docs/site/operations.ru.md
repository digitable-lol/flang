...
Из этих примеров шесть написаны здесь, остальные 268 приехали вместе с
библиотекой: подключённый модуль несёт свои примеры, и они прогоняются вместе с
вашими. (Числа на этой странице сняты с двоичного 0.7.17 11 сентября 2026; счёт
функций библиотеки — по заголовкам `функция «…»` в файлах `flang/stdlib/`.)
...
| Модуль | Файл | Функций |
| --- | --- | --- |
| «Списки» | `flang/stdlib/lists.flang` | 38 |
| «Строки» | `flang/stdlib/strings.flang` | 39 |
| «Списки строк» | `flang/stdlib/strlists.flang` | 13 |
| «Множество строк» | `flang/stdlib/sets.flang` | 16 |
...
| «Словарь хешем» | `flang/stdlib/hashmap.flang` | 25 |
...
Отчёт в 0.7.17 считает только функции и утверждения самого файла; утверждения
подключённой библиотеки в его итог не входят. Слова «с опорой, и опора не
судилась» в последней строке значат ровно это: оба утверждения оперлись на сетку
примеров, и код возврата всё равно 0 — спросить строже можно ключом `--строго`,
тогда код будет 4. Написать постусловие легко, а получить под него
