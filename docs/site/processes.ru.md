...
Правила этих объявлений частью доказываются сличением объявлений, частью
считаются вычислением на сетке чередований, и в двоичный они не втащены.
Инструментарий, который их судил, был написан на JavaScript и из дерева удалён
вместе со второй реализацией языка; нового судьи для них пока нет. То же
касается и запуска настоящего узла: исполнителей узла в дереве четыре — на
JavaScript, Python, Go и C# — и каждый из них сегодня ищет удалённый модуль.

Что это значит для читателя прямо: **модель процессов сегодня можно объявить,
прочитать и прогнать по примерам, но не поднять узлом.** Всё остальное на этой
странице — не обещание, а то, что видно на прогоне.
