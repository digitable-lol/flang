...
(Вывод снят 11 сентября 2026, двоичный 0.7.17, коммит 2c40752d0.)

Код возврата 2 значит «проверено не до конца», а не «всё хорошо»: обработчики,
их типы и их примеры проверены, у объявлений `процесс` и `надзор` сверены имена
и типы сообщений, но достижимость отказа внутри тотального обработчика двоичный
не судит — об этом он и печатает. `flang test <файл>` прогоняет примеры обработчиков и выходит кодом 0.
...
напечатанном C, Elixir, JavaScript или TypeScript.
