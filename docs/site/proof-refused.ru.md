...
Отсюда вы узнаете, что означает каждый отказ ядра доказательств, и сможете по
коду отличить свою ошибку от границы языка. Отказов ровно {{отказы.всего}}, и
вот они все: `FLANG_PROOF_NO_GOAL`, `FLANG_PROOF_AMBIGUOUS`,
`FLANG_PROOF_DUPLICATE`, `FLANG_PROOF_CLAIM_MISMATCH`, `FLANG_PROOF_UNFINISHED`,
...
Первые {{отказы.обязательств}} говорят, что теорема не сошлась с функцией, и
чинятся правкой теоремы; остальные {{отказы.вывода}} — что вывод не построен, и
половина из них называет предел, за который язык сегодня не ходит. Все
тринадцать вызваны прогоном на маленьких программах: тексты сообщений ниже —
вывод компилятора, а не пересказ (сняты в августе 2026; `FLANG_PROOF_NO_GOAL`
и «код 0 без теоремы» перепроверены 11 сентября 2026 на 0.7.17 — совпадают).
...
Хвост сообщения читать обязательно, и не ради вежливости. У трёх отказов из
шести — `FLANG_PROOF_STEP`, `FLANG_PROOF_INDUCTION_STEP`,
`FLANG_PROOF_INDUCTION_BRANCH` — именно в хвосте написано, что ядру было
известно в этом месте (`известно: предусловие функции «Сумма»`) и какие способы
оно перебрало (`правил десять — …`). Это и есть ответ на вопрос «моё или не моё»:
...
flowchart TD
  ...
  D -->|правил десять<br>из ДВУХ форм тела<br>ровно н минус 1| F([граница ядра:<br>здесь язык не умеет])
  class G vyvod
  class E glavnoe
  class F otkaz
```
...
| `FLANG_PROOF_INDUCTION_DESCENT` | спуск по `неотрицательное` не строгий | граница: читается ровно `н минус 1`, и ничего кроме |

### `FLANG_PROOF_INDUCTION_TYPE`: индукции по числу нет

Тип обязан нести принцип индукции, и нести его могут только три вещи:
объявленная сумма (из вариантов), встроенный список (из двух образцов разбора)
и отрезок `неотрицательное` (из двух границ). Довод ядро печатает вместе с отказом:
...
Чтобы построить заключение случая, ядру нужно знать, чем оказывается
`результат` в этом случае, и читает оно это из тела функции:
...
У отрезка `неотрицательное` принцип держится тем, что цепочка `н, н−1, …` дна не
проскакивает, и ядро сверяет это в теле:
...
### `FLANG_PROOF_STEP`: десять видов цели, и одиннадцатый способ
...
Здесь стояло «правил пять». Список вырос до десяти — текст выше взят из
исходника ядра (`flang/self/proof-kernel.flang`, функция, печатающая этот
отказ, коммит `2c40752d0`, 11 сентября 2026); полный прогон, воспроизводящий
отказ дословно, заново не снимался. Вида цели не спрашивает только одиннадцатый
ход, «цель есть допущение»: он сличает цель с допущением знак в знак.

Читается так: если ваша цель не одного из этих десяти видов — доказать её ядру
...
Отказов у ядра доказательств больше нет: тринадцать названных в начале
страницы — это все. Коды про разбор, типы, имена и завершаемость перечислены в
