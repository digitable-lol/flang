...
вывод шаг за шагом, ничего не ища. В дереве языка таких теорем 160, из них 53 в
...
Одиннадцать решающих правил, и каждое можно прочесть целиком; ядро называет их
по именам в текстах отказов, и счёт берётся у самого ядра
(`grep -c 'тотальная функция «Правило' flang/self/proof-kernel.flang` → 11).
...
было на день написания. Сегодня их одиннадцать, и спорить с этим числом можно
