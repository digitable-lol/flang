...
export const РАЗДЕЛЫ = [
  ...
  },
  {
    ...
    // раньше не было, теперь есть: `node scripts/build-releases-page.mjs`.
    имя: 'Что изменилось', англ: 'What changed',
    страницы: [
      // `печатается` снимает требование английской пары ИЗ КАРТЫ: обе половины
      // печатает `node scripts/build-releases-page.mjs` из тегов и заметок
  ...
  },
  {
    ...
    страницы: [
      /* ПОРЯДОК ЗДЕСЬ — ОТ ТОГО, ЧТО ПИШУТ, К ТОМУ, ЧЕМ ЭТО ЗАКРЕПЛЕНО.
         ...
         «Язык». Она не введение: это контракт на 1677 строк, где рядом с формой
  ...
  {
    /* КОНТРАКТЫ ПОВЕРХНОСТЕЙ УЕХАЛИ ЗА ДВЕРЬ УЧАСТНИКОВ, и вот довод.
       Раздел «Категорная поверхность» стоял в меню новичка, а первая страница
       за ним первой же своей строкой говорит про себя: «Черновик на
       согласование». Черновику на витрине не место — и не потому, что он плох,
       а потому, что читатель приходит за тем, чем можно пользоваться, и не
       обязан отличать принятое решение от обсуждаемого.
       Второй документ раздела — контракт модели процессов: тоже рабочий текст,
       где рядом с правилом языка лежит разбор того, чего стоило решение.
       Ни один из них не выброшен и адреса не сменил. Читателю про то же самое
       теперь отвечают «Категорная поверхность» и «Процессы, надзор,
       распределённость» в разделе «Что язык умеет». */
    имя: 'Контракты поверхностей', англ: 'Surface contracts', дляУчастников: true,
    страницы: [
      { адрес: 'spec-cat.html', имя: 'Категории и функторы — черновик контракта', из: 'flang/cat/SPEC.md' },
      ...
      /* Спецификация языка стояла в разделе «Язык» последним пунктом и читалась
         как справка для пишущего. Это не справка: контракт на 1770 строк, где
  ...
  },
  {
    ...
    страницы: [
      ...
      // Улики к странице установки: хеши, размеры, времена сборки, вывод
      // скриптов плагина. Стоят ЗДЕСЬ, а не на самой странице установки, и это
      // решение владельца: «разрабу зачем это всё читать». Между читателем и
      // командой `brew install` отчёт отдела качества стоять не должен, а
      // выбросить его нельзя — иначе слово «прогнано» не на что опереть.
      { адрес: 'install-evidence.html', имя: 'Чем проверена установка', из: 'docs/site/install-evidence.ru.md' },
    ],
  },
  {
    /* РАЗДЕЛ ПЕРЕЕХАЛ СО ВТОРОГО МЕСТА В КОНЕЦ, и довод, стоявший здесь раньше
       ...
       по тегам. Теперь такая печать есть: `node scripts/build-releases-page.mjs`
    ...
    страницы: [
      ...
      { адрес: 'about-docs.html', имя: 'Как устроена документация', из: 'docs/site/about-docs.ru.md' },
