...
export const РАЗДЕЛЫ = [
  ...
  },
  {
    ...
    // раньше не было, теперь есть: `flang io scripts/stranitsa-vypuskov.flang`.
    имя: 'Что изменилось', англ: 'What changed',
    страницы: [
      // `печатается` снимает требование английской пары ИЗ КАРТЫ: обе половины
      // печатает `flang io scripts/stranitsa-vypuskov.flang` из тегов и заметок
  ...
  },
  {
    ...
    страницы: [
      ...
      /* Страница про ИИ стоит в «Доказательствах», а не в «Начале»: её предмет —
         не «как начать», а чем доказанное обещание отличается от уверенного тона
         помощника. Раньше про это на сайте не было ни строки, хотя служба для
         помощника в двоичном есть (`flang --mcp-mode`). */
      { адрес: 'dlya-ii.html', имя: 'flang и ИИ', из: 'docs/site/dlya-ii.ru.md' },
  ...
  },
  {
    ...
    страницы: [
      /* СПЕЦИФИКАЦИЯ ЯДРА ПЕРЕЕХАЛА СЮДА ИЗ РАЗДЕЛА «ДОКАЗАТЕЛЬСТВА», где стояла
         ...
         соседей по этому разделу: контракт на 4066 строк, где рядом с правилом
  ...
  },
  {
    /* РАЗДЕЛ ПЕРЕЕХАЛ СО ВТОРОГО МЕСТА В КОНЕЦ, и довод, стоявший здесь раньше
       ...
       по тегам. Теперь такая печать есть: `flang io scripts/stranitsa-vypuskov.flang`
