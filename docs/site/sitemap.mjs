...
export const РАЗДЕЛЫ = [
  ...
  },
  {
    ...
    // раньше не было, теперь есть: `flang io scripts/site/releases-page.fscript`.
    имя: 'Что изменилось', англ: 'What changed',
    страницы: [
      // `печатается` снимает требование английской пары ИЗ КАРТЫ: обе половины
      // печатает `flang io scripts/site/releases-page.fscript` из тегов и заметок
  ...
  },
  {
    ...
    страницы: [
      /* ПОРЯДОК ЗДЕСЬ — ОТ ТОГО, ЧТО ПИШУТ, К ТОМУ, ЧЕМ ЭТО ЗАКРЕПЛЕНО.
         ...
         «Язык». Она не введение: это контракт на 1749 строк, где рядом с формой
         СНЯТО 2026-09-14 строк docs/flang/SPEC.md = 1749
      ...
      /* КАТАЛОГ СПЕК СТОИТ СРАЗУ ЗА СПЕКАМИ, и это пара, а не повтор. Соседка
         выше отвечает «что такое спека и зачем она», на сквозном примере
         маркетплейса; эта — «как из спек собирается каталог и чем он
         сторожится»: правило приёмки, слепок обещаний, подлог, границы
         `flang check` и порядок добавления спеки. Обе половины были нужны и
         раньше, но вторая жила только в `fspec/README.md` — 713 строк
         по-русски в файле с английским именем, вчетверо подробнее страницы
         сайта, то есть источник правды стоял в каталоге, а на сайте был
         огрызок (задача 0037). README каталога — указатель сюда. */
      { адрес: 'spec-catalog.html', имя: 'Каталог спек: стенд, проверка и слепок', из: 'docs/site/spec-catalog.ru.md' },
  ...
  },
  {
    ...
    страницы: [
      ...
      /* ROSETTA CODE СТОИТ СРАЗУ ЗА КАТАЛОГОМ. Каталог называет набор одной
         строкой; эта страница говорит, что в нём доказано, какие функции не
         тотальны и почему, и чего в наборе нет по устройству языка. Разбор
      ...
      /* ТРИ ЧИСТЫХ АВТОМАТА БЕЗ ХОЗЯИНА, следом за двумя программами выше и тем
         же жанром: что вышло на живом коде, что доказано и чего не
         гарантируется. Журнал упреждающей записи, распределитель памяти и
         драйвер MSI — у всех троих граница доказанного проходит по одному
  ...
  },
  {
    ...
    страницы: [
      ...
      /* СТОИТ ВТОРОЙ, сразу за «что доказано». Та страница проводит границу,
         эта показывает механику по ту сторону границы — на коде и на выводе
         прогонов: чем несётся завершаемость, где снимается предусловие и что
         именно исчезает из напечатанного C, когда постусловие доказано.
         Объяснения этого не было ни на одной странице: «зачем и как» говорит,
         ЧЕМ доказательство отличается от теста, и ни строки о том, как это
         выглядит в исходнике и в собранном коде. */
      { адрес: 'how-proofs-work.html', имя: 'Как это доказывается', из: 'docs/site/how-proofs-work.ru.md' },
      /* СТОИТ ТРЕТЬЕЙ, сразу за «что доказано». Первый вопрос к доказуемому
      ...
      /* СТОИТ ЧЕТВЁРТЫМ, сразу за «ядро отказало». Та страница отвечает на крик
      ...
      /* СТОИТ ПЯТЫМ, сразу за «какими обещаниями ядро берёт». Та страница
  ...
  },
  {
    ...
    страницы: [
      /* СПЕЦИФИКАЦИЯ ЯДРА ПЕРЕЕХАЛА СЮДА ИЗ РАЗДЕЛА «ДОКАЗАТЕЛЬСТВА», где стояла
         ...
         соседей по этому разделу: контракт на 4814 строк, где рядом с правилом
         СНЯТО 2026-09-15 строк docs/flang/proof/SPEC.md = 4814
      ...
      { адрес: 'spec-proof.html', имя: 'Ядро доказательств — контракт', из: 'docs/flang/proof/SPEC.md' },
      { адрес: 'spec-cat.html', имя: 'Категории и функторы — контракт', из: 'docs/ct/spec.md' },
      /* ИСТОЧНИК ПЕРЕЕХАЛ, АДРЕС ОСТАЛСЯ. Зеркало документации свели с каталогом
         кода: `docs/flang/conc/` → `docs/flang/concurrency/` (код переименован
         коммитом `5ce665a12`, доки за ним не поехали и полгода стояли двумя
         каталогами про одно и то же). Адрес `spec-conc.html` НЕ тронут, и
         переезда под него не заведено — вот довод, а не вкус.
         Адрес и путь здесь разведены двумя полями нарочно: `адрес` — обещание
         чужой закладке, `из` — наше внутреннее дело. Сосед по этому же разделу
         давно живёт так же: `spec-cat.html` собирается из `docs/ct/spec.md`, и
         несовпадение краткой формы с именем каталога никто бедой не считает.
         Переезд — это плата за поломку, которую мы причинили читателю ради
         чего-то; здесь читателю нечего дать: адрес не транслитный, не врущий и
         не длинный. Завести 134-й переезд значило бы сломать внешнюю ссылку
         своей рукой и тут же продать читателю пластырь. Правило шапки ПЕРЕЕЗДОВ
         («новая страница переезда не заводит») держится ровно на этом. */
      { адрес: 'spec-conc.html', имя: 'Процессы и отказоустойчивость — контракт', из: 'docs/flang/concurrency/SPEC.md' },
      /* Спецификация языка стояла в разделе «Язык» последним пунктом и читалась
         как справка для пишущего. Это не справка: контракт на 1749 строк, где
         СНЯТО 2026-09-14 строк docs/flang/SPEC.md = 1749
         рядом с формой языка стоят коды отказа, границы обещаний и раздел «Слои
         реализации» с прямой пометкой «для тех, кто развивает сам язык». Жанр
         рабочего документа, а не витрины; на витрине его место заняли
         «Справочник конструкций» и «Операции языка», которые ровно для читателя
         и написаны. */
      { адрес: 'spec.html', имя: 'Спецификация языка — контракт', из: 'docs/flang/SPEC.md' },
      /* САМОПРИМЕНЕНИЕ — четвёртый контракт того же жанра: состав `flang/self/`
         (файлы, модули, роли), точка входа, неподвижная точка раскрутки и
         долги — то, что нужно тому, кто правит компилятор, а не пишет на языке.
         Читателю про круг раскрутки отвечает `bootstrap-circle.html`. */
      { адрес: 'spec-self.html', имя: 'Самоприменение: компилятор на flang — контракт', из: 'docs/flang/self/SPEC.md' },
    ],
  },
  {
  ...
  },
  {
    /* РАЗДЕЛ ПЕРЕЕХАЛ СО ВТОРОГО МЕСТА В КОНЕЦ, и довод, стоявший здесь раньше
       ...
       по тегам. Теперь такая печать есть: `flang io scripts/site/releases-page.fscript`
    ...
    страницы: [
      ...
      // затрёт первый же `node scripts/site/build-changelog-page.mjs`. Освобождение,
