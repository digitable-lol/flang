...
Каждая программа ниже показана целиком: скопируйте её в файл, добавьте первой
строкой `модуль «Учебник»` и прогоните `flang check` и `flang test`.
...
**Теперь вы умеете:** объявить функцию с типами и примером, проверить её и
...
Причины сообщение не называет, показывает на конец строки. Занятые имена перечислены в [словаре](glossary.html) —
{{словарь.понятий}} понятий.
...
**Теперь вы умеете:** обойти список свёрткой — суммой, наибольшим, любым
накоплением — без единого цикла.
...
**Теперь вы умеете:** выбрать форму тела так, чтобы завершение доказалось, и
знать цену выбора.
...
После неё `flang check` отвечает «замечаний нет», а отчёт `flang check --proof`
называет, на чём обещание держится:
...
«Сторож, 1 место» в этом ответе — проверка, которая остаётся в работающей
программе: числа здесь машинные, и на очень больших значениях вычитание единицы
число уже не меняет. Компилятор об этом не молчит.

**Упражнение 3.** Объявите вход `неотрицательное` вместо `число` — проверка проходит и без
правки условия, потому что отрицательных входов в этом типе нет. Границу тогда
проверяет вызов:
...
**Теперь вы умеете:** прочитать отказ `FLANG_NOT_TOTAL` и починить рекурсию
ровно так, как он просит.
...
**Теперь вы умеете:** объявить свой тип из вариантов и разобрать его так, что
забытый случай не доживёт до запуска.
...
Само по себе оно доказательством не становится, и отчёт говорит, на чём оно
держится на самом деле:
...
утверждение станет ложным (на −1 результат равен −2), и ядро откажет дважды.
Сначала оно называет ложь прямо, с контрпримером — у типа `число` есть ещё и
«не число», которое ни больше, ни меньше нуля:
...
А следом отказывает и теореме, которой не на что опереться:
...
на 3`. Ответ: та же строка `требует` и те же четыре строки теоремы; отчёт
ответит «доказано: терм принят ядром, 1 шаг».

**Теперь вы умеете:** довести утверждение от «сетки» до «доказано» — индукцией
по объявленному типу или предусловием как фактом.

## Что говорит компилятор, когда вы ошиблись

Все сообщения ниже сняты прогоном `flang check` на маленьких программах
(перепроверены: прогон 11 сентября 2026, двоичный 0.7.17, коммит 2c40752d0).
...
Все отказы ядра доказательств — а `FLANG_PROOF_INDUCTION_STEP` один из них —
разобраны поимённо на странице
[Ядро отказало: чья это ошибка](proof-refused.html): там у каждого сказано,
правится он теоремой или упирается в предел языка. Остальные коды компилятора
перечислены на странице руководства (`man flang`), раздел ДИАГНОСТИКА.
