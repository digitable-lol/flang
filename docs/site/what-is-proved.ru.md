...
вправе зациклиться. Поэтому три функции цикла объявлены обычными намеренно —
`«Прогон»`, `«Виток»` и `«Дальше после шага»` в `flang/self/interpret.flang`.
Защита у них не доказательство, а предел витков: упёршись в него, вычислитель
честно отвечает `FLANG_RECURSION_LIMIT`.

Объяви их тотальными — и обещание языка стало бы неправдой на первой же
зациклившейся программе. На это смотрит отдельная проверка, так что тихо
поменять признак нельзя.

Обычных функций в репозитории — {{корпус.обычных}}. Три названные те, у
которых обычность не долг, а свойство задачи; остальные — долг.
