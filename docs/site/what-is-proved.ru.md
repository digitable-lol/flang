...
`2c40752d0`, около 13 секунд) отвечает **ДОКАЗУЕМ** и печатает четыре проверки
числами: переиграно **625 обязательств из 651 (96,01 %)**; набор подделок 33 из
33; проб на подлог 401, ни одна не принята; честных записей 197, ни одна не
отвергнута.
...
ядром (заметка к выпуску 0.7.17 в `docs/release-notes.json`; отчёт —
