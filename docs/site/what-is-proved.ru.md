...
`1218aa186`, около 13 секунд) отвечает **ДОКАЗУЕМ** и печатает четыре проверки
числами: переиграно **629 обязательств из 651 (96,62 %)**; набор подделок 34 из
34; проб на подлог 453, ни одна не принята; честных записей 215, ни одна не
отвергнута.

Доля от выпуска 0.7.19 не выросла: 625 → 629 она поднялась в 0.7.18, а
перепечатка семени дала ядру новые умения, а не прирост доли. Мест, где сверщик
верит ядру на слово, по-прежнему 12 посылок и 4 шага.

Читать 96 % надо точно: это доля мест **в собственном доказательстве
компилятора** (набор записей корпуса), где независимый сверщик переиграл ход, а
не «96 % программ доказаны». Правила вывода, которыми пользуется сверщик,
проверены ещё и чужим судьёй: 88 правил переведены в леммы Lean 4 и приняты его
ядром. Тот прогон датирован 11 сентября; с тех пор список вырос до 97
правил, и заново Lean не гоняли — его нет на этой машине (отчёт —
`docs/lean-checks-the-inference-rules.md`).
