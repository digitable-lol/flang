...
> исходникам за девять секунд и сверяются на каждый пуш
> (`sh scripts/guards/published-vs-tree.sh --числа`). Всё остальное — завершение,
> носители, места проверок в напечатанном коде, утверждения о поведении —
> печатает компилятор прогоном по всему корпусу (часы), и печатал он это
> **23 августа 2026** (коммит `252606e8`). С тех пор эти числа не пересняты.
>
> На день съёмки компилятор был собран из семени, отставшего от исходников; семя
> с тех пор перепечатано (10–11 сентября 2026, коммит `0ce948bfd`; `sh
> scripts/seed/what-lags-the-seed.sh` 11 сентября называет 3 отставших файла, 77
> функций). Пересъёмка дорогих чисел ещё не сделана; та же команда
> `published-vs-tree.sh --числа` печатает, сколько файлов `.flang` сдвинулось с
> `252606e8`.
...
Это второй конец того же сторожа — план `'Подделки остаются недоказанными'`, — и
он сегодня красный (прогон 11 сентября 2026 на 0.7.17, код возврата 1). Причина
не в том, что ядро взяло ложь: на подделке
`flang/test/fixtures/poddelka-order-arithmetic.flang` компилятор отвечает кодом 3
(«объявлено, не доказано 5»), а план ждёт другого кода и считает подделку
непроверенной. Набор подделок независимого сверщика — другой, и он зелёный,
см. следующий раздел.
...
### Доказательство переигрывает независимый сверщик

Слову «доказано» в отчёте компилятора верить на слово не нужно. `flang check
--proof --записать <файл>` кладёт само доказательство в файл, а отдельная
программа на C — `flang/proof/checker/checker.c`, — которая компилятора не видела,
берёт исходник и запись и переигрывает каждый ход заново. Прогон
`sh scripts/доказуемость.sh` 19 сентября 2026 на стволе (коммит
`a5609e322`, около трёх минут) отвечает **ДОКАЗУЕМ** и печатает четыре проверки
числами: переиграно **650 обязательств из 650 (100,00 %)**, недостижимых мест
вынесено 27; набор подделок 36 из 36; проб на подлог 533, ни одна не принята
кодом 0; честных записей 245, ни одна не отвергнута.

До 100 % доля дошла 18 сентября 2026 (задача 3348): 633 → 650 из 650. Мест, где
сверщик верил ядру на слово, не осталось ни одного — прежде их было 12 посылок и
4 шага.

Читать 100 % надо точно: это доля мест **в собственном доказательстве
компилятора** (набор записей корпуса), где независимый сверщик переиграл ход, а
не «все программы доказаны». Правила вывода, которыми пользуется сверщик,
проверены ещё и чужим судьёй: все 109 правил переведены в леммы Lean 4 и приняты
его ядром — прогон на коммите `a5609e322`: расхождений вердиктов 0, ловушек 52 и
все 52 отвергнуты, ни одного `sorry`, `axiom` или `native_decide`, `#print
axioms` на состоятельности даёт ровно три стандартных. Подробный отчёт
(`docs/lean-checks-the-inference-rules.md`) отстал: числа в нём сняты
11 сентября.
...
Код возврата у `--proof` с недоказанным утверждением — 3, а не 0 (прогон
11 сентября 2026, 0.7.17); без `--proof` тот же файл проходит кодом 0.
...
её не содержит (`grep -rl Вечно flang/test/fixtures/` пуст, 11 сентября 2026).
...
[[two-termination-rules-together-give-574]] называет для размерных графов
...
### Компилятор не проверяет собственные исходники целиком

На замере 23 августа 2026 `flang check` на исходниках компилятора упирался в
предел шагов и останавливался: `FLANG_RECURSION_LIMIT`. Разбор и связывание при
этом проходили целиком — 29 файлов вместе с импортами, ошибок ввоза ноль, — а
кончался запас на прогоне примеров.

Здесь стояло: «ключа, поднимающего предел, у `check` нет». Сегодня это не так:
`flang check --help` в 0.7.17 называет `--предел-шагов N` (латиницей
`--step-limit`) и `--предел-глубины N`; умолчание вшито в двоичный
(`FL_MAX_STEPS` в `bootstrap/flang_runtime.h`, 1 400 000 000 000) и участвует в
сверке при самосборке. Прошёл бы сегодня `check` по всем исходникам компилятора
до конца — не проверялось: прогон стоит часы. Что проверено, — то, что
доказательство компилятора переигрывает независимый сверщик (раздел выше).

Из {{корпус.файлов}} файлов отчёт вышел у 244 (замер 23 августа 2026). Оставшиеся
названы поимённо, и это три разные вещи:
