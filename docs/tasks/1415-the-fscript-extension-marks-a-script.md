...
статус: свободна (Ш1 сделан, списка два и сторож зелен; Ш2–Ш3 не закончены — пять мест ждут своей работы, умение скрипта прогоном не показано)
исполнитель: —
ветка: —
команда: вторая
карта: Куда идём
...
* `flang/self/bootstrap/emit-from-source.flang` — единственный под `flang/self`;
  в замыкание печати (`scripts/seed-fingerprint`) не входит (0 вхождений),
...
| 31 | `scripts/bootstrap-reprint.sh` (ждёт) | `find flang \( -name '*.flang' -o '*.fp' -o '*.фп' \)` (1088) — замыкание печати | О | правится той же работой, что перепечатывает семя |
...
Приём задачи — «`./ярлык расширения:проверка` зелен; `.fscript` назван в списке; У СКРИПТА
ЕСТЬ УМЕНИЕ, КОТОРОГО НЕТ У ПРОГРАММЫ, И ОНО ПОКАЗАНО ПРОГОНОМ». Прогон 22 сентября 2026
на стволе `d0763e8b6` — две трети приёма закрыты:

```
./ярлык расширения:проверка
  → расширения сведены: СПИСКА ДВА (языка: .flang, .fp, .фп, .фланг; СКРИПТА: .FSCRIPT),
    мест отбора 45 — все называют оба целиком; ЖДУТ СВОЕЙ РАБОТЫ 5, и каждое названо
    в «Осталось»; перепись дерева не нашла мест вне обоих списков        код 0
```

Порог снят: задача 4143 сделана, и `flang io` берёт доводы (`-- довод…`, см.
`flang io --help`). Но третьего условия приёма — прогона, показывающего умение, которого
нет у программы, — в задаче нет, и пять мест отбора ещё ждут работы. Не сделана; ветка
`a/1415-fscript-extension` на сервере не живёт.
