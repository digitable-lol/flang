...
статус: свободна (на 22 сентября 2026 кириллических имён вне flang/proof — 63, приём ждёт 0)
исполнитель: —
ветка: —
команда: вторая
...
Из 647 внутри `flang/proof`: 607 — в `flang/proof/checker/tests/`, 40 — прочие.
...
неотслеживаемые файлы (например, собранный `flang/proof/checker/сверщик`), —
...
подстрокой, и у общих имён (`проба.flang`, `README.md`, `ярлык`, который сидит и
...
* файлы в каталогах с кириллическим или транслитным именем (`применение/`,
  `спецификации/`, `checker/`, `fts-naslediye/`) — каталоги в 1419 не входят,
...
| 18 | `docs/flang/proof/checker/README.md` | README.md | 18 | каталог «чекер» кириллический — не 1419 |
| 19 | `docs/flang/proof/checker/tests/records/why-comments-here.md` | WHY-THERE-ARE-COMMENTS.md | 1 |  |
...
```
git -c core.quotepath=false ls-files | grep -P '[А-Яа-яЁё][^/]*$' \
  | grep -v '^flang/proof/' | wc -l      → 63   (приём ждёт 0; на день замера 61)
```

Не сделана; ветка `a/1419-cyrillic-names-outside-proof` на сервере не живёт.
