...
статус: свободна (часть 2 взята 17 сентября; 23 сентября 2026 выплачены два последних имени печати — ветка a/seed-scripts-english, ждёт ближайшей перепечатки семени)
исполнитель: —
ветка: —
команда: первая
карта: Куда идём
...
## 23 сентября 2026: два имени печати переименованы, ветка ждёт печати

Владелец потребовал снять оба транслитных имени сейчас, не дожидаясь партии №2.
Сделано на ветке `a/seed-scripts-english` от `38105062f` — всё, кроме самой печати:

- `git mv scripts/raskrutka.sh scripts/bootstrap-reprint.sh`,
  `git mv scripts/otpechatok-semeni scripts/seed-fingerprint`;
- ссылки переписаны в **233 файлах** (`.github/workflows/**`, `ярлык`,
  `ярлыки.flang`, `scripts/**`, `flang/**`, `docs/**`, `.gitattributes`,
  `packaging/flang.1`) — вместе с тремя местами внутри печатаемой части:
  `flang/self/bootstrap/compiler.flang:83` и `:1157` (комментарии),
  `flang/self/cli.flang:202` (строка справки), плюс восемь строк в
  `flang/src/emit/c/flang_repl.c` и `flang_runtime.c`;
- две строки сняты из `scripts/ledgers/translit-file-names-debt.txt`, шапка
  пересчитана прогоном: в области 2160 латинских имён, с чужими словами 105 —
  столько же строк-имён в ведомости (было «здесь 67», счёт от 17 сентября,
  устаревший ещё и приходом `flang/proof` 22 сентября);
- `sh scripts/guards/translit-file-names-guard.sh --check` — код 0,
  `--подлог` — код 1.

Цена названа заранее и заплачена сознательно: правка `flang/self/**` отбрасывает
семя назад, и до перепечатки красны заслоны семени (`semya`, `semyaosv` в
`.githooks/pre-push`). Ветка готова; печать — отдельным шагом.

**Не сделано нарочно:** `bootstrap/README.md` (строки 67, 96, 97, 98, 187) всё
ещё зовёт `scripts/raskrutka.sh`. Этот файл НЕ входит в тело семени (в нём семь
файлов: `Makefile`, `compiler_flang.{c,h}`, `flang_cli.c`, `flang_repl.c`,
`flang_runtime.{c,h}`), то есть перепечатка его не поправит — правка нужна
отдельным решением.
