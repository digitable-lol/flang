...
Замером: задача — замер, а не починка. Коммиты ветки `a/1402-printed-c-under-compcert`:

- `af8bd37d` — «docs(задачник): заведены 3467 (CompCert как выбираемый компилятор
  напечатанного C) и 3984 (имя модуля с цифры ломает печать C)»;
- `21be5f0a` — «docs(1402): напечатанный C собирается CompCert — 253 из 255 программ
  дерева и сам компилятор, ответы с cc совпадают байт в байт».

SHA здесь — ветки. Ствол вливает PR перебазированием и даёт коммитам новые SHA (так
было с 1421: коммит ветки `68b6de1c` в `main` стал `09fcccd2`), поэтому коммиты
названы и заголовками.

Число, которого ждала задача: **собирается — 253 программы из 255 напечатанных (302 в
дереве) и сам компилятор; мешают четыре имени рантайма и пять ключей ccomp, печатник
ни при чём** (задача 3467). Ошибка печатника одной породы, с CompCert не связанная, —
3984. Расхождений вывода cc и ccomp — 0 на 68 462 ответах (11 475 примеров и 56 987
граничных вызовов). ADR-0030 §9 записан числом, заметка —
`docs/zettel/portable-c99-was-only-ever-checked-by-gnu-compilers.md`.

Открытым остаётся то, чего задача и не обещала: разрыв flang → C (1401) и проход
`cparser/StructPassing.ml` вне теоремы CompCert («Чего это не даёт», 3467).
