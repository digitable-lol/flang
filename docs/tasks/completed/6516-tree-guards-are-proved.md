...
предела в `scripts/bootstrap-reprint.sh`, либо удешевлением обхода записей в ядре.
...
**Вердикт ядра снят прогоном 27 августа 2026** (`PAMYAT=45G … check
flang/scripts/proven-guard.flang --proof`, код 0):

```
утверждений 13: доказано 12 (из них без теоремы 12), сетка 0, объявлено, не доказано 1
  постусловие «длина имени файла пола» функции «Файл пола» — объявлено, не доказано:
  ни теоремы, ни примеров. Его считает рантайм после каждого возврата
```

**Единственное недоказанное утверждение файла — ровно то самое.** Остальные
двенадцать доказаны все, включая соседнее «длина пути к двоичному» (правило
«тождество после переписки допущением») — то есть ядро эту форму берёт, и
отказало оно не по бессилию. Цель замкнута, свободных имён в ней нет; не
доказать замкнутую цель ядро может ровно в одном случае — если она ложна.
