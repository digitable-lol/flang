...
- [Цель, написанная через «не равен», ядру не видна: то же утверждение через «не (… равен …)» снимается разбором случаев — 12 обязательств из 24 на одном файле](a-not-equal-goal-is-invisible-to-the-case-split-rule.md)
- [Предусловие судится по месту вызова, а не по строке, — поэтому приговора у строки `требует` не бывает никогда, а у пяти строк `закон` его нет только сегодня](a-precondition-is-judged-at-the-call-site-so-its-line-can-never-get-a-verdict.md)
- [Предусловие, записанное вызовом своей функции, не снимается НИ НА ОДНОМ вызове — включая тот, где оно очевидно истинно](a-predicate-precondition-is-never-discharged-not-even-on-literals.md)
...
- [Даровое утверждение узнаётся подменой тела заглушкой, и в двух модулях библиотеки таких оказалось 17 из 32](a-free-statement-is-exposed-by-replacing-the-body-with-a-stub.md)
- [Дизъюнкция в допущении разбирается СЛУЧАЯМИ, а расщеплять её нельзя ни в какую сторону](a-disjunction-in-a-hypothesis-is-split-by-cases-not-torn-apart.md)
- [Два правила завершаемости поодиночке дают 54 и 74, а вместе — 574: узкое место в их произведении](two-termination-rules-together-give-574.md)
...
- [Индукция по строке закрыла ОДНО утверждение на весь корпус, а не сотню: за стеной стоит вторая](string-induction-closed-one-statement-not-a-hundred.md)
...
- [Три файла компилятора обещают 788 раз, недоказанных 220, и треть из них держит ОДИН ход — деление цели по `разбор`](one-move-holds-a-third-of-what-three-compiler-files-promise.md)
...
- [Рефлексивность и цель-выбор вместе закрыли ноль из двадцати, и не хватило им двух названных шагов](reflexivity-and-goal-choice-together-give-zero.md)
...
- [Три факта о длине строки дали три утверждения на весь корпус, и самым дорогим из трёх был не факт, а место, где факт читается](three-length-facts-gave-three-statements.md)
...
- [Рост встроенной суммы «Отклик» не красит старый разбор `случай`ами, пока семя не перепечатано, и единственный обход, который бы это поймал, идёт только по тегу](a-grown-builtin-sum-stays-silent-until-the-seed-reprints.md)
...
- [Оценка витков и счётчик шагов рантайма считают разное под одним словом](оценка-витков-и-счётчик-шагов-рантайма-считают-разное-под-одним-словом.md)
- [Постусловие о сбое доказуемо для конкретного отклика и не пишется для всякого](постусловие-о-сбое-доказуемо-для-конкретного-отклика-и-не-пишется-для-всякого.md)
...
- [Цикл поручений принадлежит хозяину, а не языку: `flang io` в двоичном стоил 1289 строк C и трёх точек на flang](the-command-loop-belongs-to-the-host-not-the-language.md)
...
- [Закоммиченный в `main` двоичный отстал от исходников того же коммита: им не разбирается `stdlib/lists.flang`](the-binary-in-main-lagged-its-own-sources.md)
- [flang-близнец сторожа столкновений зеленел на трёх настоящих столкновениях](the-flang-twin-of-the-collision-guard-stayed-green-on-three-real-collisions.md)
...
- [Измеритель числа умирает раньше, чем само число в прозе, и тогда честного способа обновить страницу не остаётся](the-measurer-dies-before-the-number-in-prose.md)
...
- [Node ушёл с пути сборки: точку раскрутки печатает сам двоичный, и обе печати совпали байт в байт](node-left-the-build-path.md)
...
- [После удаления реализации на JavaScript набор проб отдаёт ноль, а не «почти всё»](after-the-second-implementation-is-deleted-the-test-suite-returns-zero.md)
...
- [Сторож столкновений имён не знал о вариантах суммы и молчал о шести настоящих столкновениях](the-collision-guard-did-not-know-sum-variants.md)
...
- [Ведомость двоичного бывает слабее ведомости на Node и никогда не сильнее — и сверять её надо в одну сторону, а не побайтово](the-binary-ledger-can-be-weaker-and-never-stronger.md)
- [Двоичный судится по именам из его таблицы символов, а не по размеру и строкам: 80 байт разницы прятали код двух невлитых веток](the-binary-is-judged-by-its-symbol-names-not-by-its-size-or-strings.md)
- [Двоичный для ведомости, собранный рядом с деревом, не видит его библиотеку и подаёт это как долг автора](a-ledger-binary-beside-the-tree-cannot-see-the-library-and-blames-the-author.md)
- [Примета приёмки, которой не досталось материала, обязана сказать «не снята», а не «сошлась»](an-acceptance-sign-without-material-must-say-not-taken-not-passed.md)
- [Счётчик шагов печати обнуляется на каждой стадии, поэтому её ход читается по отметкам времени, а не по процентам](the-reprint-step-counter-resets-per-stage-so-progress-is-read-from-tick-marks.md)
- [Ведомость доказательств при отставшем семени показывает дерево, которого нет, — поэтому сторож блокирует, а не советует](a-proof-ledger-taken-with-a-stale-seed-shows-a-tree-that-does-not-exist.md)
- [Два журнала рядом с деревом печати — это неизвестность, а не «взять посвежее»: так отказ печати прочитали как успех](two-logs-beside-a-print-tree-mean-unknown-not-take-the-newer.md)
- [Отставание от семени определяется графом импортов, а не долей отсутствующих имён: порог 80 % отнёс две трети «отставания» не туда](lag-behind-the-seed-is-told-by-the-import-graph-not-by-a-share-of-missing-names.md)
...
- [Переезд каталога поиск по полному пути не ловит: ссылка вида «../каталог» переживает его молча](a-relative-path-outlives-a-directory-move.md)
- [Цель ярлыка, объявленная в самом скрипте, упирается не в язык, а в храповики дерева](a-shortcut-declared-in-its-script-hits-the-tree-ratchets-not-the-language.md)
...
- [Выпуск был замкнут сам на себя: набор тестов требовал релизного архива, а собрать его прогону было нечем](the-release-was-closed-on-itself-the-suite-needed-an-archive-nobody-built.md)
- [Версия пишется один раз в источнике и разносится одной командой, а sha256 формулы из числа не выводится](the-version-is-written-once-and-spread-by-one-command-but-sha256-is-not-derived-from-it.md)
- [У столкновений имён три судьи, и третий молчит, пока первое столкновение не разведут переименованием](name-collisions-have-three-judges-and-the-third-is-silent-until-you-rename.md)
...
- [Прогонщик корпуса написан на flang целиком; на C осталось 59 строк невыразимого и 419 строк перевозки](the-corpus-runner-is-written-in-flang-except-for-effects.md)
...
- [«Показать» у двоичного отвечает отказом, а прогон продолжается — прежний замер устарел](show-answers-with-a-refusal-and-does-not-crash-the-run.md)
- [Проверку, которая зовёт компилятор, а не разбирает `.flang` сама, переносить на flang почти нечего — мешает только чтение ответа](a-check-that-calls-the-compiler-ports-to-flang-mechanically.md)
...
- [Перепечатанное семя может нести квантор, которым не может воспользоваться ни одна программа: разбор, ядро, печать и проверяющая программа есть, а типизатор — нет](a-reprinted-seed-can-carry-a-quantifier-that-no-program-can-use.md)
...
- [Четыре вещи, которых в `обеспечивает` написать нельзя, и одна из них ВЕШАЕТ проверку вместо отказа](four-things-a-postcondition-cannot-say.md)
...
- [У строки flang БЫЛО две меры, и делило формы между ними представление строки у цели печати, а не язык](two-string-measures-divided-the-builtin-forms.md)
...
- [Файл проверок, упавший на загрузке, в отчёте прогона неотличим от отсутствующего](a-test-file-that-failed-to-load-looks-absent-in-the-report.md)
...
- [Печать плана обещана наизнанку: семь целей молча забывают план, восьмая отказывает не про него](plan-printing-is-promised-inside-out-and-nothing-verifies-it.md)
...
- [Веер оснастки считается по ядрам, а кончается память — и правило «не больше двух прогонов» тут не помогает](the-harness-fan-is-sized-by-cores-but-memory-runs-out-first.md)
- [Ворота пропускают код команды насквозь: один код 75 не отличает отказ ворот от команды, вернувшей 75](the-gate-passes-the-command-exit-code-through-so-75-alone-proves-nothing.md)
- [Список замыкания, взятый грепом, сторожит форму записи ввоза, а не ввоз, — и пустая база при этом читается как «столкновений нет»](a-closure-list-taken-by-grep-guards-the-spelling-of-the-import-not-the-import.md)
- [Перепись целей без двоичного печатает нули и выходит кодом 0 — поэтому отсутствие прибора это «не проверено»](a-census-without-the-binary-prints-zeros-and-exits-zero.md)
- [Отказ ядра «у цели нет вида» на ограничении `таких что` был не нехваткой правила, а нераспакованной обёрткой: два места из трёх распаковывали, третье — нет](such-that-was-an-unpacked-wrapper-not-a-missing-rule.md)
...
- [Кешировать доказательство сегодня дороже, чем доказать заново: 111 мс против 145 мс на всём корпусе](caching-a-proof-costs-more-than-proving-it.md)
...
- [Законы годятся указателем, а не выводом — но в стандартной библиотеке их объявлено ноль](laws-as-a-pointer-not-a-conclusion.md)
...
- [Уроки проводника нельзя класть в `share/flang`: обе упаковки считают там каталоги и сверяют их число с числом целей печати](a-lesson-directory-under-share-flang-would-break-the-target-count-in-two-packagers.md)
...
- [Потерянный батут стоит секунд и байтов, а не только кадров: размена не было вовсе, выигрыш ноль по всем трём осям](a-lost-trampoline-costs-seconds-and-bytes-not-only-frames.md)
...
- [Символьный режим `tar --mode=u=rw,go=r,a+X` СНИМАЕТ бит исполнимости, ради которого в него и вписан `a+X`](a-symbolic-tar-mode-strips-the-execute-bit-it-promises-to-keep.md)
...
- [Проверка, знающая только один целевой язык, на общей дороге ломает ВСЕ программы: «Static» перестала проходить `flang check`, хотя её печатают в Go](a-target-specific-check-on-the-general-path-breaks-every-program.md)
...
- [Числа сайта тухнут той же правкой, что и словарь, а лечатся одной командой — которая под нагрузкой не успевает](site-numbers-go-stale-with-the-dictionary-and-one-command-cures-both.md)
...
- [Разбор цели по условию закрывает места, но доли не двигает](goal-case-split-closes-sites-but-misses-the-numerator.md)
...
- [Ведомость шагов показывала витки и называла их шагами — от восьми до трёхсот сорока восьми раз мимо](the-step-ledger-showed-ticks-and-called-them-steps.md)
...
- [Компилятор принимает имя в ёлочках, в прямых кавычках и без кавычек одинаково — провал новичка в проводнике был не от кавычек, а от вызова вне функции](компилятор-принимает-имя-в-ёлочках-в-прямых-кавычках-и-без-кавычек-одинаково.md)
