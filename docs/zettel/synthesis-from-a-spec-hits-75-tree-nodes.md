...
Внешняя проверка к [[a-signature-does-not-determine-a-function]]: если бы наша
...
> ([[code-cannot-be-derived-from-its-hash]]). Единицы разные — узлы дерева и токены, — но
...
**Чем подтверждено.** Два независимых разбора внешних источников 2026-08-18;
ссылки построчно выше, числа взяты из таблиц самих работ. **Сошлись порознь:**
69 узлов и 246 узлов у Synquid, «спека 144 против кода 137», 10–58 узлов и
десятикратная спека у SuSLik, закрытие соревнования SyGuS, 40,8 % и 17,4 % у
молотков, «tens of seconds… to overnight» у fiat-crypto, «10 to 20 lines» у
Narcissus, отказ MoSSKit на переходе от списка к словарю. Наши 59 токенов —
замер на этой машине, описан в [[code-cannot-be-derived-from-its-hash]].
...
- **Потолок «30–75 узлов» нигде не объявлен авторами как потолок.** Ближайшее —
  ...
  И именно число Myth — **единственное из ключевых, которое подтвердил только
  один разбор**: второй до этой работы не дошёл. Если оно ошибочно, потолок
  задаёт Synquid, и это 69 узлов, то есть вывод не меняется.
- **Это про дедуктивный синтез, а не про языковые модели.** Что умеют агенты с
  прувером — другое дело и другой вывод: [[a-ratchet-instead-of-derivation]].
