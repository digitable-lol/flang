...
Заявка на **flang** пишется отдельно и лежит рядом файлом
`languages-flang.yml` (приезжает веткой документации). Каталог оставлен под неё
намеренно, а не забыт: удалять его вместе с заявкой на FTS было бы удалением
чужой работы.
