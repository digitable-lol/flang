...
программы без примечаний его нет вовсе, поэтому AST всего прежнего корпуса не
меняется ни на байт.
...
ядро, ни печать. Место (`span`) — не украшение: порядок между списками
верхнего уровня в AST не хранится, и связь примечания с тем, что оно
объясняет, восстанавливается по номеру строки.
