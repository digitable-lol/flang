...
Чего точное десятичное НЕ делает: деление из точного мира выводит — `делить` в
flang IEEE-деление и целого не даёт никогда, поэтому у денежной функции с
делением тип результата `число`. Знаковых денег (возврат, сальдо) нет: оба имени
