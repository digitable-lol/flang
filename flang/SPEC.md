...
— а исполняет его хозяин. Поручений шесть; `«Перечислить каталог»` отдаёт
откликом `«Перечислено»` со списком имён, отсортированным по кодовым точкам —
порядок здесь часть контракта, а не подробность хозяина. В AST это один
необязательный список верхнего уровня:
