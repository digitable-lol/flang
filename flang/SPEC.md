...
сверяется на собранных программах всех восьми целей (сверка двери предусловия):
код и текст отказа сравниваются равенством строк. Сверять при этом стало не с
кем: вторая реализация языка, служившая вторым мнением, из дерева удалена, и
ответы, с которыми идёт сравнение, заморожены таблицей. Ловится теперь регресс,
а не расхождение двух независимых реализаций.
...
**Второй реализации нет.** Компилятор на JavaScript, служивший вторым мнением о
языке, из дерева удалён; ответы, с которыми сверяется сегодняшний, заморожены
таблицами. Разница важна и названа прямо: ловится регресс — расхождение с
записанным ответом, — а не расхождение двух независимых реализаций. Второго
мнения о языке больше нет.
...
Отдельный перевод такого документа в AST flang — объект в запись, утилиту в
тотальную функцию, свойства в постусловия — жил в удалённой второй реализации и
из дерева ушёл вместе с ней. В рабочий путь он не входил и раньше. Само чтение
этих слов разборщиком осталось: его правила описаны выше и проверяются набором
образцов в `flang/test/fixtures/`.
