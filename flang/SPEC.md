...
| `добавить … к …` | копия списка (в C постоянно: запас в арене удваивается) | копия списка |
| `приписать … к …` | копия списка (в C постоянно: запас в арене берётся спереди) | одно звено, даром |
| `код символа` | одна кодовая точка, у всех восьми одинаково | она же |

Две строки про приписывание и дописывание читаются вместе. У C постоянны обе, и
по одной причине: массив списка живёт в арене с запасом, а общая запись запаса
считает занятые ячейки с двух концов — `добавить` берёт ячейку за концом,
`приписать` перед началом, и ни одна ячейка не пишется дважды. У Elixir
постоянно только `приписать`: список BEAM односвязный, приписать звено даром, а
дописать в конец — обойти. У шести остальных целей (JS, Go, Rust, Python, Java,
C#) список — непрерывный массив, ячейки ПЕРЕД началом у него нет, и запаса
спереди не завести, не меняя представления списка целиком; поэтому там одна
копия на вызов. Одна, а не N: именно этим форма и отличается от свёртки, которой
