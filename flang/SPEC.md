...
| `добавить … к …` | C, Go, Rust, Java, C# и Python — продление за постоянное время; JS — копия всего списка, O(длины) | ячейка в голову накопленного конца, постоянное время |
...
и 25,2 с (Python) на обе точки сразу и 8,3 с (Elixir) на одну; JS на этой точке
не отвечает и за 60 с. До продления за постоянное время та же точка брала в Rust
больше 1200 с — от вечного цикла неотличимо, — в C# 273 с, в Java 63,7 с, в
Python она снималась по сроку на 60 084 мс, в Elixir — по сроку 90 с, и вместе с
...
`flang/test/emit-elixir.test.mjs`. **Значит у одной цели из восьми предел шагов
сроком не является** — долг назван здесь, а не спрятан под общим «отказ всегда
объявленный».

Как это чинится, видно в шести местах сразу: `fl_b_dobavit`
...
(`flang/src/emit/csharp/Flang.cs`) и `b_append`
(`flang/src/emit/python/flang_runtime.py`). Приём один: у общего массива есть
...
`flang/test/emit-csharp.test.mjs` и `flang/test/emit-python.test.mjs`.
