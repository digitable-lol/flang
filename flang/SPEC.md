...
Go в скобках стоит по той же причине, что C и Rust: `ChainTail` возвращает
`List(value.List[1:])` — срез того же массива, а не новый. Копия остаётся у
вычислителя, JS, Python, Java и C#: там `хвост` это `slice(1)`,
`Arrays.copyOfRange` и `Items[1..]`.
