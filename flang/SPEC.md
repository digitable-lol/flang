...
Тип := строка | число | признак | ничто
     | целое | нат                 уточнения числа: точное целое и оно же ≥ 0
     | список Тип
     | «Имя объекта»          запись
     | «Имя типа»             сумма
...
### Уточнения числа: `целое` и `нат`

`целое` и `нат` — это НЕ новые виды значения. Значение остаётся тем же
IEEE-754 double; тип говорит, в каком диапазоне оно лежит:

| тип     | значения                                    |
|---------|---------------------------------------------|
| `нат`   | точные целые из [0, 2⁵³−1]                   |
| `целое` | точные целые из [−(2⁵³−1), 2⁵³−1]            |
| `число` | любой конечный double                        |

Отсюда подтипизация `нат` ≤ `целое` ≤ `число`: `нат` годится всюду, где
объявлено `число`, а `число` там, где объявлено `нат`, — нет. Списки
ковариантны (значения неизменяемы), тип функции контравариантен по аргументам.
Другой подтипизации в языке нет: у записей, сумм и параметров типа сравнение
по-прежнему означает равенство.

Граница 2⁵³−1 взята из измерения, а не выбрана: `x минус 1` впервые не меняет
`x` на 2⁵⁴, а внутри [−(2⁵³−1), 2⁵³−1] соседние целые различимы. Поэтому на
этих двух типах постоянный целый шаг меняет число НАВЕРНОЕ — и убывание
становится теоремой, а не обещанием под сторожем (раздел 4, «Тотальность»).

**Где уточнение получается, а где нет.** Никакой проверки в рантайме уточнение
не добавляет: если границу выражения не видно статически, тип остаётся `число`,
и отказ приходит на проверке типов. Граница выводится из объявленных типов,
целых литералов и сравнений в `если`:

```
принимает н: нат                      н ∈ [0, 2⁵³−1]
если н не больше 0 … иначе            в ветви «иначе» н ∈ [1, 2⁵³−1]
н минус 1                             ∈ [0, 2⁵³−2] — снова нат
н плюс н                              за 2⁵³−1 выходит — число, а не нат
н делить на 2                         целым не бывает — число
```

Значение приходит В программу двумя путями — значением примера и `flang run
--args`, — и на каждом диапазон проверяется ОДИН РАЗ НА ЗНАЧЕНИЕ, а не на виток
цикла.

`flang/core/*` остаётся на `число` целиком: уточнение ДОБАВЛЯЕТСЯ и ничего не
заменяет, семантика ядра FTS не меняется ни в одном месте.
