...
| `добавить … к …` | C, Go, Rust, Java и C# — продление за постоянное время; JS и Python — копия всего списка, O(длины) | копия всего списка |
...
напечатанное — за 1,4 с (C), 1,7 с (Java), 3,3 с (Rust), 4,0 с (C#) и 4,1 с (Go)
на обе точки сразу; Python, Elixir и JS на этой точке не отвечают и за 60 с. До
продления за постоянное время та же точка брала в Rust больше 1200 с — от
вечного цикла неотличимо, — в C# 273 с, в Java 63,7 с, и вместе с нею не
завершались целиком ни `flang/test/emit-rust.test.mjs`, ни
`flang/test/emit-go.test.mjs`. **Значит у трёх целей из восьми предел шагов
сроком не является** — долг назван здесь, а не спрятан под общим «отказ всегда
объявленный».

Как это чинится, видно в пяти местах сразу: `fl_b_dobavit`
...
(`flang/src/emit/java/Flang.java`) и `BAppend`
(`flang/src/emit/csharp/Flang.cs`). Приём один: у общего массива есть
...
`flang/test/emit-go.test.mjs`, `flang/test/emit-java.test.mjs` и
`flang/test/emit-csharp.test.mjs`.
