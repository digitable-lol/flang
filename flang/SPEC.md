...
списка с разделителем: проверяется каждый стык. У целей, где строка — UTF-8 или
последовательность кодовых точек, такого стыка не бывает, и отказа там нет: там
две половины остаются двумя знаками, что и есть правильный ответ по мере.
