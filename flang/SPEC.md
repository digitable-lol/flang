...
| `хвост` | копия суффикса (срез без копии в C, Go и Rust) | тот же список без первой ячейки, даром |
...
срез. До него скобка называла
только C и Rust, хотя `BTail` в `flang/src/emit/go/flang_runtime.go` возвращает
`items[1:]` — тот же срез без копии. Таблица занижала цель молча, а по ней
