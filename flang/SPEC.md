...
| `добавить … к …` | C, Go, Rust и Python — продление за постоянное время; JS, Java, C# — копия всего списка, O(длины) | копия всего списка |
...
напечатанное — за 1,4 с (C), 3,3 с (Rust), 4,1 с (Go) и 25,2 с (Python) на обе
точки сразу; Java, C#, Elixir и JS на этой точке не отвечают и за 60 с. До
продления за постоянное время та же точка брала в Rust больше 1200 с — от
вечного цикла неотличимо, — а в Python снималась по сроку на 60 084 мс, и
вместе с нею не завершались целиком ни `flang/test/emit-rust.test.mjs`, ни
`flang/test/emit-go.test.mjs`, ни `flang/test/emit-python.test.mjs`.
...
(`flang/src/emit/go/flang_runtime.go`) и `b_append`
...
`flang/test/emit-go.test.mjs` и `flang/test/emit-python.test.mjs`.
