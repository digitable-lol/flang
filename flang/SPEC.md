...
| `добавить … к …` | C, Go и Rust — продление за постоянное время; JS, Python, Java, C# — копия всего списка, O(длины) | ячейка в голову накопленного конца, постоянное время |
...
напечатанное — за 1,4 с (C), 3,3 с (Rust), 4,1 с (Go) на обе точки сразу и
8,3 с (Elixir) на одну; Java, C#, Python и JS на этой точке не отвечают и за
60 с. До продления за постоянное время та же точка брала в Rust больше 1200 с —
от вечного цикла неотличимо, — а в Elixir снималась по сроку 90 с, и вместе с
нею не завершались целиком ни `flang/test/emit-rust.test.mjs`, ни
`flang/test/emit-go.test.mjs`, ни `flang/test/emit-elixir.test.mjs`.
**Значит у четырёх целей из восьми предел шагов сроком не является** — долг
назван здесь, а не спрятан под общим «отказ всегда объявленный».

Как это чинится, видно в трёх местах сразу: `fl_b_dobavit`
(`flang/src/emit/c/flang_runtime.c`), `Items::grown`
(`flang/src/emit/rust/flang_runtime.rs`) и `BAppend`
(`flang/src/emit/go/flang_runtime.go`). Приём один: у общего массива есть
