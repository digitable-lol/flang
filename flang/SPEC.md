...
- **в JavaScript объявленных 10 000 кадров не существовало НИ ДЛЯ ОДНОЙ
  ...
  - рычаг работает через `$callDeep`, а прямой вызов экспортированной функции
    считает на стеке того, кто позвал. Ровно так же устроен C: библиотека,
    вызванная мимо `fl_call_deep`, считает на стеке вызывающего;
  ...
  поток — первый сравняется со вторым, и тест покраснеет. Рассуждение и замеры —
  в шапке `src/emit/js.mjs`;
