...
| `добавить … к …` | C, Go, Rust и JS — продление за постоянное время; Python, Java, C# — копия всего списка, O(длины) | копия всего списка |
...
5 000 000 шагов и 10 000 кадров: вычислитель упирается в предел за 987 мс, а
напечатанное — за 1,4 с (C), 3,3 с (Rust), 4,1 с (Go) на обе точки сразу и 0,8 с
(JS) на каждую; Java, C#, Python и Elixir на этой точке не отвечают и за 60 с. До
продления за постоянное время та же точка брала в Rust больше 1200 с — от вечного
цикла неотличимо, — и вместе с нею не завершались целиком ни
`flang/test/emit-rust.test.mjs`, ни `flang/test/emit-go.test.mjs`; в JS она не
отвечала и за 90 с. **Значит у четырёх целей из восьми предел шагов сроком не
является** — долг назван здесь, а не спрятан под общим «отказ всегда
объявленный».

Как это чинится, видно в четырёх местах сразу: `fl_b_dobavit`
(`flang/src/emit/c/flang_runtime.c`), `Items::grown`
(`flang/src/emit/rust/flang_runtime.rs`), `BAppend`
(`flang/src/emit/go/flang_runtime.go`) и `$b_dobavit`
...
`добавить` от одного значения даёт две независимых копии. И цена, и
...
`flang/test/emit-go.test.mjs` и `flang/test/emit-js.test.mjs`.
