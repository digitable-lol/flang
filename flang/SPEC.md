...
- **лимит шагов воспроизведён не везде.** В Go, Rust и Python незавершающаяся
  функция упирается в лимит и даёт диагностику. В C и в печатаемом JS лимита
  шагов нет — там такая функция крутится вечно; в C это записано прямо в
  `src/emit/c.mjs`. Предел глубины есть везде, включая C, потому что
  переполнение стека в C — падение процесса, а не исключение;
...
Долг: лимит шагов в бэкенде C. Пока его нет, «печатается везде» верно, а
«ведёт себя везде одинаково» — нет, и разница наблюдаема ровно на программах,
