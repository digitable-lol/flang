...
- **лимиты воспроизведены во ВСЕХ восьми целях.** В C, Go, Rust, Python, Java,
  ...
  код, и ТЕКСТ отказа;
- **счётчик глубины держит обещание только пока предел НИЖЕ стека хозяина, и в C
  он там не всегда — это открытый долг.** Счётчик считает кадры, а несёт их стек,
  и сколько их влезет, зависит от толщины кадра, то есть от программы. Замер
  холодными процессами: у нехвостовой функции с одним параметром C проходит около
  20 500 кадров (стек 8 МиБ) — запас над объявленными 10 000 всего двукратный, —
  а у функции с сорока локальными связываниями только около 1 500, и на пределах
  ПО УМОЛЧАНИЮ она умирает по SIGSEGV: без кода, без текста, без возможности
  перехвата. Интерпретатор на том же входе отказывает честно. При поднятом
  `--max-depth` так же умирают Go («fatal error: stack overflow», стек горутины
  растёт до 1 ГиБ) и Rust («fatal runtime error: stack overflow, aborting», около
  309 000 кадров). Java, C# и Elixir доходят до предела и отказывают объявленно;
  Python и JavaScript, если стек кончился первым, ОТВЕЧАЮТ объявленным отказом с
  честным текстом про хозяина — и потому держат обещание при любой толщине кадра.
  Закрывается долг тем же приёмом, которым закрыт Python: считать пробегом на
  потоке с ЗАДАННЫМ стеком и соотносить `max_depth` с ним, либо отказываться при
  печати от предела, который заведомо не несёт стек. Пока это не сделано,
  «отказ всегда объявленный» — правда про шесть целей и про JavaScript, но НЕ
  про C, и записывать её как общую нельзя;
