...
`flang` — язык исполняемых спецификаций: суммы типов, коллекции, строки как
данные, рекурсия и сопоставление с образцом. Он существует, чтобы на нём можно
было писать настоящие программы — в том числе собственный инструментарий — и
при этом сохранить проверяемость: завершение доказывается, обещание о
результате проверяется, поведение сверяется с напечатанным кодом.
...
недопустимо, а восемь целей печати умеют именно этот вид числа. Точный натуральный тип `нат` (раздел 3)
...
наследия и побайтовая сверка самоприменения. Целое число минорных единиц не видит
...
| округлять к сетке масштаба | `делить` на `сотых` округляет результат к целой копейке | меняет СЕМАНТИКУ `делить` — то есть поведение восьми рантаймов, `flang/core`, моста наследия и побайтовой сверки самоприменения. Ровно то, чего уточнение типа избегает по построению |
...
поверхности наследия, которую читает тот же разборщик: запятая продолжала поля в
...
`.flang` и `.fts` дерева), но в чужом файле `принимает или: число` перестанет
...
Суррогат отвергается не из строгости. Язык обещает у восьми целей печати
ОДИНАКОВЫЕ значения, а строка в C, Go, Rust и Elixir — это UTF-8, где половина
пары не записывается по определению кодировки: `char::from_u32(0xD800)` в Rust
отдаёт `None`, `string(rune(0xD800))` в Go отдаёт U+FFFD. Пропустить её значило
бы: три цели дают значение, четыре — нет. Отказ же одинаков у всех восьми. И по
самому Unicode суррогат символом не является: скалярное значение (D76) — это
любая кодовая точка, кроме половины пары, и таких значений 1 112 064.
...
знака у языка нет, и заводить его ради одной формы значило бы добавить пятый
вид значения во все восемь целей. `разложить … на символы` уже раскладывает
...
Расходятся они ровно на одиноких половинах суррогатной пары. Такая половина
кодовой точкой является (`код символа` даёт у неё 55296…57343), а знаком
Unicode — нет, и внутри языка её не построить: `символ по коду` на суррогате
отказывает, а `\uXXXX` в исходнике до него не доходит. Приезжает она только
снаружи, и только к тем целям печати, где строка — UTF-16 (JavaScript, Java,
C#): в UTF-8 (C, Go, Rust, Elixir) и в последовательности кодовых точек (Python)
такого значения нет вовсе.

Платы за это нет там, где её ждут. У подстроки с ЦЕЛЫМИ краями совпадение по
единицам UTF-16 и совпадение по знакам — одно и то же: вхождение может разрезать
знак только тогда, когда искомое начинается низкой половиной пары или кончается
высокой. Поэтому обход границ включается лишь у такой подстроки, а у всякой
другой поиск остаётся тем же, что был.
...
списка с разделителем: проверяется каждый стык. Отказа этого нет у остальных
пяти целей, и по разным причинам: у C, Go, Rust и Elixir строка — UTF-8, где
такого стыка не бывает вовсе, а у Python строка — последовательность кодовых
точек, и две половины там просто остаются двумя знаками. Это и есть правильный
ответ по мере; UTF-16 его дать не может, поэтому вместо ответа стоит отказ.
...
ВЫЧИСЛИТЕЛЬ ПЕРЕЕХАЛ ИЗ КОПИИ В СРЕЗ 20 августа, и не правкой этой таблицы.
Считал `хвост` вычислитель на JavaScript (`элементыСписка(list, 1)`, копия
суффикса); реализации на JavaScript больше нет, и считает тот вычислитель, что
собран из `bootstrap/`. Его рантайм — `flang/src/emit/c/flang_runtime.c`, тот
же самый, что уезжает в цель C, и `хвост` там `fl_list_slice(value, 1)`, срез.
Стоимость формы у вычислителя упала с длины списка до постоянного времени.
...
без него узел побайтово тот же, что кладёт мост наследия:
...
блока (`flang/proof/SPEC.md`, «Две формы под одним словом»). Старая — из
прежней поверхности (`дано «Объект» имеет …`, `в данных`, `по морфизму`,
`следовательно «вывод»`) — уезжает в `legacy` узлом `ftsLegacy`, как уезжала
всегда, и её байты не изменились. Новая добавляет необязательный список верхнего уровня:
...
flang/
  src/
    ...
    compat.mjs       прогон примеров и словарь связей
    fts-legacy.mjs   мост наследия: документ утилит → AST flang (только для проверок)
...
| `FLANG_PLAN_UNSUPPORTED` | печать программы с объявлением `план` в цель, у которой исполнителя плана нет: функции начала и шага стали бы обычными функциями, которых никто не зовёт |
...
Перевод такого документа в AST flang существует отдельным файлом
(`flang/src/fts-legacy.mjs`: объект → запись, утилита → тотальная функция —
начальное значение плюс правила последовательностью `если`, свойства
постусловиями, примеры как `examples`). В рабочий путь он не входит и живёт
только ради проверок; трогать его без нужды не стоит — тринадцать мест в
`parser.mjs` и `types.mjs` объясняют своё устройство ссылкой на него.
