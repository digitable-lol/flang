...
ядро. В рантайме оно считается **ровно в одном месте** — на границе программы
(`interpret.mjs`, `callFunction`: `--args`, значения примеров, всё внешнее), —
и **ни строкой** не печатается ни в одну из восьми целей. Последнее измерено
побайтовым сравнением печати с предусловием и без него
(`flang/test/precondition.test.mjs`).
