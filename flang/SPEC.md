...
натуральный тип `нат` (раздел 3) НОВОГО ВИДА ЗНАЧЕНИЯ НЕ ЗАВОДИТ: его значения —
...
JSON едет то же число. Довод тот же, по которому `нат` сделан уточнением, и
здесь он сильнее.
...
поверхностях открыты 132; у остальных 17 на какой-то из поверхностей слова нет,
...
ВЫЧИСЛИТЕЛЬ ПЕРЕЕХАЛ ИЗ КОПИИ В СРЕЗ, и не правкой этой таблицы.
...
(`flang/examples/leetcode/022-generate-parentheses.flang`) при пределах 5 000
...
сверялись отказом по пределу. Сверяла это вторая реализация языка; она снята, и
проверки, которые её звали, сегодня не запускаются.
...
предусловия): код и текст отказа сравниваются равенством строк. Сверять при этом
стало не с кем: вторая реализация языка, служившая вторым мнением, из дерева
удалена, и ответы, с которыми идёт сравнение, заморожены таблицей. Ловится
теперь регресс, а не расхождение двух независимых реализаций.
