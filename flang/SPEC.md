...
случая, и в напечатанном коде его тоже не будет. Без этого правила тотальность
можно было бы обесценить законным по типам значением (комбинатор Ω выражается
на одних тотальных функциях — `flang/cat/HOF.md`).

**Что из этого сделано на 2026-08-07.** Разбор, проверка типов, анализ
завершаемости и вычисление интерпретатором. Печать в восемь целей — НЕТ:
применение даёт `FLANG_PARSE: неизвестный вид выражения «apply»`, громкий отказ
вместо тихого мусора. Слои `self/` новой формы тоже не знают, поэтому ни одна
программа репозитория — включая `stdlib` и `examples` — ею не пользуется и
пользоваться не вправе, пока `self/` не научится: корпус сверки с flang₁
собирается по маске каталога. Порядок оставшихся фаз — `flang/cat/HOF.md`.
