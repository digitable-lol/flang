...
| `добавить … к …` | C, Go и Rust — продление за постоянное время; JS, Python, Java, C# — копия всего списка, O(длины) | копия всего списка |
...
напечатанное — за 1,4 с (C), 3,3 с (Rust) и 4,1 с (Go) на обе точки сразу;
Java, C#, Python, Elixir и JS на этой точке не отвечают и за 60 с. До продления за постоянное время та же точка брала в Rust
больше 1200 с — от вечного цикла неотличимо, — и вместе с нею не завершались
целиком ни `flang/test/emit-rust.test.mjs`, ни `flang/test/emit-go.test.mjs`.
**Значит у пяти целей из восьми предел шагов сроком не является** — долг назван
здесь, а не спрятан под общим «отказ всегда объявленный».
