...
| `добавить … к …` | копия всего списка — кроме C, где он растёт в запас ёмкости и копии нет | копия всего списка |
...
42, глубина 10 000) — время до отказа `FLANG_RECURSION_LIMIT` при одном и том
же бюджете шагов:

| бюджет шагов | C | вычислитель | Java | C# | Python | JS | Rust | Elixir | Go |
|---|---|---|---|---|---|---|---|---|---|
| 100 000 | 24 | 31 | 217 | 121 | 348 | 86 | 185 | 933 | 1 448 |
| 200 000 | 38 | 40 | 302 | 330 | 864 | 762 | 802 | 2 722 | 6 428 |
| 400 000 | 68 | 72 | 710 | 1 535 | 2 328 | 3 570 | 3 014 | 11 064 | 24 186 |
| 800 000 | 130 | 135 | 2 874 | 6 236 | 7 951 | 20 797 | 13 081 | 50 216 | 98 089 |
| 1 600 000 | 271 | 286 | 11 914 | 32 369 | 34 902 | 117 506 | 128 858 | 259 603 | 417 594 |
...
рост времени в 11 и 9 раз. Там цена шага ограничена — у C потому, что копии нет
вовсе, у вычислителя потому, что его шаг в 48,6 раза мельче и на тот же бюджет
он успевает набрать список в 48,6 раза короче. У остальных семи время растёт
быстрее квадрата: удвоение бюджета стоит от трёх до пяти раз времени. Разница
между целями при одном бюджете доходит до 1500 раз (C 271 мс против Go
417 594 мс), и это разница НЕ в счётчике — счётчики у всех восьми целей
...
Долг записан: `добавить` за постоянное время нужен остальным семи, приём
известен и работает — `flang/src/emit/c/flang_runtime.c`, раздел
«„добавить“: удлинение списка за постоянное время». Пока его нет, `maxSteps`
у семи целей ограничивает число шагов, но не работу, и точки, на которых это
вылезает, названы поимённо в `flang/test/corpus-grid.mjs` (`УБЕГАЮЩИЕ`).
