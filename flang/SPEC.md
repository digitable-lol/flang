...
| `добавить … к …` | C, Go, Rust, Java, C# и Python — продление за постоянное время; JS — копия всего списка, O(длины) | копия всего списка |
...
и 25,2 с (Python) на обе точки сразу; Elixir и JS на этой точке не отвечают и за
60 с. До продления за постоянное время та же точка брала в Rust больше 1200 с —
от вечного цикла неотличимо, — в C# 273 с, в Java 63,7 с, в Python она снималась
по сроку на 60 084 мс, и вместе с нею не завершались целиком ни
`flang/test/emit-rust.test.mjs`, ни `flang/test/emit-go.test.mjs`, ни
`flang/test/emit-python.test.mjs`. **Значит у двух целей из восьми предел шагов
