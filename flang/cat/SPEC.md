...
**Не проверяется ассоциативность композиции**, и это не недоделка. `«в» после
(«б» после «а»)` и `(«в» после «б») после «а»` — в языке ДВА РАЗНЫХ ИМЕНИ, а
равенства морфизмов в языке нет вовсе (см. «Изоморфизм»: «языку с равенствами
морфизмов нужна отдельная работа»). Проверка, сравнивающая имена, продавала бы
...
`FLANG_CATEGORY_NO_IDENTITY`, `FLANG_FUNCTOR_NOT_TOTAL`,
...
1. ✅ **Категория, объект, морфизм, композиция, единица, цепочка** — и **закон
   ...
   объявлений. Ассоциативность не проверяется: равенства морфизмов в языке нет.
