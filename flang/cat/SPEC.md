...
**Что делает двоичный компилятор сегодня, замерено прогоном 22 августа 2026 на
`flang/examples/wal/append-plan.flang` по всем восьми целям, по одной цели за
...
**Семь целей из восьми попадают ровно в тот исход, который назван выше худшим.**
В их выводе нет ни описателя плана, ни исполнителя: поиск не находит ни `ioPlan`,
ни `ioRun`, ни `$io`; функции начала и шага печатаются как обычные, и звать их
некому. Модуль собирается, код возврата ноль, диагностик ноль — а работать он не
умеет. Восьмая, `js`, контракт выполняет.

Обещано было ровно обратное: печатает `js`, отказывают семь остальных кодом
`FLANG_PLAN_UNSUPPORTED`. Половина обещания сошлась (`js` печатает), вторая —
нет: семь молчат вместо отказа.
