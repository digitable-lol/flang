...
`FLANG_EQUALITY_NOT_CONGRUENT`, `FLANG_FUNCTOR_NOT_TOTAL`,
...
**Не сделано, и препятствие теперь другое.** Раньше их не было потому, что
коммутирующий квадрат — это РАВЕНСТВО ДВУХ КОМПОЗИЦИЙ, а равенства морфизмов в
языке не было вовсе. Оно появилось (раздел «Равенство морфизмов»), и с ним
квадрат стал выразим: обе стороны — стрелки с общими концами, а сравнивать их
умеет объявленное равенство кодомена, ровно тем же способом, каким сравниваются
две скобки ассоциативности.

Осталась цена ПОВЕРХНОСТИ, и она измерена, а не оценена. Занятость всех четырёх
кандидатов проверена `flang/scripts/word-occupancy.mjs` на 179 файлах корпуса:
`преобразование` — 0 голым именем, `квадрат` — 0, `коммутирует` — 0,
`действует` — 0. То есть ни одно написанное слово не пострадает, и в этом смысле
слова свободны. Но свободное слово всё равно стоит: таблица лексера живёт в ДВУХ
местах (`src/lexer.mjs` и `self/lexer.flang`, сверяются побайтово) и в ЧЕТЫРЁХ
поверхностях (русской, английской, эсперантской, китайской), а продукция — в
двух парсерах (`src/parser.mjs` и `self/parser.flang`, AST сверяется побайтово).
Это ровно то, чего равенство морфизмов не стоило вовсе: оно обошлось готовыми
`объект` и `даёт`.

Второе, не поверхностное: `из функтора «Список» в функтор «Возможно»` — это
утверждение про ПРИМЕНЕНИЯ параметрических типов с одной и той же «А» в двух
местах, а `checkFunctors` знает имя, но не применение. Это фаза 3 в
`flang/cat/POLY.md`, и она не сделана.
...
4. 🟡 **Монада и форма `в монаде` — СДЕЛАНЫ**; естественное преобразование —
   ...
   в `flang/cat/POLY.md`. Одно из двух препятствий снято 2026-08-15: квадрат
   коммутирует — это равенство двух композиций, и равенство морфизмов у языка
   теперь есть. Оставшаяся цена измерена в разделе «Естественное
   преобразование»: четыре свободных слова, но два лексера, четыре поверхности
   и два парсера — плюс сама фаза 3.
...
9. ✅ **Отношения множеств — подобъект и предел** (`flang/cat/SETS.md`).
