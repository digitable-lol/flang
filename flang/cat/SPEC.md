...
Список целей с исполнителем плана держала снятая реализация, и он сверялся с
поведением проверкой, а не читался как обещание: цели брались из каталога
генераторов кода, и напечатавшие обязаны были совпасть со списком ровно. В
выводе `flang emit` поля `возможности.план` нет и сегодня. Списка этого больше
нет нигде: он ушёл вместе со снятой реализацией, и сверять список с поведением
стало нечем.
