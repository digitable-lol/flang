...
**Сверка списка целей с поведением — `scripts/plan-v-celyah.flang`.** Различает
исходы ИЗЪЯТИЕМ, а не поиском текста по восьми языкам: программа печатается
дважды, с объявлением `план` и без него, выводы сравниваются. Совпали — значит
объявление не дало ничего. Прогон 22 августа 2026:

```
bootstrap/flang io scripts/plan-v-celyah.flang
...
таблица «цель → исход» объявлена данными в `scripts/plan-v-celyah.flang` и
