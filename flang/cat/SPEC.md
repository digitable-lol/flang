...
`flang/examples/cat/order-shipment.flang`; проверка
...
### Словарь поручений — восемнадцать действий, и набор закрыт
...
**Восемнадцать поручений, и набор закрыт** — это утверждение, а не текущее
...
11 776 136». Теперь текстовая пара на нетексте ОТКАЗЫВАЕТ кодом
`FLANG_IO_NOT_TEXT`, называя номер октета и то поручение, которым это содержимое
возится без потерь, а октетная пара возит файл байт в байт. Отказ лучше
...
**Что делает двоичный компилятор сегодня, замерено прогоном 21 августа 2026 на
`flang/examples/wal/append-plan.flang` по всем восьми целям:**

| цель | код | что вышло |
|---|---:|---|
| `c` | 0 | напечатано файлов 6, байт 395 249 |
| `go` | 0 | напечатано файлов 5, байт 168 041 |
| `rust` | 0 | напечатано файлов 7, байт 209 019 |
| `java` | 0 | напечатано файлов 8, байт 209 153 |
| `elixir` | 0 | напечатано файлов 4, байт 205 589 |
| `python` | 0 | напечатано файлов 4, байт 159 274 |
| `csharp` | 0 | напечатано файлов 9, байт 243 115 |
| `js` | 1 | `FLANG_UNKNOWN_NAME: запись не содержит поле «исходник исполнителя»` |

**Ни одна из восьми целей контракта не выполняет, и семь из них попадают ровно в
тот исход, который назван выше худшим.** В напечатанном нет ни описателя плана,
ни исполнителя: `grep` по выводу всех восьми не находит ни `ioPlan`, ни
`ioRun`, ни любого их подобия; функции начала и шага печатаются как обычные, и
звать их некому. Модуль собирается, код возврата ноль, диагностик ноль — а
работать он не умеет. Восьмая, `js`, отказывает, но не по плану: у её печатника
не хватает собственного поля.

Обещано было ровно обратное: печатает `js`, отказывают семь остальных. Исходы
переставлены местами.

Проверки, которая сверяла бы список целей с поведением, в дереве нет — потому
расхождение и прожило до случайного прогона. Как такую проверку строить, описано
в базе знаний: `docs/zettel/pechat-plana-obeshchana-naiznanku-i-sverit-eyo-nechem.md`.
...
Списка целей с исполнителем плана в дереве нет, и в выводе `flang emit` поля
`возможности.план` тоже нет. Значит сверять обещание с поведением нечем — и
расхождение выше поймано прогоном, а не проверкой.
...
`«Прочитать файл»` на содержимом, которое правильным UTF-8 не является или
несёт нулевой октет, и `«Записать файл»` с таким же содержимым отвечают
`FLANG_IO_NOT_TEXT`, называя номер октета и то поручение, которым это возится
без потерь, — `«Прочитать октеты из файла»` и `«Записать октеты в файл»`. До
22 августа 2026 отказа не было и не было предупреждения: 4096 октетов
`bootstrap/flang` через текстовую пару давали файл в 7 байт, и прогон
заканчивался кодом 0.
