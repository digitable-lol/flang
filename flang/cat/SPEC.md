...
| цель | что вышло |
|---|---|
| `c` | напечатано файлов 6, байт 395 249, код 0 |
| `js` | `FLANG_UNKNOWN_NAME: запись не содержит поле «исходник исполнителя»`, код 1 |

То есть исходы сегодня ровно обратны обещанным: цель `c` печатает и не
отказывает, цель `js` отказывает — и отказывает не по плану, а по нехватке поля
у своего печатника. Ни один из двух законных исходов на этой паре целей не
получается. Проверки, которая сверяла бы список целей с поведением, в дереве
нет.
