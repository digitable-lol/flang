...
самая важная часть главы. Реализация: `flang/self/io.flang` (словарь),
`flang/src/emit/c/flang_repl.c` (хозяин двоичного), `flang/src/emit/js/flang_io.js`
(цикл поручений в напечатанном на JavaScript), `flang/src/emit/js/flang_host_browser.js`
(хозяин вкладки), `flang/examples/io/link-report.flang` (программа, которая читает
файл, ходит в сеть и пишет отчёт).

### Словарь поручений — шестнадцать действий, и набор закрыт
...
**Шестнадцать поручений, и набор закрыт** — это утверждение, а не текущее состояние.
...
у октета не бывает половинки. Смешивать их на одном соединении можно, и смысл у
смеси определён: придержанное живёт в раскодировщике, октетное чтение заберёт его
первым.

**«Запустить процесс с вводом» — второе поручение, а не поле у первого.** Вариант
в языке строится всеми полями, и новое поле сделало бы нерабочей всякую программу
дерева, уже назвавшую программу и аргументы. Отдельный вариант не отнимает
ничего — тот же довод, по которому «Процесс завершён» и «Процесс убит» два, а не
одно поле-часовой.
...
- **служба не умела ЖДАТЬ.** `«Запросить»` — клиентское, а сервер по адресу не
  ходит, он сидит на порту. Служба на flang (`flang/examples/web/shortener`)
...
- **служба не умела ПОЙТИ.** Единственный выход наружу, `«Запросить»`, — это
  ...
  [`docs/adr/0002-ishodyashchee-soedinenie.md`](../../docs/adr/0002-ishodyashchee-soedinenie.md).
...
Поэтому у печати плана ровно два законных исхода, и третьего нет:

| цель | план |
|---|---|
| js | печатает: описатель `$io` данными, двери `ioPlan()` и `ioRun()`, исполнитель внутрь модуля |
| c, go, rust, python, java, csharp, elixir | отказ `FLANG_PLAN_UNSUPPORTED`, план назван по имени |

Что печатается у цели, которая умеет:
...
- **исполнитель** — цикл поручений, тот же, что у свидетеля (`runPlan`), но на
...
Список целей с исполнителем плана (`ЦЕЛИ_С_ИСПОЛНИТЕЛЕМ_ПЛАНА` в
`flang/src/target-plan.mjs`) сверяется с поведением проверкой, а не читается как
обещание: цели берутся из каталога бэкендов, и напечатавшие обязаны совпасть со
списком ровно. В выводе `flang emit` поля `возможности.план` при этом НЕТ, и это
названный размен: таблица возможностей живёт в рабочем пути компилятора, а у
него потолок строк JavaScript ходит только вниз. Довод целиком — в шапке
`target-plan.mjs`.
