...
**Набор закрыт** — это утверждение, а не текущее состояние: словарь растёт
правкой `flang/src/io.mjs`, а не объявлением в программе автора. Полнота здесь
дороже, чем кажется: каждое поручение обязан уметь исполнить КАЖДЫЙ хозяин,
включая напечатанного в C.

Поручений было пять, и довод к пятёрке был такой: шестое пришлось бы писать
...
поручения, а цикла хозяина и всех пяти прежних. Шестое и седьмое стоили одного
исполнителя каждое, а не восьми (`fspec/README.md`, столбец «вышло»).

Шестое и седьмое заведены не «для полноты», а под названную работу: сторож спек
обязан НАЙТИ спеки в каталоге и позвать на каждой компилятор.
