...
Проверка идёт под лимитом шагов, и упор в лимит обязан быть отдельной
диагностикой `FLANG_LAW_LIMIT`, а не молчаливым успехом.

*Объявлено, не сделано.* Кода `FLANG_LAW_LIMIT` в дереве нет, и решение это
пока не работает: проверка законов лимита не различает. Отказ вычисления на
значении сетки — любой, включая упор в предел шагов — прекращает обход
(`break авария` в `flang/src/monoid.mjs` и `flang/src/monad.mjs`), и закон,
который не смогли проверить, выглядит как закон, который проверили и не
нарушили, — ровно то, что решение запрещало. `flang/src/iso.mjs` о срыве хотя бы
говорит, но и он говорит «отказала на значении сетки», не отличая исчерпанный
предел от честной ошибки. Долг стережёт
`flang/test/code-guard.test.mjs`: он же покраснеет, когда код появится, и
потребует снять эту пометку.
...
Свои коды диагностик: `FLANG_MORPHISM_SHAPE`, `FLANG_ISO_MISMATCH`,
