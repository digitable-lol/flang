...
for N in "$@"; do
  node "$FLANG/flang/conc/bench/gen.mjs" --n="$N" --вид="$VID" ${K:+--k="$K"} ${TYAZH:+--тяж="$TYAZH"} \
    --out="$ZAMER/$METKA-$N.flang"
  printf '%s N=%s ' "$METKA" "$N"
  node "$FLANG/flang/conc/bench/build.mjs" --root="$FLANG" --src="$ZAMER/$METKA-$N.flang" --dir="$ZAMER/b-$METKA-$N"
done
