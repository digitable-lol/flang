...
//
// Решений в этом файле нет ни одного. Связь считает `link.flang`, таблицу
// процессов и планировщик — `scheduler.flang`, программу — её обработчики;
// все трое собраны в `node-benchmark.flang` и напечатаны компилятором в цель csharp.
//
...
// передаёт его НАДЗОРУ — четвёртому напечатанному модулю, `supervisor.flang`. Кого
...
public static class HozyainUzla
{
    ...
    /* Перевод, а не решение: правило «у каждого значения метка одной буквой»
       живёт в `flang/concurrency/wire.flang` («Метки значений», «Имена чисел»), и разойтись с ним нельзя. */
