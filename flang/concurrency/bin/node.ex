...
defmodule HozyainUzla do
  ...
  Ни одного решения в этом файле нет. Все три части решают напечатанные модули:
    ...
    1. связь        `flang/concurrency/link.flang`          — 11 событий, 8 велений
    2. процессы     `flang/concurrency/scheduler.flang` — 7 событий, 6 велений
    3. программа    `flang/concurrency/examples/distributed.flang` — обработчики

  Собраны они в один модуль `flang/concurrency/node-benchmark.flang` и напечатаны
  ...
  НАДЗОРУ — четвёртому напечатанному модулю, `flang/concurrency/supervisor.flang`. Кого
  ...
  # в `flang/concurrency/wire.flang` («Метки значений», «Имена чисел»), и разойтись с ним нельзя — иначе письмо с
