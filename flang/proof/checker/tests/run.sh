...
# Задача 1400: ТЕ ЖЕ ДВА ПРАВИЛА ВЕДОМОСТИ, НО ИМЕНЕМ В БЛОКЕ «вывод». Выше О9
# работает крюком по телу: запись о шаге молчит, форму читает сверщик сам. Ниже она
# названа шагом записи — «вывод 1 О9 ⟨…⟩ сам» и «вывод 3 Н6 ⟨…⟩ из 1 2», — и сверщик
# обязан переиграть ИМЕННО ту формулировку, что стоит в строке ведомости, а не
# согласиться с именем. До 26 сентября 2026 обоих имён в приёме не было: честные
# записи ниже старый сверщик отвергал кодом 1 словами «правила «Н6» в приёме нет» и
# «правила «О9» в приёме нет». Форму О9 судит один `свёртка_под_длиной` — и в крюке,
# и здесь, разойтись им нечем.
#
# Записи РУКОТВОРНЫЕ: шапку напечатало ядро (`flang check --proof --record`), блок
# «вывод» дописан руками — печать вывода в ядре другой стек (задача 6132), и до неё
# приём обязан ехать первым. Подделки — правки ОДНОЙ строки записи: программа честна,
# лжёт запись, и каждой назван свой отказ подстрокой.
opyt C "покрытие: Н6 — копара двух ветвей выбора в блоке «вывод»" 0 "$PK/n6-choice.flang" "$PK/n6-choice.record"
podd_p "покрытие: Н6 — обе посылки о ветви «то», ветвь «иначе» не покрыта" \
  "ветвь «иначе» требует «второе не меньше 0»" "$PK/n6-choice.flang" "$PK/n6-choice.record" \
  's|вывод 2 Н3 ⟨второе не меньше 0⟩|вывод 2 Н3 ⟨первое не меньше 0⟩|'
podd_p "покрытие: Н6 — посылки переставлены, ветви покрыты крест-накрест" \
  "ветвь «то» требует «первое не меньше 0»" "$PK/n6-choice.flang" "$PK/n6-choice.record" \
  's|не меньше 0⟩ из 1 2|не меньше 0⟩ из 2 1|'
podd_p "покрытие: Н6 — назван над суммой, а не над выбором" \
  "Н6 берёт выбор «если У то А иначе Б» ровно с двумя ветвями" "$PK/n6-choice.flang" "$PK/n6-choice.record" \
  's|Н6 ⟨( если первое не меньше второе то первое иначе второе )|Н6 ⟨( первое плюс второе )|'
podd_p "покрытие: Н6 — лист «сам» вместо двух посылок" \
  "Н6 берёт две посылки — основание обязано быть «из N M»" "$PK/n6-choice.flang" "$PK/n6-choice.record" \
  's|⟩ из 1 2|⟩ сам|'
opyt C "покрытие: О9 — свёртка под длиной шагом блока «вывод»" 0 "$PK/o9-derivation.flang" "$PK/o9-derivation.record"
podd_p "покрытие: О9 — начало свёртки в записи не ноль" \
  "начало свёртки «1» — не ноль" "$PK/o9-derivation.flang" "$PK/o9-derivation.record" \
  's|начиная с 0|начиная с 1|'
podd_p "покрытие: О9 — шаг записи «акк плюс 2», мера растёт быстрее длины" \
  "прибавляет к накопителю «акк» больше единицы" "$PK/o9-derivation.flang" "$PK/o9-derivation.record" \
  's|акк плюс 1|акк плюс 2|'
podd_p "покрытие: О9 — справа длина другого списка" \
  "а нужна «длина элементы» — длина РОВНО ОДНОГО довода" "$PK/o9-derivation.flang" "$PK/o9-derivation.record" \
  's|не больше ( длина элементы )⟩ сам|не больше ( длина другие )⟩ сам|'
podd_p "покрытие: О9 — имена накопителя и элемента совпали" \
  "обязаны быть различными именами" "$PK/o9-derivation.flang" "$PK/o9-derivation.record" \
  's|как акк и эл|как акк и акк|'
podd_p "покрытие: О9 — опора на строку исходника вместо листа" \
  "О9 — лист, основание обязано быть «сам»" "$PK/o9-derivation.flang" "$PK/o9-derivation.record" \
  's|⟩ сам|⟩ строка 7|'
