...
# 1412: пять законов кольца на носителе `число` (тип объявлен `целое` — носитель тот же).
# Два перестановочных и нейтральный ноль ядро выводит семейством Тожд, и запись их
# проигрывается целиком. Сочетательность, распределительность и обратный по сложению
# на этом носителе ЛОЖНЫ: встречные примеры считает вычислитель примерами того же
# файла, поэтому честная запись ЧЕСТНО ПУСТА (доказанным не числится ничего, код 0),
# а ядро такому файлу даёт 3. Три подделки ниже — запреты Тожд✗, Тожд✗✗, Тожд✗✗✗
# ведомости: ход, которым закон объявили доказанным, сверщик обязан отвергнуть.
opyt C "1412: законы кольца, которые ядро выводит — Тожд1 дважды и Тожд2 на `целое`" 0 "$UT/ring-laws-provable.flang" "$UT/ring-laws-provable.record"
opyt C "1412: законы кольца, которых на носителе числа нет — запись честно пуста" 0 "$UT/ring-laws-banned.flang" "$UT/ring-laws-banned.record"
prichina "1412: запрет Тожд✗ — сочетательность объявлена ходом «соседи»" "у цели «( ц плюс ( а плюс б ) ) равен ( а плюс ( б плюс ц ) )» стороны равенства разные" "$UT/ring-laws-banned.flang" "$UTP/ring-associativity-by-neighbours.record"
prichina "1412: запрет Тожд✗✗ — распределительность объявлена ходом «соседи»" "у цели «( ( б плюс ц ) умножить на а ) равен ( ( а умножить на б ) плюс ( а умножить на ц ) )» стороны равенства разные" "$UT/ring-laws-banned.flang" "$UTP/ring-distributivity-by-neighbours.record"
prichina "1412: запрет Тожд✗✗✗ — обратный по сложению объявлен ходом «нейтральный ноль»" "в «а плюс ( 0 минус а )» ни одно слагаемое не литерал 0" "$UT/ring-laws-banned.flang" "$UTP/ring-inverse-by-zero-neutral.record"
