...
#
#   sh flang/proof/forgeries/run.sh
...
exit 1
# ярлык «запись:проверка» sh — записанное доказательство сверяется отдельной программой: честное принято, четыре подделки отвергнуты
