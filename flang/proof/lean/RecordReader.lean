...
`«Функция»`. Читатель повторяет дисциплину сверщика (`checker.c`): те же
...
`Model.lean`: ошибка чтения не ловится теоремой, её ловит сверка вердиктов
в `run.sh`.
...
/-- Ярлык функции `функция «Ф»`: имя в ёлочках, как его пишет вызов (Term.lean,
    граница 10). -/
def «ярлык?» (s : String) : Option String :=
  let u := «ужать» s
  if u.startsWith "функция «" && u.endsWith "»" then
    let r := u.«сн» 8
    if «сколькоРаз» r '«' = 1 && «сколькоРаз» r '»' = 1 then some r else none
  else none

/-- Вызов: `«Ф» от А и Б`, `условие от эл`, `( функция «Ф» ) от А` (ярлык в скобках —
    так его ставит подстановка сверщика), либо голое `«Ф»`. -/
def «вызов?» (t : String) : Option (String × List String) :=
  let u := «ужать» t
  match «разделитьСверху» u "от" with
  | [f, args] =>
    let f' := «ужать» f
    if «имя?» f' || (f'.startsWith "«" && f'.endsWith "»") ||
       (f.«обр».startsWith "(" && («ярлык?» f').isSome) then
...
partial def «число» (s0 : String) : Option «ТермЧ» :=
  ...
  else if let some f := «ярлык?» s then some (.«ярлык» f)
...
partial def «список» (s0 : String) : Option «ТермС» :=
  ...
  -- списочная свёртка (М2, ADR-0042 §2): начало и шаг — списки
    ...
    | some («над», «хв») =>
      ...
      | none => none
    | none => none
  -- отображение (Э6, задача 4058): прообраз, имя элемента, образ-число
  else if s.startsWith "отобразить " then
    match «надвое» (s.«сн» 11) "как" with
    | some («над», «хв») =>
      match «надвое» «хв» "→" with
      | some (x, «ф») => if «имя?» x then do pure (.«отобразить» (← «список» «над») («ужать» x) (← «число» «ф»)) else none
      | none => none
    | none => none
...
partial def «текст» (s0 : String) : Option «ТермТ» :=
  ...
  if s.length ≥ 2 && s.front = '"' && s.back = '"' then some (.«литТ» («безКраёв» s))
    ...
    | _ => none
  -- число текстом (Ч5): «к строке Ч»
  else if s.startsWith "к строке " then do pure (.«кСтроке» (← «число» (s.«сн» 9)))
...
/-- Строки тел, на которые ссылаются шаги Разв3 записи (`… Разв3 ⟨Ф⟩ из N строка M`). -/
def «строкиРазв3» («текст» : String) : List Nat :=
  («текст».splitOn "\n").filterMap fun l =>
    let l' := l.«обр»
    if l'.startsWith "вывод " && («слово» l' 2) == "Разв3" then «числоПосле» ((l'.splitOn "⟩").getD 1 "") "строка "
    else none
where «слово» (s : String) (n : Nat) : String := (s.splitOn " ").getD n ""

/-- Функции с телом одной строкой — для Разв3: только те, чью строку тела называет
    шаг Разв3 записи; хозяин строки — ближайший заголовок функции выше, как у
    сверщика (`хозяин_строки`). Имя — в ёлочках, параметры из `принимает`, тело
    по объявленному типу. Тела прочих функций читатель не трогает. -/
def «определения» («строки» : List String) («нужные» : List Nat) : List «Определение» :=
  «нужные».eraseDups.filterMap fun m => do
    let «и» ← ((«строки».take m).reverse.map «имяФункции»).find? (· ≠ "")
    let f ← «функция» «строки» «и»
    let (m', t) ← f.«тело»
    if m' ≠ m then none else
    let ps := f.«строки».flatMap (fun p => match p.2 with | .«принимает» d => d.map (·.«имя») | _ => [])
    pure ⟨"«" ++ «и» ++ "»", ps, m, t⟩
...
def «утверждение» (h : String) (b : List String) («исходник» : List String)
    («опр» : List «Определение») : Except String «Утверждение» := do
...
def «запись» («текст» : String) («исходник» : List String) : Except String «Запись» := do
  ...
  let «утв» ← («утвержденияТекстом» «текст»).mapM fun (h, b) =>
    match «утверждение» h b «исходник» «опр» with
    | .ok u => pure u
...
/- ПРОБА ЧИТАТЕЛЯ (Т2, Э5, Э6 в приёмке): отображение, ярлык функции членом списка
   и ярлык на месте вызова в скобках — так их пишет подстановка сверщика. -/
#guard «список» "отобразить функции как ф → ф от 1" =
  some (.«отобразить» (.«имяС» "функции") "ф" (.«вызов» "ф" (.«ещё» (.«число» (.«лит» 1)) .«нет»)))
#guard «число» "функция «Удвоить»" = some (.«ярлык» "«Удвоить»")
#guard «список» "[функция «Удвоить», функция «Квадрат»]" =
  some (.«выписан» (.«ещё» (.«ярлык» "«Удвоить»") (.«ещё» (.«ярлык» "«Квадрат»") .«нет»)))
#guard «число» "( функция «Удвоить» ) от 1" =
  some (.«вызов» "функция «Удвоить»" (.«ещё» (.«число» (.«лит» 1)) .«нет»))
#guard «число» "функция «Удвоить» от 1" = none

/- ПРОБА ЧИТАТЕЛЯ (Ч5 в приёмке): «к строке» от длины выписанного списка. -/
#guard «текст» "( к строке ( длина [первый, второй] ) )" =
  some (.«кСтроке» (.«длина» (.«выписан» (.«ещё» (.«имя» "первый") (.«ещё» (.«имя» "второй") .«нет»)))))
