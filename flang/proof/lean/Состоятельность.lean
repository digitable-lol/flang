...
| Или1, Или2, И1, И2 | **леммы в `Правила.lean` нет** | своя, одной строкой |

Строки ведомости Или1, Или2, И1, И2 (семейство Р, ADR-0027/0029) лемм в
`Правила.lean` не имеют — это расхождение ведомости и Lean, оно записано
задачей (см. 2907); здесь их случаи закрыты леммами «или-влево»,
«или-вправо», «и-влево», «и-вправо» о булевых связках — по одной строке.

## Подмножество — числом

Теорема доказана для 29 правил приёмки (`«покрыто» = 29`, проверено
`decide`) из 70 приёмов `шаг_вывода`; из 13 именованных приёмов — для 11.
-/
...
theorem «или-влево» (l r : «Форм») (w : «Мир») (h : «Факт-из» l w) : «Факт-из» (.«или» l r) w := by
  simp only [«Факт-из», «оценитьФ»] at *; simp [h]
theorem «или-вправо» (l r : «Форм») (w : «Мир») (h : «Факт-из» r w) : «Факт-из» (.«или» l r) w := by
  simp only [«Факт-из», «оценитьФ»] at *; simp [h]
theorem «и-влево» (l r : «Форм») (w : «Мир») (h : «Факт-из» (.«и» l r) w) : «Факт-из» l w := by
  simp only [«Факт-из», «оценитьФ», Bool.and_eq_true] at *; exact h.1
theorem «и-вправо» (l r : «Форм») (w : «Мир») (h : «Факт-из» (.«и» l r) w) : «Факт-из» r w := by
  simp only [«Факт-из», «оценитьФ», Bool.and_eq_true] at *; exact h.2
...
theorem «шаг-верен» (u : «Утверждение») («преж» : List «Принятый») («ш» : «Шаг») (p : «Принятый»)
  ...
  | «Или1» n q l r ho hp h1 hf hp1 =>
    simp only [«итог»]
    rw [hf]; exact «подъём1» («или-влево» l r) (hp1 ▸ hinv q («шагПо-в» h1))
  | «Или2» n q l r ho hp h1 hf hp1 =>
    simp only [«итог»]
    rw [hf]; exact «подъём1» («или-вправо» l r) (hp1 ▸ hinv q («шагПо-в» h1))
  | «И1» n q l r ho hp h1 hp1 hf =>
    simp only [«итог»]
    rw [hf]; exact «подъём1» («и-влево» l r) (hp1 ▸ hinv q («шагПо-в» h1))
  | «И2» n q l r ho hp h1 hp1 hf =>
    simp only [«итог»]
    rw [hf]; exact «подъём1» («и-вправо» l r) (hp1 ▸ hinv q («шагПо-в» h1))
  | «ГипО» ho hp =>
  ...
  | «Разв2» n m q t ho hp h1 ht hpl hz hp1 =>
    ...
    rwa [«обновить-своё» w "результат" _ hr] at hv
...
/-- Число покрытых правил — 29; сверщик именует 70 приёмов в `шаг_вывода`. -/
theorem «покрыто-29» : «покрыто» = 29 := by decide
