...
| Т2 | — (о записи терма: переименование связанного имени) | «Т2-терм» в `Терм.lean` |
...
Теорема доказана для 74 правил приёмки (`«покрыто» = 74`, проверено
...
theorem «покрыто-число» : «покрыто» = 74 := by decide
