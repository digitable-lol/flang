...
#      Quot.sound;
#   3) отвергает ли Lean КАЖДОЕ из нарочно испорченных правил в «Ловушка.lean»
...
#      (`чекер/пробы/прогон.sh`) гоняется через обёртку, которая сохраняет
...
# Леммы ищутся в «Правила.lean» и в «Терм.lean»: правило о ЗАПИСИ терма (Т2 —
...
cat "$KAT/Правила.lean" "$KAT/Терм.lean" > "$RAB/леммы.lean"
PARY=$(awk -F'\t' '
...
echo
if ! ${CC:-cc} -std=c99 -O2 -o "$RAB/сверщик" "$CHEK/сверщик.c" 2> "$RAB/cc.out"; then
...
"$REAL" "$@" >"$d/вывод" 2>"$d/ошибки"; k=$?
echo "$k" > "$d/код"
cat "$d/вывод"; cat "$d/ошибки" >&2
exit $k
OB
chmod +x "$RAB/обёртка.sh"
SBOR="$RAB/сбор"; mkdir -p "$SBOR"
( export SBOR; REAL="$RAB/сверщик" SVERSCHIK="$RAB/обёртка.sh" FLANG_TMP="$RAB" \
  sh "$CHEK/пробы/прогон.sh" ) > "$RAB/пробы.out" 2>&1
KPROB=$?
NABOR=$(sed -n 's/^ *\(подделок [0-9]*\):.*/\1/p; s/^ *\(честных *[0-9]*\):.*/\1/p' "$RAB/пробы.out" | tr -s ' ' | paste -sd, - | sed 's/,/, /')
...
cut -f1 "$RAB/lean.tsv" | sort -u | while read -r d; do
  cat "$d/вывод" "$d/ошибки" | grep -o 'утверждение «[^»]*», вывод:' \
...
awk -F'\t' -v kras="$KRAS" -v sbros="$SBROS" -v par="$PAR" '
  ...
  END {
    ...
    printf "  вне приёмки Lean (первый непринятый шаг — правило, которого в Приёмка.lean нет): сверщик принял %d, отверг %d\n", vp, vo
...
while IFS='	' read -r d n t; do
  ...
  awk -v n="$n" '/^утверждение «/ { v = (index($0, "утверждение «" n "»") == 1) }
    v && /^ *вывод 1 / && !g { sub(/вывод 1 [^ ]+ /, "вывод 1 Н✗ "); g = 1 } { print }' "$z" > "$RAB/порча.запись"
  "$RAB/сверщик" "$i" "$RAB/порча.запись" 2>&1 \
