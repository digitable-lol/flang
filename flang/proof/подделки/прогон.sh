...
cmp -s "$RABOTA/честная" "$RABOTA/подменённый" && { say "подделка 1 не собралась: шага «по предположению» в записи нет"; exit 2; }
...
say ""
if [ "$BAD" -eq 0 ]; then
  say "все пять опытов сошлись: честное принято, четыре подделки отвергнуты"
  exit 0
fi
