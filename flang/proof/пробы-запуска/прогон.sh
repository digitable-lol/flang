...
#
#   sh flang/proof/пробы-запуска/прогон.sh            двоичным: flang run / flang io
#   sh flang/proof/пробы-запуска/прогон.sh --зондом   толкованием исходников compiler.flang
#   RABOTA=<каталог> sh flang/proof/пробы-запуска/прогон.sh --по-записям
...
#
# Что сверяется — ОЖИДАНИЕ.tsv: для каждой программы код возврата и слово в
# выводе. Код 0 — сошлось всё; код 1 — хоть одна проба разошлась, и она названа
# строкой. Слово «не доказано» и код 3 у недоказанной программы без ключа —
...
PROBY=$KOREN/flang/proof/пробы-запуска
...
# Одна проба зондом: доводы зонду — JSON с текстом программы; ответ зонда —
...
proba_zond() { # программа путь ключ → файл записи
  f=$PROBY/программы/$1; zap=$RABOTA/$1.$2.$3.txt
...
BAD=0; N=0; JOBS=0
awk -F'\t' '!/^#/ && $1 != "программа" && NF >= 5' "$PROBY/ОЖИДАНИЕ.tsv" > "$RABOTA/ожидание.tsv"
