...
#
#   sh flang/proof/пробы-недоказанного/прогон.sh
#   FLANG_BIN=<путь> sh flang/proof/пробы-недоказанного/прогон.sh
#
...
# С 0.7.21 недоказанная программа не считается вовсе (`flang/proof/пробы-запуска`
...
PROBY=$KOREN/flang/proof/пробы-недоказанного
export LC_ALL=C.UTF-8

[ -x "$FLANG" ] || { echo "нет двоичного: $FLANG (собрать: make -C bootstrap)" >&2; exit 2; }
[ -f "$PROBY/ОЖИДАНИЕ.tsv" ] || { echo "нет таблицы ожиданий: $PROBY/ОЖИДАНИЕ.tsv" >&2; exit 2; }
...
BAD=0; N=0
awk -F'\t' '!/^#/ && $1 != "программа" && NF >= 7' "$PROBY/ОЖИДАНИЕ.tsv" > "$RAB/ожидание.tsv"

while IFS="$(printf '\t')" read -r prog put fajl sreda klyuchi kod slovo; do
  N=$((N+1))
  ISHODNIK=$PROBY/программы/$prog
  if [ ! -f "$ISHODNIK" ]; then
...
for f in "$PROBY"/программы/*.flang; do
  [ -e "$f" ] || continue
  imya=$(basename "$f")
  if ! cut -f1 "$RAB/ожидание.tsv" | grep -q -a -F -x -- "$imya"; then
    printf '✗ НЕЗАЯВЛЕННАЯ ПРОГРАММА: %s лежит в каталоге, а строки о ней в ОЖИДАНИЕ.tsv нет\n' "$imya"
    BAD=$((BAD+1))
  fi
done
