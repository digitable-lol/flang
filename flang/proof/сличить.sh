...
# поднимает с миллиарда до трёхсот миллиардов (число берётся из
# `scripts/raskrutka.sh`), а тратится здесь 338 миллионов шагов — это 0,11 %
# потолка, запас 887 раз. Мерить заново незачем.
set -eu
