 ...
 *
 * ЗНАЧЕНИЕ «ДА|НЕТ» — ЧЕТВЁРТАЯ ГРАНИЦА, и до этой правки её не было вовсе.
 * `byType` — не украшение, а вывод ИЗЪЯТИЯ: ядро прогоняет то же сведение ещё
 * раз БЕЗ объявленных типов (`flang/self/proofterm.flang`, «Вердикт по
 * объявлению» → «Свести витками» с «Нет программы») и честно пишет, обошлось
 * ли без них. Повторить изъятие здесь значило бы завести в чекере второй
 * сводитель — то же самое «машинерии в чекере нет вовсе» из абзаца выше,
 * только для сведения целиком, а не для типов доводов. Проба (мутация
 * значения на честной записи) подтвердила: сегодня «да»↔«нет» не меняет ни
 * кода, ни счёта — значение принято молча. Это и есть ровно та немота, от
 * которой Ч27 велит спасаться словом «не берусь»: сведений без объявлений
 * чекер не проверяет, потому МЕСТО остаётся на слове ядра, а не в тени
 * трёх проверок выше. Считается за КАЖДЫМ валидным «по объявлению», отдельно
 * от того, снял ли что-то с этого же утверждения `perepiskoy`/`razborom_celi`
 * ниже — это другой довод (что цель верна), а не о том, нужен ли был тип. */
static void sverit_obyavlenie(Sverka *s, Sp svoi, const char *o_chyom) {
  ...
  esli_ne(s, !*pervaya_s_nachalom(svoi, "принцип тип "),
          fmt("%s: вердикт по объявлению стоит рядом с принципом, а этот маршрут его не даёт",
              o_chyom));
  /* Значение изъятия чекер не повторяет (см. врезку выше) — место остаётся
     на слове ядра, поимённо, а не молча. */
  if (*po) {
    s->na_slovo++;
    dobavit(&s->ne_vzyalsya,
            fmt("%s: значение «по объявлению %s» — сверщик не повторяет изъятие объявленного типа, место на слове ядра",
                o_chyom, slovo_posle(po, "по объявлению ")));
  }
}
