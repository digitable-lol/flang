...
/* ────────────────────────────────────────────────────────────────────────────
 * СПРОСИТЬ ДВОИЧНЫЙ О ЗНАЧЕНИИ ФУНКЦИИ ЯЗЫКА
 *
 * Выше всё спрашивается КОМАНДАМИ: `ast`, `check`, `emit`, `test`. Их у
 * двоичного одиннадцать, и каждая отвечает про целую программу. Сторожу же
 * бывает нужно не про программу, а про САМ ЯЗЫК: натуральное ли имя «нат»,
 * какое каноническое имя у формы «length», что цель печати считает занятым.
 * Команды такого вопроса не задают, и заводить под каждый ключ по команде
 * значило бы растить поверхность быстрее, чем растут вопросы.
 *
 * Отвечает на них ПРОГОНЩИК, и он у двоичного уже есть. Без доводов и без
 * терминала на входе бинарник работает протоколом «JSON на входе, JSON на
 * выходе, по запросу на строку» (`flang --help`, последний абзац). Запрос —
 * `{"fn": имя, "args": [...]}`, ответ — `{"ok": true, "value": …}`. Позвать так
 * можно любую функцию замыкания компилятора, то есть весь слой типов, всё ядро
 * доказательства и все восемь печатей.
 *
 * ── ПОЧЕМУ СЛОВАРЬ ЯЗЫКА НЕ ПЕРЕПИСЫВАЕТСЯ СЮДА ────────────────────────────
 *
 * Имена `нат`, `целое`, `вес`, точных десятичных, границы точной сетки, таблица
 * синонимов встроенных форм, наборы занятых целью имён — всё это ОТВЕТЫ ЯЗЫКА,
 * а не сведения о нём. Список, переписанный сюда, — второй ответ на тот же
 * вопрос: он не разойдётся с настоящим сегодня и разойдётся молча на первой же
 * правке слоя типов. Ровно так уже было со сравнением по ссылке в поиске
 * нарушений, и стоило это года зелёного сторожа, который ничего не стерёг.
 *
 * Поэтому здесь нет ни одного имени языка. Есть только дорога к тому, кто их
 * знает.
 *
 * ── ЦЕНА И ПАМЯТЬ ──────────────────────────────────────────────────────────
 *
 * Один процесс — семь миллисекунд, и на нём можно задать СКОЛЬКО УГОДНО
 * вопросов: протокол построчный. Поэтому вопросы копятся пачкой, а ответы на
 * уже заданное берутся из памяти: каноническое имя формы ядро доказательства
 * спрашивает десятки тысяч раз, а разных имён у него меньше полусотни.
 */
