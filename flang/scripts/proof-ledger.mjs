...
function сложить(записи) {
  const итог = {
    ...
    unaccounted: 0,
    guardSites: 0,
    /* `permits` стоит здесь, а не в `итоги_` ведомости, и разница
       содержательная: ведомость отвечает про ОДИН модуль, и раздел разрешений у
       неё появляется только там, где они есть, — а свод меряет корпус, и ноль в
       нём говорит «разрешений нет» ровно то же, что отсутствие ключа, но без
       ветвления у читателя. Тот же довод, что у `claims` строкой ниже. */
    laws: { grid: 0, assumed: 0, gridValues: 0, gridPoints: 0, permits: 0 },
...
function печать(свод, поФайлам) {
  ...
  строки.push(
    ...
    `  из них выдано разрешений: ${и.laws.permits}`,
