...
const IMPLEMENTATIONS = {
  ...
  /*
   * «приписать x к списку» — тот же список с x впереди.
   *
   * ЧТО ЭТА ФОРМА ОБЕЩАЕТ, А ЧТО НЕТ. Обещает она значение, а не стоимость, —
   * ровно как `элемент` (см. довод над ним). Здесь, в вычислителе, список это
   * массив JS, и копия неизбежна: `[x, ...список]` стоит длины. В печати цена
   * зависит от устройства цели, и она названа в SPEC, раздел «Стоимость
   * встроенных форм»: у C и Elixir постоянная, у шести остальных — одна копия
   * на вызов.
   *
   * ЗАЧЕМ ФОРМА НУЖНА, ЕСЛИ В ВЫЧИСЛИТЕЛЕ ОНА ВСЁ РАВНО КОПИРУЕТ. Потому что
   * до неё приписывание в начало выражалось единственным способом — свёрткой
   * `свёртка элементы начиная с [первый] как акк и эл → добавить эл к акк`, —
   * а это N вызовов `добавить`, каждый со своей копией: одна копия против N.
   * На «Простые до 4000» разница между этими двумя записями измерена и она не
   * в разах, а в КЛАССЕ: куб против квадрата (tmp/bench/primes-curve.mjs).
   */
  "приписать": (args, span) => {
