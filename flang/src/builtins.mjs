...
const IMPLEMENTATIONS = {
   ...
   *
   * ЧТО ВОЗВРАЩАЕТ. Строку, а не какой-то отдельный вид «символа»: вида
   * «символ» в языке нет вовсе, и заводить его ради одной формы значило бы
   * добавить скаляр во все восемь целей печати. `символы` уже раскладывает
   * строку в список ОДНОСИМВОЛЬНЫХ СТРОК, `символ N в S` тоже отдаёт строку —
   * форма не расходится с ними, а встаёт в тот же ряд.
   *
