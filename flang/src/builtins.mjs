...
const IMPLEMENTATIONS = {
   ...
   * форма не расходится с ними, а встаёт в тот же ряд. Обратный ход замкнут:
   * `код символа (символ по коду н)` равно `н`, и это доказывается ТИПОМ, а не
   * примером, — результат несёт длину не меньше 1, и частичность `код символа`
   * на нём снимается.
   *
   ...
   * ломает обещание «ровно один символ», на котором держится обратный ход, а
   * молчаливая подмена — это ровно тот дефект, который в `stdlib/http.flang`
   * назван прямым текстом («испортить молча хуже, чем не тронуть»). Отказ виден
   * и приходит с кодом `FLANG_BUILTIN_ARGS` — тем же, каким отказывают все
   * восемь остальных частичных форм, то есть множество отказов языка формой не
   * расширяется ни на один вид.
   *
   * ГРАНИЦА ОТКАЗА — ровно три условия, и все три проверяются здесь:
   *
   *   • не число или не целое      `символ по коду 65.5` — половины точки нет;
   *   • меньше нуля                 отрицательной кодовой точки не бывает;
   *   • больше 0x10FFFF             Unicode кончается здесь, и это граница
   *                                 стандарта, а не представления.
   *
   * СУРРОГАТ (0xD800…0xDFFF) ПРОПУСКАЕТСЯ, и это решение, а не недосмотр.
   * Довод первый: `код символа` УЖЕ ОТДАЁТ числа из этого отрезка — одинокая
   * половина пары приезжает снаружи, из сети и из файла, и на ней форма честно
   * считает 55296…57343. Отказать в обратную сторону значило бы разорвать
   * обратный ход ровно на тех входах, где язык сам признаёт, что такие числа у
   * него бывают. Довод второй, и он решает: множество отказов обязано быть
   * ОТРЕЗКОМ, иначе его нечем доказать. Уточнение типа в `types.mjs` умеет
   * говорить «число лежит в [низ, верх] и целое» — и не умеет говорить «лежит,
   * но не в дырке посередине». С дыркой доказуемой стала бы НИ ОДНА запись, в
   * том числе главная — `символ по коду (код символа знак)`.
   *
   ...
   * Обратный ход на нём возвращает `0`, а не `-0`, и это не потеря: кодовая
   * точка у обоих одна.
   */
