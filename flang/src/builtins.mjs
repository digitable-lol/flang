 ...
 * Правила два, оба обязательны: читать только до `длинаСписка` (за концом вида
 * лежат чужие ячейки) и не менять ничего. Взамен обход свёртки и «отобразить»
 * не платит ни копией, ни проверкой вида на каждом элементе.
 */
...
const IMPLEMENTATIONS = {
   ...
   *
   * ОБРАТНОЙ ФОРМЫ («символ по коду») ЗДЕСЬ НЕТ. Она не нужна ни хешу, ни
   * порядку — обе идут из строк в числа и обратно не возвращаются, — а стоила
   * бы восьми рантаймов и решения про то, что делать с числом, которое кодовой
   * точкой не является. Появится задача, которой она нужна, — появится и форма.
   */
