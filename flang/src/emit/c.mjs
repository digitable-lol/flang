...
//
// ── Откуда взялась форма этого бэкенда ────────────────────────────────────
// Он написан как продолжение бэкенда C старого проекта FTS (тот вынесен из
// репозитория, тег `fts-pered-udaleniem`). Там печатались только скаляры —
// хватало `double` и `const char *`, и рантайма почти не было. Здесь язык
// полный, поэтому появляется рантайм (flang/src/emit/c/flang_runtime.[ch]) с
// представлением значений, ареной и UTF-8. Два решения оттуда сохранены
// намеренно, и менять их без довода не надо:
//
...
//
// Одно решение оттуда здесь сознательно НЕ повторено: сравнение чисел с
...
const BUILTIN_HELPERS = new Map([
  ...
  ["символ по коду", "fl_b_simvol_po_kodu"],
  ...
  if (repl) {
    files.push({
      ...
      content: [
        ...
        `#define FL_PROGRAM_CALL ${prefix}_call`,
        /* Оболочка зовёт программу по имени, а `flang emit --target c` внутри
           неё просит у программы ещё и ЕЁ СОБСТВЕННУЮ границу входа: таблицу
           объявленных типов строит слой типов свидетеля, и печати самого себя
           взять её больше неоткуда. Второе имя печатается здесь, рядом с
           первым, а не выводится в оболочке из первого: как зовётся точка
           входа программы, решает бэкенд, и решать это дважды нельзя. */
        `#define FL_PROGRAM_ENTRY ${prefix}_entry`,
        "",
        REPL_SOURCE,
      ].join("\n"),
...
function renderHeader(file, moduleName, shared, concurrent = false) {
  ...
  lines.push(
    ...
    " */",
    `fl_status ${shared.prefix}_enter(fl_ctx *ctx, const char *name, const fl_value *args, size_t count,`,
    "                    fl_value *result, fl_error *error);",
    "",
    "/*",
...
function emitValue(expr, ctx, out, pad) {
  ...
  switch (node.kind) {
    ...
    case "binary": {
      /* Доказанное место считается выражением, а сверка тегов остаётся, но
         печатается ЗДЕСЬ — одним `if` в теле вызывающего. Стоит ДО вычисления
         операндов: `emitNumber` печатает их сам и по-своему — числовым
         литералам вовсе незачем становиться `fl_value`, чтобы тут же быть
         распакованными обратно. */
      if (node.числовая === true) {
        ...
        if (арифметика !== undefined) {
          ...
          const п = emitNumber(node.right, ctx, out, pad)
          сверкаЧисел(л, п, `fl_not_numbers(ctx, ${cstring(node.op)}`, out, pad)
          return `fl_number(${арифметика(л.число, п.число)})`
        }
        const знак = ПРЯМОЙ_ПОРЯДОК.get(node.op)
        if (знак !== undefined) {
          const л = emitNumber(node.left, ctx, out, pad)
          const п = emitNumber(node.right, ctx, out, pad)
          сверкаЧисел(л, п, "fl_not_order(ctx", out, pad)
          return `fl_flag(${л.число} ${знак} ${п.число})`
        }
      }
 ...
 *
 * Отдаётся тройка: `число` — выражение типа `double`; `значение` — то же самое
 * как `fl_value`, нужное ТОЛЬКО тексту отказа; `сверять` — надо ли смотреть тег
 * перед чтением.
 *
 * Три случая, и порядок между ними содержательный.
 *
 * ЧИСЛОВОЙ ЛИТЕРАЛ отдаётся самим числом и сверки не требует: тег у него стоит
 * при печати. Заворачивать `2` в `fl_number(2.0)`, чтобы следующей же строкой
 * прочитать оттуда `.as.number`, — работа, которой незачем появляться даже в
 * тексте.
 *
 * ВЛОЖЕННАЯ ДОКАЗАННАЯ АРИФМЕТИКА складывается в одно выражение: `(а плюс б)
 * умножить на ц` печатается как `(a + b) * c`, а не как две упаковки в
 * `fl_value` и две распаковки. Сверки она тоже не требует — числом её сделала
 * предыдущая строка, — а вот в текст отказа попасть может, и для этого рядом
 * лежит `fl_number(…)`. Скобки ставятся на вложении, потому что приоритеты C и
 * flang совпадать не обязаны.
 *
 * ВСЁ ОСТАЛЬНОЕ печатается обычным путём, сверяется по тегу и распаковывается
 * полем.
 */
function emitNumber(expr, ctx, out, pad) {
  ...
  if (node.kind === "literal" && typeof node.value === "number") {
    ...
    return { число: cnumber(node.value), значение: `fl_number(${cnumber(node.value)})`, сверять: false }
  }
  if (node.kind === "binary" && node.числовая === true) {
    ...
    if (арифметика !== undefined) {
      ...
      const п = emitNumber(node.right, ctx, out, pad)
      сверкаЧисел(л, п, `fl_not_numbers(ctx, ${cstring(node.op)}`, out, pad)
      const текст = `(${арифметика(л.число, п.число)})`
      return { число: текст, значение: `fl_number(${текст})`, сверять: false }
    }
  }
  const ident = emitValue(node, ctx, out, pad)
  return { число: `${ident}.as.number`, значение: ident, сверять: true }
}
 ...
 * `«Отобразить» от Удвоить и [1, "два"]` без этой строки давало
 * 1.32e-309 вместо FLANG_TYPE — то есть отказ менялся на неверный ответ.
 *
 * ЧТО ПРИ ЭТОМ ВСЁ-ТАКИ ВЫИГРАНО. Не сама проверка, а то, что вокруг неё:
 * межмодульный вызов, передача двух значений по 32 байта, возврат через
 * указатель и ветвь отказа посреди горячего кода. Здесь остаётся одно сравнение
 * тега в теле вызывающего, а всё остальное считается выражением, которое
 * компилятор держит в регистрах.
 *
 ...
 */
function сверкаЧисел(л, п, отказ, out, pad) {
  const условия = [л, п].filter((о) => о.сверять).map((о) => `${о.значение}.tag != FL_NUMBER`)
  if (условия.length === 0) return
  out.push(`${pad}if (${условия.join(" || ")}) FL_TRY(${отказ}, ${л.значение}, ${п.значение}, error));`)
}
...
function renderDispatch(shared) {
  ...
  lines.push(
    ...
    `fl_status ${shared.prefix}_enter(fl_ctx *ctx, const char *name, const fl_value *args, size_t count,`,
    "                    fl_value *result, fl_error *error) {",
    `  FL_TRY(fl_check_entry(ctx, ${shared.prefix}_entry(), name, args, count, error));`,
    `  return ${shared.prefix}_call(ctx, name, args, count, result, error);`,
    "}",
  )
  return lines.join("\n")
}
...
/* ═══════════════════════════ конкурентность ═══════════════════════════
   ...
   тем же для всех программ, иначе сверять со свидетелем пришлось бы не модель, а
