 ...
 *
 * ── Второй вход: `flang repl` ──────────────────────────────────────────────
 * Когда этой программой оказывается сам компилятор flang, у бинарника есть и
 * человеческий вход — интерактивная оболочка. Она тоже прогонщик: те же вызовы
 * функций по имени, только запросы придумывает не инструмент, а человек, и
 * ответы печатаются поверхностью языка, а не JSON. Живёт она в СОСЕДНЕМ файле
 * (flang_repl.c), и не для порядка: оболочка обязана спрашивать у мира, где
 * `cc` и где каталоги установки, а этот файл обязан остаться переносимым C99,
 * который ни от чего не зависит и ничего не спрашивает. Здесь остаётся одна
 * строка — передача управления.
 *
...
/*
 * Интерактивная оболочка — соседний файл печати (flang_repl.c), и печатается он
 * по просьбе, а не всегда: осмысленна оболочка ровно у одной программы — у
 * самого компилятора flang, потому что только у него есть точки входа, которые
 * она зовёт. Просьба приезжает тем же способом, что и имя функции вызова, —
 * `#define` перед этим файлом.
 */
#ifdef FL_WITH_REPL
extern int fl_repl_main(int argc, char **argv, const char *self);
#endif
...
int main(int argc, char **argv) {
  ...
  char *input = NULL;
#ifdef FL_WITH_REPL
  /* `flang repl` — человеческий вход; без него всё как было, JSON со стдина.
     Разбор аргументов нарочно однословен: у прогонщика ключей нет и не должно
     быть, иначе его протокол перестал бы быть протоколом. */
  if (argc > 1 && strcmp(argv[1], "repl") == 0) {
    return fl_repl_main(argc - 1, argv + 1, argv[0]);
  }
#else
  (void)argc;
  (void)argv;
