 ...
 * flang/stdlib одним файлом кеша — это 4427 разных ключей. Но многочленный
 ...
 *
 * ЦЕНА ЧЕСТНОГО КЛЮЧА НАЗВАНА, И ОНА ЗАВИСИТ ОТ ТОГО, ЧЕЙ SHA-256. На
 * base64.flang (125 обязательств, вперемежку, минимум из повторов): горячий
 * кеш 3,09 с многочленом, 19,4 с через sha256 НА FLANG и 3,03 с через
 * НАТИВНЫЙ sha256 рантайма — тот самый, каким снят отпечаток двоичного выше.
 * То есть дорог не sha256, а его flang-овая запись списками байтов
 * (flang/stdlib/sha256.flang): она съедает 85 % выгоды кеша и делает холодный
 * проход на 71 % дороже, чем вовсе без кеша. Нативный ключ стоит НОЛЬ в
 * пределах шума, но требует встроенного слова языка, а встроенное слово —
 * девяти печатей; это отдельная работа, и её цена здесь названа замером.
 *
