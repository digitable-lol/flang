 ...
 *
 * ЦЕНА ЧЕСТНОГО КЛЮЧА НАЗВАНА, И ОНА СЪЕДАЕТ КЕШ ЦЕЛИКОМ. На base64.flang
 * (125 обязательств, вперемежку, минимум из повторов) горячий кеш стоит 3,10 с
 * многочленом и 23,0 с с sha256 в обоих слоях — при 22,2 с ВОВСЕ БЕЗ КЕША.
 * То есть честный кеш на этом файле не экономит ничего, а холодный проход с ним
 * стоит 42,5 с против 22,2 с. Дорог не sha256, а его flang-овая запись списками
 * байтов (flang/stdlib/sha256.flang): тот же ключ НАТИВНЫМ sha256 — тем самым,
 * каким снят отпечаток двоичного выше, — стоит 3,03 с, ноль в пределах шума.
 * Нативный ключ требует встроенного слова языка, а встроенное слово — девяти
 * печатей; это отдельная работа, и её выгода здесь названа замером. Пока её
 * нет, выбор сделан в пользу честного ключа, а не быстрого обмана.
 *
 * ПОКА НЕ ПРОШЛА ПЕРЕПЕЧАТКА, СЕМЯ ДЕРЖИТ СТАРЫЙ КЛЮЧ: правка лежит в
 * flang/self/proofterm.flang, а собранный bootstrap/flang считает по-прежнему
 * многочленом. Кеш ЭТОГО двоичного подделывается за две минуты.
 */
