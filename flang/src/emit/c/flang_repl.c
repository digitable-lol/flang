...
static const char HELP_EMIT_2[] =
    ...
    "Ключа «печатать без проверки» здесь НЕТ намеренно. Единственный, кому он был бы\n"
    "нужен, — пересборка компилятора самим собой, а она обязана идти через проверку:\n"
    "ключ в скопированном рецепте становится привычкой за день.\n"
    "\n"
