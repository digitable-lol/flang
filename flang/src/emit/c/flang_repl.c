...
#define EMIT_TARGETS_SAY \
  "целей здесь ДЕСЯТЬ — «c», «cpp», «go», «rust», «java», «js», «ts», «elixir», «python», «csharp»"
...
static const char HELP_MACHINE[] =
    "flang --machine [<файл>]        (кириллицей: --машина)\n"
    "\n"
    ...
    "\n"
    "  flang --machine           снять постоянную здесь и сейчас и напечатать\n"
    "  flang --machine <файл>    взять снятую раньше и напечатать как есть\n"
    "\n"
...
static const char FLANG_HELP[] =
    ...
    "  flang test <файл|каталог>          прогон примеров: один файл или целый каталог\n"
...
static const char HELP_CHECK[] =
    ...
    "\n"
    "  --fast             только связывание, типы, исчерпываемость и\n"
    ...
    "                     ведомость — это ответ ядра. Кириллицей: --быстро\n"
    ...
    "  --strict           вместе с --proof: четыре исхода разъезжаются кодами.\n"
    "                     0 — ДОКАЗАНО, и только если у КАЖДОГО утверждения\n"
    "                     вердикт «доказано»; 1 — ОПРОВЕРГНУТО; 3 — НЕ УДАЛОСЬ\n"
    "                     ДОКАЗАТЬ (опора на сетку значений автора, на\n"
    "                     недоказанную посылку или «объявлено, не доказано»);\n"
    "                     2 — НЕ ПОДДЕРЖИВАЕТСЯ. Код 4 остаётся за «--fast».\n"
    "                     Без ключа опора остаётся нулём и называется строкой.\n"
    "                     Кириллицей: --строго\n"
    "  --record <файл>    вместе с --proof: положить САМО доказательство в файл\n"
    ...
    "                     FLANG_RECURSION_LIMIT с числом. Кириллицей: --предел-шагов\n"
    "  --depth-limit N    поднять предел ГЛУБИНЫ вызовов на этот прогон. Ключ\n"
    ...
    "                     этот — в текущий прогон. Кириллицей: --предел-глубины\n";
...
static const char HELP_TEST[] =
    ...
    "это НАБОР файлов, а не один файл:\n"
    "\n"
    "  flang test flang/stdlib/                весь каталог, вглубь\n"
    "  flang test \'examples/**/*.flang\'  по маске (кавычки — от оболочки)\n"
    "\n"
    "У набора печатается каждый не прошедший пример и каждый не взятый файл, а\n"
...
static const char HELP_EMIT[] =
    ...
    "                        [--no-postconditions]\n"
    "\n"
    "Печатает программу без Node — во все десять целей. Каталог из «--out»\n"
    ...
    "  --protocol файл   рядом с C напечатать протокол перевода (только у цели «c»,\n"
    "                    ADR-0030): что во что перешло и по какому правилу; его\n"
    "                    переигрывает flang/translation/matcher.c. Кириллицей —\n"
    "                    --протокол\n"
...
static const char HELP_EMIT_3[] =
    ...
    "    («scripts/seed-fingerprint»), а собранный из такого семени двоичный толще\n"
...
static const char HELP_IO_3[] =
    ...
    "\n"
    "«Размер экрана» отдаёт «Экран» с «ширина» и «высота» — знаков в строке и\n"
    "строк на экране. Нулевой или нечитаемый размер — отказ, а не 80×24: кадр по\n"
    "выдуманной ширине рисуется молча и неверно.\n"
    "\n"
    "Терминала нет — вывод уведён в трубу или в файл, CI, nohup — и все три\n"
    "поручения отвечают FLANG_IO_NO_SCREEN, ровно как отвечали всегда. Чего у\n"
    "двоичного хозяина нет по-прежнему — СВОЕГО шифрования. Нехватка названа\n"
...
static const char HELP_NEW[] =
    ...
    "\n"
    "  --force   переписать каталог, если он уже существует. Кириллицей: --силой.\n"
    "            Без ключа существующий каталог — отказ\n"
 ...
 *
 * ЗАЧЕМ ОНА. Перепечатка семени (`scripts/bootstrap-reprint.sh`) идёт от пяти с
 ...
 * шагов на весь «Суд ядра о программе», шапка scripts/bootstrap-reprint.sh), получила
...
static fl_status repl_call_within(const char *name, const fl_value *args, size_t count, fl_value *result) {
  ...
  if (!repl_call_quiet && repl_ctx.max_steps > 0 && repl_ctx.steps > repl_ctx.max_steps / 2) {
    fprintf(stderr,
            ...
            "следующий раз не поместится. Умолчание задаётся в scripts/bootstrap-reprint.sh,\n"
 ...
 * 1,98 млн витков в секунду» в шапке scripts/bootstrap-reprint.sh снято ИМЕННО с
 ...
 * (`flang/test/встроенные-формы.flang`). Шестьдесят четыре килобайта берут
 ...
 * (`flang/proof/checker/README.md`) и у `check --быстро` выше:
 *
 ...
 *
 * ── ЧЕТЫРЕ ИСХОДА ПОД `--строго` (задача 5502, пункт 5 внешнего аудита) ────
 * Аудит требует различать «доказано», «опровергнуто», «не удалось доказать» и
 * «не поддерживается» и запрещает успех без независимой проверки ВСЕХ
 * обязательств. Исходов под ключом ровно четыре, и НОВЫХ ЧИСЕЛ НЕ ЗАВЕДЕНО НИ
 * ОДНОГО: ADR-0010 §2 обещает четыре кода, ADR-0045 §4 тем же доводом отказался
 * заводить пятое число для `run`.
 *
 *   ДОКАЗАНО             0  у КАЖДОГО утверждения вердикт «доказано»
 *   ОПРОВЕРГНУТО         1  найден контрпример: «нарушено» ≥ 1
 *   НЕ УДАЛОСЬ ДОКАЗАТЬ  3  противоречия нет, доказано не всё: «объявлено, не
 *                           доказано», «отвергнуто», закон на веру, сетка,
 *                           «условно», закон на сетке
 *   НЕ ПОДДЕРЖИВАЕТСЯ    2  объявлено то, чего прибор не судит вовсе; ветка
 *                           стоит ВЫШЕ этого приговора, в `proof_file`
 *
 * ЧТО ИМЕННО ПОМЕНЯЛОСЬ ОДНИМ ЗНАКОМ: сетка под `--строго` уезжает с 4 на 3.
 * Код 4 остаётся за `--быстро` и значит ровно одно — «часть проверок не
 * запускалась»; сетка же не «не запускали», а «запускали и не доказали», и это
 * исход 3. В умолчании не меняется ни знака: без ключа сетка по-прежнему ноль,
 * и названа она СТРОКОЙ, как и была.
 *
 * ДЫРА, РАДИ КОТОРОЙ ПРАВКА (прогон 19 сентября 2026). Программа с ЛОЖНЫМ
 * постусловием `результат не больше 100` при теле `н плюс н` и парой строк
 * `пример` получала ноль от `check`, от `check --proof` и от независимого
 * сверщика разом, а `run --на-веру` на н = 100 печатал FLANG_PROPERTY и код 1.
 * Ноль ПОКУПАЛСЯ примером: тот же файл без `пример` давал 3. Слова у приборов
 * были честные, код возврата — нет, и сборочный сценарий, читающий `$?`,
 * пропускал ложь. Проба — `flang/proof/probes/strict/`.
 *
 * СЕТЬ ПОД НУЛЁМ, и она не про сегодняшние разряды. Перечисленные `if`-ы —
 * закрытый список ведомости НА СЕГОДНЯ; заведись в ней завтра новый разряд, он
 * не попал бы ни в один из них, и успех приехал бы МОЛЧА — ровно та порода
 * беды, ради которой ключ и заведён. Поэтому под `--строго` ноль требует ещё и
 * равенства «доказано == всего»: неизвестный разряд ловится без имени. Пустая
 * ведомость остаётся нулём и здесь: 0 == 0.
 */
static int proof_verdict(const proof_tally *t, bool strict) {
  ...
  if (t->grid > 0 || t->conditional > 0 || t->laws_grid > 0) {
    return 3;
  }
  return t->proved == t->total ? 0 : 3;
}
 ...
 *
 * СЛОВА УМОЛЧАНИЯ НЕ ТРОНУТЫ НИ БУКВОЙ (задача 5502): их читают руководства,
 * страницы сайта и чужие сценарии, и менять их заодно с кодом значило бы
 * ломать два договора вместо одного. Под `--строго` печатается ИМЯ ИСХОДА из
 * четырёх, а рядом с «НЕ УДАЛОСЬ ДОКАЗАТЬ» — чем именно не удалось: пробелом
 * («объявлено, не доказано») или опорой («сетка»). Старшинство то же, что у
 * приговора: пробел важнее опоры, и на программе, где есть ОБА, называется
 * пробел — код при этом 3, а не 4, и проба на это стоит отдельная.
 *
 * Ветки `code == 4` здесь нет: приговор её больше не ставит, а «--быстро» до
 * этой печати не доходит вовсе (ведомости у него нет). Мёртвая ветка читалась
 * бы как «исход возможен», а он невозможен.
 */
static void proof_say_verdict(const char *path, const proof_tally *t, int code, bool strict) {
  const bool leaning = t->grid > 0 || t->conditional > 0 || t->laws_grid > 0;
  const bool gaps = t->declared > 0 || t->refused > 0 || t->laws_assumed > 0;
  const char *word = code == 1   ? (strict ? "ОПРОВЕРГНУТО" : "НЕ СОШЛОСЬ")
                     : code == 3 ? (strict ? (gaps ? "НЕ УДАЛОСЬ ДОКАЗАТЬ — ОБЪЯВЛЕНО, НЕ ДОКАЗАНО"
                                                   : "НЕ УДАЛОСЬ ДОКАЗАТЬ — ОПОРА НЕ СУДИЛАСЬ")
                                            : "НЕ ПРОВЕРЕНО")
                     : strict    ? "ДОКАЗАНО"
 ...
 * (`corpus-share.sh`). С абсолютным путём выходит «$root//srv/…», исходник не
...
static int proof_file(const char *path, bool json, const char *record, bool strict) {
   ...
   * доказательства НАПЕЧАТАЛАСЬ. Сверщик записи (`flang/proof/checker`) её
   ...
   */
  {
    ...
    if (has_program) {
       ...
       * печататься у ЧЕТЫРНАДЦАТИ из шестнадцати `flang/concurrency/examples` — тех
       ...
       * общему списку стоил «68 обязательств в 17 файлах каталога flang/concurrency», и
       ...
       */
      if (unjudged) {
        /* ИМЯ ИСХОДА — только под ключом: в умолчании здесь печатается ровно
           то же, что и вчера, а под `--строго` четвёртый исход обязан быть
           назван так же внятно, как три остальных. */
        if (strict) {
          fprintf(stderr,
                  "%s: НЕ ПОДДЕРЖИВАЕТСЯ — названное выше прибор не судит вовсе — код возврата 2\n",
                  path);
        }
        fflush(stderr);
        strings_free(&paths);
...
static int emit_file(int argc, char **argv, const char *self) {
  ...
  if (code == 0) {
    ...
    if (code == 0) {
       ...
       * `scripts/bootstrap-reprint.sh` стоит под `set -e`, и любой ненулевой код печати
...
static fl_value io_perform(io_host *host, fl_value order) {
  ...
  if (io_order_is(order, "Размер экрана")) {
    if (!host->screen) {
      return io_fail("FLANG_IO_DENIED", "хозяину запрещено смотреть на экран");
    }
    /* Порядок тот же, что у соседей: запрет спрашивается ДО терминала, терминал
       — двумя условиями сразу (isatty(1) и открытый «/dev/tty»), и в трубе,
       в файле, в CI и под nohup ответ остаётся прежним FLANG_IO_NO_SCREEN. */
    if (!io_screen_open()) {
      return io_screen_none(order);
    }
#ifdef TIOCGWINSZ
    {
      struct winsize frame;
      fl_value fields[2];
      memset(&frame, 0, sizeof(frame));
      if (ioctl(io_screen_fd, TIOCGWINSZ, &frame) != 0) {
        return io_fail_errno("FLANG_IO_NO_SCREEN", "размер экрана у терминала не спрашивается");
      }
      /* Нулевой размер — не «ответа нет», а ответ, которому нельзя верить: так
         отвечает терминал без рамки. 80×24 ЗДЕСЬ НЕ ВЫДУМЫВАЕТСЯ: кадр,
         посчитанный по придуманной ширине, рисуется молча и неверно, и ни
         хозяин, ни план об этом не узнают. Взять 80×24 волен сам план — но
         тогда это его решение, названное в его же коде. */
      if (frame.ws_col == 0 || frame.ws_row == 0) {
        return io_fail("FLANG_IO_NO_SCREEN",
                       "терминал отдал нулевой размер экрана: ширину и высоту взять неоткуда, а 80×24 здесь не выдумывается");
      }
      fields[0] = io_pair("ширина", io_number((double)frame.ws_col));
      fields[1] = io_pair("высота", io_number((double)frame.ws_row));
      return io_variant("Экран", fields, 2);
    }
#else
    /* Сборка без TIOCGWINSZ: спрашивать размер нечем, и это тоже отказ, а не
       выдуманное число. */
    return io_fail("FLANG_IO_NO_SCREEN",
                   "эта сборка хозяина не знает TIOCGWINSZ: спросить размер экрана нечем");
#endif
  }
