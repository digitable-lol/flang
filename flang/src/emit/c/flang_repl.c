 ...
 * На base64.flang (125 обязательств, вперемежку, минимум из повторов) три
 * двоичных, отличающихся РОВНО одной функцией «Отпечаток строки кеша»:
 *
 *   прогон       многочлен  sha256 на flang  хеш256
 *   без кеша       22,80 с        22,89 с    22,60 с
 *   холодный       23,76 с        40,40 с    22,72 с
 *   горячий         3,30 с        20,18 с     3,10 с
 *
 * То есть выигрыш кеша вернулся — 7,3 раза, — и ключ при этом остался честным:
