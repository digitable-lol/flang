...
static const char HELP_EMIT[] =
    ...
    "Ключа «печатать без проверки» здесь НЕТ намеренно: у свидетеля на Node он есть\n"
    "(«--no-check», для отладки самой печати), а здесь единственный, кому он был бы\n"
    "нужен, — пересборка компилятора без Node, и она обязана идти через проверку.\n"
    "\n"
