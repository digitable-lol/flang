...
/*
 * Продлевает последний выданный блок на месте, не копируя.
 *
 * Удаётся только если блок кончается ровно на кромке выдачи — то есть после
 * него арена не выдала ничего другого. Тогда байты за его концом не принадлежат
 * никакому значению, и занять их безопасно: ни одно уже созданное значение не
 * меняется, потому что каждое несёт свою длину и дальше неё не смотрит.
 *
 * Это и есть условие линейности: продлить список может только тот, кто держит
 * его последнюю версию. Любой другой держатель (или тот же, но во второй раз)
 * кромку не угадает, получит false и пойдёт обычным путём — через копию. Так
 * неизменяемость значений сохраняется без единой проверки владения.
 *
 * Возвращает false и когда места в куске не хватило: вызывающий копирует.
 */
...
/** Список из готового массива значений (массив обязан жить в арене). */
