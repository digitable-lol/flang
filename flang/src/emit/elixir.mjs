...
//    интерпретатора и у остальных бэкендов она квадратична. Даром достаётся и
//    неизменяемость при «добавить»: там, где C, Go и Rust держат отметку
//    занятого в общем массиве и уводят ветвление на копию, здесь ветвление —
//    две головы поверх общего конца (flang_runtime.ex, moduledoc, раздел 5).
//
...
const BUILTIN_HELPERS = new Map([
  ...
  ["символ по коду", "b_char_from_code"],
  ["содержит", "b_contains"],
 ...
 *
 * −0 печатается ВЫЗОВОМ `Flang.Rt.neg_zero()`, а не литералом `-0.0`. До OTP 27
 * `-0.0 =:= 0.0` истинно, и компилятор BEAM вправе считать `{:num, 0.0}` и
 * `{:num, -0.0}` одним термом — а он и считает: на OTP 25 разбор с образцами
 * `0` и `-0` в одной функции сливал оба литерала, и ветвь минус нуля
 * становилась недостижимой. Литерал в напечатанном коде опирался бы на версию
 * машины, где этот код будут собирать, а её нам знать неоткуда.
 */
function exnumber(value) {
  if (Number.isNaN(value)) return ":nan"
  if (value === Infinity) return ":inf"
  if (value === -Infinity) return ":ninf"
  if (Object.is(value, -0)) return "Flang.Rt.neg_zero()"
...
function emitValue(expr, ctx, out, pad) {
  ...
  switch (node.kind) {
    ...
    case "builtin": {
      ...
      return `Flang.Rt.${помощникФормы(canonical, node, BUILTIN_HELPERS, СУФФИКС_ДОКАЗАННОГО)}(${rendered.join(", ")})`
    }
    case "binary": {
    ...
    case "cons":
      ...
      if (pattern.tail !== undefined && pattern.tail !== null) {
        /* Единственное место, где Elixir дешевле остальных целей: список
           ...
           Оговорка одна: если по этому списку успело пройти «добавить»,
           накопленный конец разворачивается — один раз на всю цепочку «хвостов»,
           а не на каждый (flang_runtime.ex, moduledoc, раздел 5).
 ...
 *
 * Сверяет таблицу `Flang.Rt.check_entry` — один и тот же текст для всех
 * программ, а строит её `таблицаВхода` из flang/src/types.mjs, то есть тот же
 * файл, что отвечает на этот вопрос для `flang run --args`.
 */
function renderEntry(таблица) {
  const список = (строки, хвост) =>
    строки.length === 0 ? [`      []${хвост}`] : ["      [", ...строки, `      ]${хвост}`]
  return [
    "  @doc \"\"\"",
    "  Граница входа: объявленные типы параметров данными.",
    "",
    ...
    "  def entry do",
    "    {",
    ...список(таблица.типы.map((запись) =>
      `        {${ВИДЫ_ТИПА_EX.get(запись.вид) ?? ":unknown"}, ${exstring(запись.имя)}, ` +
      `${exstring(запись.владелец)}, ${запись.ничто}, ${запись.целое}, ${запись.отрезок}, ` +
      `${exnumber(запись.низ)}, ${exnumber(запись.верх)}, ${запись.элемент}, ` +
      `${запись.полеС}, ${запись.полей}, ${запись.вариантС}, ${запись.вариантов}},`), ","),
    ...список(таблица.поля.map((поле) => `        {${exstring(поле.имя)}, ${поле.тип}},`), ","),
    ...список(таблица.варианты.map((вариант) =>
      `        {${exstring(вариант.имя)}, ${вариант.полеС}, ${вариант.полей}},`), ","),
    ...список(таблица.параметры.map((параметр) =>
      `        {${exstring(параметр.функция)}, ${exstring(параметр.параметр)}, ${параметр.тип}},`), ""),
    "    }",
    "  end",
  ].join("\n")
}
...
/* ═══════════════════════════ конкурентность ═══════════════════════════
   ...
   тем же правилам, что планировщик свидетеля: печать здесь ничего не решает, она
...
function renderConcurrency(program, processes, shared) {
  ...
  const processEntries = processes.map((node) => {
    /* Ограниченный ящик (А3) в BEAM не ложится ни на что, и печатать здесь
       что-нибудь похожее было бы враньём. Ящик процесса BEAM неограничен
       устройством машины, а `отправить` там не может отказать: `send/2`
       возвращает сообщение и никогда не сообщает, что адресат не справляется.
       Подделать потолок можно было бы только опросом
       `Process.info(pid, :message_queue_len)` из ОТПРАВИТЕЛЯ — это чтение
       чужого ящика в другой момент времени, то есть не тот потолок, который
       объявлен, а гонка вместо него. Поэтому печать отказывается, а не
       выдумывает: программу с объявленным ящиком на BEAM не отправишь. */
    if (Number.isFinite(node.mailbox) && node.mailbox > 0) {
      throw flangError(
        "FLANG_EMIT",
        `процесс «${node.name}» объявил ящик на ${Math.trunc(node.mailbox)} сообщений, а у процесса BEAM ` +
          "ящик неограничен и «отправить» там отказать не может: печати в Elixir у ограниченного ящика нет",
        node.span,
      )
    }
    /* Запас витков ставится ТОЛЬКО нетотальному обработчику. Про тотальный
       ...
       планировщик свидетеля (conc.mjs), и разойтись здесь было бы нельзя: от
