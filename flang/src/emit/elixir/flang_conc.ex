...
defmodule Flang.Proc do
  ...
  Контракт модели конкурентности (`docs/flang/concurrency/SPEC.md`, раздел «Свою BEAM писать
  ...
  окно, пропадает. У свидетеля такого окна нет вовсе.
  ...
  # Решает признак тотальности, а не наличие запаса, — ровно как у свидетеля: у
  ...
  # диагностик дословно те же, что у планировщика свидетеля (conc.mjs).
  ...
  # ── действия ────────────────────────────────────────────────────────────────
  #
  # Порядок исполнения — порядок списка, как у свидетеля. «Остановить» не
  ...
  defp perform(actions, st, message) do
    ...
    cond do
      ...
      outcome.stop != nil ->
        ...
        # надзор, перезапуск»), и код у него тот же, что у свидетеля.
  ...
  # от этого расходился со свидетелем (остановленный процесс попадал в живые) — на
  ...
  # Задержка таймера — миллисекунды настоящих часов. У свидетеля единица времени
...
defmodule Flang.Conc do
  ...
  ## Где это расходится со свидетелем

  1. **Порог считает не то же самое.** У свидетеля в окно порога попадают ВСЕ
  ...
  2. **Окно измеряется по-разному.** У свидетеля единица — пробег обработчика (это
     записано в контракте как расхождение шага 3), у OTP — секунда настоящих
     часов, и меньше секунды окна не бывает вовсе.
  3. **«Остановить» по порогу останавливает группу.** У свидетеля запасная
  ...
  У свидетеля итог прогона виден просто так — он весь в его памяти. На BEAM
  ...
  У свидетеля исход «покой» точен: он видит, что очередь готовых пуста. На BEAM
  ...
  #
  # Это не педантизм. «Дано «Счётчик» принимает …» у свидетеля значит «сообщение
  ...
  # которого у свидетеля не бывает вовсе, и в программе с `отложить` оно наблюдаемо
  ...
  триста прогонов, под нагрузкой — раз на несколько десятков. Свидетель такой щели
