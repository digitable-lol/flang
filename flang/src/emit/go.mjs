...
export function emitGo(program, options = {}) {
  ...
  /* Одно пространство имён на пакет, поэтому один именователь на всё, что
     торчит наружу: функции, фабрики записей, конструкторы вариантов, шаги
     батута. Имена, которые печатает сам бэкенд, занимаются первыми. */
  const exported = createNamer((name) => safeIdent(pascal(name)), [...GO_RESERVED, ...EXPORTED_BY_BACKEND])

  const functionIdents = new Map()
  const factoryIdents = new Map()
  const variantIdents = new Map()
  for (const name of prepared.records.keys()) factoryIdents.set(name, exported(`Создать ${name}`))
  for (const name of prepared.variants.keys()) variantIdents.set(name, exported(name))
  for (const name of prepared.functions.keys()) functionIdents.set(name, exported(name))
