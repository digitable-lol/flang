...
export function emitJava(program, options = {}) {
  ...
  требуетИсполнителяПлана(program, "java")
  ...
  const className = обойтиЗанятоеЦелью(wanted, "java")
...
function renderPreconditions(fn, shared, pad) {
  ...
  for (const property of fn.preconditions) {
    ...
    out.push(
      ...
      `${pad}if (!Flang.pre(ctx, ${check}, ${javastring(property.name)}, ${javastring(fn.name)})) {`,
      `${pad}  throw Flang.fail(${javastring(property.code)}, ${javastring(message)});`,
      `${pad}}`,
    )
  }
  return out
