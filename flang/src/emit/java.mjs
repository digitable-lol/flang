 ...
 * «Trojan Source» (CVE-2021-42574). javac такой файл принимает молча, даже под
 * `-Xlint:all -Werror`, и это здесь не смягчающее обстоятельство, а отягчающее:
 * ревьюер видит один порядок слов, JVM исполняет другой, и предупредить об этом
 * некому. Набор общий с остальными бэкендами — в C и Rust это прямая ошибка
 * сборки, в Elixir отказ разбора, — поэтому печатать их сырыми нельзя ни в
 * одном. В литерал они попадают законно: таблица блоков лексера
 ...
 *
 * Двунаправленные управляющие уезжают в `\uXXXX`: другой формы для них у Java
 * нет вовсе — `\xNN` язык не знает, а восьмеричные (`\NNN`) кончаются на \377 и
 * кодовую точку U+202E выразить не могут. Все двенадцать лежат в BMP, поэтому
 * каждая — ровно одно экранирование, та же кодовая точка и та же длина строки в
 * char: значение литерала не меняется, меняется только запись.
 */
