...
// Предел шагов, который срабатывает через двадцать минут, — не предел. Точка
// `«Строить скобки» от 42 и 0 и 0 и "" и []`
// (`flang/examples/leetcode/022-generate-parentheses.flang`) при объявленных
// 5 000 000 шагов НЕ ОТВЕЧАЛА и за 90 с, тогда как интерпретатор упирается в
// предел за 987 мс. Считал счётчик верно — дорог был ШАГ: `добавить` печаталось
...
// индексу), а цена накопления падает классом: 200 000 `добавить` — 129 мс
// вместо 199 000 мс. Обмен принят осознанно: до него всякая программа с
