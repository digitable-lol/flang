...
const SURFACE_TABLE = {
  ...
  /* Уточнения числа. Это НЕ новый вид значения: значение остаётся тем же
     IEEE-754 double, а тип говорит, в каком диапазоне оно лежит (`целое` —
     точное целое, `нат` — оно же неотрицательное). Отсюда и место в таблице:
     сразу за `число`, рядом с тем, что уточняется.

     СЛОВО ЗДЕСЬ ОДНО И МЕНЯЕТСЯ ОДНОЙ ПРАВКОЙ. Ниже, в парсере и в типизаторе,
     живут только идентификаторы `tNatural` и `tInteger`; русское слово названо
     ровно в этой строке (и в её близнеце — таблице `flang/self/lexer.flang`).
     `нат` — имя рабочее; кандидаты на замену: `натуральное` (склоняется и
     точнее рифмуется с `целое`, но по-русски натуральное число обычно
     начинается с единицы, а этот тип включает ноль) и `неотрицательное`
     (точнее всех и длиннее всех). Замена — правка двух строк, и больше нигде. */
  tNatural: { ru: ["нат"], en: ["natural"], eo: ["natura"], zh: ["自然数"] },
  tInteger: { ru: ["целое", "целого", "целым", "целому"], en: ["integer"], eo: ["entjero"], zh: ["整数"] },
