 ...
 *   ВОСЕМЬ СЛОВ ДОКАЗАТЕЛЬСТВА без эсперанто и без китайского — `ensures`,
 *     `forall`, `claim`, `induction`, `byProperty`, `byExample`,
 *     `byHypothesis`, `qed`. Довод ровно тот же, по которому молчит
 *     `decreases`, и это не совпадение: слова одного слоя. Два из восьми —
 *     `для всех` и `индукция по` — в математике устоялись на всех четырёх
 *     поверхностях, остальные шесть не устоялись НИГДЕ: `по свойству`, `по
 ...
 *     восьми: `ensures` в этом смысле стоит в Eiffel, JML и Dafny, `induction
 *     on` и `for all` — в любом учебнике.
 *
...
const SURFACE_TABLE = {
  ...
  /* ── доказательство (flang/proof/SPEC.md) ──────────────────────────────────
     Восемь слов, и все восемь заведены, чтобы стало возможно ВЫСКАЗАТЬ
     ...
     читает язык: `flang/scripts/word-occupancy.mjs`. У всех восьми — ноль
     ...
     ЧЕТЫРЕ СЛОВА ИЗ ВОСЬМИ — ФРАЗЫ, и не ради красоты речи. Одинокое
     ...
     ЧЕГО ЗДЕСЬ НЕТ. Нет `требует` — предусловия. Слово свободно (ноль голых
     вхождений), и соблазн завести его вместе с `обеспечивает` велик: они
     ходят парой в Eiffel, JML и Dafny. Но постусловие ложится на уже
     работающую машинерию (`postconditions` в `interpret.mjs`, куда его сегодня
     кладёт `compat.mjs`), а предусловие — это НОВЫЙ рантайм в интерпретаторе и
     во всех восьми целях печати, и до тех пор оно было бы утверждением, за
     которым не стоит ничего. Роль гипотезы в теореме несёт уже стоящее `дано`.
     Заводить слово раньше того, что оно означает, — это ровно то, из-за чего
     `теорема` три версии разбиралась и не читалась никем. */
