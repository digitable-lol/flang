...
export const KEYWORDS = {
  ...
  /* Кодовая точка символа: `код символа СТРОКА` → число.

     ПОЧЕМУ ФРАЗА, А НЕ СЛОВО. Одиночное «код» занять нельзя, и это не догадка,
     а тот же счёт, каким уже потеряны «символы», «группа», «обратный», «на»,
     «начальное» и «порог»: токенизацией всех 149 файлов `.flang` и `.fts`
     репозитория с отбором незакавыченных имён.

       код          — 226 голых вхождений (одни только `http-request.fts` и
                      разбор отказа `«Не разобрано» с код как код` дают
                      большинство);
       код символа  — 0 голых вхождений;
       character    — 0, `code` — 0, `character code` — 0.

     Пара слов свободна, и свободна она не случайно: фразой из двух слов
     переменную не назвать никогда — тем же доводом в языке появились
     `разложить … на символы`, `обратный элемент` и `порог отказов`. Склейка
     жадная, длинные фразы идут первыми, поэтому `с код как код` разбирается
     как раньше: там за «код» стоит «как», а не «символа». Обе половины под
     тестом в `flang/test/lexer.test.mjs` — на слово этого проверять нельзя.

     Поверхностей у языка четыре, и остальные две заводит ветка поверхностей;
     для неё запись здесь читается так:
       charCode: { ru: ["код символа"], en: ["character code"],
                   eo: ["kodo de signo"], zh: ["字符码"] }
     Эсперанто фразой по той же причине, что и русский; китайский словом —
     там пробелов нет, и `字符码` именем быть не может так же, как фраза. */
  charCode: ["код символа", "character code"],
