...
const SURFACE_TABLE = {
  ...
  /* Кодовая точка символа: `код символа СТРОКА` → число. Фраза, а не слово:
     ...
     Обратная форма («символ по коду») ЕСТЬ, и довод «ни хешу, ни порядку она не
     нужна» её больше не держит: он верен ровно для тех двух задач и неверен для
     сети. Там кодовая точка приезжает ЧИСЛОМ снаружи — процентная запись `%D0%9C`
     — и другого пути к символу у программы нет. Замер, которым это названо:
     раскодирование адресов в `flang/stdlib/http.flang` доводило до символа 95
     кодовых точек из 1 114 112, потому что мост из числа в строку там был
     таблицей печатных ASCII. */
  charCode: { ru: ["код символа"], en: ["character code"], eo: ["kodo de signo"], zh: ["字符码"] },
  /* Обратная к `код символа`: `символ по коду ЧИСЛО` → строка ровно из одного
     символа. Фраза из ТРЁХ слов, и первое из них — уже ключевое `символ`.
     Столкновения это не даёт: после `символ` в языке всегда стоит выражение
     (`символ N в СТРОКА`), а `по` выражения не начинает никогда. Проверено
     счётом, а не рассуждением: `flang/scripts/word-occupancy.mjs` на 374 файлах
     корпуса даёт у «символ по коду» 0 голых вхождений (2 в ёлочках, 9 в
     комментариях — они ключевыми не становятся).

     Все четыре поверхности названы, потому что парсер цитирует эту фразу в
     диагностике, а `flang/test/surfaces.test.mjs` требует от цитируемого
     понятия слова на каждой поверхности. Новых знаков в алфавит не приходит:
     `按`, `码`, `字`, `符` уже стоят в таблице у `по` и `字符码`. */
  charFromCode: { ru: ["символ по коду"], en: ["character by code"], eo: ["signo per kodo"], zh: ["按码字符"] },
  ...
  /* Приписывание в начало: `приписать ЭЛЕМЕНТ к СПИСОК`. Предлог тот же, что у
     `добавить … к …`, и это нарочно: операции противоположны только концом
     списка, поэтому и различаться обязаны ровно одним словом — глаголом.
     Порядок слов в таблице значения не имеет, но запись стоит рядом с `add`
     нарочно: правку одной надо видеть вместе с другой.

     ЦЕНА СЛОВА ИЗМЕРЕНА тем же счётом, каким уже потеряны «символы», «группа»,
     «начальное», «порог» и выиграно одиночное «элемент»: токенизацией всех 156
     файлов `.flang` и `.fts` репозитория, которые лексер читает, с отбором
     незакавыченных имён (`tmp/bench/words.mjs`, каталог в .gitignore).

       приписать   — 0 голых вхождений;
       начало      — 42 голых вхождения («Числа от и до», факториал, ядро);
       слева       — 76 голых вхождений (обходы деревьев).

     Отсюда и выбор: «приписать» одиночным словом можно, «начало» — нельзя
     никогда, а пара «в начало» была бы можно (`grep`-ом и токенизацией: слова
     «в» и «начало» ни разу не стоят рядом голыми) — но стоила бы вдвое больше
     поверхности при том же смысле.

     СЛОВО ВЫБИРАЕТ ВЛАДЕЛЕЦ, и это правило, а не вежливость. «приписать» здесь
     стоит как РАБОЧЕЕ: заменяется одной строкой этой таблицы плюс одной строкой
     той же таблицы в `flang/self/lexer.flang`. Канонические имена встроенных
     форм в языке совпадают с русским словом (`добавить`, `остаток от`), поэтому
     смена слова тянет ещё и механическое переименование имени формы — оно
     делается одним `sed` по репозиторию и в диагностику попадает уже новым
     словом. Отвергнутые варианты и цена каждого — в `flang/SPEC.md`, раздел
     «Приписывание в начало». */
  prepend: { ru: ["приписать"], en: ["prepend"], eo: ["antaŭmeti"], zh: ["前置"] },
  ...
  /* Родительный падеж `функтора` — не новое слово, а вторая форма того же, ровно
     как `поля` рядом с `поле` и `числа` рядом с `число`. Нужен он одному месту:
     естественное преобразование пишется `из функтора «Ф» в функтор «Г»`
     (flang/cat/SPEC.md), и без падежа фраза была бы не по-русски. Другим трём
     поверхностям падеж не нужен — `from functor … to functor …`,
     `de funktoro … al funktoro …` и `从函子…到函子…` стоят в исходной форме.
     Занятость мерена `flang/scripts/word-occupancy.mjs` на 179 файлах корпуса:
     голым именем — ноль, значит ни одна написанная программа не меняет ни
     одного своего токена. Печатается в диагностике всегда первая форма. */
  functor: { ru: ["функтор", "функтора"], en: ["functor"], eo: ["funktoro"], zh: ["函子"] },
