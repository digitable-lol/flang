...
export async function linkProgram(entryFile, source, parse, options = {}) {
  ...
  const категории = { morphisms: [], isomorphisms: [], bifunctors: [], monoids: [] }
  ...
  const merge = (program, file, visible) => {
    ...
    /* Видимость считается по тому же набору имён, что у типов и функций: у
       ...
       Дубликат по имени здесь намеренно не ловится: `checkMorphisms` в
       types.mjs уже сообщает «морфизм «х» объявлен дважды», и второе
       сообщение о том же было бы шумом. Слияние плоское — порядок объявлений
       сохраняется, а разбирать композиции по мере готовности проверка умеет
       сама, поэтому порядок файлов на неё не влияет. */
    for (const поле of Object.keys(категории)) {
      for (const узел of program[поле] ?? []) {
        if (visible !== null && !visible.has(узел.name)) continue
