...
const КАТЕГОРНЫЕ = [
  ...
  /* Монада — из того же ряда и пропущена была по той же невнимательности, что и
     категория: в списке её не было, и `checkMonadLaws` на слитой программе не
     видел ни одного объявления. Сегодня в дереве это никого не задевает — оба
     файла с `монада` (`examples/cat/monoid-and-monad.flang`,
     `examples/monad/order-total.flang`) написаны без `использует`, и дыра
     латентна. Латентная дыра тем и опасна, что откроется первым же файлом,
     который импорт добавит, — и откроется молчанием, а не отказом. */
  ["monads", "монада", "объявлена"],
]
...
export async function linkProgram(entryFile, source, parse, options = {}) {
  ...
  /* ТЕОРЕМЫ КАЖДОГО ФАЙЛА — по файлу, а не по имени: доносятся они ПО РЕБРУ
     ИМПОРТА (см. ниже), и знать, из какого файла теорема пришла, обязательно.
     Сличать теоремы по одному имени нельзя: `удвоенное неотрицательно` есть и в
     `proof/examples/precondition.flang`, и — про ДРУГУЮ функцию, в файле без
     единого импорта — в `proof/examples/four-words.flang`. Донеси такую по
     имени, и в ведомость приехало бы доказательство про чужую функцию, то есть
     аксиома под другим именем. Мерено, а не придумано. */
  const теоремыФайла = new Map()
  ...
  const merge = (program, file, visible) => {
    ...
    if (Array.isArray(program.theorems) && program.theorems.length > 0) {
      теоремыФайла.set(file, program.theorems)
    }
  ...
  for (const поле of ["processes", "supervisors", "runs", "plans", "requirements"]) {
    if (Array.isArray(entry?.[поле]) && entry[поле].length > 0) linked[поле] = entry[поле]
  }
  /* ТЕОРЕМА ЕДЕТ ЗА СВОЕЙ ФУНКЦИЕЙ, И ВОТ ПОЧЕМУ ЭТО ИСПРАВЛЕНИЕ, А НЕ ПОБЛАЖКА.
   *
   * Стояло здесь обратное: теоремы брались только у входного файла, а довод был
   * записан так — «слить теоремы импортированных модулей значило бы
   * перепроверять чужое доказательство в каждом, кто модуль использует, и ни
   * одного нового факта за неё». Довод верен ровно наполовину, и вторая
   * половина стоила дорого.
   *
   * ЧТО БЫЛО НА САМОМ ДЕЛЕ. Постусловие функции едет через импорт ВСЕГДА — оно
   * часть подписи. Значит и ОБЯЗАТЕЛЬСТВО по нему возникает в каждом
   * импортирующем модуле. Не ехало только ДОКАЗАТЕЛЬСТВО. То есть работа
   * «столько раз, сколько импортов» делалась и раньше — но делалась ХУДШИМ из
   * доступных способов: без теоремы ядро закрывало эти обязательства
   * ВЫЧИСЛЕНИЕМ ЗАМКНУТОЙ ЦЕЛИ, самым дорогим своим ходом. Измерено прогоном по
   * всему дереву (18 августа): таких обязательств 12, и ВСЕ 12 — это ровно те,
   * у кого теорема потерялась на импорте. Других замкнутых целей у ядра на
   * корпусе нет ни одной.
   *
   * ЧТО СТАЛО. Теорема доносится, и те же 12 закрываются ТЕРМОМ — тем самым,
   * который автор написал в родном модуле. Ход вычисления при этом не зовётся
   * ни разу.
   *
   * АКСИОМ НЕ ПРИБАВИЛОСЬ, И ЭТО ГЛАВНОЕ. Донесённая теорема не объявляется
   * доказанной — она ПРОВЕРЯЕТСЯ на месте прибытия тем же ядром и по тем же
   * правилам, что и своя: `checkProofs` не отличает донесённую от написанной
   * здесь. Теорема, «доехавшая» непроверенной, была бы аксиомой под другим
   * именем; эта доезжает термом, который надо предъявить заново.
   *
   * ДОНОСИТСЯ ПО РЕБРУ ИМПОРТА, А НЕ ПО ИМЕНИ. Теорема из файла Ф едет только
   * тогда, когда из Ф же приехала функция, у которой есть постусловие с этим
   * именем. Совпадение имён между несвязанными файлами (`удвоенное
   * неотрицательно` в `precondition.flang` и в `four-words.flang`, который не
   * импортирует ничего) ничего не переносит.
   *
   ...
   *
   * Ключ не выкладывается, когда теорем нет: программа без доказательств
   * обязана остаться побайтово прежней, иначе неподвижная точка самоприменения
   * сошлась бы к другому AST. */
  const своиТеоремы = Array.isArray(entry?.theorems) ? entry.theorems : []
  const названо = new Set(своиТеоремы.map((т) => т?.name).filter((и) => typeof и === "string"))
  const донесённые = []
  for (const [file, теоремы] of теоремыФайла) {
    if (file === resolve(entryFile)) continue
    /* Постусловия, приехавшие ИЗ ЭТОГО файла, — по ним и решается, что донести. */
    const отсюда = new Set()
    for (const fn of functions) {
      if (originOfFunction.get(fn.name) !== file) continue
      for (const п of fn?.postconditions ?? []) {
        if (typeof п?.name === "string") отсюда.add(п.name)
      }
    }
    for (const т of теоремы) {
      if (typeof т?.name !== "string" || названо.has(т.name) || !отсюда.has(т.name)) continue
      названо.add(т.name)
      донесённые.push(т)
    }
  }
  const теоремы = [...своиТеоремы, ...донесённые]
  if (теоремы.length > 0) linked.theorems = теоремы
  return linked
}
