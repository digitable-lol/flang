...
/**
 * Русские имена → идентификаторы целевых языков.
 *
 * ── Откуда файл здесь ───────────────────────────────────────────────────────
 * Написан он был для старого проекта FTS и лежал в `tools/ftsc/src/naming.mjs`.
 * Проект вынесен из репозитория (тег `fts-pered-udaleniem`), а файл ПЕРЕЕХАЛ,
 * а не скопировался: все восемь целей печати языка (`flang/src/emit/*.mjs`)
 * зовут именно его, и второй копии быть не должно. Разъехавшиеся копии дали бы
 * расхождение не в момент правки, а в момент печати чужой программы.
 *
 * В программах имена кириллические и с пробелами: «Рассчитать скидку», «сумма без
