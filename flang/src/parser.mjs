  ...
  parseDeclaration() {
    ...
    switch (token.value) {
      ...
      /* `требует` У ЛЕВОГО КРАЯ — требование к ещё не написанной функции.
         ...
         строка отвечала `не разобрана конструкция: неожиданное 'требует'`.
         Значит поток токенов корпуса не сдвигается, таблица поверхностей не
         трогается вовсе, и побайтовая сверка с `self/lexer.flang` не задета. */
      case "requires":
  ...
  parseRequirement() {
    ...
    if (this.enterBlock()) {
      while (!this.atBlockEnd()) {
        ...
        this.fail(
          "не разобрана конструкция: в требовании ожидаются 'принимает', 'возвращает' или 'обеспечивает'",
        )
      }
      this.exitBlock()
    }
