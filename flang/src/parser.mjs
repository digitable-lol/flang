...
class Parser {
    ...
    if (this.atPunct("[")) {
      ...
      while (!this.atPunct("]")) {
        ...
        /* Разделитель — запятая или `и`, как у `список из a, b и c`: одна и та
           же мысль записывается одинаково в обеих формах списка. `и` здесь
           достижимо только тогда, когда выражение элемента уже остановилось, —
           внутренние формы (аргументы вызова, поля конструктора) съедают его
           раньше. Поэтому ни одна написанная программа разбор не меняет: на
           этом месте раньше был отказ. */
   ...
   *
   * Разделитель между полями — `и` или запятая, и оба заняты снаружи: запятая
   * разделяет ещё и элементы списочного литерала, `и` — аргументы вызова. Пока
   * разделитель съедался безусловно, список из двух конструкторов записать было
   * нельзя: в `[вариант «М» с имя равным "а", вариант «М» с имя равным "б"]`
   * запятая читалась как «дальше ещё поле», парсер ждал имя поля и спотыкался о
   * слово `вариант`. Один элемент работал, два — нет, и это било по примерам:
   * `дано` из нескольких записей не задать.
   *
   * Отсюда взгляд на два токена вперёд: разделитель продолжает поля, только
   * если за ним действительно присвоение — имя и `равным`. Иначе он остаётся
   * тому, кто нас позвал.
   *
   * AST существующих программ от этого не сдвинулся ни на байт: пока условие
   * выполнялось, старый код разбирал ровно то же, а когда не выполнялось —
   * отказывал. Отказ заменён разбором, разбор не заменён ничем.
   */
  ...
  /** Разделитель, за которым и правда ещё одно присвоение поля: `… равным`. */
  atFieldSeparator() {
    if (!this.atKw("and") && !this.atPunct(",")) return false
