  ...
  parseTheorem() {
    ...
    if (this.enterBlock()) {
      while (!this.atBlockEnd()) {
        this.skipNewlines()
        if (this.atBlockEnd()) break
        if (this.atKw("given") && this.atName(1) && !this.atKw("has", 2)) {
          /* `дано <имя>: <тип>` — введённая переменная, она же квантор «для
             всех» этой теоремы, и `дано <имя> <утверждение…>` — гипотеза.
             Различает их ровно то, чем различает их `принимает`: двоеточие или
             `является`. Третий случай — `дано <имя> имеет …` — старая форма, и
             он отсечён выше.

             ПОЧЕМУ ТЕМИ ЖЕ ДВУМЯ ЗНАКАМИ, ЧТО У ПАРАМЕТРА. Переменная теоремы
             и параметр функции — одно и то же: имя с типом, о котором говорят
             для всех значений. Третья запись того же самого заставила бы
             читателя помнить, чем `дано х: число` отличается от `принимает х:
             число`, а не отличается оно ничем. */
          if (this.atPunct(":", 2) || this.atKw("is", 2)) {
            новая = true
            const слово = this.next()
        ...
        } else if (this.atKw("given") && !this.atName(1)) {
          новая = true
          const слово = this.next()
          доказательство.given.push({ expr: this.parseExpression(), span: слово.span })
