...
class Parser {
  ...
  parseProgram() {
    ...
    if (this.processes.length > 0 && !this.types.some((type) => type.name === ACTION_TYPE.name)) {
      /* Седьмое действие (`поручить`) приписывается НЕ ВСЕМ, а только той
         ...
         `flang/conc/examples` не изменился ни на байт, и близнец этого парсера
  ...
  parseIndexBase() {
    ...
    if (this.базаНомера !== null) {
      /* Числа в сообщении нет НАРОЧНО: близнец на flang читает `прочие` списком
   ...
   * flang её не написать, и жила бы она только в эталоне на JS. Ядро всё равно
