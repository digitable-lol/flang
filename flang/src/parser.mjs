...
import { ACTION_TYPE, ORDER_ACTION } from "./conc.mjs"
import { IO_TYPE_NAMES, IO_VARIANT_OWNER, actionTypeWithOrder, withIoTypes } from "./io.mjs"
...
/**
 * Имя, под которым в постусловии виден возвращённый результат.
 *
 * Слово НЕ новое и заводится не здесь: тем же словом то же самое названо в
 * `fts-legacy.mjs` (`RESULT_BINDING`, свойство утилиты наследия FTS) и в значении
 * по умолчанию интерпретатора (`interpret.mjs`, `DEFAULT_RESULT_BINDING`).
 * Здесь оно повторено третий раз намеренно и с открытыми глазами: связать три
 * слоя импортом значило бы, что парсер зависит от интерпретатора — порядок
 * слоёв, на котором стоит самоприменение, запрещает это прямо. Цена повтора
 * названа: если слово меняют, менять надо в трёх местах, и все три названы
 * друг в друге.
 */
...
class Parser {
  constructor(tokens, external) {
    ...
    this.isomorphisms = []
    this.bifunctors = []
    /* Категория, называющая свои стрелки. Список пуст у всех, кто пишет
       `категория «Х»` заголовком документа FTS: там она — пространство имён и
       ничего не называет. Появляется он только там, где членство написано. */
    this.categories = []
    /* Естественные преобразования. Список отдельный, а не строка в `morphisms`:
       у преобразования концы — ФУНКТОРЫ, а не объекты, и стыковать их надо
       иначе. Появляется он только там, где преобразование написано. */
    ...
    this.intersections = []
    this.monoids = []
    /* Объявленные свойства (`flang/cat/ZAKONY.md`): дистрибутивность,
       идемпотентность, частичный порядок, монотонность, коммутативность.
       Список отдельный от моноида, и это не мелочь: моноид называет ТРИ вещи
       разом (носитель, операцию, единицу) и получает три закона; свойство
       называет ОДНУ вещь и получает ровно один закон и ровно одно разрешение.
       Появляется ключ только там, где свойство написано. */
    this.properties = []
    /* Требования: утверждение о функции, высказанное ДО того, как функция
       написана. Список отдельный от функций ровно потому, что функции ещё
       нет: требование живёт без тела и переживает его отсутствие.
       Появляется ключ только там, где требование написано. */
    this.requirements = []
    ...
    this.plans = []
    this.usesIo = false
    /* Седьмое действие (`поручить`) — шов между двумя словарями, и признак у
       него отдельный от `usesIo` намеренно: словарь ввода-вывода нужен всякому,
       кто строит поручение (даже без плана и без процессов), а седьмое действие
       нужно только тому, кто это поручение ВЫДАЁТ из процесса. Два разных
       вопроса — два признака; один на оба приписал бы семь действий программе,
       где `поручить` не написано, и AST девяти примеров конкурентности
       изменился бы без всякой на то причины. */
    this.usesOrderAction = false
    ...
    this.legacy = []
    /* Теоремы новой формы. Старая по-прежнему уезжает в `legacy` целиком, и
       пустой список сюда не выкладывается вовсе — тем же правилом, что у
       `plans` выше: программа без доказательств обязана дать побайтово прежний
       AST, иначе неподвижная точка самоприменения сошлась бы к другому. */
    this.theorems = []
    /* БАЗА НОМЕРА, объявленная АВТОРОМ: `элемент начиная с 0` у левого края.
       `null` значит «не объявлено», и это НЕ то же самое, что объявленная
       единица: неоглашённое умолчание кладёт единицу читателям поля
       (`базаНомераПрограммы`), а ключа в AST не появляется вовсе — иначе
       побайтовый AST всякой сегодняшней программы изменился бы. */
    this.базаНомера = null
    this.module = ""
  ...
  resolveLocal(name, span) {
    ...
    /* Самый частый способ получить это сообщение — написать инфиксное действие
       префиксом (`умножить 3.14 на р`): голова формы становится именем, и оно
       ничем не связано. Поэтому текст называет и то, чем имя вводят, и то, что
       действия языка стоят МЕЖДУ значениями. */
    throw flangError(
      "FLANG_UNKNOWN_NAME",
      `имя «${name}» не связано: имя вводят 'принимает', 'пусть' или образец 'случай'; `
  ...
  /** Имя в потоке; `ahead` нужен теореме, чтобы отличить старую форму от новой. */
  atName(ahead = 0) {
    return this.at("name", ahead) || this.at("string", ahead)
  }
  ...
  parseProgram() {
    ...
    if (this.processes.length > 0 && !this.types.some((type) => type.name === ACTION_TYPE.name)) {
      /* Седьмое действие (`поручить`) приписывается НЕ ВСЕМ, а только той
         программе, где оно написано, — и правило это взято с другой стороны шва.
         Словарь ввода-вывода приписывается ровно так же, признаком по именам в
         тексте (`usesIo` ниже), а `поручить` принадлежит обоим словарям сразу:
         оно единственное действие, чьё поле объявлено типом из словаря
         поручений. Из двух правил берётся более строгое.
         Замер, ради которого это важно: AST девяти примеров
         `flang/conc/examples` не изменился ни на байт, и эталон этого парсера
         на flang (`flang/self/parser.flang`, где словарь выписан данными)
         остался с ним побайтово согласен без единой правки. */
      ...
      this.types.push(structuredClone(this.usesOrderAction ? actionTypeWithOrder() : ACTION_TYPE))
    }
    ...
    /* Ключ последний и появляется только при непустом списке: порядок ключей —
       это байты канонического JSON, и вставить новый ключ в середину значило бы
       переписать AST каждой программы репозитория. */
    if (this.theorems.length > 0) program.theorems = this.theorems
    /* И этот — по той же причине, следом за ним: `категория «Х»` без списка
       стрелок остаётся ровно тем, чем была (узлом наследия и только), и AST
       сорока моделей `.fts` репозитория не меняется ни на байт. */
    if (this.categories.length > 0) program.categories = this.categories
    /* И это — следом и по той же причине: ключ последний, значит AST всякой
       программы, где преобразования не написано, не меняется ни на байт. */
    if (this.transformations.length > 0) program.transformations = this.transformations
    /* И это — следом и по той же причине: ключ последний, значит AST всякой
       программы, где ни одного свойства не объявлено, не меняется ни на байт. */
    if (this.properties.length > 0) program.properties = this.properties
    /* И это — следом и по той же причине: ключ ПОСЛЕДНИЙ, значит AST всякой
       программы, где ни одного требования не написано, не меняется ни на байт.
       Проверено прогоном по всему корпусу: `self-parser.test.mjs`. */
    if (this.requirements.length > 0) program.requirements = this.requirements
    /* И этот — последним из последних, и по той же причине. Ключ появляется
       ТОЛЬКО у программы, где `элемент начиная с N` написано: молчание значит
       единицу, а единица, выложенная в AST явно, переписала бы байты каждой
       программы репозитория ради числа, которое и так известно. */
    if (this.базаНомера !== null) program.базаНомера = this.базаНомера
  ...
  noteIoVariant(name) {
    ...
    /* Седьмое действие ставит ОБА признака сразу, и второй — не про запас:
       поле `поручение` объявлено типом «Поручение», а адресат обязан принимать
       «Отклик». Программа, где написано `поручить`, но не написано ни одного
       имени из словаря ввода-вывода, получила бы семь действий и ни одной суммы,
       которой седьмое действие пользуется, — то есть «неизвестный тип» на
       собственном словаре языка. */
    if (name === ORDER_ACTION) {
      this.usesOrderAction = true
      this.usesIo = true
    }
    return name
  }
  ...
  parseDeclaration() {
    ...
    switch (token.value) {
      ...
      case "property":
        return this.properties.push(this.parseProperty())
      /* `требует` У ЛЕВОГО КРАЯ — требование к ещё не написанной функции.
         Внутри функции то же слово ограничивает вход (`parseRequires`); здесь
         оно ограничивает КОД, которого ещё нет. Различает не смысл слова, а
         место — ровно так же, как у `свойство` (предел утилиты с отступом,
         объявление у левого края) и у `теорема` (две формы одного слова).
         Занято ноль новых слов из ноля: `^требует` в корпусе `.flang` и `.fts`
         не встречается ни разу (замерено `grep`-ом), и до сегодняшнего дня
         строка у левого края отвечала «не разобрана конструкция: неожиданное
         слово». Значит поток токенов корпуса не сдвигается, таблица
         поверхностей не трогается вовсе, и побайтовая сверка с
         `self/lexer.flang` не задета. */
      case "requires":
        return this.requirements.push(this.parseRequirement())
      case "item":
        return this.parseIndexBase()
      case "monad":
      ...
      case "theorem": {
        /* Одно слово, две формы: старая уезжает в `legacy`, как уезжала всегда,
           новая — в свой список. Решает не эта строка, а сам разбор блока: см.
           `parseTheorem`. */
        const узел = this.parseTheorem()
        return узел.kind === "theorem" ? this.theorems.push(узел) : this.legacy.push(узел)
      }
      case "functor":
        return this.legacy.push(this.parseFunctorFile())
      default:
        /* Слово языка у левого края, которым объявление не начинается: либо
           внутренняя строка потеряла отступ, либо забыто объявление, которому
           она принадлежит (`состояние` без `процесс`). Названы оба лекарства. */
        return this.fail(
          `не разобрана конструкция: неожиданное '${token.text ?? token.value}' — этим словом объявление `
            + "не начинается: у левого края стоят заголовок модуля, 'тип', 'объект', 'функция', "
            + "'тотальная', 'процесс', 'прогон'; строка внутри объявления пишется с отступом",
        )
    }
  }
  ...
  readModuleLine(value) {
    ...
    /* Отступ под `модуль` — самая частая беда того, кто читал выжимку языка, а
       не спецификацию: строка модуля стоит сама по себе, блока объявлений она
       не открывает, и сдвинутое вправо `тотальная функция` упирается сюда.
       Поэтому сообщение называет не только то, что тут разрешено, но и то,
       чем это лечится: снять отступ.

       Понятие `module` здесь НЕ цитируется: на поверхности eo у него нет
       слова, и сторож `surfaces.test.mjs` справедливо ловит цитату, которую
       нечем перевести. Заголовок модуля назван прозой. */
    this.fail(
      "не разобрана конструкция: с этим отступом стоят только 'экспортирует' и 'использует' — заголовок "
        + "модуля блока объявлений не открывает; снимите отступ, 'тип', 'объект', 'функция', 'процесс' "
        + "пишутся у левого края",
    )
  }

  /**
   * Категория: имя и — с 2026-08-14 — список её стрелок.
   *
   * ```flang
   * категория «Продажи»
   *   морфизм «отгрузить»
   ...
   * ```
   *
   * ЗАЧЕМ ЭТО ПОЯВИЛОСЬ. Имена категорий у функтора (`из «Продажи» в
   * «Биллинг»`) были пометкой для читателя и ничем больше: категория отдельной
   * сущностью не объявлялась, и утверждать, что стрелка принадлежит именно ей,
   * было не на чем. Отсюда же дыра в проверке функтора: он мог отобразить две
   * стрелки из пяти, и никто не возражал, потому что «пять» было неизвестно.
   * Список членов и есть то, чего не хватало.
   *
   * ФОРМА ВЗЯТА ИЗ УЖЕ ЗАНЯТЫХ СЛОВ, и это не экономия. Новое слово пришлось бы
   * завести в таблице лексера — а её сверяет побайтово `self/lexer.flang`, и
   * ровно на такой правке сверка ломалась на этой неделе. `морфизм «имя»` без
   * `из` и без блока сегодня отвергается разбором («требует строки 'если' и
   * 'то'»), значит эта запись НИЧЕГО не отнимает у написанного: она занимает
   * то, что было ошибкой.
   *
   * РАЗЛИЧАЕТСЯ ОДНИМ ВЗГЛЯДОМ ВПЕРЁД. `морфизм «имя»` и конец строки — член
   * категории; всё остальное (`из … в …`, `это … после …`, блок `если/то`) —
   * обычное объявление, разбираемое тем же кодом, что и всегда. Поэтому
   * заголовок документа FTS (`категория «Продажи»` с объектами и утилитами под
   * ним) читается ровно как читался, и `categories` у него не появляется вовсе.
   *
   * ── Своё равенство на значениях объекта (2026-08-15) ────────────────────────
   *
   * ```flang
   * категория «Отгрузки»
   *   морфизм «отгрузить»
   ...
   * ```
   *
   * Категория обогащена сетоидами: она объявляет СВОЁ отношение равенства, а
   * равенство стрелок из него выводится поточечно (`flang/cat/SPEC.md`,
   * «Равенство морфизмов»). Без него не выразима ассоциативность композиции:
   * `«в» после («б» после «а»)` и `(«в» после «б») после «а»` — два разных
   * ИМЕНИ, а сравнение имён продавало бы сличение строк за аксиому.
   *
   * НИ ОДНОГО НОВОГО СЛОВА, и по той же причине, по которой их нет у членства:
   * таблицу лексера сверяет побайтово `self/lexer.flang`, а слово в ней — это
   * ещё и четыре поверхности, и продукция у парсера на flang. `объект «Х»` с
   * `даёт` в той же строке разбор прежде ОТВЕРГАЛ (за именем записи идёт блок
   * полей, а не ключевое слово), значит эта запись занимает то, что было
   * ошибкой, и не отнимает ничего у написанного. `даёт` здесь означает ровно
   * то же, что у стрелки и у моноида, — «вот функция, которая это считает».
   */
  parseCategory() {
    const start = this.next()
    ...
    if (this.enterBlock()) {
      for (;;) {
        this.skipNewlines()
        if (this.atBlockEnd()) break
        if (this.atKw("morphism") && this.atName(1) && this.членКатегории(2)) {
          this.next()
          стрелки.push(this.expectName("ожидалось имя морфизма"))
          this.endLine()
          continue
        }
        if (this.atKw("object") && this.atName(1) && this.atKw("gives", 2)) {
          this.next()
          const объект = this.expectName("ожидалось имя объекта")
          this.expectKw("gives", "после имени объекта ожидалось 'даёт'")
          const функция = this.expectName("ожидалось имя функции после 'даёт'")
          this.endLine()
          равенства.push({ object: объект, gives: функция })
          continue
        }
        this.parseDeclaration()
      }
    ...
    /* Условие «или равенства» — не полнота ради полноты. Категория, где
       написано только равенство, — это объявление без единой стрелки, и молча
       выбросить его значило бы потерять написанное: пусть о нём скажет
       проверка типов, у которой для этого есть слова. */
    if (стрелки.length > 0 || равенства.length > 0) {
      /* Ключ `equalities` появляется только там, где равенство написано, — по
         той же причине, по какой сам `categories` появляется только у категории
         со списком стрелок: AST категории, объявленной вчера, обязан остаться
         прежним байт в байт. */
      this.categories.push({
        kind: "category",
        name,
        morphisms: стрелки,
        ...(равенства.length > 0 ? { equalities: равенства } : {}),
        span: start.span,
      })
    }
  }
   ...
   *
   * Конец строки одного мало: у морфизма-утверждения FTS (`морфизм «имя»` с
   * блоком `если … то …`) за именем тоже стоит перевод строки, а следом —
   * отступ. Приняв его за членство, разбор съел бы заголовок и споткнулся о
   * собственный блок; так и вышло с первого захода, и тест наследия это поймал.
   */
  членКатегории(ahead) {
    if (this.at("dedent", ahead) || this.at("eof", ahead)) return true
    return this.at("newline", ahead) && !this.at("indent", ahead + 1)
  }
  ...
  parseFunction() {
    ...
    if (this.enterBlock()) {
      while (!this.atBlockEnd()) {
        ...
        /* Постусловие обычной функции: `обеспечивает «имя» <утверждение>`.
           Стоит там же, где `убывает`, — в подписи, до тела, — и по той же
           причине: это обещание автора о том, что функция вернёт, а не шаг
           вычисления. */
        if (this.atKw("ensures") || this.atKw("forall")) {
          postconditions.push(this.parseEnsures(name, params))
          continue
        }
        /* Предусловие: `требует «имя» <утверждение>`. Стоит там же, где
           `обеспечивает`, и ПЕРЕД ним по чтению — «что нужно на входе» раньше,
           чем «что будет на выходе», — но порядок строк не навязывается: обе
           конструкции читаются в любом месте подписи, как читаются `принимает`
           и `возвращает`. */
        if (this.atKw("requires")) {
          preconditions.push(this.parseRequires(name, params))
          continue
        }
    ...
    /* Тем же правилом, что `decreases`: поле появляется ТОЛЬКО у функции с
       постусловием. Программа без `обеспечивает` обязана давать побайтово
       прежний AST — иначе побайтовая неподвижная точка самоприменения рухнула
       бы на первом же файле корпуса. Имя поля не новое: `postconditions` уже
       есть в AST (SPEC, раздел 9) — сегодня его кладёт туда только
       `compat.mjs` из свойств утилиты FTS и `defunc.mjs` из сторожа меры.
       Поверхности у него не было; теперь есть, и это ровно то, ради чего
       заведено слово. */
    if (postconditions.length > 0) node.postconditions = postconditions
    /* Тем же правилом, что `decreases` и `postconditions`: поле появляется
       ТОЛЬКО у функции с предусловием. Программа без `требует` обязана давать
       побайтово прежний AST — иначе побайтовая неподвижная точка
       самоприменения рухнула бы на первом же файле корпуса. */
  ...
  /**
   * Предусловие функции: то, чем в языке ОГРАНИЧИВАЮТ вход.
   *
   * ```flang
   * требует «накопитель неотрицателен» предыдущее не меньше 0
   * ```
   *
   * КТО ЕГО СНИМАЕТ — ВЫЗЫВАЮЩИЙ, и в этом весь смысл слова. Проверка на входе
   * функции говорила бы о том входе, который пришёл, а утверждение нужно обо
   * всех; и стоила бы она новой строки в рантайме интерпретатора и в каждой из
   * восьми печатей. Обязательство на вызывающем не стоит ни одной строки
   * рантайма и взамен делает предусловие ИЗВЕСТНЫМ ФАКТОМ внутри тела: ровно
   * то, ради чего слово и заведено. Так же это устроено в Dafny, и по той же
   * причине.
   *
   * ПОЧЕМУ ЗДЕСЬ НЕТ `результат`. Предусловие говорит о том, что было ДО
   * вызова, а результата до вызова не существует. Связывать имя, которого не
   * может быть, значило бы разрешить написать утверждение, для которого нет
   * момента, когда его можно посчитать. Постусловие связывает `результат`
   * (`parseEnsures`), и разница между двумя словами ровно в этом.
   *
   * ПОЧЕМУ ЗДЕСЬ НЕТ `для всех`. Квантор при постусловии называет параметр, по
   * которому пойдёт индукция; предусловие индукцией не доказывается — оно
   * СНИМАЕТСЯ у каждого вызова по отдельности, и параметр для индукции ему не
   * нужен ни в одном месте машинерии.
   *
   * ПОЧЕМУ ИМЯ ОБЯЗАТЕЛЬНО — по той же причине, что у постусловия: по имени
   * предусловие называет отказ вызывающему и ведомость, а безымянное условие
   * читалось бы как «что-то здесь не подошло».
   */
  parseRequires(fnName, params) {
    const start = this.next()
    const name = this.expectName("ожидалось имя предусловия")
    /* Область видимости — параметры функции, и ничего сверх них: они уже
       связаны `bind`-ом в `parseFunction`, потому что `принимает` читается
       раньше. Отдельной области здесь не заводится намеренно — предусловие
       говорит ровно о том же, о чём говорит подпись. */
    ...
    return {
      name,
      expr,
      /* Код отказа свой, а не `FLANG_PROPERTY`: нарушенное постусловие — это
         ошибка ВЫЗЫВАЕМОГО (он обещал и не сделал), а невыполненное
         предусловие — ошибка ВЫЗЫВАЮЩЕГО (он не имел права звать). Один код на
         обе беды заставил бы читателя выяснять, кого чинить. */
      code: "FLANG_PRECONDITION",
      message: `не выполнено требование «${name}» функции «${fnName}»`,
      span: start.span,
    }
  }

  /**
   * Постусловие обычной функции: то, чем в языке ВЫСКАЗЫВАЮТ утверждение.
   *
   * ```flang
   * обеспечивает «длина неотрицательна» результат не меньше 0
   * для всех список обеспечивает «длина неотрицательна» результат не меньше 0
   * ```
   *
   * ПОЧЕМУ ЭТО НЕ НОВЫЙ РАНТАЙМ. Постусловие в AST было и работало: его
   * проверяет интерпретатор (`interpret.mjs`, кадр `post`) и печатают все восемь
   * целей, а кладёт его туда `compat.mjs` из `свойство` утилиты наследия FTS.
   * Не было ПОВЕРХНОСТИ: обычная функция flang сказать о себе не могла ничего.
   * Поэтому здесь заведено слово и ровно оно — ни одной новой проверки в
   * рантайме, ни одной новой строки в восьми печатях. Цена нововведения
   * измерима и равна нулю, а высказать утверждение стало можно.
   *
   * ЧТО ЗНАЧИТ `для всех`. Ничего для рантайма и всё для доказательства.
   * Постусловие и без него — утверждение обо ВСЕХ входах: тело одно, проверка
   * стоит после каждого возврата. `для всех список` называет ТОТ параметр, по
   * которому утверждение потом доказывают индукцией, и без этого имени
   * обязательство (`obligations.mjs`) не знало бы, что предъявлять ядру.
   * Поэтому квантор — не украшение записи, а единственное место, где автор
   * говорит, по чему идёт индукция; проверка от него не меняется, и это прямо
   * видно по тому, что `bind`, `code` и `expr` у обеих форм одни и те же.
   *
   * ПОЧЕМУ ИМЯ ОБЯЗАТЕЛЬНО. По имени на постусловие ссылается шаг
   * доказательства (`по свойству «…»`) и по нему же его называет отказ
   * рантайма. Безымянное постусловие нельзя ни процитировать, ни найти в
   * ведомости — оно читалось бы как «что-то здесь нарушено».
   *
   * ПОЧЕМУ `результат`, А НЕ ВЫДУМАННОЕ СЛОВО. Так уже названо то же самое в
   * свойстве утилиты FTS (`fts-legacy.mjs`, `RESULT_BINDING`) и в значении по
   * умолчанию интерпретатора (`interpret.mjs`, `DEFAULT_RESULT_BINDING`).
   * Третьего слова об одном и том же в языке быть не должно.
   */
  parseEnsures(fnName, params) {
    const start = this.peek()
    let forall = null
    if (this.eatKw("forall")) {
      forall = this.expectName(`после '${this.kw("forall")}' ожидалось имя параметра`)
      if (!params.some((param) => param.name === forall)) {
        /* Квантор по чужому имени — не описка в прозе, а обязательство, которое
           нечем предъявить: индукция пойдёт по параметру, которого у функции
           нет. Молчать нельзя, догадываться не о чем. */
        this.fail(
          `у функции «${fnName}» нет параметра «${forall}», а квантор назван по нему`
            + `${params.length === 0 ? " (у функции нет параметров вовсе)" : ""}`,
          start,
        )
      }
      this.expectKw("ensures", `ожидалось '${this.kw("forall")} имя ${this.kw("ensures")} «имя» утверждение'`)
    } else {
      this.next()
    }
    const name = this.expectName("ожидалось имя постусловия")
    /* `результат` виден только внутри утверждения — там, где он и есть: до
       возврата его нет, и в теле такое имя означало бы другое. */
    this.pushScope([RESULT_BINDING])
    const expr = this.parseExpression()
    this.popScope()
    this.endLine()
    const node = {
      ...
      expr,
      bind: RESULT_BINDING,
      /* Код тот же, что у постусловия из AST по умолчанию: отказ обычной
         функции обязан быть неотличим от отказа утилиты — читателю сообщения
         всё равно, каким словом обещание было записано. */
      code: "FLANG_PROPERTY",
      message: `нарушено свойство «${name}» функции «${fnName}»`,
      span: start.span,
    }
    if (forall !== null) node.forall = forall
    return node
  }
  ...
  parseLiteralValue() {
    ...
    /* Чаще всего сюда приходит имя варианта или запись без слова впереди:
       `ожидается «Звено» с значение равным 1`. Сообщение поэтому перечисляет,
       что литералом БЫВАЕТ, и называет пропущенное слово. */
    return literalValue(expression, () =>
      this.fail(
        "не разобрана конструкция: ожидалось литеральное значение — примеры не вычисляются: "
          + "годятся число, строка, 'да', 'нет', 'ничто', список из них, 'запись «Имя» с «поле» равным …' "
          + "и 'вариант «Имя» с поле равным …'; одному имени в ёлочках нужно слово 'вариант' или "
          + "'запись' впереди",
        token,
      ),
    )
  ...
  parsePattern() {
    ...
    /* Сюда доходят те, кто написал в `случай` условие или выражение. Числа,
       строки и `да`/`нет` образцами БЫВАЮТ (выше), поэтому сообщение
       перечисляет разрешённое целиком и называет место условия — `если`. */
    return this.fail(
      "не разобрана конструкция: ожидался образец — в 'случай' годятся 'пусто', 'голова' и 'хвост', "
        + "'вариант «Имя»', имя варианта с прописной, число, строка, 'да', 'нет', 'ничто' и имя со "
        + "строчной (оно связывает любое значение); условие образцом не бывает — его пишут через 'если'",
      token,
    )
  }
  ...
  parseComparison() {
     ...
     *
     * ЗАЧЕМ. Вариант различал только `разбор`, а `разбор` требует блока с
     * отступом и потому в постусловие не помещается — оно однострочное. Из-за
     * этого правду о функции вида «Успешно» (`stdlib/result.flang`) сказать было
     * НЕЧЕМ: попытка написать `результат равен (итог это вариант «Успех»)`
     * упиралась в `FLANG_PARSE: ожидался знак ')'`, и в замере цены
     * доказательства (`docs/benchmark-proof-cost-2.md`) этот случай стоит строкой
     * «на языке не выразить», а не «ядро не берёт».
     *
     * НОВЫХ СЛОВ НОЛЬ. `это` — уже ключевое (`alias`: `тип «Х» это …`),
     * `вариант` — уже ключевое, а сочетание `это вариант` в позиции выражения
     * разбор ПРЕЖДЕ отвергал. Запись занимает то, что было ошибкой, и не
     * отнимает ничего у написанного: голых вхождений `это` в корпусе 0
     * (`npm run word:occupancy это`).
     *
     * ЧТО ЭТО ЗА УЗЕЛ. Не новый вид узла и не встроенная форма, а `разбор`,
     * написанный разбором же: ветвь названного варианта даёт `да`, `любое` —
     * `нет`. Оттого ни типизатор, ни вычислитель, ни восемь печатей не узнают о
     * новой записи ничего — они видят тот же `match`, который читают с первого
     * дня. Проверку исчерпывающести, разбор образцов и печать это наследует
     * целиком, а не повторяет второй раз рядом.
     *
     * ПОРЯДОК ВЕТВЕЙ ОБЯЗАТЕЛЕН ИМЕННО ТАКОЙ: `любое` покрывает всё, и случай
     * после него типизатор отвергает (`FLANG_MATCH_UNREACHABLE`). */
    if (this.atKw("alias") && this.atKw("variant", 1)) {
      const слово = this.next()
      this.next()
      const имя = this.expectName("ожидалось имя варианта")
      return {
        kind: "match",
        target: left,
        cases: [
          { pattern: { kind: "variant", name: имя, bind: {} }, body: { kind: "literal", value: true, span: слово.span } },
          { pattern: { kind: "any" }, body: { kind: "literal", value: false, span: слово.span } },
        ],
        span: слово.span,
      }
    }
  ...
  parsePrimary() {
    ...
    switch (token.value) {
      ...
      /* Захват части аргументов — теми же словами, какими поля присваивает
         ...
         сверка разбора с эталоном разошлась бы на каждой такой программе — при
      ...
      default:
        ...
        /* Сюда приходит слово, которое ЯЗЫК уже занял: `состояние`, `цепочка`,
           `в`, `от`. Прежнее «неожиданное 'состояние'» не называло ни причины
           («слово занято»), ни лекарства, и на этом умирали не только модели,
           но и написанные человеком решения. Лекарства ровно два, и оба
           проверены: имя в ёлочках годится как имя, а лишнее слово формы
           убирается. */
        return this.fail(
          `не разобрана конструкция: неожиданное '${token.text}' — это слово занято языком, `
            + `именем оно быть не может: напишите имя в ёлочках («${token.text}») или переименуйте; `
            + "а если слово попало в форму лишним — уберите его",
        )
    }
  }
   ...
   *   принимает «Команда счёта» с ящиком 100 сообщений
   *   обрабатывает «шаг счёта» с запасом 100000 витков
   * ```
   *
   * `с ящиком N` необязательно и приехало шагом А3 карты
   * (`flang/conc/RESILIENCE.md`): без него ящик неограничен, как было, и
   * заливающий отправитель кончается исчерпанием памяти; с ним переполнение —
   * названный отказ отправителя, который разбирает надзор.
   *
  ...
  parseProcess() {
    ...
    const node = {
      ...
      handler: null,
      budget: null,
      /* Ящик: `null` — неограничен, как было до А3; число — объявленный
         потолок. Стоит он на строке `принимает`, а не своей: ящик — свойство
         приёма, и разводить их по двум строкам значило бы разрешить объявить
         размер ящика процессу, который ничего не принимает. */
      mailbox: null,
      span: start.span,
    }
    ...
    if (this.enterBlock()) {
      while (!this.atBlockEnd()) {
        ...
        if (this.eatKw("accepts")) {
          ...
          if (this.eatKw("mailbox")) {
            node.mailbox = this.expectNumber("после 'с ящиком' ожидался размер ящика")
  ...
  /**
   * Объявленное свойство: пять законов, у каждого своё разрешение.
   *
   * ```flang
   ...
   *   функция «Номер сборки»
   * ```
   *
   * ── НИ ОДНОГО НОВОГО СЛОВА, и число этому есть ──────────────────────────────
   *
   * `свойство` ключевым словом было и до сегодняшнего дня — им объявляется предел
   * утилиты внутри `утилита` (70 вхождений в `.fts` репозитория, все с отступом,
   * все внутри утилиты). А ВЕРХНЕУРОВНЕВОГО `свойство` в дереве нет ни одного:
   * `grep -c '^свойство'` по всем `.flang` и `.fts` даёт 0, и `parseDeclaration`
   * отвечал на него `не разобрана конструкция: неожиданное 'свойство'`. То есть
   * эта запись занимает то, что БЫЛО ОШИБКОЙ, и не отнимает ничего у написанного:
   * ни одна строка корпуса не меняет смысла, поток токенов не сдвигается,
   * побайтовая сверка таблицы лексера с `self/lexer.flang` не задета вовсе.
   *
   * `носитель`, `операция`, `по`, `функция` — тоже все четыре уже ключевые.
   * Занято ноль новых слов из ноля.
   *
   * ── Имя свойства — из закрытого словаря, и это не экономия ──────────────────
   *
   * Пять имён (`«дистрибутивность»`, `«идемпотентность»`, `«частичный порядок»`,
   * `«монотонность»`, `«коммутативность»`) пишутся в ёлочках, а не ключевыми
   * словами, — ровно как стратегия надзора (`стратегия «перезапустить»`), и по
   * тому же доводу: закавыченное имя ключевым не становится никогда, значит пять
   * существительных не отбираются у авторов во всех файлах репозитория. В корпусе
   * ни одно из пяти не встречается ни разу (0 вхождений на все пять).
   *
   * Проверяет словарь `types.mjs`, а не разбор, — тем же порядком, каким там же
   * проверяется стратегия надзора: дело разбора — форма, дело проверки типов —
   * устройство. Здесь отвергается ровно одно: свойство, не назвавшее ни операции,
   * ни функции. Объявление, которое ничего не обещает, хуже отсутствующего,
   * потому что читается как гарантия.
   */
  /**
   * БАЗА НОМЕРА — объявлением АВТОРА, а не ключом запуска: `элемент начиная с 0`.
   *
   * ── ПОЧЕМУ ЭТИ ДВА СЛОВА, А НЕ НОВОЕ ────────────────────────────────────────
   * Оба уже стоят в таблице поверхностей и оба открыты на ВСЕХ ЧЕТЫРЁХ:
   * `item` (`элемент` / `item` / `ero` / `元素`) и `startingWith`
   * (`начиная с` / `starting with` / `komencante per` / `起始于`). Новое слово
   * пришлось бы заводить четырежды, а из 149 понятий на всех четырёх открыто
   * 132 — дыры в таблице сегодня все до одной в словаре доказательства, и
   * пополнять их незачем.
   *
   * Правило выбора то же, каким выше отвергнуто `по операции`: нового слова не
   * заводим, пока хватает старого. Мерой служит корпус — сочетания
   * `элемент начиная с` в 751 файле `.flang` нет НИ ОДНОГО, а `элемент` у левого
   * края сегодня даёт ошибку разбора («этим словом объявление не начинается»),
   * значит поток токенов ни одной написанной программы от этой записи не
   * меняется. Тем же приёмом у левого края завели `требует`.
   *
   * ── ЧТО ЗНАЧИТ МОЛЧАНИЕ ─────────────────────────────────────────────────────
   * Умолчание — ЕДИНИЦА, и весь сегодняшний корпус написан при ней. Программа
   * без этой строки обязана считаться и печататься ровно как прежде, поэтому
   * ключа `базаНомера` в её AST не появляется вовсе (см. `parseProgram`).
   *
   * ── ТРИ ОТКАЗА, И ВСЕ ТРИ НАЗВАНЫ ──────────────────────────────────────────
   *   • не 0 и не 1 — отказ. Третьей базы у языка нет: `at = номер − база`
   *     осмысленно ровно при двух числах, и всякое другое молча сдвинуло бы
   *     каждый доступ по списку.
   *   • не число вовсе — отказ там же и тем же кодом.
   *   • ДВА объявления в одной программе — отказ. Порядок строк не вправе
   *     решать, какое из двух чисел настоящее.
   */
  parseIndexBase() {
    const start = this.next()
    if (!this.eatKw("startingWith")) {
      this.fail(
        "не разобрана конструкция: после 'элемент' у левого края ожидалось 'начиная с' и число 0 или 1"
          + " — этим словом у левого края начинается только объявление базы номера",
        start,
      )
    }
    const token = this.peek()
    if (token.kind !== "number" || (token.value !== 0 && token.value !== 1)) {
      this.fail(
        "база номера объявляется числом 0 или 1: `элемент начиная с 0` значит, что первый элемент"
          + " списка имеет номер 0, `элемент начиная с 1` — что единицу; третьей базы у языка нет",
        token,
      )
    }
    if (this.базаНомера !== null) {
      /* Числа в сообщении нет НАРОЧНО: эталон на flang читает `прочие` списком
         узлов, и достать из него первое число значило бы завести там читателя
         числа из узла ради одной строки отказа. Место названо точно и без
         него — вторая строка та, на которой стоит курсор. */
      this.fail(
        "база номера объявлена дважды: объявление в программе одно —"
          + " иначе смысл программы решал бы порядок строк",
        start,
      )
    }
    this.next()
    this.базаНомера = token.value
    this.endLine()
  }
  ...
  parseProperty() {
    ...
    if (this.enterBlock()) {
      while (!this.atBlockEnd()) {
        ...
        if (this.atKw("carrier")) {
          const at = this.next()
          if (node.carrier !== null) this.fail(`у свойства «${name}» больше одного носителя`, at)
          node.carrier = this.parseTypeExpression()
          this.endLine()
          continue
        }
        if (this.atKw("operation")) {
          const at = this.next()
          if (node.operation !== null) this.fail(`у свойства «${name}» больше одной операции`, at)
          node.operation = this.expectName("ожидалось имя функции-операции")
          this.endLine()
          continue
        }
        /* `по «Сложить»` — вторая операция дистрибутивности. Роль различает
           СЛОВО, а не порядок строк: `операция` называет ту, что раскрывается,
           `по` — ту, по которой раскрывают. Порядок, решающий смысл, здесь
           запрещён тем же доводом, каким два равенства на одном объекте — беда:
           выбирал бы между ними порядок строк.

           Не `по операции «Сложить»`, хотя по-русски это и ровнее: ключевое
           слово у понятия одно — `операция`, а `операции` пришлось бы завести
           СКЛОНЕНИЕМ в таблице лексера (как `число | числа | числом`). Слово
           свободно — голых вхождений в корпусе 0, — но правило здесь строже
           удобства: нового слова не заводим, пока хватает старого. */
        if (this.atKw("by")) {
          const at = this.next()
          if (node.over !== null) this.fail(`у свойства «${name}» больше одной второй операции`, at)
          node.over = this.expectName("после 'по' ожидалось имя второй операции")
          this.endLine()
          continue
        }
        if (this.atKw("function")) {
          const at = this.next()
          if (node.function !== null) this.fail(`у свойства «${name}» больше одной функции`, at)
          node.function = this.expectName("ожидалось имя функции")
          this.endLine()
          continue
        }
        this.fail(
          "не разобрана конструкция: в свойстве ожидаются 'носитель', 'операция', 'по' или 'функция'",
        )
      }
      this.exitBlock()
    }

    if (node.operation === null && node.function === null) {
      this.fail(
        `свойство «${name}» не назвало ни операции, ни функции: проверять было бы нечего`,
        start,
      )
    }
    return node
  ...
  /**
   * ТРЕБОВАНИЕ: утверждение о функции, высказанное ДО того, как функция
   * написана.
   *
   * ```flang
   * требует «Скидка»
   ...
   *   возвращает число
   *   обеспечивает «скидка не больше 30%» результат не больше (сумма умножить на 0.3)
   * ```
   *
   * ── Чего сегодня нельзя, и это замерено ────────────────────────────────────
   *
   * `обеспечивает` вешается ТОЛЬКО на функцию, а функция без тела отвергается:
   * `flang check` на такой программе даёт `FLANG_PARSE` и текст «функция
   * «Скидка» не содержит тела», код выхода 1. То есть требование нельзя
   * записать, пока код не написан, — а спека ДО кода и есть весь смысл затеи.
   *
   * ── Ни одного нового слова, и число этому есть ─────────────────────────────
   *
   * `требует`, `принимает`, `возвращает`, `обеспечивает` — все четыре уже
   * ключевые. `^требует` у левого края в корпусе `.flang` и `.fts` — 0
   * вхождений, и до сегодняшнего дня строка отвечала «не разобрана
   * конструкция». Эта запись занимает то, что БЫЛО ОШИБКОЙ, — ровно тем же
   * доводом, каким верхнеуровневое `свойство` заняло свою строку. Поэтому:
   * таблица поверхностей не трогается, четырёх поверхностей заводить не надо,
   * поток токенов корпуса не сдвигается, побайтовая сверка `KEYWORDS` с
   * `self/lexer.flang` не задета вовсе.
   *
   * ── ЧЕМ ТРЕБОВАНИЕ СВЯЗЫВАЕТСЯ С РЕАЛИЗАЦИЕЙ — ИМЕНЕМ, И ЭТО РЕШЕНИЕ ───────
   *
   * Связывать ПОДПИСЬЮ нельзя. Две функции `(число) → число` — «Скидка» и
   * «Налог» — по подписи неразличимы; требование, притянутое к функции по
   * форме входа и выхода, молча прицепилось бы к соседней, а смена типа
   * параметра так же молча его отцепила бы. Подпись — не личность.
   *
   * Имя — единственная связь, которая НАПИСАНА, а значит видна в diff-е,
   * проверяема прогоном и переживает отсутствие функции: пока кода нет, имя
   * всё равно названо, и именно поэтому «реализации нет» можно СКАЗАТЬ. Так же
   * связывается `свойство «монотонность» функция «Номер сборки»` — здесь не
   * заводится нового правила, здесь берётся уже принятое.
   *
   * Подпись при этом не выбрасывается: она объявляется тут же и сверяется с
   * реализацией (`types.mjs`), потому что утверждение о параметре `сумма`
   * бессмысленно у функции, которая `сумму` не принимает.
   *
   * ── Что уходит в напечатанный код — НИЧЕГО ────────────────────────────────
   *
   * Требование — проверка времени сборки и только. Своего списка в AST оно не
   * получает нигде, кроме программы, и ни одна из восьми целей печати его не
   * читает: печатается то, что лежит в `functions`, а требование там не лежит.
   * Постусловие, которое требование несёт, печатается ровно тогда и ровно так,
   * как печаталось раньше, — с той функции, где оно написано.
   */
  parseRequirement() {
    const start = this.next()
    const name = this.expectName("ожидалось имя функции, к которой требование")

    const params = []
    const postconditions = []
    let returns = null

    /* Область видимости заводится та же, что у функции: `принимает` связывает
       имена, и утверждение говорит о них. Без неё `сумма` в утверждении была
       бы несвязанным именем — и требование молча принимало бы опечатку. */
    this.pushScope([])
    if (this.enterBlock()) {
      while (!this.atBlockEnd()) {
        this.skipNewlines()
        ...
        /* Утверждение читается ТЕМ ЖЕ `parseEnsures`, каким читается
           постусловие функции, и даёт ТОТ ЖЕ узел. Это не экономия строк, а
           само устройство проверки: сверять требование с реализацией — значит
           сравнивать два узла одной формы, а не переводить один в другой. */
        if (this.atKw("ensures") || this.atKw("forall")) {
          postconditions.push(this.parseEnsures(name, params))
          continue
        }
        /* СЛОВО УТВЕРЖДЕНИЯ ЗДЕСЬ НЕ ЦИТИРУЕТСЯ, и это не небрежность.
           `обеспечивает` названо в таблице поверхностей только по-русски и
           по-английски — намеренно, и довод записан там же: устоявшегося слова
           у понятия нет ни в эсперанто, ни в китайском, а придуманное хуже
           отсутствующего. Сторож `surfaces.test.mjs` стережёт ровно эту связь:
           слово, которое парсер цитирует в сообщении, обязано быть у таблицы на
           всех четырёх поверхностях, иначе автор чужого файла увидит русское
           слово, которого не писал. Улика прямая и замерена: с цитатой
           `обеспечивает` в этой строке сторож краснел — один новый красный на
           весь прогон, и ровно он. Поэтому третья строка блока названа тем, чем
           она является, а не своим ключевым словом. */
        this.fail(
          "не разобрана конструкция: в требовании ожидаются 'принимает', 'возвращает' или утверждение",
        )
      }
      this.exitBlock()
    }
    ...
    /* Требование без утверждения ХУЖЕ отсутствующего: оно читается как
       гарантия, а проверять в нём нечего. Тем же доводом отвергается свойство,
       не назвавшее ни операции, ни функции. */
    if (postconditions.length === 0) {
      this.fail(`требование к функции «${name}» не высказало ни одного утверждения`, start)
    }

    const node = { kind: "requirement", name, params, returns, postconditions, span: start.span }
    return node
  }
  ...
  parseMorphism() {
    ...
    if (this.atKw("from")) {
      this.next()
      /* …или ЕСТЕСТВЕННОЕ ПРЕОБРАЗОВАНИЕ: `из функтора «Ф» в функтор «Г»`.
         Разводит их один взгляд вперёд — за `из` стоит либо имя объекта, либо
         ключевое слово `функтор`, а ключевым словом имя быть не может никогда.

         СЛОВО ТО ЖЕ, И ЭТО НЕ ЭКОНОМИЯ. Естественное преобразование — это
         морфизм в категории функторов, и других прочтений у записи нет: `из
         функтора … в функтор …` называет концы так же, как их называет стрелка
         и как их называет функтор. Новое слово стоило бы правки в двух таблицах
         лексера (`src/lexer.mjs` и `self/lexer.flang`, они сверяются
         побайтово), четырёх поверхностей и двух парсеров — за то же самое. */
      if (this.atKw("functor")) return this.parseTransformation(name, start)
   ...
   *
   * ```flang
   * морфизм «выпрямить» из функтора «Через отгрузку» в функтор «Напрямую»
   *   объект «Заказ» отображается в морфизм «сдвиг заказа»
   *   объект «Позиция» отображается в морфизм «сдвиг позиции»
   * ```
   *
   * ЗАЧЕМ ЭТО СТАЛО ВОЗМОЖНО ТОЛЬКО СЕЙЧАС. Закон естественности — это
   * КОММУТИРУЮЩИЙ КВАДРАТ, то есть равенство двух композиций: `Г(ф) после
   * альфа_Х` и `альфа_У после Ф(ф)` — две разные стрелки с общими концами. Пока
   * равенства морфизмов в языке не было, сказать «они равны» было нечем, и
   * контракт (`flang/cat/SPEC.md`) держал преобразование в задуманном. Равенство
   * появилось, и вместе с ним появился квадрат.
   *
   * `отображается в морфизм` — то же слово, каким функтор называет образ
   * стрелки, и та же разница с `отображается в`: там объект переходит в объект,
   * здесь объекту сопоставлена СТРЕЛКА. Различие, которое функтор уже проводит,
   * второй раз изобретать незачем.
   *
   * Преобразование без единой компоненты отвергается разбором — по тому же
   * доводу, по какому отвергается закон без примеров: объявление, которое
   * ничего не обещает, хуже отсутствующего, потому что читается как гарантия.
   */
  parseTransformation(name, start) {
    this.expectKw("functor", "после 'из' ожидалось 'функтор'")
    const from = this.expectName("ожидалось имя исходного функтора")
    this.expectKw("to", "после имени функтора ожидалось 'в'")
    this.expectKw("functor", "после 'в' ожидалось 'функтор'")
    const to = this.expectName("ожидалось имя целевого функтора")
    this.endLine()
    ...
    if (this.enterBlock()) {
      while (!this.atBlockEnd()) {
        this.skipNewlines()
        if (this.atBlockEnd()) break
        this.expectKw("object", "у преобразования ожидалось 'объект'")
        const object = this.expectName("ожидалось имя объекта")
        this.expectKw("mapsToMorphism", "ожидалось 'отображается в морфизм'")
        components.push({ object, morphism: this.expectName("ожидалось имя морфизма") })
        this.endLine()
      }
      this.exitBlock()
    }
    if (components.length === 0) {
      this.fail(
        `преобразование «${name}» требует хотя бы одной компоненты: ` +
          `'объект «Х» отображается в морфизм «…»'`,
        start,
      )
    }

    this.transformations.push({ kind: "transformation", name, from, to, components, span: start.span })
    return null
  }
   ...
   *
   * ── Почему одно слово, а не второе ──────────────────────────────────────────
   * Слово `теорема` в языке стоит с самого начала: им написаны пять теорем
   * наследия FTS, и разбирались они всегда. Читал их только человек: узел уезжал
   * в `legacy`, и оттуда его не брал никто — ни типизатор, ни ведомость. То
   * есть высказать теорему было можно, а следствий из неё не было никаких.
   *
   * Завести для доказательства ВТОРОЕ слово значило бы сказать, что теорема,
   * которую доказывают, — это не теорема. Формы различаются не словом, а тем,
   * ЧТО в них написано, и различие решается одним токеном вперёд:
   *
   *   `дано «Объект» имеет …`   старая: за именем стоит `имеет`
   *   `дано <утверждение>`      новая: за именем стоит что угодно другое
   ...
   *   `утверждаем`, `индукция по`, `случай`, `по свойству`, `по примеру`,
   *   `по закону`, `по предположению`, `затем` — только новая
   *
   * Решение принимается ПОСЛЕ разбора блока, а не до: узел новой формы
   * появляется ровно тогда, когда в блоке встретилось хоть одно слово новой
   * формы. Все пять теорем наследия отдают ПОБАЙТОВО тот же `ftsLegacy`, что
   * отдавали, — иначе рухнула бы полнота подмножества FTS, а вместе с ней
   * побайтовая неподвижная точка самоприменения.
   *
   * ── Поверхность взята не из головы ──────────────────────────────────────────
   * Это Isar из Isabelle: структурированное доказательство, которое ЧИТАЕТСЯ, —
   * `утверждаем` (`show`), `дано` (`assume`), `затем` (`also`/`moreover`),
   * `по свойству «…»` (`by (rule …)`), `следовательно доказано` (`qed`). Isar
   * придуман ровно ради читаемости, и переизобретать поверхность, которая уже
   * доказала себя, не за чем. Чего здесь нет и не будет — ТАКТИК: тактика
   * ворочает состояние цели поиском, а поиск нетотален по природе, значит на
   * flang её не написать, и жила бы она только в свидетеле на JS. Ядро всё равно
   * проверяет ТЕРМ, поэтому тактики не входят в контур доверия и их можно
   * добавить когда угодно позже — но начинать с автоматики значило бы завести
   * вторую дыру в самоприменённом слое к той одной, что уже есть.
   */
  parseTheorem() {
    const start = this.next()
    const value = {
    ...
    /* Новая форма собирается рядом и в дело идёт только если что-то в неё
       попало. Пустой сбор ничего не стоит и ничего не меняет. */
    const доказательство = { vars: [], given: [], claim: null, induction: null, steps: [], qed: false }
    let новая = false
    /* Область видимости теоремы: имена, введённые `дано`, плюс `результат`.
       Без неё `стопка` в утверждении читалось бы как вызов функции без
       аргументов (`bindNullaryCalls`) и упиралось бы в «имя не связано» — то
       есть высказать утверждение о параметре было бы нельзя. */
    this.pushScope([RESULT_BINDING])
    ...
    if (this.enterBlock()) {
      while (!this.atBlockEnd()) {
        this.skipNewlines()
        if (this.atBlockEnd()) break
        /* `дано <имя>: <тип>` — введённая переменная, она же квантор «для
           всех» этой теоремы; `дано <имя> <утверждение…>` — гипотеза;
           `дано «Объект» имеет …` — старая форма. Различает первую от второй
           ровно то, чем различает их `принимает`: двоеточие или `является`.

           ПОЧЕМУ ТЕМИ ЖЕ ДВУМЯ ЗНАКАМИ, ЧТО У ПАРАМЕТРА. Переменная теоремы и
           параметр функции — одно и то же: имя с типом, о котором говорят для
           всех значений. Третья запись того же самого заставила бы читателя
           помнить, чем `дано х: число` отличается от `принимает х: число`, а не
           отличается оно ничем.

           ИМЯ, СОВПАВШЕЕ СО СЛОВОМ ЯЗЫКА, тоже имя, и здесь оно не читалось.
           `expectNameToken` принимает ключевое слово в позиции имени давно —
           оттого и работает `принимает число: число` у половины библиотеки, —
           но эта строка спрашивала `atName`, а `число` приезжает токеном
           `keyword`, не `name`. `дано число: число` уходило в гипотезу,
           гипотеза читалась выражением, и выражение упиралось в «имя «число» не
           связано»: переменная не вводилась вовсе, и теорему о такой функции
           написать было нечем. Признак остался прежний — двоеточие или
           `является` вторым токеном, — расширился только вопрос о первом. */
        if (this.atKw("given") && (this.atName(1) || this.at("keyword", 1)) && (this.atPunct(":", 2) || this.atKw("is", 2))) {
          новая = true
          const слово = this.next()
          const переменная = this.parseParameter()
          this.bind(переменная.name)
          доказательство.vars.push({ ...переменная, span: переменная.span ?? слово.span })
        } else if (this.atKw("given") && !(this.atName(1) && this.atKw("has", 2))) {
          новая = true
          const слово = this.next()
          доказательство.given.push({ expr: this.parseExpression(), span: слово.span })
        } else if (this.eatKw("given")) {
        ...
        } else if (this.atKw("claim")) {
          новая = true
          const слово = this.next()
          if (доказательство.claim !== null) {
            this.fail(`не разобрана конструкция: у теоремы больше одного '${this.kw("claim")}'`, слово)
          }
          доказательство.claim = { expr: this.parseExpression(), span: слово.span }
        } else if (this.atKw("induction")) {
          новая = true
          if (доказательство.induction !== null) {
            this.fail("не разобрана конструкция: у теоремы больше одной индукции")
          }
          доказательство.induction = this.parseInduction(value.title)
          continue
        } else if (this.atAnyKw(["nextStep", "byProperty", "byExample", "byHypothesis", "law"])) {
          новая = true
          доказательство.steps.push(this.parseProofStep(value.title))
          continue
        } else if (this.atKw("qed")) {
          новая = true
          const слово = this.next()
          if (доказательство.qed) {
            this.fail(`не разобрана конструкция: '${this.kw("qed")}' стоит дважды`, слово)
          }
          доказательство.qed = true
      ...
      if (новая) {
        this.popScope()
        return this.finishTheorem(value, доказательство, start)
      }
    }
    this.popScope()
  ...
  /**
   * Собрать узел теоремы новой формы.
   *
   * Смешение форм — отказ, а не склейка. `по морфизму` рядом с `утверждаем`
   * читалось бы как обоснование шага, а означает совсем другое (стрелку
   * наследия FTS); принять оба значило бы завести конструкцию, у которой два
   * смысла и ни одного объявленного.
   */
  finishTheorem(value, доказательство, start) {
    const наследие = [
      [value.structure !== null, "дано «Объект» имеет «поле»"],
      [value.collection !== null, "в данных"],
      [value.morphisms.length > 0, "по морфизму"],
      [value.conclusion !== null, "следовательно «вывод»"],
    ].filter(([есть]) => есть).map(([, слово]) => слово)
    if (наследие.length > 0) {
      this.fail(
        `теорема «${value.title}» смешала две формы: ${наследие.join(", ")} — из старой,`
          + " а рядом стоят слова доказательства. Выберите одну форму",
        start,
      )
    }
    if (доказательство.claim === null) {
      this.fail(`теорема «${value.title}» ничего не утверждает: нет строки '${this.kw("claim")}'`, start)
    }
    const узел = { kind: "theorem", name: value.title, claim: доказательство.claim }
    if (доказательство.vars.length > 0) узел.vars = доказательство.vars
    if (доказательство.given.length > 0) узел.given = доказательство.given
    if (доказательство.induction !== null) узел.induction = доказательство.induction
    if (доказательство.steps.length > 0) узел.steps = доказательство.steps
    узел.qed = доказательство.qed
    узел.span = start.span
    return узел
  }

  /**
   * `индукция по <параметр> убывает <мера>` и случаи под ней.
   *
   * ПОЧЕМУ ИНДУКЦИЯ НЕ НОВОЕ ПОНЯТИЕ. Индуктивный тип — это начальная алгебра,
   * и её универсальное свойство даёт СРАЗУ И свёртку, и принцип индукции.
   * Свёртка в языке есть с самого начала (`свёртка … начиная с …`), объявленные
   * суммы и списки — её начальные алгебры, а `разбор` по сумме исчерпывающий
   * (это доказывает типизатор). Значит принцип индукции здесь не заводится, а
   * ЧИТАЕТСЯ с того же объявления: случаев ровно столько, сколько
   * конструкторов, и в рекурсивном случае доступно предположение о части
   * значения. Отдельного принципа не выдумано ни одного (`flang/cat/SPEC.md`,
   * «начальная алгебра»).
   *
   * ПОЧЕМУ `убывает` НЕОБЯЗАТЕЛЕН И ЧТО ОН ЗДЕСЬ ЗНАЧИТ. По объявленной сумме
   * убывание доказывать не надо: часть значения меньше значения, цепочка частей
   * конечного дерева обрывается сама — тот же довод, которым носитель
   * «структурой» держит 149 функций корпуса. Мера нужна там, где убывает не
   * часть, а число: тогда автор называет её тем же словом, каким называет меру
   * функции, и по ней ядро сверяет шаг. Второго слова об одном и том же нет.
   *
   * ПОЧЕМУ ОБРАЗЦЫ — ТЕ ЖЕ, ЧТО У `разбор`. Не ради экономии: исчерпывающность
   * случаев считает типизатор, и считает он её по образцам `разбор`. Свои
   * образцы у индукции означали бы вторую проверку исчерпывающности, обязанную
   * совпадать с первой, — и разошлась бы она молча.
   */
  parseInduction(title) {
    const start = this.next()
    const param = this.expectName(`после '${this.kw("induction")}' ожидалось имя параметра`)
    let measure = null
    if (this.atKw("decreases")) {
      this.next()
      measure = this.parseExpression()
    }
    const cases = []
    if (!this.enterBlock()) this.fail(`у индукции теоремы «${title}» нет ни одного 'случай'`, start)
    while (!this.atBlockEnd()) {
      this.skipNewlines()
      if (this.atBlockEnd()) break
      const caseStart = this.expectKw("case", "не разобрана конструкция: ожидалось 'случай'")
      const pattern = this.parsePattern()
      /* Имена, связанные образцом, видны в шагах случая: в рекурсивном случае
         именно они называют часть значения, о которой говорит предположение. */
      this.pushScope(this.patternBindings(pattern))
      const steps = []
      if (this.eatKw("then")) {
        steps.push(this.parseProofStep(title))
      } else if (this.enterBlock()) {
        this.skipNewlines()
        this.eatKw("then")
        while (!this.atBlockEnd()) {
      ...
      this.popScope()
      if (steps.length === 0) this.fail("у 'случай' нет ни одного шага", caseStart)
      cases.push({ pattern, steps, span: caseStart.span })
    }
    this.exitBlock()
    if (cases.length === 0) this.fail(`у индукции теоремы «${title}» нет ни одного 'случай'`, start)
    const узел = { param, cases, span: start.span }
    if (measure !== null) узел.measure = measure
    return узел
  }
   ...
   * верить. Поэтому у шага нет формы «затем <утверждение>» без `по …` — такой
   * шаг отвергается с указанием, что назвать.
   *
   ...
   * обязательно — им и держится порядок шагов.
   *
   * ПОЧЕМУ `затем` — УЖЕ СТОЯЩЕЕ СЛОВО. Оно в языке с теорката: `цепочка` со
   * `сначала`/`затем` — композиция в порядке чтения. Смысл тот же самый —
   * следующее звено цепочки, — и заводить второе слово об этом значило бы
   * держать читателю два словаря вместо одного. Новых слов у шага поэтому
   * четыре, а не пять.
   */
  parseProofStep(title) {
    const start = this.peek()
    const продолжение = this.eatKw("nextStep")
    let claim = null
    if (продолжение) {
      /* `затем по свойству «…»` законно: шаг продолжает цепочку и не выписывает
         утверждение заново — оно то же, что у цели. */
      if (!this.atAnyKw(["byProperty", "byExample", "byHypothesis", "law"])) {
        claim = this.parseExpression()
      }
    }
    const by = this.parseJustification(title, start)
    this.endLine()
    const узел = { by, span: start.span }
    if (claim !== null) узел.claim = claim
    if (продолжение) узел.next = true
    return узел
  }

  /** `по свойству «…»`, `по примеру «…»`, `по закону «…»`, `по предположению`. */
  parseJustification(title, start) {
    if (this.eatKw("byProperty")) {
      return { rule: "property", name: this.expectName(`после '${this.kw("byProperty")}' ожидалось имя постусловия`) }
    }
    if (this.eatKw("byExample")) {
      return { rule: "example", name: this.expectName(`после '${this.kw("byExample")}' ожидалось имя примера`) }
    }
    if (this.eatKw("law")) {
      return { rule: "law", name: this.expectName(`после '${this.kw("law")}' ожидалось имя закона`) }
    }
    if (this.eatKw("byHypothesis")) {
      /* У предположения индукции имени нет: оно одно на случай, и называть его
         значило бы разрешить сослаться на предположение чужого случая. */
      return { rule: "hypothesis", name: null }
    }
    this.fail(
      `шаг доказательства теоремы «${title}» ничем не обоснован: назовите факт —`
        + ` '${this.kw("byProperty")} «…»', '${this.kw("byExample")} «…»',`
        + ` '${this.kw("law")} «…»' или '${this.kw("byHypothesis")}'`,
      start,
    )
  }
    ...
    if (this.enterBlock()) {
      while (!this.atBlockEnd()) {
        ...
        if (this.eatKw("object")) {
          ...
          /* ПЕРЕВОД ДАННЫХ: `объект «Заказ» отображается в «Отгрузка» даёт «Ф»`.
             Без него связь двух модулей — сличение имён и ничего больше: функтор
             говорит «этот объект соответствует тому», а ЧЕМ соответствует, в
             программе не написано, и проверить нечего.

             НИ ОДНОГО НОВОГО СЛОВА, и это не экономия: слово в таблице лексера —
             это ещё и четыре поверхности (ru/en/eo/zh), и правка `self/lexer.flang`,
             которую сверяет побайтово тест самораскрутки. `даёт` уже ключевое, и
             значит здесь ровно то же, что у стрелки и у моноида, — «вот функция,
             которая это считает». А сама запись занимает то, что было ошибкой:
             `объект «А» отображается в «Б» даёт «Ф»` разбор ПРЕЖДЕ отвергал
             («не разобрана конструкция: непонятная строка функтора»), потому что
             за именем образа шёл либо конец строки, либо отступной блок полей.

             Ключ `gives` встаёт ПОСЛЕ `fields` и появляется только там, где
             написан: AST сорока моделей `.fts` репозитория, где переводов нет,
             не меняется ни на байт. */
          if (this.eatKw("gives")) {
            mapping.gives = this.expectName("после 'даёт' ожидалось имя функции перевода")
          }
          if (this.enterBlock()) {
            while (!this.atBlockEnd()) {
              this.skipNewlines()
