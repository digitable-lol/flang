 ...
 * отказом и произвести отказное имя не может: приставка `flang_` появляется
 * только у имени, занятого целью, а `runtime` и `cli` ни одной цели не заняты.
 *
...
export const ЗАНЯТО_ЦЕЛЬЮ = {
  ...
  js: { имена: [], хвосты: [], приставка: "flang_", суффикс: "" },
}
...
/**
 * Обход занятого имени: ОДНА приставка (Go — один суффикс) занятому, и ничего
 * свободному.
 *
 * Одного шага хватает не по счастью, а по инварианту: ни одно имя ни в одном из
 * восьми наборов не начинается с `flang_`/`Flang` и не кончается на `_flang`,
...
export function обойтиЗанятоеЦелью(имя, цель) {
  ...
  return `${набор.приставка}${имя}${набор.суффикс}`
}
...
export function обходСнимаетЗанятость() {
  return Object.entries(ЗАНЯТО_ЦЕЛЬЮ).map(([цель, набор]) => {
    ...
    const занятые = [...набор.имена, ...набор.хвосты.map((хвост) => `flang_${хвост}`)]
