 ...
 *
 * Лежит рядом с `naming.mjs`, а НЕ в `src/emit/`: каталог `src/emit` — это
 * реестр бэкендов (`bin/flang.mjs`, `emitTargets`), и всякий `.mjs` в нём
 * считается ещё одной целью печати. Файл общих таблиц стал бы там девятой
 * «целью» — с ней уже покраснела сверка двунаправленных, прежде чем он
 * переехал сюда.
 *
 * ── Чего не хватало ─────────────────────────────────────────────────────────
 *
 ...
 *
 * ОБОЙТИ — суффикс (`_flang`, `Flang`) ровно тем именам, что столкнулись. Это
 * чинит корень: имя `«JSON»` остаётся законным, печатается всегда, а в цели
 * получает имя, которого у цели нет. Цена — только у затронутых программ: у всех
 * прочих суффикс не появляется и вывод не меняется НИ НА БАЙТ.
 *
 * Обход именно СУФФИКСОМ, а не приставкой, по двум причинам. Первая: приставка
 * `flang_` уже занята бэкендом — так зовутся `flang_runtime.py` и `flang_cli.py`,
 * и по этой приставке напечатанное отличают от программы (`flang/test/
 * emit-python.test.mjs` ищет модуль программы ровно так). Приставка сделала бы
 * `модуль «JSON»` неотличимым от файла рантайма. Вторая: у Go занят ХВОСТ имени
 * файла, и приставка его не снимает вовсе. Один приём на восемь целей вместо
 * двух — и он же единственный, который работает у всех восьми.
 *
 ...
 *   js      — набор ПУСТ, и это измеренный ответ, а не пропуск: печатается ОДИН
 *             самодостаточный файл, прогонщик берёт его по файловому URL, а
 *             голый спецификатор Node в относительный файл не резолвится
 *             никогда. Занять стандартную библиотеку JavaScript именем модуля
 *             нельзя. Пустой набор стоит здесь ЯВНО, чтобы восьмая цель была
 *             посчитана, а не забыта.
 *
 ...
 *
 * Он не трогает имена САМОГО БЭКЕНДА (`flang_runtime`, `flang_cli`): там своя
 * политика — Python, Java, C# и Elixir по ним ОТКАЗЫВАЮТ, и отказ этот уместен,
 * потому что имена те бэкенд раздаёт сам и набор их закрыт. Здесь набор чужой,
 * открытый и подвижный от версии цели, поэтому здесь обход. Обход бежит ПЕРЕД
 * отказом и произвести отказное имя не может: суффикс появляется только у имени,
 * занятого целью, а ни `flang_runtime`, ни `flang_cli` ни одной цели не заняты.
 *
 * Эталоны печати на самом языке (`flang/self/emit-*.flang`) держат ТЕ ЖЕ
 ...
 * `суффикс` — чем обходить.
 */
export const ЗАНЯТО_ЦЕЛЬЮ = {
  c: { имена: C_HEADERS, хвосты: [], суффикс: "_flang" },
  go: { имена: [], хвосты: GO_BUILD_TAGS, суффикс: "_flang" },
  rust: { имена: RUST_CRATE_NAMES, хвосты: [], суффикс: "_flang" },
  python: { имена: PYTHON_STDLIB, хвосты: [], суффикс: "_flang" },
  java: { имена: JAVA_TYPES, хвосты: [], суффикс: "Flang" },
  csharp: { имена: CSHARP_TYPES, хвосты: [], суффикс: "Flang" },
  elixir: { имена: ELIXIR_ALIASES, хвосты: [], суффикс: "Flang" },
  js: { имена: [], хвосты: [], суффикс: "_flang" },
}
...
/**
 * Обход занятого имени: ОДИН суффикс занятому, и ничего свободному.
 *
 * Одного шага хватает не по счастью, а по инварианту: ни одно имя ни в одном из
 * восьми наборов не кончается на `_flang` или на `Flang`,
 * поэтому обойдённое имя занятым быть уже не может. Инвариант этот не принят на
 * веру — его проверяет `обходСнимаетЗанятость` ниже и сторож корпуса: припиши
 * кто-нибудь в набор имя `FlangError`, и проверка назовёт место, а не отдаст
 * молча имя, которое цель по-прежнему занимает.
 *
 * Свободное имя возвращается ТЕМ ЖЕ значением: у программы, чьё имя цель не
 * занимала, вывод не меняется ни на байт. Это не украшение — это условие
 * побайтовой сверки печати на всём корпусе.
 *
 * Эталоны на самом языке (`flang/self/emit-*.flang`, «Обойти занятое целью …»)
 * делают ровно этот один шаг: две реализации, одна форма.
 *
 * @param {string} имя
 * @param {"c"|"go"|"rust"|"python"|"java"|"csharp"|"elixir"|"js"} цель
 * @returns {string}
 */
export function обойтиЗанятоеЦелью(имя, цель) {
  ...
  if (!занятоЦелью(имя, набор)) return имя
  return `${имя}${набор.суффикс}`
}

/**
 * Инвариант «один шаг снимает занятость» — проверяемый, а не обещанный.
 *
 * Считает по каждой цели, сколько занятых имён после обхода перестали быть
 * занятыми. Ответ обязан быть «все»: иначе один шаг оставил бы имя занятым, и
 * печать снова отдала бы цели её собственное имя — молча.
 *
 * @returns {{ цель: string, занятых: number, снято: number }[]}
 */
export function обходСнимаетЗанятость() {
  return Object.entries(ЗАНЯТО_ЦЕЛЬЮ).map(([цель, набор]) => {
    /* У хвостов проверяется имя С хвостом: занят не сегмент сам по себе, а имя,
       которым он кончается («setka_windows», а не «windows»). */
    const занятые = [...набор.имена, ...набор.хвосты.map((хвост) => `setka_${хвост}`)]
    const снято = занятые.filter((имя) => !занятоЦелью(обойтиЗанятоеЦелью(имя, цель), набор)).length
    return { цель, занятых: занятые.length, снято }
  })
}
