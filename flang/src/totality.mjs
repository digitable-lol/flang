 ...
 *
 * ── Когда сторож НЕ НУЖЕН: уточнение числа ─────────────────────────────────
 *
 * Абзац выше говорит: «Анализом это не закрыть: понадобилась бы верхняя граница
 * на параметр, а её в `если н не больше 0` нет и взяться ей неоткуда». Это было
 * верно, пока у языка не появилось слово, которым верхнюю границу МОЖНО
 * НАПИСАТЬ. Теперь оно есть: `принимает н: нат` — целое в [0, 2⁵³−1], `целое` —
 * целое в [−(2⁵³−1), 2⁵³−1] (`range.mjs`).
 *
 * Внутри этой области постоянный целый шаг меняет число НАВЕРНОЕ: соседние
 * целые там различимы (мерено: первое x, у которого `x − 1` равен x, — это 2⁵⁴).
 * Значит цепочка v₀ > v₁ > … с целым шагом ≥ 1 и дном K обрывается не позже чем
 * через (v₀ − K) витков, и обрывается она НА ЧИСЛАХ, а не «на вещественных
 * числах, если повезёт с представлением». Разрыва между доказательством и
 * исполнением, ради которого ставился сторож, здесь просто нет.
 *
 * Отсюда пятый носитель обещания: `exact` — «точным шагом». Условий у него
 * четыре, и каждое обязательно:
 *
 *   1. позиция убывает мерой на каждом ребре компоненты — то же, что и раньше;
 *   2. параметр на этой позиции объявлен `нат` или `целое` У КАЖДОЙ функции
 *      компоненты. Не у одной: цепочка идёт по всем, и звено, объявленное
 *      `числом`, вернуло бы значение за границу мантиссы;
 *   3. шаг ЦЕЛЫЙ. `н минус 0.5` внутри области точности убывает, но за 2·v₀
 *      витков вместо v₀ — и, что важнее, само значение перестаёт быть целым,
 *      то есть перестаёт быть `натом`, и типизатор такой вызов не пропустит.
 *      Условие здесь стоит всё равно: анализ завершаемости не вправе полагаться
 *      на то, что типы кто-то проверил;
 *   4. шаг-параметр (`н минус ш`) тоже объявлен точным — иначе про целость
 *      шага не известно ничего.
 *
 * ЧТО ЧИТАЕТ АНАЛИЗ И ЧЕГО ОН НЕ ЧИТАЕТ. Читается одно поле одного узла AST:
 * `params[j].type.refine`. Это не «анализ завершаемости стал читать типы» —
 * таблицы сигнатур здесь по-прежнему нет, псевдонимы не разворачиваются, и
 * `тип «Счётчик» это нат` сюда НЕ доходит: у него остаётся сторож, то есть
 * ровно то, что было. Консерватизм тут не оговорка, а условие: не доказали —
 * значит платим сторожем, и хуже прежнего не становится никогда.
 *
 * ЧТО ДЕРЖИТ ОБЕЩАНИЕ ТИПА. Что значение `нат` действительно лежит в диапазоне
 * — отвечает типизатор, и отвечает без единой проверки в рантайме: сумма двух
 * `нат` есть `число`, а не «`нат` со сторожем», и `нат` из `числа` не получается
 * молча ни в одном месте. Граница входа (значение примера, `flang run --args`)
 * проверяется ОДИН РАЗ НА ЗНАЧЕНИЕ. Отсюда и весь смысл этапа: сторожей стало
 * меньше, а проверок не прибавилось ни одной.
 *
...
/* Уточнение числа (`нат`, `целое`) читается ОДНИМ полем объявленного типа —
   `refine` на узле параметра, — и предикат для этого живёт в `range.mjs`.
   Импорт односторонний: `range.mjs` не импортирует ничего вовсе, поэтому
   круга, который отверг бы связывание (`FLANG_IMPORT_CYCLE`), тут нет и
   появиться ему не из чего. */
import { точныйТип } from "./range.mjs"
 ...
 *   guards: object[], descents: object[], structures: object[], exact: object[],
 *   cycles: string[][] }}
 ...
 *   `exact` — рёбра, доказанные ТОЧНЫМ шагом: параметр объявлен `нат` или
 *   `целое`, шаг целый, и потому сторожа нет. Отдельным списком от `guards`
 *   именно поэтому: `guards` читает печать и ставит сторожа, а здесь стеречь
 *   нечего — список нужен ведомости, чтобы назвать носителя обещания и
 *   показать, что сторож СНЯТ, а не потерян.
...
export function checkTotality(program) {
  ...
  for (const name of totalNames) {
    ...
    const state = { from: name, params, exact: exactParams(fn), calls, tags, functions }
  ...
  for (const call of calls) {
    ...
    /* Точность параметров едет НА РЕБРЕ обеими сторонами: у вызывающей она
        собрана вместе с их именами, у вызываемой — здесь, рядом с её именами и
        по той же причине. Достать её из таблицы функций было бы можно, но копия
        анализа на самом языке читает список только с головы, и счёт позиций
        руками разошёлся бы с этим на первом же несовпадении арности. */
    edges.push({ ...call, calleeParams: paramNames(callee), calleeExact: exactParams(callee) })
  }
  ...
  for (const component of stronglyConnectedComponents([...totalNames], edges)) {
    checkComponent(component, edges, failed, report, guards, descents, functions, structures, exact)
  ...
  return {
    ...
    /* Рёбра, доказанные точным шагом: сторожа у них нет и печать о них не
       узнаёт ничего — как и о структурном убывании. Наружу они выходят по той
       же причине, по какой выходят `structures`: «доказано точным шагом» без
       имени убывающего аргумента нельзя перепроверить чтением программы. */
...
function checkComponent(component, edges, failed, report, guards, descents, functions, structures = [], exact = []) {
  const members = new Set(component)
  ...
  if (мерой.length > 0) {
    /* Точный шаг пробуется ПЕРВЫМ среди мер, и по тому же правилу, по которому
       структура пробуется раньше меры: дешёвое доказательство побеждает.
    ...
    if (точные.length > 0) {
      ...
      for (const { edge } of decreasing) {
        exact.push({
          ...
          from: edge.from,
          to: edge.to,
          param: edge.params[точная] ?? null,
          declared: объявленноеУточнение(functions.get(edge.from), точная),
        })
      }
      return
    }
...
/** Слово уточнения из объявления — ведомости, чтобы назвать причину. */
function объявленноеУточнение(fn, position) {
  const params = Array.isArray(fn?.params) ? fn.params : []
  const refine = params[position]?.type?.refine
