...
/**
 * Формы, чья частичность СНИМАЕТСЯ непустотой.
 *
 * Их четыре из восьми частичных (`failures.mjs`, `ЧАСТИЧНЫЕ`), и это не
 * произвол выбора: у этих четырёх условие отказа — «длина ноль», одно и то же
 * с точностью до того, чья длина. У остальных четырёх оно другое:
 * `элемент`, `символ` и `подстрока` требуют ДВУСТОРОННЕЙ границы номера, а
 * `к числу` частична неустранимо — никакой тип строки не обещает, что она
 * записывает число.
 */
export const СНИМАЮТСЯ_НЕПУСТОТОЙ = Object.freeze(["голова", "хвост", "код символа", "разделить"])
