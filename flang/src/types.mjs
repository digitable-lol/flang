...
function checkMonoids(program, ctx) {
  ...
  for (const узел of моноиды) {
    ...
    /* Носитель приводится к тому же виду, в каком лежат типы параметров функций.
       ДО ЭТОЙ ПРАВКИ ЭТОГО ШАГА НЕ БЫЛО, и цена была не теоретическая: носитель
       `число` проходил (скаляр совпадает с собой сам), а `моноид` на ИМЕНОВАННОМ
       типе — `носитель «Частичный итог»` — не собирался никогда, потому что
       `sameType` сравнивал сырой узел разбора с нормализованным типом параметра
       и не сходился ни разу. В корпусе этого не было видно: моноидов на
       именованном типе там нет ни одного — ровно потому, что объявить их было
       нельзя. А `flang/cat/SPEC.md` обещает моноид «Вектору», «Словарю» и
       «Очереди», то есть именно именованным типам. Правка снимает запрет и не
       снимает ни одной проверки: имя, которого нет, `normalizeType` называет
       сам. */
