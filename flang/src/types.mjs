...
function checkMonoids(program, ctx) {
  ...
  for (const узел of моноиды) {
    ...
    /* НОСИТЕЛЬ НОРМАЛИЗУЕТСЯ, А НЕ БЕРЁТСЯ СЫРЫМ ИЗ AST, и до этой строки не
       ...
       неизвестный тип».
       ПОЧЕМУ ЭТО НЕ ЗАМЕТИЛИ РАНЬШЕ. `число`, `строка` и `признак` — КЛЮЧЕВЫЕ
       слова, парсер отдаёт их готовым узлом (`{kind: "number"}`), и `sameType`
       сличает его с сигнатурой напрямую. Имена уточнений ключевыми словами не
       являются НАМЕРЕННО (см. шапку `SCALAR_ALIASES`): они приезжают тем же
       узлом `{kind: "named"}`, каким приезжает всякий объявленный тип, и без
       `normalizeType` остаются именем, а не типом. Пока уточнений не было, все
       носители были ключевыми словами, и разницы не существовало.
       Отсюда цена молчания: ни один моноид над `нат`, `целое` или `вес` не
       собирался вовсе — и узнать об этом было неоткуда, потому что программ с
       таким носителем в корпусе не было. Чинится там же, где чинится всё
       прочее с именами типов. */
