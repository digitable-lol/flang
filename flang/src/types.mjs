 ...
 * названа числом: из 35 мест, где стоял `sameType`, на `годится` переведены 14
 * — ровно те, где значение ВТЕКАЕТ в объявленную позицию (аргумент, тело против
 * `возвращает`, элемент списка, накопитель свёртки, поле варианта). Остальной 21
 * `sameType` остался равенством и обязан им остаться: там сверяются НОСИТЕЛИ
 ...
 * (он симметричен, и это было незаметно), поэтому каждое из 14 мест выверено
 * поимённо, а не заменой по образцу.
 */
