...
function checkMonoids(program, ctx) {
  ...
  for (const узел of моноиды) {
    ...
    /* НОСИТЕЛЬ НОРМАЛИЗУЕТСЯ, А НЕ БЕРЁТСЯ СЫРЫМ ИЗ AST, и до этой строки не
       ...
       (эта ветка, `work/zakony`) и ни один над уточнением — `нат`, `целое`,
       `вес` (`work/beskonechnost`, где та же строка правится тем же
       `normalizeType`). Первая половина стоила обещания `flang/cat/SPEC.md`:
       моноид обещан «Вектору», «Словарю» и «Очереди», то есть именно
       именованным типам. Правка снимает запрет и не снимает ни одной проверки:
       имя, которого нет, `normalizeType` называет сам. */
