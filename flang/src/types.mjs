...
function checkMonoids(program, ctx) {
  ...
  for (const узел of моноиды) {
    ...
    /* НОСИТЕЛЬ НОРМАЛИЗУЕТСЯ, А НЕ БЕРЁТСЯ СЫРЫМ ИЗ AST, и до этой строки не
       нормализовался — отчего `носитель нат` (и всякое другое имя уточнения)
       давал «носитель объявлен как неизвестный тип» на совершенно исправной
       программе. Улика мерена, а не вспомнена: то же самое печаталось на
       моноиде из четырёх строк над двумя `нат`.

       Причина была в том, что `число`, `строка` и `признак` — КЛЮЧЕВЫЕ слова, и
       парсер отдаёт их готовым узлом `{kind: "number"}`, который `sameType`
       понимает без всякой нормализации. Имена уточнений ключевыми словами не
       являются НАМЕРЕННО (лексер и близнец на flang не меняются ни на байт), и
       приезжают они узлом `{kind: "named"}` — тем же, каким приезжает
       «Возможно». Сравнивать его с нормализованным типом параметра значило бы
       сравнивать написанное с разобранным.

       Чинится там же, где чинится всё прочее: тем же `normalizeType`, каким
       нормализуются типы параметров и возврата. Второго словаря не заводится. */
...
const GENITIVE_ЧИСЛА = new Map([
  ["число", "числа"], ["нат", "нат"], ["целое", "целого"],
