...
/**
 * Диапазон значений, пришедших В программу извне, против объявленных типов.
 *
 * ── Зачем это отдельная функция, а не часть проверки типов ─────────────────
 *
 * Уточнение числа обещает диапазон, и обещание держится тем, что каждое место,
 * где значение ВХОДИТ в программу, диапазон проверяет. Мест таких два, и оба —
 * граница, а не виток цикла:
 *
 *   значение примера        — проверяет `checkValue` вместе с типом;
 *   `flang run --args`      — проверяет вот это, потому что типов `run` не
 *                             проверял никогда и проверять их здесь значило бы
 *                             менять поведение команды заодно.
 *
 * Отсюда узость: проверяется РОВНО диапазон и ничего больше. Аргумент не того
 * вида (строка вместо числа) отсюда уходит молча — как уходил и раньше, и
 * говорить о нём начнёт тот же `run`, когда типы у него появятся. Прибавить
 * проверку, не отняв ни одной, важнее полноты: `нат` без границы входа обещал
 * бы то, чего никто не держит.
 *
 * ОДНА ПРОВЕРКА НА ЗНАЧЕНИЕ, А НЕ НА ВИТОК. Это и есть весь размен: сторож меры
 * стоял внутри цикла и срабатывал на каждом шаге, а здесь значение проверяется
 * один раз, когда приходит. Ради этого уточнение и заводилось.
 *
 * @param {object} program AST модуля
 * @param {string} name имя вызываемой функции
 * @param {object} args аргументы по именам параметров
 * @returns {object[]} диагностики в формате ядра FTS; пустой массив — всё в диапазоне
 */
export function checkEntryRanges(program, name, args) {
  const signature = checkTypes(program).types.get(name)
  if (!signature || args === null || typeof args !== "object") return []
  const беды = []
  for (const param of signature.params) {
    if (!Object.hasOwn(args, param.name)) continue
    проверитьДиапазон(args[param.name], param.type, `аргумент «${param.name}»`, беды)
  }
  return беды
}

/** Диапазон значения и его частей. Список проверяется поэлементно: `список нат`
 *  обещает диапазон о каждом элементе, а не о самом списке. */
function проверитьДиапазон(value, type, label, беды) {
  if (!type) return
  if (type.kind === "list") {
    if (!Array.isArray(value)) return
    value.forEach((item, index) => проверитьДиапазон(item, type.of, `${label}[${index}]`, беды))
    return
  }
  const интервал = интервалТипа(type)
  if (интервал === null) return
  if (type.optional === true && (value === null || value === undefined)) return
  if (typeof value === "number" && вместимо(интервалЛитерала(value), интервал)) return
  беды.push({
    code: "FLANG_TYPE",
    message: `${label}: ${JSON.stringify(value ?? null)} не соответствует типу ${typeName(type)} — `
      + `нужно целое из [${интервал.min}, ${интервал.max}]. Проверка стоит на границе программы `
      + "и делается один раз на значение: дальше диапазон уже доказан",
    severity: "error",
    span: null,
  })
}
...
function подсказкаУточнения(actual, expected) {
  ...
  /* Подсказка договаривает до КОПИРУЕМОЙ формы, а не останавливается на «мало».
     Замер того, как модель пишет на языке, показал: 15 % всех падений — этот
     самый класс, «нижняя граница числа не доказана», и приходит он вместе с
     `FLANG_NOT_TOTAL` про ту же недостачу. Чинится он одной строкой в
     ИСХОДНИКЕ, и строку эту надо назвать: подсказка, из которой не видно, что
     писать, читается как «не получилось» — и на неё отвечают тем же кодом. */
  if (есть.min < ждём.min) {
    return `: значение может быть ${есть.min}, а нужно не меньше ${ждём.min}. `
      + `Границу снизу даёт проверка-неравенство: после «если … не больше ${ждём.min}» `
      + `в ветви «иначе» значение уже больше ${ждём.min}, и вычитание единицы из диапазона не выходит`
  }
  if (есть.max > ждём.max) {
    return `: значение может дойти до ${есть.max}, а нужно не больше ${ждём.max}. `
      + "Границу сверху даёт та же проверка с другим знаком — или объявление `число` там, где границы нет"
  }
  return ""
}
