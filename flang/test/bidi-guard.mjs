 ...
 *
 * Правило одно — «ни один бэкенд не печатает двунаправленные управляющие
 * сырыми», — и способ его проверить тоже один: flang/test/emit-bidi.test.mjs
 * перебором по всем восьми целям печати.
 *
 * Помощник писался общим на два компилятора репозитория: тем же набором
 * пользовался прогон старого проекта FTS. Проект вынесен (тег
 * `fts-pered-udaleniem`), помощник переехал сюда вместе с языком, и
 * потребитель у него теперь один. Отдельным файлом он остаётся по прежней
 * причине: набор кодовых точек обязан жить не там, где реализация.
 *
 * Набор кодовых точек здесь СВОЙ, а не импортированный из flang/src/bidi.mjs, и это
