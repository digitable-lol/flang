...
/*
 * Форма зовётся так, как её видит ЧИТАТЕЛЬ ЗНАЧЕНИЙ, а не машина: результат
 * пропускается через `материализовать`. Внутри вычислителя «добавить» отдаёт
 * список с запасом (вид поверх общего массива — `builtins.mjs`), и наружу этот
 * вид не выходит: границу держит `interpret.mjs`. Скаляры, строки и списки,
 * построенные не «добавить», проходят через `материализовать` тем же объектом.
 */
const вызов = (имя, ...аргументы) => материализовать(callBuiltin(имя, аргументы))
...
test("символ по коду даёт строку ровно из одного символа", () => {
  assert.equal(вызов("символ по коду", 65), "A")
  assert.equal(вызов("символ по коду", 1071), "Я")
  // Кодовая точка за основной плоскостью — ОДИН символ строки, хотя в UTF-16
  // это два кода: длина в flang считается кодовыми точками, и обе стороны моста
  // обязаны считать одинаково.
  assert.equal(вызов("символ по коду", 128512), "😀")
  assert.equal(Array.from(вызов("символ по коду", 128512)).length, 1)
})

test("символ по коду и код символа — взаимно обратны на скалярных значениях", () => {
  /* Круг замыкается на обеих сторонах, и проверяется он не парой примеров, а
     проходом по краям: границы плоскостей и края суррогатной дырки — там, где
     реализации обычно и расходятся. */
  const точки = [0, 1, 65, 1071, 0xd7fe, 0xd7ff, 0xe000, 0xe001, 0xfffd, 0xffff, 0x10000, 128512, 0x10fffe, 0x10ffff]
  for (const код of точки) {
    assert.equal(вызов("код символа", вызов("символ по коду", код)), код, `круг разошёлся на ${код}`)
  }
  for (const текст of ["a", "Я", "😀", "é", "abc", "🙂🙃"]) {
    assert.equal(вызов("символ по коду", вызов("код символа", текст)), вызов("символ", 1, текст))
  }
})

test("символ по коду отвергает всё, что не кодовая точка", () => {
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("символ по коду", -1))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("символ по коду", 0x110000))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("символ по коду", 65.5))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("символ по коду", Number.NaN))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("символ по коду", Number.POSITIVE_INFINITY))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("символ по коду", "65"))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("символ по коду"))
})

test("символ по коду отвергает суррогат — все 2048 половин пары", () => {
  /* Дырка проверяется целиком, а не краями: половина пары не является символом
     ни в одной цели печати, и пропусти форма хоть одну — три цели с UTF-16
     внутри дали бы значение, а четыре с UTF-8 не дали бы вовсе. */
  let отказов = 0
  for (let код = 0xd800; код <= 0xdfff; код += 1) {
    ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("символ по коду", код))
    отказов += 1
  }
  assert.equal(отказов, 2048)
  // Соседи дырки при этом целы с обеих сторон.
  assert.equal(вызов("код символа", вызов("символ по коду", 0xd7ff)), 0xd7ff)
  assert.equal(вызов("код символа", вызов("символ по коду", 0xe000)), 0xe000)
})

test("символ по коду на минус нуле даёт тот же символ, что на нуле", () => {
  /* Правило класса «минус ноль»: всякая форма, читающая число, считается на
     `-0` до попадания в дерево. Здесь он безопасен — целый, не меньше нуля, и
     кодовая точка у него та же. */
  assert.equal(вызов("символ по коду", -0), вызов("символ по коду", 0))
  assert.equal(вызов("код символа", вызов("символ по коду", -0)), 0)
})
...
test("все встроенные формы из спецификации на месте", () => {
  const обязательные = [
    ...
    "добавить", "приписать", "остаток от", "процентов от", "символ по коду",
  ]
