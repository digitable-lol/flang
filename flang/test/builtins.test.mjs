...
test("код символа даёт кодовую точку, а не единицу UTF-16", () => {
  assert.equal(вызов("код символа", "A"), 65)
  assert.equal(вызов("код символа", "Я"), 1071)
  // Эмодзи — одна точка (U+1F600), а не пара суррогатов 0xD83D/0xDE00.
  assert.equal(вызов("код символа", "😀"), 128512)
  // Берётся первый символ: то же, что «символ 1 в …», а не «ровно один символ».
  assert.equal(вызов("код символа", "abc"), 97)
  assert.equal(вызов("код символа", "😀!"), 128512)
})

test("код символа и символ 1 в … согласованы на любом входе", () => {
  for (const текст of ["a", "Я", "😀", "é", "abc", "🙂🙃"]) {
    assert.equal(вызов("код символа", текст), вызов("код символа", вызов("символ", 1, текст)))
  }
})

test("код символа отвергает пустую строку и не строку", () => {
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("код символа", ""))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("код символа", 5))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("код символа", ["a"]))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("код символа"))
})
...
test("элемент индексируется от единицы, как символ, и отказывает за границей", () => {
  assert.equal(вызов("элемент", 1, [10, 20, 30]), 10)
  assert.equal(вызов("элемент", 3, [10, 20, 30]), 30)
  assert.deepEqual(вызов("элемент", 2, [[1], [2, 3]]), [2, 3])
  /* Текст отказа — часть контракта: он сверяется с восемью целями печати
     дословно, поэтому проверяется целиком, а не «начинается с». */
  assert.equal(
    ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("элемент", 4, [10, 20, 30])).message,
    "«элемент»: индекс 4 вне списка длиной 3",
  )
  assert.equal(
    ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("элемент", 1, [])).message,
    "«элемент»: индекс 1 вне списка длиной 0",
  )
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("элемент", 0, [10]))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("элемент", 1.5, [10, 20]))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("элемент", 1, "строка"))
  ожидаемКод("FLANG_BUILTIN_ARGS", () => вызов("элемент", "а", [10]))
})

test("элемент уважает нулевую базу индексов — той же опцией, что и символ", () => {
  assert.equal(callBuiltin("элемент", [0, [10, 20]], undefined, { indexBase: 0 }), 10)
  assert.equal(callBuiltin("элемент", [1, [10, 20]], undefined, { indexBase: 0 }), 20)
  ожидаемКод("FLANG_BUILTIN_ARGS", () => callBuiltin("элемент", [2, [10, 20]], undefined, { indexBase: 0 }))
})
...
test("все встроенные формы из спецификации на месте", () => {
  const обязательные = [
    "длина", "символ", "символы", "код символа", "подстрока", "соединить", "разделить", "содержит",
    "начинается с", "к числу", "к числу или беда", "к строке", "пусто", "голова", "хвост", "элемент",
    "добавить", "приписать", "остаток от", "процентов от",
  ]
