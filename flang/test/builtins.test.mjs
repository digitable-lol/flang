...
test("символ по коду и код символа — взаимно обратны на всём диапазоне", () => {
  /* Круг замыкается на обеих сторонах, и проверяется он не парой примеров, а
     проходом по диапазону: границы плоскостей и сами суррогаты — там, где
     реализации обычно и расходятся. */
  const точки = [0, 1, 65, 1071, 0xd7ff, 0xd800, 0xdb3f, 0xdfff, 0xe000, 0xfffd, 0xffff, 0x10000, 128512, 0x10fffe, 0x10ffff]
