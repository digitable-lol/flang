 ...
 * проходит ничего.
 *
 * ── Что померено этой же работой ────────────────────────────────────────────
 *
 * Корпус дерева ДО правки (798 файлов `.flang`, ствол `440ca5ac`): прогнано
 * 7558 примеров, не сошлось 17 в 13 файлах, и `flang check` не отвергал ни
 * одного из тринадцати. После правки краснеют ровно эти тринадцать — ни одним
 * больше и ни одним меньше. Цена: сумма времени `check` по всем 798 файлам
 * 299,4 с → 381,3 с, медиана роста на файл 3 мс.
 *
