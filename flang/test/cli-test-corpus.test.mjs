...
test("каталог с ложным примером: упавший назван поимённо, код ненулевой", (t) => {
  ...
  assert.match(прогон.stdout, /файлов 2, взято 2, отказано 0, примеров 4, прошло 3, не прошло 1/)
...
test("--json: тот же итог машине, и он разбирается", (t) => {
  ...
  assert.deepEqual(
    [свод.files, свод.taken, свод.refused, свод.total, свод.passed, свод.failed],
    [2, 2, 0, 4, 3, 1],
  )
...
test("маска берёт из каталога один чистый файл, и код нулевой", (t) => {
  ...
  assert.deepEqual([свод.files, свод.total, свод.failed], [1, 2, 0])
