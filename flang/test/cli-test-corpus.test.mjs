...
/*
 * `--ledger`: по строке на файл, и её сверяют диффом.
 *
 * Ради этого ключа он и заведён: `flang/проверки/обход-примеров.sh` держит
 * ведомость на всё дерево и сверяет её целиком, потому что файл, который
 * двоичный перестал брать, отдаёт ноль примеров и ноль отказов — то есть
 * выглядит как чистый. Свод в JSON для этого не годится: он одна строка на весь
 * корпус, и дифф по ней говорит «строка изменилась».
 */
test("--ledger: по строке на файл, со своим числом примеров", (t) => {
  const прогон = двоичным(t, ["test", фикстуры, "--ledger"])
  if (прогон === null) return
  assert.deepEqual(прогон.stdout.trimEnd().split("\n"), [
    `${фикстуры}/chistyy.flang\t2 2 0 2`,
    `${фикстуры}/lozhnyy.flang\t2 1 1 2`,
  ])
  assert.notEqual(прогон.status, 0, "ведомость не отменяет кода возврата")
})
...
test("файл без своих примеров назван, а не засчитан чужими", (t) => {
  ...
  /* И то же самое строкой ведомости: четвёртое число — ноль, и это единственное,
     что отличает её от строки настоящего слоя с девяноста одной своей проверкой. */
  const ведомость = двоичным(t, ["test", "flang/self/carriers.flang", "--ledger"])
  assert.match(ведомость.stdout, /^flang\/self\/carriers\.flang\t\d+ \d+ 0 0$/m)
})
