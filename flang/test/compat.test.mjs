...
/* ─────────────────────── пределы интерпретатора у `run` ─────────────────────
 * `--max-steps` разбирается на общем разборе ключей, поэтому команда `run` его
 * ПРИНИМАЕТ — и молча выбрасывает: `commandRun` звал `evaluateFlang` без
 * четвёртого аргумента. Ключ, который принят и не действует, хуже отвергнутого:
 * отвергнутый виден сразу, а этот обещает работу и не делает её. Заметно это
 * было только на большом входе — «запустить как есть» упиралось в предел по
 * умолчанию (миллион шагов, около 33 000 витков числового цикла), и поднять его
 * у `run` было нечем, хотя тот же ключ действует у `repl` (src/repl.mjs, поле
 * `пределы`) и у `emit` (`emitOptions`).
 *
 * Проверяется ОБА направления, и это не педантизм. Проверка «подняли предел —
 * посчиталось» зеленеет и на выброшенном ключе, если предел по умолчанию и так
 * велик: она не отличает «ключ подействовал» от «он не понадобился». Поэтому
 * рядом стоит проверка на ОПУЩЕННЫЙ предел — она может зеленеть только если
 * число из ключа дошло до вычислителя. */

const пределФайл = join(temporary, "predel.flang")
await writeFile(
  пределФайл,
  `модуль «Предел»

тотальная функция «Считать вниз»
  принимает осталось: число
  возвращает число
  пример «Ноль»
  ...
  если осталось не больше 0
    то 0
    иначе «Считать вниз» от (осталось минус 1)

тотальная функция «Глубоко»
  принимает осталось: число
  возвращает число
  пример «Ноль»
    дано осталось равно 0
  ...
  если осталось не больше 0
    то 0
    иначе (1 плюс («Глубоко» от (осталось минус 1)))
`,
)

test("flang run: --max-steps опускает предел, и он бьёт", async () => {
  /* Одного шага не хватит даже на разбор входа, поэтому это про «ключ дошёл»,
     а не про «сколько именно стоит виток». */
  const result = await cli(["run", пределФайл, "--function", "Считать вниз", "--args", JSON.stringify({ "осталось": 10 }), "--max-steps", "1"])
  assert.equal(result.code, 1, `ожидался отказ по пределу, а вышло: ${result.stdout}${result.stderr}`)
  const output = JSON.parse(result.stderr)
  assert.equal(output.diagnostics[0].code, "FLANG_RECURSION_LIMIT")
  assert.match(output.diagnostics[0].message, /\(1\)/u, "в сообщении обязан стоять предел из ключа, а не из умолчания")
})

test("flang run: --max-steps поднимает предел, и без него тот же вход отказывает", async () => {
  const аргументы = ["run", пределФайл, "--function", "Считать вниз", "--args", JSON.stringify({ "осталось": 60000 })]

  /* Сначала улика, что вход и правда за пределом по умолчанию: без этой
     половины проверка ниже ничего не значила бы. */
  const без = await cli(аргументы)
  assert.equal(без.code, 1, "вход выбран так, чтобы не проходить с пределом по умолчанию")
  assert.equal(JSON.parse(без.stderr).diagnostics[0].code, "FLANG_RECURSION_LIMIT")

  const с = await cli([...аргументы, "--max-steps", "50000000"])
  assert.equal(с.code, 0, `--max-steps не подействовал: ${с.stderr}`)
  assert.equal(JSON.parse(с.stdout).result, 0)
})

test("flang run: --max-depth тоже доходит до вычислителя", async () => {
  /* Тот же четвёртый аргумент несёт два предела, и второй потерялся бы тем же
     способом, поэтому он проверяется отдельно.
     Считает глубину только НЕхвостовой вызов: «Считать вниз» выше рекурсирует
     хвостом, и на нём машина кадров не копит — `--max-depth 1` проходит на любом
     входе, и проверка глубины по нему зеленела бы, ничего не проверяя. Поэтому
     здесь «Глубоко», у которой вызов стоит под «плюс». */
  const глубоко = ["run", пределФайл, "--function", "Глубоко", "--args", JSON.stringify({ "осталось": 10 })]
  const без = await cli(глубоко)
  assert.equal(без.code, 0, `с умолчанием этот вход обязан считаться: ${без.stderr}`)
  assert.equal(JSON.parse(без.stdout).result, 10)

  const result = await cli([...глубоко, "--max-depth", "1"])
  assert.equal(result.code, 1, `ожидался отказ по глубине, а вышло: ${result.stdout}${result.stderr}`)
  const диагностика = JSON.parse(result.stderr).diagnostics[0]
  assert.equal(диагностика.code, "FLANG_RECURSION_LIMIT")
  assert.match(диагностика.message, /предел глубины вызовов \(1\)/u, "предел обязан прийти из ключа")
})
