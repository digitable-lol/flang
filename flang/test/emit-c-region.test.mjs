 ...
 * сортировка слиянием четырёх тысяч чисел берёт 1 655 МиБ вместо 3,5, а
 * сортировка вставками полутора тысяч не досчитывает вовсе.
 *
...
test("разделяемый подграф не разворачивается: обмер с бюджетом обрывает копию", async () => {
   ...
   *
   * Разделения перекладка не сохраняет, и это НЕ починено, а ОГРАНИЧЕНО: копия,
   * если она всё-таки делается, не больше четверти отданного мусора. Больше
   * памяти после области, чем до неё, не станет никогда.
   *
   * И то же самое верно про уже сделанную копию сообщения в конкурентности
   * (`fl_conc_clone` в flang_conc.c, шаг А2): она разворачивает разделяемый
   * подграф ровно так же — но БЕЗ бюджета, то есть без этой границы. Там это не
   * записано и этой правкой не тронуто; здесь оно названо, потому что механизм
   * один и тот же.
   */
  const lines = await probe(`${HEAD}
int main(void) {
