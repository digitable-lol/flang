 ...
 * файл. Свидетель-интерпретатор на той же точке упирается в предел за 926 мс.
 *
...
test("точка сетки, на которой печать зависала: предел шагов срабатывает за секунды", (t) => {
  ...
  /* Текст сверен не с образцом, а со свидетелем: дословно, знак в знак. */
...
test("кодировка прогонщика: ответ побайтово тот же при любой локали хозяина", (t) => {
  ...
  /* И то же самое, но со свидетелем-интерпретатором, а не с образцом. */
  const свидетель = outcome(() => interpret(program, "Слова", ["привет"], {}))
  assert.ok(sameOutcome(свидетель, answerOutcome(ответы[1])))
...
test("сторож частичной формы: все восемь форм отказывают у Elixir кодом и текстом свидетеля", async (t) => {
  if (!toolchain) {
    missingToolchain(t, "elixir", "тулчейн Elixir не найден — пропуск")
    return
  }
  ...
  [...ЧАСТИЧНЫЕ_ВХОДЫ].forEach(([форма, [fn, args]], index) => {
    ...
    assert.equal(ответ.ok, false, `«${форма}»: Elixir не отказал там, где отказал свидетель`)
