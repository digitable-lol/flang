 ...
 * необязательным полем и сумма с полями. Без него сетка проверяла бы только
 * числа и списки, а виды `запись` и `сумма` в таблице остались бы словом.
 *
...
const СЕТКА = [
  ...
  /* Годный вход обязан считаться — иначе зелень означала бы, что дверь просто
     заперта. Пять штук, и каждая про свой вид типа: `нат`, `целое` (у которого
     −2 внутри типа), список числа, запись и вложенная сумма.

     Необязательного поля записи в сетке нет НАРОЧНО: `fieldMap` в types.mjs
     нормализует `field.type`, а пометка `иногда является` живёт на самом поле —
     и `checkValue` эталона требует такое поле как обязательное. Таблица
     повторяет эталон в точности (`ничто: false`), то есть сверка целей от этого
     не страдает; чинить сам эталон — отдельная работа и не про эту дверь. */
