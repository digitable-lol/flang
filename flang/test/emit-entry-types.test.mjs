 ...
 *
 * ── Чем сверяется ──────────────────────────────────────────────────────────
 * Свидетель здесь один и тот же, что у `flang run --args`: `checkArguments` из
 ...
 *
 * Ожидание не выписано в тесте руками, а взято у свидетеля: выписанное руками
 * разошлось бы со свидетелем при первой же правке текста. Поэтому отдельно
 * проверяется, что свидетель на каждой лжи действительно ОТКАЗЫВАЕТ, — иначе
 ...
 * параметров (для свидетеля) и они же по порядку (для прогонщика).
 *
 * `ложь` говорит, чего мы ждём: `true` — свидетель обязан отказать, и напечатанная
...
const СЕТКА = [
  ...
  /* Годный вход обязан считаться — иначе зелень означала бы, что дверь просто
     ...
     единственная форма, где свидетель и таблица могли бы разойтись молча. Таблица
     помечает такой тип `ничто: true` и пропускает отсутствие; свидетель
...
/** Ожидание свидетеля: отказ (код и текст) либо «посчиталось». */
function свидетель(точка) {
  const итог = checkArguments(программа, точка.имя, точка.args)
  if (точка.ложь) {
    assert.equal(итог.ok, false, `${точка.имя}: свидетель обязан отвергнуть ${JSON.stringify(точка.args)}`)
    return { ok: false, code: итог.diagnostics[0].code, message: итог.diagnostics[0].message }
  }
  assert.equal(итог.ok, true, `${точка.имя}: свидетель обязан принять ${JSON.stringify(точка.args)}`)
  return { ok: true }
}
