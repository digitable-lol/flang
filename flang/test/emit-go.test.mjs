 ...
 * голого разбора теряет 456 605 байт Go, и слово `FLANG_MEASURE` встречается в
...
test("хвост списка — срез, а не копия: длинный список обходится линейно", async (t) => {
  ...
  assert.match(built.runtime, /return List\(value\.List\[1:\]\)/u, "срез обязан быть сдвигом начала, а не копией")
...
test("стоимость взятия по номеру: массив, значит удвоение n удваивает время", (t) => {
  ...
  t.diagnostic(`взятие по номеру (Go): 20 000 за ${времена[0]} мс, 40 000 за ${времена[1]} мс`)
})
