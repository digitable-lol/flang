      ...
      if (point.runaway) {
        ...
        assert.ok(
          ...
          `${file} / «${point.name}» на входе ${JSON.stringify(point.args) ?? "?"}: свидетель упёрся ` +
            `в предел шагов, а при ${ПРЕДЕЛ_УБЕГАЮЩЕЙ} своих шагов собранный Go дал ${describeOutcome(byEmitted)}`,
        )
        return
      }
      assert.ok(
        ...
        `${file} / «${point.name}» на входе ${JSON.stringify(point.args) ?? "?"}: ` +
          `интерпретатор дал ${describeOutcome(point.byInterpreter)}, собранный Go дал ${describeOutcome(byEmitted)}`,
      )
      points += 1
    })
  }
...
/*
 * Имя этого теста скромнее, чем у эталонов в emit-c и emit-rust, и это не
...
/* ═════════ 7а. цена «добавить»: предел шагов обязан быть СРОКОМ ═════════ */
 ...
 * и за 60 с, и вместе с нею НЕ ЗАВЕРШАЛСЯ весь этот файл — прогон снимался по
 * сроку 420 с. Свидетель-интерпретатор на той же точке упирается в предел за
 * 987 мс, напечатанный C — за 1,3 с.
 *
 ...
 * правильность (продление на месте не имеет права портить исходный список).
...
test("накопление списка линейно: 200 000 «добавить» — это доли секунды, а не минуты", (t) => {
  ...
  /* Порог с запасом в два порядка, а не «на глаз». Между линией и квадратом
     здесь разница классов сложности, а не нагрузки машины. */
  assert.ok(
    времена[2] < 30_000,
...
test("«добавить» за постоянное время не портит исходный список: ветвление и хвост", (t) => {
  ...
  /* Ветвление — то самое место, где приём «занять ячейку за концом» ломается,
     ...
     «основа» обязана остаться прежней. «Хвост» здесь не для полноты: он отдаёт
     срез ЧУЖОГО массива, и продлить его на месте нельзя. */
  const program = parse([
...
/* ═════════════ 10б. сторож меры: отметка обязана ПОЯВИТЬСЯ ═════════════════ */
...
function упоминанийМеры(program) {
  const весь = emitGo(program).files.map((file) => file.content).join("")
  return (весь.match(/FLANG_MEASURE/gu) ?? []).length
}

test("сторож меры ПОЯВЛЯЕТСЯ в напечатанном Go, а не теряется обеими сторонами", () => {
  /* Главная сверка двусторонняя, и этим слепа: снятая отметка теряется ОБЕИМИ
     ...
     программе он досчитает ровно то, что досчитает непомеченный Go, — сверка
...
test("сторож меры: отказ у собранного Go дословно тот же, что у интерпретатора", async (t) => {
  if (!goBin) {
    missingToolchain(t, "go", "тулчейн Go не найден — пропуск")
    return
  }
  ...
  const built = build(program)
  assert.match(built.source, /FLANG_MEASURE/u, "сторож не доехал до напечатанного Go")
  ...
  /* Отказ обязан ПРИЙТИ С МАШИНЫ, а не только совпасть: на 2⁵⁴+4 шаг ничего не
     меняет, и собранный Go обязан отказать шестым видом. */
  const [ответ] = ask(built, [{ fn: "До нуля", args: [encode(18014398509481988)] }])
  assert.equal(ответ.ok, false, "на входе, где мера не убывает, собранный Go обязан отказать")
...
/* ═════════ 10в. сторож ОБЪЯВЛЕННОЙ меры: 98 мест корпуса из ста ════════════ */
...
test("сторож объявленной меры ПОЯВЛЯЕТСЯ в напечатанном Go всеми тремя условиями", () => {
  /* Довод тот же, что у сторожа постоянного шага, и здесь он весомее: условий
     ...
     Убери проверку целости — интерпретатор и собранный Go согласно закрутят
  ...
  const весь = emitGo(markMeasureGuards(parse(descentSource))).files.map((file) => file.content).join("")
  for (const условие of [/не убыла/u, /ушла ниже нуля/u, /перестала быть целой/u]) {
    assert.match(весь, условие, `условие сторожа не доехало до напечатанного Go: ${условие}`)
  }
})

test("сторож объявленной меры: отказ у собранного Go дословно тот же, что у интерпретатора", async (t) => {
  if (!goBin) {
    missingToolchain(t, "go", "тулчейн Go не найден — пропуск")
    return
  }
  ...
  const built = build(program)
  assert.match(built.source, /FLANG_MEASURE/u, "сторож объявленной меры не доехал до напечатанного Go")
  ...
  for (const [ответ, условие] of [[дробная, /перестала быть целой/u], [ниже, /ушла ниже нуля/u],
    [неУбыла, /не убыла/u]]) {
    assert.equal(ответ.ok, false, `на входе без спуска собранный Go обязан отказать: ${условие}`)
    assert.equal(ответ.code, "FLANG_MEASURE", `с машины пришёл ${ответ.code}, а не шестой вид отказа`)
...
/* ═════════ 10г. сторож частичной формы: 278 мест корпуса ═══════════════════ */
...
test("сторож частичной формы: все восемь форм отказывают у Go кодом и текстом свидетеля", async (t) => {
  if (!goBin) {
    missingToolchain(t, "go", "тулчейн Go не найден — пропуск")
    return
  }
  ...
  [...ЧАСТИЧНЫЕ_ВХОДЫ].forEach(([форма, [fn, args]], index) => {
    ...
    assert.equal(ответ.ok, false, `«${форма}»: Go не отказал там, где отказал свидетель`)
