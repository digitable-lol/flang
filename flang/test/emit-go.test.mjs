 ...
 * сроку 420 с. Эталон-интерпретатор на той же точке упирается в предел за
...
test("сторож частичной формы: все восемь форм отказывают у Go кодом и текстом эталона", async (t) => {
  if (!goBin) {
    missingToolchain(t, "go", "тулчейн Go не найден — пропуск")
    return
  }
  ...
  [...ЧАСТИЧНЫЕ_ВХОДЫ].forEach(([форма, [fn, args]], index) => {
    ...
    assert.equal(ответ.ok, false, `«${форма}»: Go не отказал там, где отказал эталон`)
