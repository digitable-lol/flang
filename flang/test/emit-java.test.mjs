 ...
 * предел 63,7 с. Свидетель-интерпретатор на той же точке отвечает за 950 мс,
...
test("сторож частичной формы: все восемь форм отказывают у Java кодом и текстом свидетеля", async (t) => {
  if (!toolchain) {
    missingToolchain(t, "java", "JDK не найден — пропуск")
    return
  }
  ...
  [...ЧАСТИЧНЫЕ_ВХОДЫ].forEach(([форма, [fn, args]], index) => {
    ...
    assert.equal(ответ.ok, false, `«${форма}»: Java не отказал там, где отказал свидетель`)
