...
/* Отказ свидетеля на том же входе. С него списывается ТЕКСТ, а не только код.
...
const свидетельскийОтказ = (() => {
  try {
    interpret(толстая, "Спуск", [ПРЕДЕЛ * 4], { maxDepth: ПРЕДЕЛ, maxSteps: 1_000_000_000 })
  } catch (беда) {
    return { code: беда.code, message: беда.message }
  }
  return assert.fail("свидетель обязан упереться в предел глубины на этом входе")
})()

test("прогонщик доводит до ОБЪЯВЛЕННОГО предела глубины и отказывает текстом свидетеля", async (t) => {
  assert.equal(свидетельскийОтказ.code, "FLANG_RECURSION_LIMIT")
  assert.equal(
    свидетельскийОтказ.message,
  ...
  assert.equal(ответ.code, свидетельскийОтказ.code)
  assert.equal(ответ.message, свидетельскийОтказ.message, "текст отказа обязан совпасть со свидетелем дословно")
  ...
  t.diagnostic(`при ${СВЯЗЫВАНИЙ} связываниях прогонщик дошёл до ${ПРЕДЕЛ + 1} и отказал текстом свидетеля`)
})
...
test("изъятие: тот же вход прямым вызовом до объявленного предела НЕ доходит", async (t) => {
  ...
  assert.notEqual(свой.message, свидетельскийОтказ.message, "прямой вызов не имеет права дать текст свидетеля")
...
/** Отказ свидетеля на том же вызове — код и текст, без выдумывания их руками. */
