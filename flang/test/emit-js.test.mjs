...
/* ═════════════════ 12. корпус: stdlib и leetcode целиком ═══════════════════ */
 ...
 *
 * У программы без сумм типов конструктора нет ни одного, и чужой вариант на её
 * границе непредставим: интерпретатору он приезжает вариантом, а модулю — тем,
 * что модуль зовёт записью, и расходятся не движки, а два разных входа. Такие
 * точки в прогон не идут, но и не прячутся: их 739 из 8799, счёт им ведётся,
 * назван в отчёте теста и ограничен сверху.
 *
 ...
 * Предел шагов там ИСПРАВЕН у обеих сторон, но шаг напечатанного кода много
 * крупнее шага интерпретатора: интерпретатор отказывает `FLANG_RECURSION_LIMIT`
 * за полторы секунды, а напечатанный JS набирает свои 5 000 000 шагов не
 * минутами — на этой точке он не досчитал и за 25 минут. То же у Go, Rust,
 * Python и Elixir; Java и C# ту же программу проходят. К уравниванию по мере
 * это отношения не имеет: прогон с ГОЛЫМ `parse` встаёт ровно там же.
 *
 ...
 * разошедшийся ответ. Исключены ТОЧКИ, а не программа: остальные 66 точек
 * `022-generate-parentheses` сверяются как у всех.
 */
const ДОЛГИЕ_ТОЧКИ = [
