 ...
 * Предел шагов там ИСПРАВЕН у обеих сторон — при малом пределе напечатанный JS
 * отказывает `FLANG_RECURSION_LIMIT` как положено, — но шаг у него ДОРОЖАЕТ по
 * ходу, потому что накопитель копируется на каждом шаге. Измерено на этой самой
 * точке: 10 000 шагов — 7 мс, 100 000 — 73 мс, 200 000 — 729 мс, 400 000 —
 * 4257 мс; удвоение шагов даёт ушестерение времени, и до 5 000 000 отсюда часы.
 * Интерпретатор на той же точке отказывает за 1378 мс. То же встаёт у Go, Rust,
