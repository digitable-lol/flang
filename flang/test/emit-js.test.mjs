...
test("сторож частичной формы: все восемь форм отказывают у JavaScript кодом и текстом эталона", async (t) => {
  ...
  for (const [форма, [fn, args]] of ЧАСТИЧНЫЕ_ВХОДЫ) {
    ...
    assert.equal(ответ.ok, false, `«${форма}»: JavaScript не отказал там, где отказал эталон`)
    ...
    assert.equal(ответ.message, эталон.message, `«${форма}»: текст от модуля разошёлся с эталоном`)
