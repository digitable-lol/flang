...
/* Прогонщик едет аргументом `-e`, а не файлом рядом с модулем: файл пришлось бы
   ...
   бы разбора чужой командной строки. */
const ДРАЙВЕР = `const модуль = await import(process.env.FLANG_МОДУЛЬ)
const { функция, аргументы, пределы } = JSON.parse(process.env.FLANG_ЗАПРОС)
...
function спросить(path, вызов, срок) {
  ...
  try {
    вывод = execFileSync(process.execPath, ["--input-type=module", "-e", ДРАЙВЕР], {
      ...
      env: {
        ...process.env,
        FLANG_МОДУЛЬ: pathToFileURL(path).href,
        FLANG_ЗАПРОС: JSON.stringify({ пределы: null, ...вызов }),
      },
    })
  } catch (беда) {
