...
import { INPUT_PARAM } from "../src/fts-legacy.mjs"
 ...
 *
 * Раньше она приезжала из модели `.fts` через мост, и сверялась со свидетелем FTS:
 * «код и текст обязаны совпасть с ядром дословно». Свидетель вынесен из
...
test("нарушение постусловия: авторский код и текст едут в JavaScript литералами", async () => {
  const program = violatingProgram
  const { module, content } = await build(program)

  /* Код и текст едут в AST данными — значит и в напечатанном коде они литералы,
     а не знание, зашитое в эмиттер. Знать этот код эмиттеру неоткуда: он
     посторонний. */
  assert.match(content, new RegExp(`"${АВТОРСКИЙ_КОД}"`, "u"))
  ...
  assert.equal(byEmitted.ok, false)
  assert.equal(byEmitted.code, АВТОРСКИЙ_КОД)
  assert.equal(byEmitted.message, АВТОРСКИЙ_ТЕКСТ, "текст постусловия обязан доехать дословно")
...
test("строковые формы: кириллица, суррогатные пары и границы индексов", async (t) => {
  ...
  points += compare(stringProgram, module, "Приписать", [[1, []], [1, [2]], [1, "строка"]])
 ...
 * параметр типа. Мест в корпусе у первого два из ста:
...
function упоминанийОбъявленнойМеры(program) {
  const весь = emitJs(program).files.map((file) => file.content).join("")
  return (весь.match(/FLANG_MEASURE/gu) ?? []).length
}

test("сторож объявленной меры ПОЯВЛЯЕТСЯ в напечатанном JavaScript всеми тремя условиями", () => {
  /* Главная сверка двусторонняя, и этим слепа: снятая отметка теряется ОБЕИМИ
     ...
     напечатанный JavaScript согласно закрутят Евклида на дробной мере, сверка
     останется зелёной, а обещание «тотальная» станет ложным. Поэтому
     проверяется ПОЯВЛЕНИЕ, и по тексту на условие. */
  assert.equal(упоминанийОбъявленнойМеры(parse(descentSource)), 0,
    "у голого разбора сторожа объявленной меры нет вовсе — иначе улика ниже ничего не значит")
  assert.equal(упоминанийОбъявленнойМеры(markMeasureGuards(parse(descentSource))), 3,
    "у сторожа объявленной меры три постусловия — по одному на условие")

  const весь = emitJs(markMeasureGuards(parse(descentSource))).files.map((file) => file.content).join("")
  for (const условие of [/не убыла/u, /ушла ниже нуля/u, /перестала быть целой/u]) {
    assert.match(весь, условие, `условие сторожа не доехало до напечатанного JavaScript: ${условие}`)
  }
})

test("сторож объявленной меры: отказ у напечатанного JavaScript дословно тот же, что у интерпретатора", async (t) => {
  const program = markMeasureGuards(parse(descentSource))
  const { module, content } = await build(program)
  assert.match(content, /FLANG_MEASURE/u, "сторож объявленной меры не доехал до напечатанного JavaScript")

  const points = compare(program, module, "НОД", descentGrid)

  /* Отказ обязан ПРИЙТИ ОТ НАПЕЧАТАННОГО МОДУЛЯ, а не только совпасть, и по
     всем трём условиям: совпадение отказов двух движков, взявших текст из
     одного поля AST, доказывает согласие, а не срабатывание.

     ВХОД «не убыла» — `НОД(10, −3)`, тот же, что у семи остальных целей.
     Здесь модуль зовётся напрямую, границы входа на пути нет, и `НОД(1, ∞)`
     сработал бы; но сторож меры ОДИН на восемь целей, и входы, на которых он
     обязан сработать, обязаны быть одни — иначе «один на всех» перестаёт быть
     проверяемым утверждением. Бесконечности остались в сетке выше. */
  const нод = module[exportName("НОД")]
  for (const [args, условие] of [[[1071.5, 462], /перестала быть целой/u], [[-10, 3], /ушла ниже нуля/u],
    [[10, -3], /не убыла/u]]) {
    const ответ = outcome(() => нод(...args))
    assert.equal(ответ.ok, false, `на входе без спуска напечатанный JavaScript обязан отказать: ${условие}`)
    assert.equal(ответ.code, "FLANG_MEASURE", `от модуля пришёл ${ответ.code}, а не шестой вид отказа`)
    assert.match(ответ.message, условие, `от модуля пришёл не тот текст: ${ответ.message}`)
  }
  t.diagnostic(`сверенных входов: ${points}, условий от модуля: 3`)
})
...
test("сторож частичной формы: все восемь форм отказывают у JavaScript кодом и текстом свидетеля", async (t) => {
  ...
  const { module, content } = await build(stringProgram)
  for (const [форма, [fn, args]] of ЧАСТИЧНЫЕ_ВХОДЫ) {
    ...
    /* И отказ обязан ПРИЙТИ ОТ НАПЕЧАТАННОГО МОДУЛЯ, а не только совпасть. */
    const ответ = outcome(() => module[exportName(fn)](...args))
    assert.equal(ответ.ok, false, `«${форма}»: JavaScript не отказал там, где отказал свидетель`)
    assert.equal(ответ.code, "FLANG_BUILTIN_ARGS", `«${форма}»: от модуля пришёл ${ответ.code}`)
    assert.equal(ответ.code, свидетель.code, `«${форма}»: код разошёлся со свидетелем`)
    assert.equal(ответ.message, свидетель.message, `«${форма}»: текст от модуля разошёлся со свидетелем`)
    /* У JavaScript рантайм печатается ПО ПОТРЕБНОСТИ (см. «рантайм печатается
       по потребности, а не целиком»), и это даёт то, чего нет у остальных
       семи: отрицательный контроль. Сторож обязан быть в печати именно потому,
       что форма в программе есть. */
    assert.ok(content.includes(`«${форма}»:`), `«${форма}»: сторожа нет в напечатанном модуле`)
  }
  t.diagnostic(`частичных форм от модуля: ${ЧАСТИЧНЫЕ_ВХОДЫ.size}`)
})

test("сторож частичной формы не печатается там, где формы нет: улика к тесту выше", async () => {
  /* Отрицательный контроль возможен только у JavaScript: остальные семь целей
     печатают рантайм целиком, и у них «в печати есть» верно и без формы.
     Здесь же программа без единой частичной формы обязана не нести ни одного
     их сторожа — иначе утверждение «сторож есть» выше не значило бы ничего. */
  const { content } = await build({
    flang: 1,
    module: "Без частичных",
    functions: [{ name: "Тождество", total: true, params: [{ name: "н" }], body: { kind: "var", name: "н" } }],
  })
  for (const форма of ЧАСТИЧНЫЕ.keys()) {
    assert.ok(!content.includes(`«${форма}»:`),
      `сторож «${форма}» напечатан в программе, которая этой формы не зовёт`)
  }
})
...
test("детерминированность: два вызова emitJs дают побайтово одно и то же", async () => {
  const programs = [violatingProgram, listProgram, treeProgram, mutualProgram, countdownProgram, stringProgram]
...
test("рантайм печатается по потребности, а не целиком", async () => {
  ...
  /* Порог поднят с 40 до 50, и вот за что. К именам функций в таблице добавилась
     ГРАНИЦА ВХОДА — объявленные типы параметров данными (`$PROGRAM.entry`), по
     которым прогонщик сверяет пришедшее снаружи ДО вызова. У программы вроде
     этой параметров нет вовсе, и таблица типов пуста; платится за неё ровно
     десять строк — четыре пустых списка, скобки и три строки объяснения, — то
     есть 33 стало 43. Мерено, а не прикинуто. */
  const цена = content.split("\n").length - голый.split("\n").length
  assert.ok(цена < 50, `таблица прогонщика обязана оставаться дешёвой: ${цена} строк на одну функцию`)
})
 ...
 * сроку 90 с. Свидетель-интерпретатор на той же точке упирается в предел за 919 мс.
 *
 ...
 *     импортировал;
 *   • сам предел — на той самой точке, дословным текстом свидетеля.
...
test("точка сетки, на которой печать зависала: предел шагов срабатывает за секунду", async (t) => {
  ...
  /* Текст сверяется со свидетелем ДОСЛОВНО, а не по форме: на этой точке виток
     печати совпал с витком интерпретатора вплоть до глубины, на которой
     кончился бюджет. */
  const свидетель = outcome(() => interpret(program, "Строить скобки", [42, 0, 0, "", []], пределы))
  assert.equal(свидетель.ok, false)
  assert.equal(свидетель.code, "FLANG_RECURSION_LIMIT")
  assert.equal(ответ.message, свидетель.message)
  assert.equal(
    ответ.message,
  ...
  assert.equal(сверху.message, свидетель.message)
 ...
 * бы со свидетелем не ту программу, которую печатает настоящая команда.
 *
 ...
 * идут, но и не прячутся: их 774 из 8799 (все — чужой `Нет такого` у программ
 ...
 * программа целиком.
 */
/* СПИСОК ЗДЕСЬ НЕ СВОЙ, А ОБЩИЙ, и это правка сборки `work/svodka2`. Ветка
   `work/js-measure` завела его копией из двух точек, а `work/corpus-runnable`
   в тот же день вынесла тот же список в `corpus-grid.mjs` (`УБЕГАЮЩИЕ`) для
   шести остальных целей. Две копии одного утверждения о корпусе — это два
   места, которые разойдутся молча: у одной цели точку припишут, у другой
   забудут, и обе останутся зелёными. Теперь список один на восемь целей, а
   зубы у него в `corpus-runaway.test.mjs` — в обе стороны. */
const ДОЛГИЕ_ТОЧКИ = УБЕГАЮЩИЕ.map((т) => ({ file: т.файл, fn: т.функция, args: т.аргументы }))
...
test("stdlib и leetcode: напечатанный JS совпадает с интерпретатором", async (t) => {
  ...
  /* Потолок поднят с 739 до 774 при сборке `work/svodka3`, и поднят по РОСТУ
     КОРПУСА, а не по слабости: библиотека выросла с 185 функций до 196
     (`work/stdlib-grow`), и каждая новая функция без объявленной суммы приносит
     свои точки того же вида. Природа дыры не изменилась ни на знак — это
     по-прежнему чужой `Нет такого` у программ без сумм. */
  assert.ok(unrepresentable <= 774,
    `непредставимых точек стало ${unrepresentable} против 774 — дыра растёт, а не закрывается`)
})
