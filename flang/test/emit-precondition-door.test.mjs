 ...
 *
 * Там свидетель — `checkArguments` (значение против объявленного ТИПА). Здесь
 * свидетель — сам ИНТЕРПРЕТАТОР, потому что предусловие типом не выражается
...
const рабочий = рабочийКаталог("precondition-door")
...
/**
 * Ожидание свидетеля — самой ДВЕРИ интерпретатора, а не второго свода правил.
 *
 ...
 * `flang run --args` (flang/bin/flang.mjs), и брать свидетелем одну половину
 ...
 */
function свидетель(точка) {
  const типы = checkArguments(программа, точка.имя, точка.args)
  if (!типы.ok) {
    assert.equal(точка.нарушает, true, `${точка.имя}: свидетель обязан принять ${JSON.stringify(точка.args)}`)
  ...
  try {
    ...
    assert.equal(точка.нарушает, false, `${точка.имя}: свидетель обязан отвергнуть ${JSON.stringify(точка.args)}`)
    return { ok: true, значение }
  } catch (беда) {
    assert.equal(точка.нарушает, true, `${точка.имя}: свидетель обязан принять ${JSON.stringify(точка.args)}: ${беда.message}`)
    assert.equal(typeof беда.code, "string", `${точка.имя}: у отказа свидетеля нет кода`)
    return { ok: false, code: беда.code, message: беда.message }
  }
}

const ОЖИДАНИЯ = СЕТКА.map(свидетель)

/* Сам свидетель обязан отвечать ДОГОВОРОМ там, где нарушен договор, а не чем
...
test("свидетель: нарушенный договор — это FLANG_PRECONDITION, а не отказ постусловия", () => {
  ...
  for (const [точка, ждём] of договорные) {
    assert.equal(ждём.code, "FLANG_PRECONDITION", `${точка.имя}: свидетель отказал не договором, а ${ждём.code}`)
...
/** Сверка ответов прогонщика со свидетелем — равенством строк, и кода, и текста. */
  ...
  ОЖИДАНИЯ.forEach((ждём, индекс) => {
    ...
    if (ждём.ok) {
      ...
      assert.deepEqual(
        ...
        `${кто}: значение разошлось со свидетелем`,
      )
      return
    }
