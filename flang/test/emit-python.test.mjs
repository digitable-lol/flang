...
test("сторож частичной формы: все восемь форм отказывают у Python кодом и текстом эталона", async (t) => {
  if (!pythonBin) {
    missingToolchain(t, "python", "интерпретатор Python не найден — пропуск")
    return
  }
  ...
  [...ЧАСТИЧНЫЕ_ВХОДЫ].forEach(([форма, [fn, args]], index) => {
    ...
    assert.equal(ответ.ok, false, `«${форма}»: Python не отказал там, где отказал эталон`)
