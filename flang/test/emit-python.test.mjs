      ...
      if (point.runaway) {
        ...
        assert.ok(
          ...
          `${file} / «${point.name}» на входе ${JSON.stringify(point.args) ?? "?"}: свидетель упёрся ` +
            `в предел шагов, а при ${ПРЕДЕЛ_УБЕГАЮЩЕЙ} своих шагов напечатанный Python дал ${describeOutcome(byEmitted)}`,
        )
        return
      }
      assert.ok(
        ...
        `${file} / «${point.name}» на входе ${JSON.stringify(point.args) ?? "?"}: ` +
          `интерпретатор дал ${describeOutcome(point.byInterpreter)}, напечатанный Python дал ${describeOutcome(byEmitted)}`,
      )
      points += 1
    })
  }
 ...
 * ДОСЧИТЫВАЛСЯ весь этот файл. Свидетель-вычислитель на той же точке упирается в
 * предел за 892 мс.
 *
...
test("«добавить» за постоянное время не портит исходный список: ветвление, хвост и обход", (t) => {
  ...
  сетка.forEach(([основа], индекс) => {
    const свидетель = outcome(() => interpret(program, "Свёртка по себе", [основа], {}))
    const напечатано = answerOutcome(ответы[индекс])
    assert.ok(
      sameOutcome(свидетель, напечатано),
      `«Свёртка по себе» от ${JSON.stringify(основа)}: интерпретатор дал ${describeOutcome(свидетель)}, ` +
...
test("точка сетки, на которой печать зависала: предел шагов срабатывает за секунды", (t) => {
  ...
  /* Дословно текстом свидетеля, а не «по форме»: у этой точки счётчики двух
  ...
  assert.equal(ответы[0].message, свидетель.message)
...
test("сторож объявленной меры ПОЯВЛЯЕТСЯ в напечатанном Python всеми тремя условиями", () => {
  /* Довод тот же, что у сторожа постоянного шага, и здесь он весомее: условий
     ...
     Убери проверку целости — интерпретатор и собранный Python согласно закрутят
  ...
  const весь = emitPython(markMeasureGuards(parse(descentSource))).files.map((file) => file.content).join("")
  for (const условие of [/не убыла/u, /ушла ниже нуля/u, /перестала быть целой/u]) {
    assert.match(весь, условие, `условие сторожа не доехало до напечатанного Python: ${условие}`)
  }
})

test("сторож объявленной меры: отказ у собранного Python дословно тот же, что у интерпретатора", async (t) => {
  if (!pythonBin) {
    missingToolchain(t, "python", "интерпретатор Python не найден — пропуск")
    return
  }
  ...
  const built = build(program)
  assert.match(built.source, /FLANG_MEASURE/u, "сторож объявленной меры не доехал до напечатанного Python")
  ...
  for (const [ответ, условие] of [[дробная, /перестала быть целой/u], [ниже, /ушла ниже нуля/u],
    [неУбыла, /не убыла/u]]) {
    assert.equal(ответ.ok, false, `на входе без спуска собранный Python обязан отказать: ${условие}`)
    assert.equal(ответ.code, "FLANG_MEASURE", `с машины пришёл ${ответ.code}, а не шестой вид отказа`)
...
test("сторож частичной формы: все восемь форм отказывают у Python кодом и текстом свидетеля", async (t) => {
  if (!pythonBin) {
    missingToolchain(t, "python", "интерпретатор Python не найден — пропуск")
    return
  }
  ...
  [...ЧАСТИЧНЫЕ_ВХОДЫ].forEach(([форма, [fn, args]], index) => {
    ...
    assert.equal(ответ.ok, false, `«${форма}»: Python не отказал там, где отказал свидетель`)
