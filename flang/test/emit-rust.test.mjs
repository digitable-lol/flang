 ...
 * (flang/test/toolchain-guard.mjs) — тем же способом, что у бэкендов Go и
 ...
 * двадцать минут» без срока не задать вовсе — прогонщик без него просто висит,
 * и тест, вместо того чтобы покраснеть, не кончается никогда.
 */
function ask(built, requests, срок = undefined) {
  const input = `${requests.map((request) => JSON.stringify(request)).join("\n")}\n`
  let output
  try {
    output = execFileSync(built.cli, {
      input,
      encoding: "utf8",
      ...
      if (point.runaway) {
        ...
        assert.ok(
          ...
          `${file} / «${point.name}» на входе ${JSON.stringify(point.args) ?? "?"}: эталон упёрся ` +
            `в предел шагов, а при ${ПРЕДЕЛ_УБЕГАЮЩЕЙ} своих шагов собранный Rust дал ${describeOutcome(byEmitted)}`,
        )
        return
      }
      assert.ok(
        ...
        `${file} / «${point.name}» на входе ${JSON.stringify(point.args) ?? "?"}: ` +
          `интерпретатор дал ${describeOutcome(point.byInterpreter)}, собранный Rust дал ${describeOutcome(byEmitted)}`,
      )
      points += 1
    })
  }
 ...
 * хорошо. Свидетель обязан потолок перейти — n копий по n значений стоят n²/2
 * значений, то есть под две сотни мегабайт на четырёх тысячах против потолка в
 * единицы мегабайт.
 */
const копияИсточник = [
  "модуль «Копия хвоста»",
 ...
 * машине и на пустой (замерялось трижды подряд — все три раза 3456 КиБ на
...
test("хвост списка — суффикс, а не копия: длинный список обходится линейно", async (t) => {
  ...
  /* В JS «хвост» копирует, потому что массив нельзя разделить с суффиксом.
     ...
     памяти меньше. Именно ради этого выбран `Rc`, а не `Box`. */
  const built = build(listProgram)
  assert.match(built.source, /rt::chain_tail\(&[a-z0-9_]+\)/u, "хвост обязан браться суффиксом рантайма")

  /* ДОКАЗАТЕЛЬСТВО, а не замер: у суффикса в напечатанном рантайме нет ни
     обхода, ни копирования — счётчик `Rc` растёт на единицу, начало сдвигается,
     массив остаётся общим. Никакой замер этого не даёт: он говорит про один
     компилятор на одной машине, а эти строки — про всякий.

     Проверяется ТЕЛО, а не сигнатура: до сегодняшнего дня здесь стояло
     `assert.match(built.runtime, /fn tail\(&self\) -> Items \{/u)` — то есть
     утверждалось только, что функция с таким именем существует. Подделка, у
     которой `tail` собирала новый `Vec` поэлементно, проходила тест целиком. */
  const хвост = телоФункции(built.runtime, "pub fn chain_tail(value: &Value) -> Value")
  assert.match(хвост, /Value::List\(items\) => Value::List\(items\.tail\(\)\)/u, "хвост списка обязан уходить в суффикс")
  const суффикс = телоФункции(built.runtime, "pub fn tail(&self) -> Items")
  /* Пробелы между полями не значат ничего, и требовать их значило бы стеречь
     раскладку файла вместо устройства суффикса: `Items::tail` в рантайме
     записан по полю на строку, и однострочный образец краснел бы на верном
     коде. Стережётся то, ради чего тест написан, — ТОТ ЖЕ массив под `Rc` и
     начало, сдвинутое ровно на единицу. */
  assert.match(
    суффикс,
    /Items\s*\{\s*data:\s*Rc::clone\(&self\.data\),\s*start:\s*self\.start\.saturating_add\(1\)/u,
    "суффикс обязан быть общим массивом со сдвинутым началом, а не копией",
  )
  for (const [имя, тело] of [["chain_tail", хвост], ["Items::tail", суффикс]]) {
    assert.doesNotMatch(
      тело,
      /\b(for|while|loop|iter|collect|to_vec|push|extend)\b/u,
      `в ${имя} появился обход — хвост перестал быть суффиксом`,
    )
  }
  /* `Vec` и `Items::new` в `chain_tail` есть законно — пустым списком отвечает
     ветка «не цепочка», и заводить его больше не из чего. В самом суффиксе их
     быть не может: новый массив там означал бы копию. */
  assert.doesNotMatch(
    суффикс,
    /\b(Vec|Items::new)\b/u,
    "в Items::tail появился новый массив — суффикс перестал быть общим",
  )
  ...
  /* МЕРА — и она проверяет НЕ то же, что проверка выше. Исходник отвечает за
     ...
     связывание образца. Отвечает мера, и мера в БАЙТАХ, а не в секундах.

     «Сумма» не хвостовая, поэтому хвост каждого шага жив в своём кадре до конца
     прохода: копирующий суффикс держал бы разом n²/2 значений — под две сотни
     мегабайт на 4000 элементах, — а общий массив не стоит ничего сверх самого
     списка и кадров прохода (замерено: 1,25 КиБ на элемент).

     Потолок растёт ЛИНЕЙНО по n и проверяется на n и на 2n: пройдя оба, проход
     доказал, что его аппетит линеен, — это и есть вторая половина имени. 4 КиБ
     на элемент втрое выше измеренного аппетита суффикса (1,25 КиБ — это в
     основном кадры стека: `-D warnings` собирает без оптимизации, и кадр
     нехвостовой рекурсии толст) и вдесятеро ниже квадрата; между этими числами
     границу можно провести где угодно. */
  const запрос = (n) => ({
    fn: "Сумма",
  ...
  t.diagnostic(`на элемент: ${((бюджет[1] - база) / 8000).toFixed(2)} КиБ при потолке 4 КиБ`)
  for (const [номер, n] of [4000, 8000].entries()) {
    assert.ok(
      ...
      `проход по ${n} элементам потребовал ${бюджет[номер]} КиБ при потолке ${потолок(n)}: ` +
        "хвост перестал быть суффиксом, и проход стал квадратичным",
    )
  }
  ...
  /* Свидетель: тот же порог, копирующий хвост — и он обязан порог перейти,
     ...
     а не о цене копии. */
  const свидетель = build(parse(копияИсточник))
...
/*
 * Предел шагов, который срабатывает через двадцать минут, — не предел.
 *
 * Улика. Точка сетки «Строить скобки» от 42 и 0 и 0 и "" и [] (её строит порча
 * аргумента в главной сверке выше) при объявленных 5 000 000 шагов НЕ
 * ЗАВЕРШАЛАСЬ: 1200 с при 98,9% CPU и `st` = 0,0 в `top` — то есть не голодание
 * процессора, а настоящая работа. Эталон-интерпретатор на той же точке упирался
 * в предел за 987 мс.
 *
 * Причина была не в счётчике: тот же перебор без списка-накопителя брал те же
 * 5 000 000 шагов за 0,91 с. Цена была в «добавить» — оно копировало весь
 ...
 *
 * Здесь два теста, и они проверяют разное:
 *   • цену — накопление обязано быть линейным (200 000 «добавить» — сотни
 *     миллисекунд, а не пять минут);
...
test("накопление списка линейно: 200 000 «добавить» — это доли секунды, а не минуты", (t) => {
  ...
  /* Порог с запасом в два порядка, а не «на глаз»: до починки те же три точки
     давали 33 858 мс, 125 863 мс и больше 300 000 мс (снято по сроку), после —
     119 мс, 221 мс и 432 мс. Между 432 и 30 000 нет ни машины помедленнее, ни
  ...
  /* И отдельно — сама форма роста: удвоение длины обязано удваивать время, а
     не учетверять. Порог 3 (а не 2) оставлен под шум машины; квадрат дал бы
     около 4 и здесь. */
  assert.ok(
...
test("«добавить» за постоянное время не портит исходный список: ветвление и хвост", (t) => {
  ...
  /* Ветвление — то самое место, где приём «занять ячейку за концом» ломается,
     ...
     «Хвост» здесь не для полноты: он отдаёт суффикс ЧУЖОГО массива, и продление
     такого суффикса не имеет права дописать в чужие ячейки. */
  const program = parse([
  ...
  /* Явные ожидания сверх сверки с эталоном: сверка ловит расхождение движков, а
     эти три строки ловят согласованную ошибку обоих — на случай, если чинить
...
test("точка сетки, на которой печать зависала: предел шагов срабатывает за секунды", (t) => {
  ...
  const начало = Date.now()
  /* Срок 120 с — это в шестьдесят раз больше, чем нужно после починки (1,9 с),
     и в десять раз меньше того, что было до неё (больше 1200 с). */
  ...
  t.diagnostic(`обе точки упёрлись в предел за ${мс} мс на двоих (было: больше 1 200 000 мс на одной)`)
})
 ...
 * «хвост» — копия, и обход звеньями стоил бы там n³ работы; свидетель у всех
 * пяти целей один и тот же, чтобы числа сходились. Витков языка при этом ровно
 * столько же — н на одно взятие, — а меряются здесь именно витки.
 */
const обходИсточник = [
  "модуль «Обход»",
  "",
...
test("стоимость взятия по номеру: массив, значит удвоение n удваивает работу", (t) => {
  ...
  /* ДОКАЗАТЕЛЬСТВО, а не замер: у формы в напечатанном рантайме нет обхода
     вовсе — номер превращается в индекс вектора, и в `Items::get` тоже. Никакой
     замер этого не даёт: он говорит про один компилятор на одной машине, а эти
     три строки — про всякий. Сменят вектор на звенья — упадёт здесь и сразу, а
     не замедлится молча. */
  const тело = /pub fn b_element\([^)]*\)[^{]*\{([\s\S]*?)\n\}/u.exec(built.runtime)?.[1]
  assert.ok(тело !== undefined, "в напечатанном рантайме не нашлось тела b_element")
  assert.match(тело, /match items\.get\(at as usize\)/u, "взятие обязано быть индексом вектора")
  assert.doesNotMatch(тело, /\b(for|while|loop)\b|\.iter\(\)/u, "во взятии по номеру появился обход — форма перестала быть постоянной")
  /* Второй этаж, и без него первый ничего не значит. С тех пор как список —
     вид на общий массив с запасом («добавить» за постоянное время), сам индекс
     живёт в `Items::get`: она складывает начало вида с номером и берёт ячейку.
     Сменят вектор на звенья — обход появится ЗДЕСЬ, и смотреть надо сюда. */
  const взятие = /pub fn get\(&self, index: usize\) -> Option<Value> \{([\s\S]*?)\n    \}/u.exec(built.runtime)?.[1]
  assert.ok(взятие !== undefined, "в напечатанном рантайме не нашлось тела Items::get")
  assert.match(взятие, /self\.data\.borrow\(\)\.get\(at\)\.cloned\(\)/u,
    "Items::get обязана быть индексом вектора: иначе постоянство взятия держится на слове")
  assert.doesNotMatch(взятие, /\b(for|while|loop)\b|\.iter\(\)/u,
    "в Items::get появился обход — массив сменили на звенья")
  ...
  t.diagnostic(`формой на ${n} номерах (Rust): ${мс} мс — время печатается, но не утверждается: оно шумит`)
...
test("сторож частичной формы: все восемь форм отказывают у Rust кодом и текстом эталона", async (t) => {
  if (!rustcBin) {
    missingToolchain(t, "rust", "тулчейн Rust не найден — пропуск")
    return
  }
  ...
  [...ЧАСТИЧНЫЕ_ВХОДЫ].forEach(([форма, [fn, args]], index) => {
    ...
    assert.equal(ответ.ok, false, `«${форма}»: Rust не отказал там, где отказал эталон`)
