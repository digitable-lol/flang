 ...
 * процессора, а настоящая работа. Свидетель-интерпретатор на той же точке упирался
 * в предел за 987 мс.
 *
...
test("сторож частичной формы: все восемь форм отказывают у Rust кодом и текстом свидетеля", async (t) => {
  if (!rustcBin) {
    missingToolchain(t, "rust", "тулчейн Rust не найден — пропуск")
    return
  }
  ...
  [...ЧАСТИЧНЫЕ_ВХОДЫ].forEach(([форма, [fn, args]], index) => {
    ...
    assert.equal(ответ.ok, false, `«${форма}»: Rust не отказал там, где отказал свидетель`)
