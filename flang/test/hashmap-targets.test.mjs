...
/*
 * Эталон считается ДВАЖДЫ, и разница между двумя ответами — не мелочь.
 *
 * `эталон` — то, что скажет прогонщик: у входа ИЗВНЕ стоит граница объявленных
 ...
 *
 * `эталон библиотеки` — то, что скажет вычислитель: цель `js` зовётся не
...
const эталонБиблиотеки = вопросы.map((в) => {
  try {
...
for (const цель of ЦЕЛИ) {
  test(`hashmap.flang в цели ${цель.имя}: ${вопросы.length} входов, расхождений ноль`, async (ctx) => {
    ...
    /* Цель `js` зовётся не прогонщиком, а прямо — значит и сверяется с
       библиотечным эталоном, у которого границы входа нет. */
    const ждём = цель.имя === "js" ? эталонБиблиотеки : эталон
    let расхождений = 0
    const первые = []
