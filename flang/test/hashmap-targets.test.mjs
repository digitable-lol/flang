 ...
 *
 * Сверяется значение: вычислитель — свидетель, каждая цель обязана дать то же
...
/*
 * Свидетель считается ДВАЖДЫ, и разница между двумя ответами — не мелочь.
 *
 * `свидетель` — то, что скажет прогонщик: у входа ИЗВНЕ стоит граница объявленных
 * типов (`checkArguments`, см. through-entry.mjs), потому что значения приезжают
 * JSON-ом и программой не являются. Так работает `flang run`, и так теперь
 * работает напечатанный `flang_cli` во всех целях с прогонщиком.
 *
 * `свидетель библиотеки` — то, что скажет вычислитель: цель `js` зовётся не
 * прогонщиком, а прямо, экспортированными функциями модуля. Библиотечный вход
 * границы не имеет и иметь не должен — она про вход извне.
 */
const свидетель = вопросы.map((в) => {
  const итог = черезГраницу(программа, в.имя, в.аргументы, БОЛЬШОЕ)
  return итог.ok ? { ok: true, значение: итог.value } : { ok: false, код: итог.code }
})

const свидетельБиблиотеки = вопросы.map((в) => {
  try {
...
function разложить(печать) {
  счёт += 1
  /* Имя каталога — латиницей, и это не вкусовщина. `javac` и `java` читают
     путь текущего каталога через `sun.jnu.encoding`, а она берётся из локали
     машины; там, где локаль не UTF-8 (`LC_CTYPE=UTF-8` — имя несуществующей
     локали, и JDK откатывается к ANSI_X3.4-1968), кириллица в пути становится
     «??», и javac отвечает «error: file not found: Value.java» на файл, который
     лежит прямо перед ним. Тест обязан проверять печать, а не локаль машины. */
  const каталог = join(работа, `target${счёт}`)
...
for (const цель of ЦЕЛИ) {
  test(`hashmap.flang в цели ${цель.имя}: ${вопросы.length} входов, расхождений ноль`, async (ctx) => {
    ...
    /* Цель `js` зовётся не прогонщиком, а прямо — значит и сверяется с
       библиотечным свидетелем, у которого границы входа нет. */
    const ждём = цель.имя === "js" ? свидетельБиблиотеки : свидетель
    let расхождений = 0
    const первые = []
      ...
      if (!сошлось) {
        ...
        if (первые.length < 3) {
          первые.push(
            ...
            `свидетель ${JSON.stringify(наш).slice(0, 200)}, цель ${JSON.stringify(их).slice(0, 200)}`,
          )
        }
      }
    }
