...
/*
 * Слово «элемент» стало ключевым, и цена этому измерена: в 128 файлах .flang и
 * .fts репозитория голых (незакавыченных) вхождений у него НОЛЬ, а у
 * множественного «элементы» — 430. Значит ключевым обязано стать РОВНО
 * единственное число, и проверить это на слово нельзя: склейка сверяет слово
 * целиком, а такое правило легко потерять при правке таблицы.
 *
 * Вторая половина — про «обратный элемент» моноида: пара слов обязана
 * побеждать одиночное. Без этого правила запись моноида сломалась бы молча.
 */
test("«элемент» ключевое, «элементы» имя, «обратный элемент» цел", () => {
  assert.equal(tokenize("элемент 1 в с\n")[0].value, "item")
  assert.equal(tokenize("item 1 in с\n")[0].value, "item")
  assert.equal(keywordId("элемент"), keywordId("item"))

  const [первый] = tokenize("элементы\n")
  assert.equal(первый.kind, "name", "«элементы» обязано остаться именем")
  assert.equal(первый.value, "элементы")
  assert.equal(keywordId("элементы"), null)

  assert.equal(tokenize("обратный элемент «Минус»\n")[0].value, "inverseElement")
  assert.equal(tokenize("«элемент»\n")[0].kind, "name", "имя в ёлочках ключевым не становится")
})

/*
 * «код символа» — фраза, а не слово, и цена измерена тем же счётом: в 149
 * файлах .flang и .fts репозитория голых вхождений у пары НОЛЬ, а у одинокого
 * «код» — 226 (`«Не разобрано» с код как код` из самой спецификации языка и
 * весь `http-request.fts`). Значит ключевой обязана стать РОВНО пара, а
 * одинокое «код» — остаться именем. Проверить это на слово нельзя: правило
 * теряется при первой же правке таблицы.
 */
test("«код символа» ключевое, одинокий «код» — по-прежнему имя", () => {
  assert.equal(tokenize("код символа т\n")[0].value, "charCode")
  assert.equal(tokenize("character code t\n")[0].value, "charCode")
  assert.equal(keywordId("код символа"), keywordId("character code"))

  const [одинокий] = tokenize("код\n")
  assert.equal(одинокий.kind, "name", "«код» обязано остаться именем")
  assert.equal(одинокий.value, "код")
  assert.equal(keywordId("код"), null)

  // Разбор отказа «к числу или беда» из SPEC связывает «код» именем дважды.
  const связывание = tokenize("с код как код и сообщение как весть\n")
  assert.equal(связывание[1].kind, "name")
  assert.equal(связывание[1].value, "код")
})
