...
/**
 * Список из нескольких конструкторов: разделитель полей отдаётся хозяину.
 *
 * До 7 августа 2026 записать `[вариант «М» с имя равным "а", вариант «М» с имя
 * равным "б"]` было нельзя: запятая после значения поля всегда читалась как
 * «дальше ещё поле». Один элемент работал, два — нет, и из-за этого примеры в
 * `stdlib/dictionary.flang` стояли на словаре из одной связи, а словарь из двух
 * проверялся тестом на JS.
 *
 * Проверяются обе стороны разведения: разделитель ПРОДОЛЖАЕТ поля, когда за ним
 * `имя равным`, и УХОДИТ наружу, когда там что угодно другое. Одной стороны
 * мало — проверка только второй прошла бы и на грамматике, где `и` вообще
 * перестал разделять поля.
 */
test("разделитель после поля продолжает конструктор только перед новым полем", () => {
  const один = { kind: "construct", variant: "М", fields: { имя: "а" } }
  const два = { kind: "construct", variant: "М", fields: { имя: "б" } }
  ...
  for (const разделитель of [",", " и"]) {
    ...
    assert.deepEqual([узел.items[0].variant, узел.items[1].variant], [один.variant, два.variant])
  }

  /* Обратная сторона: перед `имя равным` разделитель по-прежнему съедается, и
     оба поля попадают в ОДИН конструктор, а не в два элемента списка. */
  for (const разделитель of [",", " и"]) {
    const узел = без(body(`функция «Ф»\n  возвращает число\n  [вариант «М» с имя равным "а"${разделитель} год равным 2026]\n`))
    assert.equal(узел.items.length, 1, `разделитель «${разделитель}»`)
    assert.deepEqual(Object.keys(узел.items[0].fields), ["имя", "год"])
  }
