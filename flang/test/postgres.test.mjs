...
test("НАСТОЯЩАЯ БАЗА: план на flang создаёт таблицу, вставляет с параметрами, выбирает и получает отказ", async (t) => {
  ...
  try {
    ...
    /* Разбор ОБЯЗАН оборваться на ключе отмены — в нём случайное 32-битное
       число, — и причина у обрыва бывает ДВУХ видов, потому что случайные
       октеты ведут себя по-разному. Если они негодны как UTF-8, каждый из них
       приезжает знаком замены: «есть знак замены». Если они СЛУЧАЙНО сложились
       в правильную многооктетную последовательность, знаков стало меньше, чем
       октетов, и граница сообщения разрезала знак пополам: «знак не уместился».
       Оба вида — одно и то же событие, и ждать ровно одного значит писать
       проверку, которая краснеет от чужой случайности. */
    assert.match(строки[0], /ОБРЫВ: (в теле сообщения «K» есть знак замены|знак не уместился в границу сообщения)/)
