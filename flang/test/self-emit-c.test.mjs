...
test("сторож меры: обе реализации понижения ставят его одинаково", async (t) => {
  ...
  /* Программа, у которой сторож меры ПЕЧАТАЕТСЯ. Раньше их было три, и все три
     оказались таковыми случайно — `stdlib/numbers.flang`, `stdlib/strings.flang`
     и решение LeetCode; уточнение числа сторожа у них сняло, и предмет сверки
     исчез бы вместе с ними. Поэтому предмет теперь НАЗВАН: образец, где та же
     функция написана дважды — на `числе` со сторожем и на `нат` без него.
     Объявленные меры (`euclid.flang`, `binary-search.flang`) сюда не добавлены:
     на них вскрывается расхождение `FL_MAX_ARGS`, к уточнению числа не
     относящееся, и долг этот назван в flang/test/self-totality.test.mjs. */
  const отмеченные = ["flang/examples/measure/natural.flang"]
  let сторожей = 0
