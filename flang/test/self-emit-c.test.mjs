 ...
 *
 * ЧИСЛО ИЗМЕРЕНО, А НЕ ПРИКИНУТО, и оба его значения стоит знать. До сторожа
 * объявленной меры печать стоила 96 607 421 шаг при лимите 100 000 000 — то
 * есть запаса было 3,5 %, и любое пополнение `self/defunc.flang` выбило бы
 * лимит независимо от того, чем именно. Сторож спуска добавил в этот файл 64
 * функции, и печать стала стоить 116 119 140 шагов: рост 1,20 против роста
 * самой программы в 1,14 — то есть по размеру печатаемого, а не по классу
 * сложности. Мерено двоичным поиском по `maxSteps` на обоих деревьях.
 *
 * Двести миллионов — это 1,7 запаса, и запас нужен ровно затем, зачем нужен был
 * прежний: чтобы следующее пополнение слоя ловилось замером, а не падением
 * теста, где числа не видно.
 */
