 ...
 *
 * В СОБРАННОМ ДЕРЕВЕ ПРИЧИН ДВЕ, А НЕ ОДНА. Второй раз тот же лимит поднимала
 * печать границы входа (`work/emit-entry-types`): она добавила файлу десяток
 * функций, а цена печати растёт быстрее числа функций — `Идентификаторы для`
 * просматривает таблицу целиком на каждой. Обе прибавки сложились, и число
 * ниже перемерено двоичным поиском на СОБРАННОМ дереве, а не сложено из двух
 * замеров на разных.
 *
 * Двести миллионов — это запас над замером, и нужен он ровно затем, зачем нужен
 * был прежний: чтобы следующее пополнение слоя ловилось замером, а не падением
