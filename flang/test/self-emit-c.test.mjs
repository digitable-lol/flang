...
test("сторож меры: обе реализации понижения ставят его одинаково", async (t) => {
  ...
  /* Список — программы репозитория, у которых сторож меры ЕЩЁ ПЕЧАТАЕТСЯ, то
     есть убывающий параметр объявлен `числом`. `flang/stdlib/numbers.flang` из
     него ушёл нарочно: там параметр объявлен `нат`, и сторожа больше нет вовсе
     (`flang/src/range.mjs`). Пустеть этому списку нельзя — сверять постановку
     сторожа стало бы не на чем, — поэтому три программы на `числе` в дереве
     оставлены сознательно, и это записано у них в тексте. */
  const отмеченные = [
