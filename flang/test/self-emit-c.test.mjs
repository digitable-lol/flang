...
/**
 * Лимит шагов поднят с миллиона до ста двадцати: печать в C — работа с целым
 * AST сразу, и самая большая программа репозитория (`flang/self/emit-c.flang`,
 * 3400 строк, 100 КБ напечатанного C) укладывается между сотней и ста пятью
 * миллионами шагов (мерено бисекцией: 100 000 000 не хватает, 105 000 000
 * хватает). Лимит здесь не формальность: он ловит превращение печати в
 * перебор. Глубина — по вложенности AST, а не по числу узлов, поэтому её
 * хватает штатной.
 *
 * Было сто миллионов, пока в близнеце не появилась печать границы входа: она
 * добавила файлу десяток функций, а цена печати растёт быстрее числа функций —
 * `Идентификаторы для` просматривает таблицу целиком на каждой.
 */
const ШАГИ = { maxSteps: 120_000_000, maxDepth: 10_000 }
...
test("сторож меры: обе реализации понижения ставят его одинаково", async (t) => {
  ...
  /* СПИСОК ПЕРЕСОБРАН 15 августа 2026 при сборке `work/svodka`, и пересобран
     он не по вкусу, а потому что две прежние программы перестали быть
     примером: `flang/stdlib/numbers.flang` и
     `flang/examples/leetcode/070-climbing-stairs.flang` объявляют свои
     убывающие параметры типом `нат`, и после слияния `origin/work/axiom`
     анализ несёт их обещание ПЯТЫМ носителем — «точный шаг», у которого
     сторожа нет вовсе. Сторожить в них стало нечего, и оставить их здесь
     значило бы требовать сторожа там, где его сняли по делу.

     Взамен взяты программы, у которых носитель со сторожем настоящий и того же
     рода, что был: `flang/stdlib/strings.flang` и `flang/stdlib/lists.flang` —
     постоянный шаг (`guards`).

     ЧЕГО ЗДЕСЬ НЕТ И ПОЧЕМУ — это находка, а не умолчание. Второй носитель со
     сторожем — ОБЪЯВЛЕННАЯ МЕРА (`descents`), и этой сверкой он не покрыт
     никогда не был: прежние три программы несли постоянный шаг и `нат`. Попытка
     покрыть его при сборке `work/svodka` показала, что покрывать пока нечего:
     на `flang/examples/measure/euclid.flang` печать близнеца расходится с
     эталоном уже в `flang_runtime.h` (строка 13), на
     `flang/examples/measure/binary-search.flang` — в заголовке модуля
     (строка 55, эталон печатает функцию «объявленная мера убывает», близнец на
     её месте — другую). Это НЕ следствие слияния: обе программы и обе
     реализации понижения пришли такими, какими были. Долг назван здесь, чтобы
     его нельзя было принять за покрытое: пока он не закрыт, эта сверка отвечает
     за постоянный шаг и молчит об объявленной мере. */
  const отмеченные = ["flang/stdlib/strings.flang", "flang/stdlib/lists.flang"]
  let сторожей = 0
