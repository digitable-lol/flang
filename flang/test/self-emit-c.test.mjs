 ...
 * что печатает свидетель `flang/src/emit/c.mjs`, **побайтово** — включая
 ...
 * делает ни у свидетеля (`emitC` зовёт `таблицаВхода` из src/types.mjs), ни у
 * эталона. Сверяется здесь то, что эталон и правда делает: как эти данные
...
/** Побайтовая сверка со свидетелем; при расхождении показывает первую строку. */
function сверить(имя, ast, опции = {}) {
  const свидетель = emitC(ast, опции)
 ...
 * `self-emit-c` перестаёт краснеть на эталоне, у которого доказанная
...
test("программы репозитория: C совпадает со свидетелем побайтово", async () => {
...
test("сторож меры: обе реализации понижения ставят его одинаково", async (t) => {
  /* Понижений два — свидетель `src/defunc.mjs` и копия на самом языке, — и с
     ...
     настоящая: печать эталона расходилась со свидетелем на
     `examples/measure/euclid.flang` и `examples/measure/binary-search.flang`.
     Расходилась она потому, что эталон меру только РАЗБИРАЛ; теперь он её
  ...
  for (const относительный of отмеченные) {
     ...
     * края (`flang/bin/flang.mjs`), и стоят они парой потому, что свидетель печатает
     * только программу с обеими. Печатник читает `доказана` и `числовая`; из восьми
     * эталонов их читает один — этот. Без них сверка шла бы по ветви, которой в
     * работе нет, и молчала бы о доказанной арифметике: свидетель печатает выражением,
     * эталон — вызовом помощника.
     */
...
test("доказанная непустота: обе печати снимают сторожа в одних и тех же местах", async (t) => {
  /* Отметку `доказана` кладёт `разобрать` — тем же `markProven`, каким её
     ...
     всё это у свидетеля и у эталона написано порознь. */
  const отмеченные = [
  ...
  for (const относительный of отмеченные) {
    ...
    assert.ok(вызовов > 0, `${относительный}: свидетель не позвал ни одного помощника без сторожа`)
    сверить(относительный, помеченная)
    снято += вызовов
  }
...
test("доказанный тип числа: обе печати ставят выражение вместо вызова помощника", async (t) => {
  /* Вторая отметка того же прохода, и проверяется она тем же способом: сверка
     ...
     у процента и голая печать числового литерала написаны у свидетеля и у
     эталона порознь. */
  const отмеченные = [
  ...
  for (const относительный of отмеченные) {
    ...
    assert.ok(мест > 0, `${относительный}: свидетель не напечатал ни одного доказанного места`)
    сверить(относительный, помеченная)
    распаковок += мест
  }
...
/**
 * `к числу или беда` — форма языка, которую эталон печати НЕ ЗНАЛ.
 *
 ...
 * головы эталона — проверка типов и печать — про неё не слышали. Программы
 ...
 */
test("«к числу или беда» печатается тем же вызовом рантайма, что у свидетеля", () => {
...
test("шаг батута без единого значения в хвосте гасит result и собирается", (t) => {
  /* Регрессия на дефект, который эта печать нашла в свидетеле на собственном
     исходнике. Компонента взаимной хвостовой рекурсии, у которой ВСЕ хвостовые
     позиции — отскоки, даёт шаг батута, ни разу не пишущий в *result. Свидетель
     ...
     со свидетелем, а сборка — что результат вообще компилируется. */
  const кольцо = {
    flang: 1,
    module: "Кольцо",
    functions: [
