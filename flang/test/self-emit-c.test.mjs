...
test("сторож меры: обе реализации понижения ставят его одинаково", async (t) => {
  ...
  /* СПИСОК ПЕРЕСОБРАН 15 августа 2026 при сборке `work/svodka`, и пересобран
     ...
     Взамен взяты программы, у которых носитель со сторожем настоящий:
     `flang/stdlib/strings.flang` и `flang/stdlib/lists.flang` — постоянный шаг
     (`guards`), `flang/examples/measure/binary-search.flang` — объявленная мера
     (`descents`). Обе половины нужны: сторож у них ставится разными путями, и
     сверять одну значило бы не сверять вторую. */
  const отмеченные = [
