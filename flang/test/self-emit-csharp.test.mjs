...
function напечатать(ast, опции = {}) {
  const итог = вызвать("Печать программы шарп", { "программа": значение(ast), "настройки": настройки(опции) })
...
function расхождение(имя, ast, опции = {}) {
  ...
  /* ДОЛГ ЭТАЛОНА, НАЗВАННЫЙ И ВЫЧТЕННЫЙ: печати ГРАНИЦЫ ВХОДА у него нет — это
     сказано и в шапке `self/emit-csharp.flang`, — а у свидетеля она появилась
