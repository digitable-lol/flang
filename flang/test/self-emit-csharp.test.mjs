 ...
 * что печатает эталон `flang/src/emit/csharp.mjs`, **побайтово** — включая
 ...
 *
 * Тест МЕРЯЕТ, а не только краснеет: пока близнец не дописан, он печатает
...
/**
 * Побайтовая сверка с эталоном. Возвращает `null` при совпадении и текст
 * первого расхождения иначе — краснеть решает вызывающий, потому что на
 * недописанном близнеце важнее СЧЁТ совпавших, чем первый же отказ.
 */
function расхождение(имя, ast, опции = {}) {
  let эталон
  try {
    эталон = emitCsharp(ast, опции)
  } catch (ошибка) {
    return `${имя}: эталон отказал (${ошибка.message}) — программа не годится для сверки`
  }
  let мой
  try {
  ...
  /* ДОЛГ БЛИЗНЕЦА, НАЗВАННЫЙ И ВЫЧТЕННЫЙ: печати ГРАНИЦЫ ВХОДА у него нет — это
     сказано и в шапке `self/emit-csharp.flang`, — а у эталона она появилась
     после того, как ветка была написана. Блок вырезается по двум меткам, всё
     остальное сверяется побайтово, и пропажа блока красит сверку отдельно —
     см. `flang/test/entry-debt.mjs`. */
  let блоков = 0
  for (const [индекс, файл] of эталон.files.entries()) {
    const наш = мой.files[индекс].content
    const { текст: ожидаемое, былаГраница, беда } = безГраницы(файл.content, "csharp")
...
test("программы репозитория: C# совпадает с эталоном побайтово", async (t) => {
