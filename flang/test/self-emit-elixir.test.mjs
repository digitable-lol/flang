 ...
 * файлы, которые печатает «Печать программы Elixir» на flang, обязаны совпасть с тем,
 * что печатает свидетель `flang/src/emit/elixir.mjs`, **побайтово** — включая
 ...
 *
 * Пока эталон не дописан, тест ЧЕСТНО КРАСЕН и при этом МЕРЯЕТ: он печатает,
...
function настройки(ast, опции = {}) {
  ...
  return {
    ...
    ...РАНТАЙМ,
    /* ГРАНИЦА ВХОДА приезжает снаружи, как и текст рантайма: какой у параметра
       объявленный тип, решает слой типов (`таблицаВхода`), а не печать. Печать
       отвечает за байты, и вот их сверяет этот тест. */
...
function напечатать(ast, опции = {}) {
  const итог = вызвать("Печать программы Elixir", { "программа": значение(ast), "настройки": настройки(ast, опции) })
  return { files: итог.файлы.map((файл) => ({ path: файл.путь, content: файл.содержимое })), error: итог.ошибка }
}

/**
 * Побайтовая сверка со свидетелем. Возвращает `null` при совпадении либо текст
...
function расхождение(имя, ast, опции = {}) {
  ...
  try {
    свидетель = emitElixir(ast, опции)
  } catch (ошибка) {
  ...
  /* ДОЛГ ЗАКРЫТ: печать ГРАНИЦЫ ВХОДА эталон теперь делает сам («Печать границы
     входа Elixir»), и вычитание блока из вывода свидетеля снято целиком. Сверка
     идёт по ПОЛНОМУ тексту обеих сторон, без единого исключения. */
...
test("программы репозитория: Elixir совпадает со свидетелем побайтово", async (t) => {
  const беды = []
  ...
  /* Сверка, у которой корпус опустел, «проходит» с нулём расхождений из нуля
     программ — и молчит об этом. У соседних целей такой сторож стоит
     (`программ стало подозрительно мало`), здесь его не было. */
  assert.ok(всего >= 30, `программ стало подозрительно мало: ${всего}`)
...
test("испорченный AST: те же отказы подготовки, что у свидетеля", () => {
