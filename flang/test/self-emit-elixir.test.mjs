...
function напечатать(ast, опции = {}) {
  const итог = вызвать("Печать программы Elixir", { "программа": значение(ast), "настройки": настройки(опции) })
...
function расхождение(имя, ast, опции = {}) {
  ...
  for (const [индекс, файл] of свидетель.files.entries()) {
    ...
    const { текст: ожидаемое, былаГраница, беда } = безГраницы(файл.content, "elixir")
