...
/** Лимит тот же, что у печати в C: печать — работа с целым AST сразу. */
...
function напечатать(ast, опции = {}) {
  const итог = вызвать("Печать программы в Go", { "программа": значение(ast), "настройки": настройки(опции) })
...
test("настройки печати: путь, база индексации, пределы, без прогонщика", () => {
  ...
  assert.ok(свои.files.some((файл) => файл.path === "flang/svoyo.go"), "свой путь не доехал")
})
