...
/* Граница входа приходит эталону НАСТРОЙКАМИ, как и рантайм: строит её слой
   типов (`таблицаВхода`), а не печать, и у свидетеля она считается ровно там же
   и по той же программе — ДО дефункционализации. */
...
function напечатать(ast, опции = {}, шаги = ШАГИ) {
  const итог = вызвать("Печать программы в Go", { "программа": значение(ast), "настройки": настройки(ast, опции) }, шаги)
 ...
 *
 * Здесь стояло вычитание: эталон не печатал ГРАНИЦУ ВХОДА (таблицу объявленных
 * типов параметров, по которой прогонщик сверяет пришедшее снаружи значение ДО
 * вызова), и из вывода свидетеля этот блок вырезался перед сверкой. Долг был
 * назван честно, но цена его оказалась выше, чем «один блок комментария»:
 * `flang_cli.go` зовёт `flang.Entry()`, а без блока такой функции в пакете нет
 * вовсе — то есть напечатанное эталоном НЕ СОБИРАЛОСЬ, пока печатался прогонщик.
 *
 * Теперь блок печатает и эталон («Печать границы входа Go»), а таблицу ему, как
 * и рантайм, подаёт вызывающий: строит её слой типов (`таблицаВхода`), которого
 * на flang нет ни строки. Сверка от этого стала СТРОЖЕ — она снова побайтовая на
 * целом файле, без единого выреза.
 *
 * Метка блока остаётся здесь одним утверждением: блок обязан БЫТЬ у обоих. Она
 * стоит вместо прежнего вычитания и краснеет, если печать границы пропадёт с
 * любой из двух сторон. */
...
function расхождение(имя, ast, опции = {}, шаги = бюджет(имя)) {
  ...
  for (const [индекс, файл] of свидетель.files.entries()) {
    ...
    return (
      `${имя}: ${файл.path} разошёлся на строке ${строка + 1} (строк: свидетель ${слева.length}, flang ${справа.length})\n` +
      `  свидетель: ${JSON.stringify(слева[строка])}\n` +
      `  flang:  ${JSON.stringify(справа[строка])}`
    )
  }
  /* Блок границы обязан БЫТЬ у обоих. Это то, что осталось от прежнего
     вычитания: пропади печать границы с любой стороны — сверка ещё сойдётся
     (обе печатают одинаково мало), а вот это утверждение покраснеет. */
  if (блоковГраницы === 0 && свидетель.files.some((файл) => файл.content.includes("func ") || файл.content.length > 400)) {
    return `${имя}: границы входа нет ни в одном файле — печать границы пропала, сверка ослабла молча`
  }
  return null
}
