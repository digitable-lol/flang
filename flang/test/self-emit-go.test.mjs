...
function расхождение(имя, ast, опции = {}, шаги = бюджет(имя)) {
  ...
  for (const [индекс, файл] of эталон.files.entries()) {
    ...
    return (
      `${имя}: ${файл.path} разошёлся на строке ${строка + 1} (строк: эталон без границы ${слева.length}, flang ${справа.length})\n` +
  ...
  if (блоковГраницы === 0 && эталон.files.some((файл) => файл.content.includes("func ") || файл.content.length > 400)) {
    return `${имя}: эталон не напечатал границы входа ни в одном файле — вычитание блока стало прикрытием, снимите его`
  }
  return null
}
