 ...
 * с тем, что печатает свидетель `flang/src/emit/java.mjs`, **побайтово** — включая
 ...
 * Пока эталон не дописан, дифференциальный тест краснеет — и он обязан при
 ...
 * печати. Печать отвечает за то, что эти данные лягут в Java правильными
 * байтами, и вот это здесь и сверяется — побайтово, без всякого вычитания.
 *
 * Свидетель зовёт `таблицаВхода` на той же программе, что приходит сюда, и ДО
 * понижения (`emitJava`: таблица считается перед `defunctionalize`). Позови её
 * на понижённой — параметры диспетчера приехали бы в таблицу лишними строками.
 */
...
function напечатать(ast, опции = {}) {
  const итог = вызвать("Печать программы в Java", { "программа": значение(ast), "настройки": настройки(ast, опции) })
  return { files: итог.файлы.map((файл) => ({ path: файл.путь, content: файл.содержимое })), error: итог.ошибка }
}

/** Первая строка блока границы входа: по ней сверка считает, что блок напечатан. */
const МЕТКА_ГРАНИЦЫ = "  /* Граница входа: объявленные типы параметров данными. Прогонщик сверяет по"
...
/**
 * Сверка со свидетелем; возвращает `null` при побайтовом совпадении и текст
...
function расхождение(имя, ast, опции = {}) {
  let свидетель = null
  try {
    свидетель = emitJava(ast, опции)
  } catch (ошибка) {
    return `${имя}: свидетель отказал (${ошибка.message}), а сверять надо печать`
  }
  let мой = null
  try {
  ...
  const слеваПути = свидетель.files.map((файл) => файл.path)
  const справаПути = мой.files.map((файл) => файл.path)
  if (слеваПути.join("|") !== справаПути.join("|")) {
    return `${имя}: набор файлов не совпал\n  свидетель: ${слеваПути.join(", ")}\n  flang:  ${справаПути.join(", ")}`
  }
  /* ДОЛГ ЭТАЛОНА СНЯТ, И ВЫЧИТАНИЕ УБРАНО. Здесь стояло вычитание блока ГРАНИЦЫ
     ВХОДА: эталон его не печатал, и блок вырезался из вывода свидетеля перед
     сравнением. Теперь «Печать границы входа Java» есть в эталоне, таблица
     приезжает настройками, и сверяются ПОЛНЫЕ файлы, байт в байт, без среза.

     Требование «блок обязан быть» осталось и стоит теперь на ОБЕИХ сторонах:
     перестань печатать границу любая из двух — сверка покраснеет на первой же
     программе сама, без отдельного утверждения-подпорки. */
  let блоков = 0
  for (const [индекс, файл] of свидетель.files.entries()) {
    const наш = мой.files[индекс].content
    const ожидаемое = файл.content
    if (ожидаемое.includes(МЕТКА_ГРАНИЦЫ)) блоков += 1
  ...
  /* Граница обязана БЫТЬ напечатана: без неё прогонщик не собрался бы вовсе —
     `FlangCli.java` зовёт `entry()` по имени. Требование стоит на наборе файлов,
     а не на каждом: блок печатается ровно в один файл — класс программы. */
  if (блоков === 0) return `${имя}: границы входа нет ни в одном файле — печать таблицы пропала`
  return null
}
...
test("понижение высшего порядка зовётся оттуда же, откуда его зовёт свидетель", () => {
...
test("программы репозитория: Java совпадает со свидетелем побайтово", async (t) => {
...
test("сторож меры: обе реализации понижения ставят его одинаково", async (t) => {
  ...
  for (const относительный of программыРепозитория) {
    ...
    /*
     * ПРЕДОБРАБОТКА СВИДЕТЕЛЯ: markMeasureGuards, markProven, defunctionalize — обе отметки переднего
 ...
 * предмет: понижение эталона сверяет self-defunc.test.mjs, здесь сверяется
 * печать.
     */
