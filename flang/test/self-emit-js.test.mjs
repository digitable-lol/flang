 ...
 *
 * Причина измерена прямо: на `003-longest-substring…` эталонное понижение
...
test("сторож меры: расходится только понижение, и ровно на измеренном числе программ", async (t) => {
  ...
  for (const относительный of программыРепозитория) {
    ...
    отмеченных += 1
    const эталонный = emitJs(помеченная).files[0].content
    assert.match(эталонный, /FLANG_MEASURE/u, `${относительный}: сторожа нет даже у эталона`)
    const беда = сверить(относительный, помеченная)
    if (беда === null) {
      совпало += 1
      continue
    }
    /* Разделяем две беды. Сторож меры, которого нет у близнеца, — долг
