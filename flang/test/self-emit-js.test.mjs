...
/* ГРАНИЦА ВХОДА ПРИЕЗЖАЕТ НАСТРОЙКАМИ, а не считается эталоном, — ровно как у
   печати в C. Считает её слой типов (`таблицаВхода`), и второй такой обход
   разошёлся бы с первым на первом же параметре, чей тип объявлен синонимом.
   Раньше блока у эталона не было вовсе и сверка ВЫЧИТАЛА его у свидетеля: то
   есть на 7,66 % вывода (218 158 байт корпуса из 2 848 619) не сравнивала
   ничего. Теперь вычитания нет, и сравниваются все байты. */
function настройки(программа, опции = {}) {
  const граница = таблицаВхода(программа)
  return {
    "путь": опции.path ?? "",
    ...
    /* Имена полей у слоя типов и у эталона печати разные («полеС» против
       «поле с»), и переклад тот же, что в `self-bootstrap.test.mjs`. */
    "типы входа": граница.типы.map((тип) => ({
      "вид": тип.вид,
      "имя": тип.имя,
    ...
    "поля входа": граница.поля.map((поле) => ({ "имя": поле.имя, "тип": поле.тип })),
    "варианты входа": граница.варианты.map((вариант) => ({
      "имя": вариант.имя,
      "поле с": вариант.полеС,
      "полей": вариант.полей,
    })),
    "параметры входа": граница.параметры.map((параметр) => ({
...
function напечатать(ast, опции = {}) {
  const итог = вызвать("Печать программы JS", { "программа": значение(ast), "настройки": настройки(ast, опции) })
...
function сверить(имя, ast, опции = {}) {
  ...
  /* НИЧЕГО НЕ ВЫЧИТАЕТСЯ. Граница входа у эталона теперь печатается
     («Граница входа JS»), и сравниваются все байты обоих файлов. */
