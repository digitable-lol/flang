...
/**
 * Граница входа приезжает ТАБЛИЦЕЙ, как и текст рантайма.
 *
 * Строит её слой типов (`таблицаВхода` в `src/types.mjs`) — тот же, что отвечает
 * на вопрос «подходит ли значение объявленному типу» для `flang run --args`.
 * Печать её только раскладывает в байты, и это не деление работы для красоты:
 * второй ответ на вопрос об объявленном типе разошёлся бы с первым молча.
 *
 * Читается таблица ДО дефункционализации — ровно там же, где её читает
 * свидетель: после понижения параметр, объявленный функцией, становится суммой
 * тегов, и таблица описала бы не то, что объявлял автор.
 */
...
function напечатать(ast, опции = {}) {
  const итог = вызвать("Печать программы Python", { "программа": значение(ast), "настройки": настройки(ast, опции) })
...
function сверить(имя, ast, опции = {}) {
  ...
  /* ВЫЧИТАНИЯ ЗДЕСЬ БОЛЬШЕ НЕТ, и это главное изменение сверки. Раньше из вывода
     свидетеля вырезался блок ГРАНИЦЫ ВХОДА общим местом на пять целей: у
     эталона его не было вовсе. Долг был назван честно, но названный долг всё
     равно молчал о том, без чего напечатанный модуль не работает — `flang_cli.py`
     зовёт `program.entry()` безусловно. Теперь сверяются ПОЛНЫЕ файлы, и общего
     места с вычитанием не осталось вовсе: все восемь целей печатают границу
     входа сами. */
  let файлов = 0
  let беда = null
