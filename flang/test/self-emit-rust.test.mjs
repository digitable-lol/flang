 ...
 * с тем, что печатает свидетель `flang/src/emit/rust.mjs`, **побайтово** — включая
 ...
 * такое». Пока эталон не дописан, тест краснеет честно и с числом.
 *
 ...
 * печати. Печать отвечает за то, что эти данные лягут в Rust правильными
 ...
 * понижения (`emitRust`: таблица считается ПЕРЕД `defunctionalize`). Позови её
...
function напечатать(ast, опции = {}) {
  const итог = вызвать("Печать программы в Rust", { "программа": значение(ast), "настройки": настройки(ast, опции) })
 ...
 *
 * Здесь стояло вычитание: эталон писался от `main`, свидетель после него получил
 * печать ГРАНИЦЫ ВХОДА (`work/emit-entry-types`), и из вывода свидетеля этот
 * блок ВЫЧИТАЛСЯ, а остаток сверялся побайтово. Долг закрыт — «Печать границы
 * входа Rust» есть в эталоне, таблица приезжает настройками, — и вычитание
 * снято целиком: сверяются полные файлы, байт в байт, без единого среза.
 *
 * Требование «блок обязан быть» осталось и стоит теперь на ОБЕИХ сторонах: если
 * границу перестанет печатать любая из двух, сверка покраснеет на первой же
 * программе сама, без отдельного утверждения-подпорки. */
const МЕТКА_ГРАНИЦЫ = "/// Граница входа: объявленные типы параметров данными."
/**
 * Побайтовая сверка со свидетелем. Возвращает `null` при совпадении, иначе текст
 * первого расхождения — тест сам решает, падать сразу или считать до конца.
 */
function расхождение(имя, ast, опции = {}) {
  let свидетель = null
  try {
    свидетель = emitRust(ast, опции)
  } catch (ошибка) {
    return `${имя}: свидетель отказал (${ошибка.message}) — сверять нечего`
  }
  let мой = null
  try {
  ...
  for (const [индекс, файл] of свидетель.files.entries()) {
    ...
    if (ожидаемое.includes(МЕТКА_ГРАНИЦЫ)) блоковГраницы += 1
  ...
  /* Граница обязана БЫТЬ напечатана: без неё прогонщик не собрался бы вовсе —
     `src/cli.rs` зовёт `program::entry()`. Требование стоит на наборе файлов, а
     не на каждом: блок печатается ровно в один файл — реализацию модуля. */
  if (блоковГраницы === 0) {
    return `${имя}: границы входа нет ни в одном файле — печать перестала её давать`
  }
  return null
}
...
test("программы репозитория: Rust совпадает со свидетелем побайтово", async (t) => {
...
test("сторож меры: программа с ним печатается побайтово так же", async (t) => {
  /* Программы выше приходят БЕЗ отметок: отметку кладёт анализ завершаемости
     ...
     Понижение делается ЗАРАНЕЕ и свидетельское (`defunctionalize`), и обеим
     ...
     `пусть «довод 1»`, эталон — голый вызов. Это долг понижения, он уже
