 ...
 * даёт свидетельский `checkPlans` из `flang/src/io.mjs` — код, текст сообщения и
 ...
 *
 * СВИДЕТЕЛЬ СПРАШИВАЕТСЯ ЧЕРЕЗ `checkTypes`, а не напрямую, и это не обход:
 ...
 * ней одной была бы зелёной у эталона, который не делает ничего: «диагностик
 ...
 * ловят эталон, который жалуется ТАМ, ГДЕ ЖАЛОВАТЬСЯ НЕ НА ЧТО.
 */
...
const свидетельские = (ast) => checkTypes(ast).diagnostics.filter((д) => д.code === "FLANG_PLAN").map(образцовая)
...
function расхождение(ast) {
  const свидетель = свидетельские(ast)
  const свои = наши(ast)
...
test("корпус планов: каждый случай даёт те же диагностики, что и свидетель", () => {
...
test("корпус планов: сверка не пуста — свидетель на нём поднимает шестнадцать диагностик", () => {
  const всего = случаи.reduce((сумма, случай) => сумма + свидетельские(случай.ast).length, 0)
  assert.equal(всего, 16, `свидетель поднял ${всего}`)
})
...
test("настоящая программа с планом сверяется со свидетелем", async () => {
  ...
  assert.deepEqual(свидетельские(ast), [])
  assert.deepEqual(наши(ast), [])
})

/**
 * Программы БЕЗ планов — против эталона, который жалуется на пустом месте.
...
test("программы без планов не поднимают ни одной диагностики ни у свидетеля, ни у эталона", async () => {
  for (const путь of БЕЗ_ПЛАНОВ) {
    ...
    assert.deepEqual(свидетельские(ast), [], `${путь}: свидетель`)
    assert.deepEqual(наши(ast), [], `${путь}: эталон`)
  }
})
