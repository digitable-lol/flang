/**
 * Свидетель для НАПЕЧАТАННОГО ПРОГОНЩИКА — это `flang run`, а не голый вычислитель.
 *
 ...
 *
 * Поэтому сверка «напечатанное против свидетеля» на входах, которые снаружи
 * приходят, обязана идти через ту же дверь. Одна функция на все бэкенды:
 * разойдись они в том, что считать свидетелем, — и «совпало» перестало бы
 * что-либо значить.
 */
