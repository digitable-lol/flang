...
/* «приписать» и «добавить» проверяются одной ветвью, но диагностика обязана
   называть написанное слово: сказать «добавить», когда написано «приписать»,
   значит отправить читателя искать не ту строку. */
test("«приписать» проверяет тип элемента и называет своё слово", () => {
  const result = checkTypes(программа([], [
    функция({
      name: "Приписать не то",
      params: [{ name: "э", type: список(число) }],
      returns: список(число),
      body: форма("приписать", лит("строка"), пер("э")),
    }),
  ]))
  assert.deepEqual(коды(result), ["FLANG_BUILTIN_ARGS"])
  assert.match(result.diagnostics[0].message, /«приписать»/u)
})
...
/**
 * ГИПОТЕЗА ТЕОРЕМЫ — ПРИЗНАК, а не что попало.
 *
 * УЛИКА, С КОТОРОЙ ЭТО НАЧАЛОСЬ. Слова `theorems` в `types.mjs` не было ни
 * одного, и три программы ниже — `дано свет плюс "строка"`, `дано 5`,
 * `дано "строка"` — проходили `flang check` с ответом `valid: true` и НУЛЁМ
 * диагностик. Проверялась у гипотезы только область видимости, и проверял её
 * парсер; типа не проверяло ничто.
 *
 ...
 * ложным. `5` не является утверждением ни о чём.
 *
 * ПОДДЕЛКА И ЕЁ ОБРАТНАЯ ПОЛОВИНА. Без второй половины проверка ловила бы не
 * тип гипотезы, а само наличие `дано`: верная гипотеза (`н больше 0`) и
 * гипотеза про `результат` — имя, которое парсер держит в области видимости всей
 * теоремы, — обязаны проходить молча.
 */
const ТЕОРЕМА = (гипотеза) => `модуль «Дано»
...
test("гипотеза «дано» обязана быть признаком: три улики, каждая своим типом", () => {
  const дурные = [
    /* `нат`, а не `число`: литерал сужается до отрезка, и сообщение обязано
       называть выведенный тип, а не тот, которым его назвал бы автор. */
    ["нат", "5"],
    ["строка", '"строка"'],
    /* Арифметика по строке: сперва жалоба на сам операнд, потом на то, что
       утверждением это не стало. Две диагностики об одном месте — не описка:
       выражение неверно и как выражение, и как гипотеза. */
    ["число", 'н плюс "строка"'],
  ]
  for (const [тип, гипотеза] of дурные) {
    const итог = checkTypes(parse(ТЕОРЕМА(гипотеза)))
    const беда = итог.diagnostics.find((д) => /гипотеза обязана быть признаком/u.test(д.message))
    assert.ok(беда !== undefined, `«${гипотеза}»: ${JSON.stringify(итог.diagnostics)}`)
    assert.equal(беда.code, "FLANG_TYPE", `«${гипотеза}»: код отказа`)
    assert.match(беда.message, new RegExp(`теорема «половина неотрицательна»: гипотеза даёт ${тип},`, "u"))
  }
})

test("верная гипотеза проходит молча — иначе ловилось бы наличие «дано», а не тип", () => {
  for (const гипотеза of [null, "н больше 0", "результат не меньше 0"]) {
    assert.deepEqual(
      коды(checkTypes(parse(ТЕОРЕМА(гипотеза)))),
      [],
      `гипотеза «${String(гипотеза)}» отвергнута, а она верна`,
    )
  }
})
