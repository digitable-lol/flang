...
class Flang < Formula
  ...
  url "https://github.com/digitable-lol/flang/releases/download/v0.7.1/flang-0.7.1-c.tar.gz"
  ...
  #
  # Первая попытка (b0d4ce0d…) была собрана вручную тем же рецептом
  # (io-запуск release-in-c.flang был недоступен) и не совпала с архивом,
  # который собрал сам конвейер выпуска на теге: `gh run` `Сверить хеш
  # архива с формулой Homebrew` упал 3 сентября 2026, назвав РЕАЛЬНЫЙ хеш
  # архива. Число ниже — он, взят из вывода `sha256sum` самого конвейера, а
  # не пересчитан вручную повторно; чем разошлась ручная сборка с конвейерной
  # (право доступа, время правки файла, порядок — рецепт `tar` целится ровно
  # в это), не разбиралось — второй попытки на руках не потребовалось.
  ...
  sha256 "2f5d92e3eb6ef7b2ae11a6b1f06d5b4154bc856974f6e43d9d1d8cda907fcb8a"
