...
class Flang < Formula
  ...
  url "https://github.com/digitable-lol/flang/releases/download/v0.7.2/flang-0.7.2-c.tar.gz"
  ...
  #
  # Метка v0.7.1 не удалась: архив там был собран вручную (io-запуск
  # release-in-c.flang был недоступен на тот момент), и sha256 не совпал с
  # тем, что собрал сам конвейер выпуска на теге — шаг «Сверить хеш архива
  # с формулой Homebrew» упал ДО публикации, ничего не вышло под v0.7.1
  # (ни GitHub Release, ни npm). Тег не удалялся (запись запрещена), просто
  # пропущен.
  #
  # Число ниже — с версией 0.7.2: посчитано 3 сентября 2026 ЭТИМ рецептом
  # (`bootstrap/flang io scripts/release-in-c.flang`, тем же io-запуском,
  # каким пойдёт и сам конвейер, не вручную), 4 447 763 байта. Совпадение
  # с конвейером остаётся утверждением до подтверждения самим прогоном на
  # теге — здесь только то, что посчитано локально тем же путём.
  ...
  sha256 "1b2c173fe9123003056b5839f603be510ff460e2e5c465c396629fe85a1eb210"
