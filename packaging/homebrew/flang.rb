...
class Flang < Formula
  ...
  url "https://github.com/digitable-lol/flang/releases/download/v0.7.7/flang-0.7.7-c.tar.gz"
  ...
  # не удалялись (запись запрещена), просто пропущены. v0.7.3 дошла.
  #
  # 3 сентября 2026 release.yml (прежде publish-npm.yml) лишился
  # workflow_dispatch — задача 8649 убрала npm и сузила запуск до одних
  # лишь тегов; сухого прогона до метки этим приёмом больше нет. v0.7.4 и
  # v0.7.5 поймали это на себе подряд: v0.7.4 несла хеш от 0.7.3, v0.7.5 —
  # хеш, снятый ЛОКАЛЬНО той же документированной командой (дословно) и
  # всё равно не совпавший — местная сборка архива не побайтово равна
  # тому, что даёт конвейер, причина не разобрана. Оба раза конвейер сам
  # остановился ДО публикации и сам назвал настоящий хеш в тексте ошибки
  # — ни разу ничего битого не вышло. Оба тега не удалялись, просто
  # пропущены, как v0.7.1/v0.7.2 раньше. Число ниже для v0.7.6 взято
  # ПРЯМО из текста ошибки конвейера на теге v0.7.5, не пересчитано вручную.
  ...
  # 2 995 862 байта.
  sha256 "573e3a567b2b66630364ffaa4fe51ad1236908410e181626bbcb92fdf64dd7f2" # ЗАВЕДОМО ЧУЖОЕ (это хеш архива 0.7.6) — v0.7.7 нарочно ставится на провал, чтобы получить настоящий хеш из текста ошибки конвейера
