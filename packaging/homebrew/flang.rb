...
class Flang < Formula
  ...
  #     | gzip -9n > output/flang-0.7.0-c.tar.gz
  #
  ...
  #
  # Формула на dev отстала от выпуска: version и sha256 всё ещё называли 0.6.2,
  # а тег v0.7.0 вышел раньше (сторож `формула:проверка` в install-path.yml
  # красен этим же расхождением как минимум с 29 августа). Число ниже снято
  # 3 сентября 2026 НЕ пересборкой дерева (дерево dev с тега v0.7.0 уже ушло
  # вперёд, и пересборка дала бы sha256 другого содержимого) — а прямым
  # скачиванием и sha256sum самого опубликованного актива релиза: `curl -sL
  # https://github.com/digitable-lol/flang/releases/download/v0.7.0/flang-0.7.0-c.tar.gz
  # | sha256sum`. 4 447 759 байт. Внутри десять имён, ни одного `.o` и ни
  # одного собранного двоичного — проверено распаковкой (`tar -tzf`). Прежде
  # здесь стояло b8d82b1c… от выпуска 0.6.2 (2 995 862 байта, 22 августа 2026).
