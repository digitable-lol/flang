...
class Flang < Formula
  ...
  url "https://github.com/digitable-lol/flang/releases/download/v0.7.4/flang-0.7.4-c.tar.gz"
  ...
  #
  # ЧИСЛО НИЖЕ — ЕЩЁ ОТ 0.7.3, ЗАВЕДОМО ЧУЖОЕ ЭТОМУ РЕЛИЗУ. 3 сентября 2026
  # release.yml (прежде publish-npm.yml) лишился workflow_dispatch — задача
  # 8649 убрала npm и сузила запуск до одних лишь тегов, и сухого прогона до
  # метки этим приёмом больше нет. Метка v0.7.4 ставится ДО пересчёта, а
  # число здесь правится СРАЗУ ПОСЛЕ прямым скачиванием опубликованного
  # актива и sha256sum по нему — тем же приёмом, каким снят был хеш 0.7.0
  # (см. историю dev-ветки этой формулы, коммит 3 сентября). Метки v0.7.1 и
  # v0.7.2 не удались именно из ручного пересчёта архива, не из этого приёма:
  # у обеих ручная сборка не совпала с тем, что собрал сам конвейер
  # (0.7.1 — расхождение полное; 0.7.2 — 4 байта из 4 447 763, причина не
  # разобрана). Оба тега не удалялись (запись запрещена), просто пропущены.
  # Прежде здесь стояло ddf21617… от выпуска 0.7.0, чей архив был байт в байт
  # тем же кодом, но с #define FLANG_VERSION "0.6.2" — строкой, которую
  # забыли бампнуть вместе с package.json (хотфикс: release-versions/9959.md,
  # и тот же класс беды снова нашёлся и починен для 0.7.4 — задача 9983).
  # b8d82b1c… от выпуска 0.6.2 стоял ещё раньше, 2 995 862 байта.
  sha256 "c101dae42007afa697893d68358dea1d9d220c76839555147676bd791f435159" # ЗАВЕДОМО СТАРОЕ — правится после публикации
