...
# Второй путь печати — `scripts/bootstrap-reprint.sh`; числа там обязаны совпадать.
set -eu
...
# То же число уже стояло в scripts/bootstrap-reprint.sh (MAX_STEPS), а здесь отстало;
...
# flang/stdlib/json.flang, — но подняли ТОЛЬКО в scripts/bootstrap-reprint.sh, и правило
...
#
# Здесь число возвращено к одному. Довод самого числа — в scripts/bootstrap-reprint.sh
...
# обоих путях сразу. Коротко, полностью — в scripts/bootstrap-reprint.sh и в
...
# целиком — над MAX_STEPS в scripts/bootstrap-reprint.sh.
#
# 23 сентября 2026: 1 400 000 000 000 → 4 000 000 000 000, и снова в обоих путях
# сразу. Заход печати 22–23 сентября НЕ ДОШЁЛ — съел 1 400 021 420 760 шагов и
# уткнулся в прежний потолок. Запас здесь меньше трёхкратного (2,86), и делится
# он на НИЖНЮЮ оценку: настоящая цена шага больше и пока не измерена. Довод
# целиком — над MAX_STEPS в scripts/bootstrap-reprint.sh.
FLANG_MAX_STEPS='4000000000000'
FLANG_MAX_DEPTH='20000'
# Та же измеренная цена, что в scripts/bootstrap-reprint.sh, и по тому же доводу:
...
#
# 23 сентября 2026 она ею СНОВА СТАЛА: заход 22–23 сентября оборвался на
# 1 400 021 420 760 шагах, то есть цена больше этого числа, а насколько —
# неизвестно до первого дошедшего захода. Имя переменной оставлено прежним:
# его читает `same_numbers_in_second_print` в scripts/bootstrap-reprint.sh, и разъезд
# имён стоил бы дороже оговорки. Оговорка — эта.
#
# СОВПАДЕНИЕ ЭТИХ ТРЁХ ЧИСЕЛ С scripts/bootstrap-reprint.sh ТЕПЕРЬ ПРОВЕРЯЕТСЯ, а не
...
# вершины в 1,66 раза.
FLANG_MEASURED_COST='2412102536357'
