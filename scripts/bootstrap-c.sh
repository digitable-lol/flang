...
#
# 29 августа 2026, вечер: 300 000 000 000 → 1 400 000 000 000, и снова в обоих
# путях сразу. Самый дорогой шаг печати впервые ДОСЧИТАН до конца — заход
# 15:49:37 → 22:12:27, 6 ч 23 мин, код 0: «Суд ядра о программе» стоит от
# 455 989 437 934 до 456 857 834 234 шагов. Прежние 300 миллиардов были НИЖЕ
# этой цены, то есть семя с таким клеймом следующую перепечатку не осилило бы.
# Новое число — верхний конец вилки × 3, запас на месяц роста дерева. Довод
# целиком — над MAX_STEPS в scripts/raskrutka.sh.
FLANG_MAX_STEPS='1400000000000'
FLANG_MAX_DEPTH='20000'
# Та же измеренная цена, что в scripts/raskrutka.sh, и по тому же доводу:
# печатать с потолком ниже уже измеренного шага — значит заведомо не дойти до
# конца. Прежде звалась FLANG_POL_IZMERENNYY (пол, нижняя оценка); нижней
# оценкой она быть перестала — шаг досчитан, и это измеренная цена.
#
# СОВПАДЕНИЕ ЭТИХ ТРЁХ ЧИСЕЛ С scripts/raskrutka.sh ТЕПЕРЬ ПРОВЕРЯЕТСЯ, а не
# только обещается: `same_numbers_in_second_print` там читает их отсюда и
# отказывается работать кодом 2, если они разошлись. До 29 августа 2026 правило
# «число одно на оба пути» было записано и не исполнялось — 23 августа числа
# разъехались, и заметил это человек глазами, а не сторож.
...
if [ "$FLANG_MAX_STEPS" -lt "$FLANG_MEASURED_COST" ]; then
  echo "bootstrap-c.sh: потолок $FLANG_MAX_STEPS ниже измеренной цены шага $FLANG_MEASURED_COST —" >&2
