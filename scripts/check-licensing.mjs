...
// `.c`, `.h` и `.rs` попали в этот список не для полноты. Рантаймы бэкендов
// (`flang/src/emit/c/*`, `flang/src/emit/rust/*`) печатаются в чужую программу
...
// планировщик JS и не видел ни одного из восьми файлов C и Rust: JS-шапку он
// поймал, а расхождение осталось невидимым.
const SUFFIXES = ['.ts', '.mjs', '.js', '.cjs', '.c', '.h', '.rs'];
