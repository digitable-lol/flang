...
/*
  ...
  Правила про `.ts` и `dist/src` ниже оставлены НАМЕРЕННО, хотя ни одного `.ts`
  в дереве больше нет: старый проект FTS вынесен 16 августа 2026 (тег
  `fts-pered-udaleniem`). Довод — не «жалко удалять», а то, что гейт обязан
  пережить появление первого нового `.ts` без правки; правило, снятое за
  ненадобностью, возвращают уже после того, как файл без шапки уехал наружу.
...
// БАЙТ В БАЙТ — это наш код, который получатель распространяет дальше уже как
// часть своего исходника, а пункт 1 BSD-2-Clause требует сохранять при этом
// уведомление. Пока гейт смотрел только на `.js`, он видел печатаемый
// планировщик JS и не видел НИ ОДНОГО из остальных двадцати шести файлов
// рантайма: JS-шапку он поймал, а расхождение осталось невидимым.
const SUFFIXES = [
  '.ts', '.mjs', '.js', '.cjs',
  '.c', '.h', '.rs', '.go', '.java', '.cs', '.ex', '.exs', '.py',
];
...
/* Заголовки самого tree-sitter, лежащие в дереве копией. Чужой код: своё
   авторство в них не дописывают, а MIT-уведомление в них уже есть. Тот же
   случай, что `/vendor/`, только каталог назван по-другому. */
const isVendoredTreeSitter = relative =>
  relative.startsWith('editors/tree-sitter-fts/src/tree_sitter/');
...
/* Файл, напечатанный `tree-sitter generate` по нашей грамматике. Шапку в него
   ...
   из самого файла, а не из списка имён, — тогда правило переживёт переименование
   и не станет дырой, через которую пройдёт написанный руками файл. */
const isGeneratedByTreeSitter = relative =>
  /\.(c|h)$/.test(relative)
  && read(relative).startsWith('/* Automatically @generated by tree-sitter */');
...
const isExcluded = relative =>
  ...
  || isVendoredTreeSitter(relative)
  || isGeneratedByTreeSitter(relative);
