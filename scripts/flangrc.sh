...
#
# ИМЕНА ПЕРЕМЕННЫХ ЗДЕСЬ ЛАТИНИЦЕЙ, как в `ярлык` и `scripts/bootstrap-reprint.sh`:
...
sreda_klyucha() {
  case $1 in
    ...
    # РЕШАЕТ ПО ЭТОМУ КЛЮЧУ САМ ДВОИЧНЫЙ (`flangrc_key` в flang_repl.c):
    # поставленный `flang` живёт без дерева и наших сценариев рядом не имеет.
    # Печатаем всё равно: до 23 сентября 2026 `flangrc.sh недоказанное` отвечал
    # пустой строкой и кодом 0 — тишиной о воротах, через которые в сборку
    # проходит непроверенная программа.
...
# действует: замер 23 сентября 2026 на 0.7.21 — `.flangrc: недоказанное =
# разрешено` даёт у `flang run` код 2 и три строки словами. Сценарий, ответивший
# бы «отказ», сказал бы правду о своём разборе и ложь о том, что случится.
# Судит язык — «Слово недоказанного» в scripts/settings-file.flang.
