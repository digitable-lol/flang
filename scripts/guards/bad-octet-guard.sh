...
#
#   scripts/guards/bad-octet-guard.sh              судить все девять целей
#   scripts/guards/bad-octet-guard.sh --цель rust  судить одну
#   scripts/guards/bad-octet-guard.sh --порча      проверить самого сторожа
#
# Коды возврата:
...
#      девять похожих текстов — это и есть девять разных поведений.
#
...
#
#   PAMYAT=150G flang-vorota -- scripts/guards/bad-octet-guard.sh
#
...
# Свой каталог сторож за собой убирает, ЧУЖОЙ — нет. Заданный через RABOTA=
# принадлежит позвавшему: он мог положить туда своё и ждать вывода. Разница
# замерена, а не выведена: на 6 сентября 2026 в /srv/tmp лежало 86 каталогов
# «storozh-oktet.*» на 525 МБ, старейший от 22 августа, — сторож не убирал за
# собой ни разу. Машина общая на 27 человек.
if [ -n "${RABOTA:-}" ]; then
  WORK=$RABOTA; SVOY=нет
else
  WORK=$(mktemp -d -p "${FLANG_TMP:-/srv/tmp}" storozh-oktet.XXXXXX); SVOY=да
  KRASNYH=0
  # Убирается ЗЕЛЁНЫЙ прогон. Красный каталог оставляется: сторож на него
  # ссылается строкой «работа: …», и снести то, чем разбираются, значит
  # отчитаться о находке и тут же её спрятать.
  trap '[ "$SVOY" = да ] && [ "$KRASNYH" = 0 ] && rm -rf "$WORK"' EXIT INT TERM
fi
mkdir -p "$WORK"
...
SRC=docs/examples/measure/euclid.flang
...
if [ "$SVOY" = да ] && [ "$KRASNYH" = 0 ]; then SUDBA="работа: $WORK (свой, убран)"
else SUDBA="работа: $WORK"; fi
echo "красных: $KRASNYH; без тулчейна: $BEZ_TULCHEJNA; $SUDBA"
...
PY
  ...
  PORCHA_RAB=$(mktemp -d -p "${FLANG_TMP:-/srv/tmp}" storozh-porcha.XXXXXX)
  RANTAJM="$PORTA" RABOTA="$PORCHA_RAB" "$0" --цель rust
  PKOD=$?
  rm -rf "$PORTA" "$PORCHA_RAB"
  if [ "$PKOD" = 0 ]; then
...
[ "$KRASNYH" = 0 ] || exit 1
exit 0
# ярлык «октет:проверка» bash — негодный октет во входе прогонщика: девять целей отказывают одинаково, а не молчат
# ярлык «октет:порча» bash --порча — проверка негодного октета краснеет на подложенной тишине и молчит, когда чисто
