...
# flang/proof/** не судится ПОКА: там имя файла — ключ артефактов доказательств
# (задача 1420). Довод снят по существу: ключ не запрещает переименование, он требует
# двигать файл и строку манифеста одним изменением. Решение — ADR-0046, каталог
# переводится на английские имена порциями; исключение уходит с последней (задача 6421).
