...
#
# КИРИЛЛИЦЫ В ИМЕНАХ ФАЙЛОВ НЕ ПРИБАВЛЯЕТСЯ — храповик по ведомости.
#
...
# ИСКЛЮЧЕНИЯ flang/proof/** БОЛЬШЕ НЕТ (ADR-0046, задача 6421, 22 сентября 2026).
# Оно стояло с доводом «имя файла — ключ артефактов доказательств» (задача 1420).
# Довод снят по существу, а не росчерком: ключ не запрещает переименование, он требует
# двигать файл и строку манифеста ОДНИМ изменением. Каталог переведён на английские
# имена тремя порциями, после каждой набор проб давал «сошлось всё» с теми же числами.
# Единственное оставшееся кириллическое имя — образец расширения «.фп» — стоит в
# ведомости долга ниже, с причиной.
...
if [ "$n" -eq 0 ] && [ "$m" -eq 0 ]; then
  ...
  echo "кириллица в именах файлов: $vsego, все в ведомости — новых нет"
  exit 0
fi
[ "$n" -eq 0 ] || { echo "КИРИЛЛИЦА В ИМЕНИ ФАЙЛА — новых $n (имя файла — английскими словами, без транслита; задача 1419):"; sed 's/^/  · /' "$RAB/novye"; }
