...
#
#   sh scripts/guards/guards-without-forgery-probe.sh            # перепись целиком
#   sh scripts/guards/guards-without-forgery-probe.sh --check     # сверка с ведомостью, код 1 при расхождении
#   sh scripts/guards/guards-without-forgery-probe.sh --список    # только имена беспробных, по одному в строке
#
# ── Зачем ───────────────────────────────────────────────────────────────────
...
#   · Шесть проверок чекера (`checker.c:1199`) НЕ ОТРАБАТЫВАЛИ НИ РАЗУ: они
#     читают исходник по привязке `пример строка N`, а записи корпуса её не
#     несут — напечатаны двоичным постарше (задача 8690).
#   · Проба под правило 9999 ЛЕЖАЛА В ДЕРЕВЕ, а `run.sh` её не звал —
...
# Прибор `who-calls-the-guards.sh` отвечает на вопрос «кого CI зовёт»; этот
...
# Поэтому `--check` сверяет с ведомостью `scripts/ledgers/guards-without-forgery-probe.json`
# В ОБЕ СТОРОНЫ, как заведено у сторожа ключей (flang/scripts/cli-keys-debt.json)
# и у переписи зова (scripts/ledgers/uncalled-guards.json):
...
VEDOMOST=scripts/ledgers/guards-without-forgery-probe.json
REZHIM=${1:-}
...
without_probe = [i for i in storozha if i not in with_probe]
...
if rezhim != "--check":
    ...
    if os.path.exists(vedomost):
        prichiny = {k: v for k, v in json.load(io.open(vedomost, encoding="utf-8")).items()
                    if not k.startswith("//")}
    tolko_postroena = sorted(i for i in postroena if i not in with_probe)
    print("ПЕРЕПИСЬ ПРОБ ПОРЧИ: у кого показано, что он краснеет\n")
    print(f"  сторожей в дереве              {len(storozha)}")
    print(f"  ПРОБА ПОЗВАНА (CI требует красноты)   {len(with_probe)}")
    print(f"  проба построена, но НЕ позвана        {len(tolko_postroena)}")
    print(f"  ПРОБА НЕ ПОКАЗАНА (долг)              {len(without_probe)}")
    print("\nПРОБА ПОЗВАНА, и где стоит:\n")
    for imya in sorted(with_probe):
        fajl, stroka = with_probe[imya]
...
novye = [i for i in without_probe if i not in zapisano]        # долг вырос
zakrytye = [i for i in zapisano if i not in without_probe]     # долг закрыт

if not novye and not zakrytye:
    print(f"сторожа без пробы порчи: {len(without_probe)},"
