...
#
# ── Как звать ────────────────────────────────────────────────────────────────
#   sh scripts/guards/hand-written-lists.sh             перепись: два числа и все адреса
#   sh scripts/guards/hand-written-lists.sh --check     то же и код 1, если расхождений
#                                                больше, чем записано в ведомости
#   sh scripts/guards/hand-written-lists.sh --targets   только перечни целей печати
#
# Коды: 0 — сошлось; 1 — не сошлось; 2 — судить нечем (нет двоичного, нет
# ведомости). Второй — «не проверено», а НЕ отрицательный приговор.
#
# Ведомость известных расхождений — scripts/ledgers/hand-written-lists-ledger.tsv. Она
...
LEDGER=scripts/ledgers/hand-written-lists-ledger.tsv
...
# отказа — scripts/targets/identical-declarations.sh).
...
exit 0
# ярлык «перечни:перепись» sh — сколько перечней в дереве набрано руками и сколько из них разошлось с деревом — оба числа своим прогоном
# ярлык «перечни:проверка» sh --check — новое расхождение перечня с деревом красит; запись ведомости, переставшая быть расхождением, красит тоже
