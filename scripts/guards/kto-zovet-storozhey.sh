...
#
#   sh scripts/guards/kto-zovet-storozhey.sh            # перепись целиком
#   sh scripts/guards/kto-zovet-storozhey.sh --check     # сверка с ведомостью, код 1 при расхождении
#   sh scripts/guards/kto-zovet-storozhey.sh --список    # только имена незваных, по одному в строке
#
# ── Зачем ───────────────────────────────────────────────────────────────────
...
# `--check` сверяет с ведомостью `scripts/ledgers/storozha-bez-zova.json` В ОБЕ
...
VEDOMOST=scripts/ledgers/storozha-bez-zova.json
REZHIM=${1:-}
...
if rezhim != "--check":
    ...
    print("\nСверка с ведомостью — тем же прибором:"
          "\n  sh scripts/guards/kto-zovet-storozhey.sh --check")
    sys.exit(0)
