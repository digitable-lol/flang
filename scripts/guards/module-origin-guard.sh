...
#
#   sh scripts/guards/module-origin-guard.sh              # опись: модуль → файл, из которого он приедет
#   sh scripts/guards/module-origin-guard.sh --кратко      # только чужое
#   sh scripts/guards/module-origin-guard.sh --подлог      # проба порчи на игрушечном дереве
#   sh scripts/guards/module-origin-guard.sh --корень DIR  # судить не это дерево, а названное
#
...
# `bootstrap/flang check scripts/site/releases.flang` отвечает «замечаний нет», кодом
...
if [ "$REZHIM" = "--подлог" ]; then
  ...
  printf 'модуль «Проба»\n  использует «Списки»\n' > "$RAB/над/дерево/scripts/probe.flang"
...
sys.exit(0)
PY
# ярлык «изоляция:проверка» sh — откуда приедет каждый модуль: поиск по имени поднимается ВЫШЕ корня дерева, и чужой файл в каталоге-предке подменяет модуль языка молча — 8 сентября 2026 «JSON» брался из /srv/tmp/json.baseline.flang, а сборка отвечала кодом 0
# ярлык «изоляция:подлог» sh --подлог — проверка изоляции краснеет на файле, подложенном выше корня, и зеленеет, когда его убрали — обе стороны прогоном, на игрушечном дереве во временном каталоге
