...
#
#   sh scripts/guards/storozha-bez-podloga.sh            # перепись целиком
#   sh scripts/guards/storozha-bez-podloga.sh --check     # сверка с ведомостью, код 1 при расхождении
#   sh scripts/guards/storozha-bez-podloga.sh --список    # только имена беспробных, по одному в строке
#
# ── Зачем ───────────────────────────────────────────────────────────────────
...
#         "коды:проверка"  … flang/scripts/code-guard.flang --plan 'Коды целы'
#         "коды:подлог"    … flang/scripts/code-guard.flang --plan 'Подлог кода'
...
# Поэтому `--check` сверяет с ведомостью `scripts/ledgers/storozha-bez-podloga.json`
# В ОБЕ СТОРОНЫ, как заведено у сторожа ключей (flang/scripts/cli-keys-debt.json)
# и у переписи зова (scripts/ledgers/storozha-bez-zova.json):
...
VEDOMOST=scripts/ledgers/storozha-bez-podloga.json
REZHIM=${1:-}
...
if rezhim != "--check":
    ...
    print("\nСверка с ведомостью — тем же прибором:"
          "\n  sh scripts/guards/storozha-bez-podloga.sh --check")
    sys.exit(0)
