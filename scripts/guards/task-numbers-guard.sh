...
# и спрашивает у `scripts/bootstrap-reprint.sh --telo`, то ли соберётся, — а отпечаток
...
#     СРАЗУ ПО ЧЕТЫРЁМ каталогам: `docs/tasks/`, `completed/`, `rejected/`, `archive/`;
#   • номер в шапке (`номер:`) сходится с именем файла;
#   • номер — ровно четыре цифры;
#   • у каждой задачи в `docs/tasks/rejected/` статус «отклонена», раздел
...
# так же, как их не считает `flang/scripts/tasks.fscript`.
#
# ── Почему четыре каталога, а не один ───────────────────────────────────────────
# До 8 сентября 2026 сторож смотрел только в `docs/tasks/`. Номер, занятый закрытой
...
# ярлык «задачник:номера» sh — номер задачи не занят дважды НИ В ОДНОМ из трёх каталогов (tasks, completed, rejected) и сходится с именем файла, а у отклонённой названы довод и дата; без двоичного и без отпечатка семени — 31 августа две ветки от одного основания раздали семь одинаковых номеров, и не покраснел никто
# ярлык «задачник:номера-подлог» sh --подлог — проверка номеров задач краснеет на двойном номере, на двойнике через каталоги и на отказе без довода, а зеленеет на законном отказе — пять сторон прогоном, во временной копии каталога
