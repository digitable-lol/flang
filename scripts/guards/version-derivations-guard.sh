...
# .flangrc ↔ FLANG_VERSION — этого мало: .TH страницы man и три числа формулы
...
#
# Источник версии один: функция «Версия» в scripts/release/emit-package.flang. Из
# неё ярлык `версия` разносит число во все производные места (до 17 сентября
# 2026 первым из них был `package.json`; файл выброшен задачей 3570, и его
# читатели читают `.flangrc`). Разошлось — не правьте руками, запустите:
#
...
# Этот сторож сверяет ТОЛЬКО число версии (и четыре строки .flangrc рядом с
# ним) и дёшево. Набор man держит `справка:проверка`, sha256 и архив —
# `формула:проверка`, отсутствие самого package.json — `без-пакета:проверка`.
# Спора нет: здесь подмножество-по-числу, дешёвое и в хуке.
set -u
...
# Тело любой функции источника: первая строка вида «‹два пробела›"…"» после её
# заголовка. `LC_ALL=C` — grep -F сличает заголовок по байтам, и кириллица в
# чужой локали ему не помеха.
...
sverit_klyuch() { # ключ .flangrc, имя функции источника
  ...
  if [ -z "$ozhidaem" ]; then
    dobavit "$ISTOCHNIK: у функции «$2» не найдено тело вида «\"…\"» — ключ $1 в .flangrc сверять не с чем"
    return
  fi
  est=$(sed -n 's/^'"$1"'[[:space:]]*=[[:space:]]*//p' .flangrc | head -1)
  if [ "$est" != "$ozhidaem" ]; then
...
echo "сторож производных версии: источник $ISTOCHNIK объявляет $V_SRC — и .flangrc (все пять ключей), flang_repl.c, .TH страницы man и три числа формулы Homebrew сошлись с ним. sha256 формулы, changelog и семя здесь не судятся (см. шапку)."
exit 0
