...
#
#   sh scripts/guards/who-calls-the-guards.sh            # перепись целиком
#   sh scripts/guards/who-calls-the-guards.sh --check     # сверка с ведомостью, код 1 при расхождении
#   sh scripts/guards/who-calls-the-guards.sh --список    # только имена незваных, по одному в строке
#
# ── Зачем ───────────────────────────────────────────────────────────────────
...
#   ПО ФАЙЛУ:  `run: bootstrap/flang io scripts/guards/license-guard.fscript`
...
# `--check` сверяет с ведомостью `scripts/ledgers/uncalled-guards.json` В ОБЕ
...
VEDOMOST=scripts/ledgers/uncalled-guards.json
REZHIM=${1:-}
...
primerov = TEKST_YARLYKOV.count("запись «Ярлык» с «имя» равным") - len(yarlyki)
...
if rezhim != "--check":
    ...
    print("\nСверка с ведомостью — тем же прибором:"
          "\n  sh scripts/guards/who-calls-the-guards.sh --check")
    sys.exit(0)
...
sys.exit(1)
PY
# ярлык «зов:проверка» sh --check — проверки, которых CI не зовёт, сходятся с описью — в обе стороны
