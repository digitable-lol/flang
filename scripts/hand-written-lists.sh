...
# Двоичный, которым дерево судят, собран из СЕМЕНИ, и своих целей у него восемь:
# девятая (cpp) в семя ещё не перепечатана. Перечень, называющий восемь, не лжёт
# — он описывает двоичный. Лжёт тот, который не сходится НИ С ОДНИМ из двух.
# Поэтому перепись держит оба источника и говорит, с каким сошлось.
...
git -c core.quotepath=false ls-files \
  | LC_ALL=C.UTF-8 /usr/bin/grep -a -E '\.(flang|mjs|sh|js|py|awk|json|yml|yaml|md|tsv)$' > "$TMP/файлы"
...
[ -f "$LEDGER" ] || { printf 'ведомости %s нет — сверять не с чем\n' "$LEDGER" >&2; exit 1; }
