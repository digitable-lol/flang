...
// Какие файлы дерева САМИ становятся страницами сайта. Нужно, чтобы отличить
// ссылку внутри сайта от ссылки на сайт снаружи: первую проверяет сборка по
// адресам страниц, вторую не проверяет никто, кроме этого сторожа.
const ИСТОЧНИКИ_СТРАНИЦ = new Set();
for (const раздел of РАЗДЕЛЫ) {
  for (const страница of раздел.страницы) {
    if (!страница.из) continue;
    ИСТОЧНИКИ_СТРАНИЦ.add(страница.из);
    // Английская пара объявлена не строкой карты, а именем файла рядом.
    if (страница.из.endsWith('.ru.md')) {
      ИСТОЧНИКИ_СТРАНИЦ.add(страница.из.slice(0, -'.ru.md'.length) + '.md');
    }
  }
}
// Заметки базы знаний страницами становятся все, вместе с указателем.
const страницаСайта = (файл) =>
  ИСТОЧНИКИ_СТРАНИЦ.has(файл) || файл.startsWith(БАЗА_ЗНАНИЙ.каталог + '/');
...
for (const файл of файлы) {
  ...
  for (const м of безКода.matchAll(ССЫЛКА)) {
    ...
    // Ссылка на собранную страницу сайта. Пропускаем её по ИСТОЧНИКУ, а не по
    // виду цели: если ссылающийся файл сам становится страницей, то ссылку уже
    // проверил docs/site/build.mjs — по адресам страниц, а не по файлам дерева,
    // и `../spec.html` со страницы `en/` для него правильный адрес, а для нас
    // путь в никуда. Если же файл страницей не становится, его ссылку на .html
    // не проверяет никто, кроме нас, — и мы её проверяем.
    if (цель.endsWith('.html') && страницаСайта(файл)) continue;
