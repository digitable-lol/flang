...
# `scripts/predel-pamyati.sh`, и если места нет — ограничитель прогон НЕ
# начинает.
#
...
for fajl in "$katalog"/*.flang; do
  ...
  ./scripts/predel-pamyati.sh -- ./bootstrap/flang ast "$fajl" > "$rab/ast-$imya.json" 2>"$rab/ast-$imya.err"
  ./scripts/predel-pamyati.sh -- ./bootstrap/flang check "$fajl" --proof > "$rab/led-$imya.txt" 2>&1
done
