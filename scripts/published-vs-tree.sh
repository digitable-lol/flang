...
dolya_dokazannogo() {
  ...
  # ── ЗНАМЕНАТЕЛЬ РАЗЛОЖЕН: что в него входит и что из него ВЫНЕСЕНО ──────────
  #
  # «Не доказано» и «доказать нельзя» — разные вещи, и из знаменателя выносится
  # только второе. Иначе доля лжёт в другую сторону: потолок объявлен стом
  # процентами, которых достичь нельзя ни при какой работе (задача 1729).
  #
  # НИКОГДА — 92 строки, и причина у каждой по существу, а не «руки не дошли»:
  #   • «требует» (предусловие) — ДОПУЩЕНИЕ, а не утверждение о функции.
  #     Список приговоров строится фильтром `kind == "postcondition"`
  #     (flang/self/proof.flang), предусловие уезжает в ФАКТЫ доказательства
  #     постусловия, а наружу по нему идёт не вердикт, а счёт МЕСТ вызова
  #     ({sites, discharged}). Единица суда у него другая — вызов, а не строка:
  #     отказ зовётся FLANG_PRECONDITION_CALL и называет вызов. Строк столько
  #     же, мест — ноль, одно или сто.
  #   • «закон» в подделках flang/test/fixtures/binary-rules/** — файл написан
  #     затем, чтобы быть отвергнутым. У программы с замечаниями ведомости не
  #     бывает и не будет никогда.
  #
  # ОСТАЮТСЯ В ЗНАМЕНАТЕЛЕ «закон» честных программ: сегодня двоичный отвечает
  # по ним кодом 2 («судьи нет: правил не написано ни строкой»), но это долг
  # дерева, а не свойство обязательства. ⚠ Вшивание судьи их в числитель ещё не
  # приведёт: алфавит вердиктов у закона — "grid" и "violated", «доказано» у
  ...
  segodnya=$((zak_vse - zak_pod))
  znam_doli=$((znam - nikogda))
  printf '  знаменатель разложен:  %s = %s постусловий + %s требует + %s закон\n' \
         "$znam" "$((znam - treb - zak_vse))" "$treb" "$zak_vse"
  printf '    НИКОГДА в числитель:  %s (требует %s + закон подделок %s) — вынесены\n' \
         "$nikogda" "$treb" "$zak_pod"
  printf '    нельзя сегодняшним двоичным: %s (закон честных программ) — остаются\n' \
         "$segodnya"
  skazat "знаменатель доли" "$(znach 'знаменатель доли' "$DOLYA")" "$znam_doli"

  # Опись обязана знать те же 97: поле «без приговора» считается по исходнику,
  # двоичный для него не нужен, и потому оно заполнено и там, где ведомости нет.
  bez_opis=$(awk -F'|' '/^\/\// || NF < 3 {next} $6 ~ /^[0-9]+$/ {s += $6} END {print s + 0}' "$VEDOMOST")
  skazat "без приговора в описи" "$bez_opis" "$((treb + zak_vse))"
