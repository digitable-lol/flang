...
#    в 54 % сверх того.
#
# ЧТО ЭТО СТОИТ, названо прямо: программа, которая НЕ ЗАВЕРШАЕТСЯ, теперь
# считается в пять раз дольше, прежде чем её остановят. Предел ловит зацикливание,
# и поднимать его вслепую значит ослаблять эту ловлю — поэтому он поднят по
# замеру и ровно один раз, а не «с запасом на всякий случай».
#
