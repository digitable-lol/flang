...
#
# Поднять версию одной командой: число пишется в источник (функция «Версия» в
# scripts/release/emit-package.flang) и разносится в .flangrc (все пять ключей),
# flang_repl.c (#define FLANG_VERSION), packaging/flang.1, формулу Homebrew и её
# копию в кране; плагин asdf сверяется. sha256 формулы, changelog.json и семя не
# трогает — их поднимает выпуск и перепечатка.
#
# Как звать:
#   ./ярлык версия 0.7.13     поставить 0.7.13 всюду
#   ./ярлык версия            показать текущую версию и что из неё вытекает
#
# Коды: 0 — поднято (или показано); 2 — число не вида X.Y.Z; 3 — нет корня.
# см. docs/zettel/the-version-is-written-once-and-spread-by-one-command-but-sha256-is-not-derived-from-it.md
set -u
...
echo
echo "готово. Проверить сведение: sh scripts/guards/version-derivations-guard.sh"
