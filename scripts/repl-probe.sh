...
#
#   sh scripts/repl-probe.sh                 судить bootstrap/flang
#   sh scripts/repl-probe.sh <двоичный>      судить названный
#   sh scripts/repl-probe.sh --отпечаток     напечатать верный FLANG_SHELL_ID
#
# Коды возврата:
...
RAB=${FLANG_TMP:-/srv/tmp}/repl-probe.$$
mkdir -p "$RAB" || exit 2
...
if [ "$ID" != "$V_FILE" ]; then
  ...
  echo "  sh scripts/repl-probe.sh --отпечаток"
  BEDA=1
fi
