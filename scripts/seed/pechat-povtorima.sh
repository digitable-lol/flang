...
#
# Как звать:
#   sh scripts/seed/pechat-povtorima.sh                  сверка (вход flang/self/lexer.flang)
#   sh scripts/seed/pechat-povtorima.sh --вход <файл>    сверка на другом входе
#   sh scripts/seed/pechat-povtorima.sh --подлог         проба самого прибора
...
while [ $# -gt 0 ]; do
  case $1 in
    ...
    *) beda "непонятный ключ: $1"
       beda "звать: sh scripts/seed/pechat-povtorima.sh [--вход <файл>|--подлог]"
       exit 2 ;;
  esac
  shift
done
