...
#      Улика и цена его пропуска — в `scripts/proba-variantov.flang` рядом.
#
...
# напечатал сам flang; `scripts/imena-v-c.awk` только читает готовое, второй
...
#
# ── Прогон ──────────────────────────────────────────────────────────────────
#   scripts/stolknoveniya-celey.sh                           всё
...
IMENA="$ROOT/scripts/imena-v-c.awk"
TAB=$(printf '\t')
...
# `cc`, и именно её идентификаторы займёт новый модуль. Свежесть точки стережёт
