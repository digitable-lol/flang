...
# Судит flang: связывание (flang ast), печать в C (flang emit) и проверка типов;
# оболочка раскладывает ответы по трём стопкам и печатает карту переименований.
#
# Как звать:
#   scripts/targets/target-collisions.sh                           всё
#   scripts/targets/target-collisions.sh --без-пар                 без 21 пары
#   FLANG=/путь/к/двоичному scripts/targets/target-collisions.sh   судит двоичный
#   KARTA=<файл.tsv> …                                             записать карту
#
# Коды: 0 — замер снят; 3 — мерить нечем (двоичного нет) или печать отказала.
# см. docs/zettel/name-collisions-have-three-judges-and-the-third-is-silent-until-you-rename.md
set -u
...
IMENA="$ROOT/scripts/targets/names-in-c.awk"
TAB=$(printf '\t')
