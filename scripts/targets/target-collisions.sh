...
#      Улика и цена его пропуска — в `scripts/targets/variant-probe.flang` рядом.
#
...
# напечатал сам flang; `scripts/targets/names-in-c.awk` только читает готовое, второй
...
#   scripts/targets/target-collisions.sh: line 132: …/bootstrap/flang: No such file or directory
#
# Читается это как «печать сломалась, чините печать», хотя чинить надо сборку.
# Задача 1312; образец отказа — scripts/targets/identical-declarations.sh.
if [ ! -x "$FL" ]; then
