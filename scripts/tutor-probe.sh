...
#
#   sh scripts/tutor-probe.sh              судить bootstrap/flang
#   sh scripts/tutor-probe.sh <двоичный>   судить названный
#
...
#              (раздел «впиши» — что набирается в пропуски), — проводник
#              обязан дойти до конца и ответить нулём. Подсказка и проверка
#              берутся из одного места, разойтись им негде.
...
#   СЕАНС.     Ввод владельца от 11 сентября 2026 слово в слово: имя модуля
#              без кавычек, вызов «Удвоить» от 21 внутри «Удвоить», вопрос
#              словами. На каждую строку — своя реакция, и урок проходится.
#   СЛОВА.     Список слов языка в проводнике совпадает с таблицей лексера
#              `flang/self/lexer.flang` — иначе подсветка врёт.
...
RAB=${FLANG_TMP:-/srv/tmp}/tutor-probe.$$
mkdir -p "$RAB" || exit 2
...
otvet() { # номер урока → что набирается в пропуски
  f=$UROKI/$(printf '%s\n' "$SPISOK" | sed -n "$1p")
  awk '$0=="=впиши=" {v=1;next} /^=[^=]*=$/ {v=0} v {print}' "$f"
}
...
# ── сеанс владельца: три ошибки, три реакции, урок пройден ──────────────────
printf '%s\n' 'Урок' 'х умножить на 2' '"Удвоить" от 21' 'и непонятно как это работает' '' > "$RAB/seans"
vesti "$RAB/seans" "$RAB/seans.out" --только 1
KOD=$?
if [ "$KOD" != 0 ] || ! grep -q "урок 1 пройден" "$RAB/seans.out"; then
  echo "сеанс: ввод владельца дал код $KOD, а урок 1 не пройден"; tail -8 "$RAB/seans.out"; BEDA=1
elif ! grep -q "принял как «Урок»" "$RAB/seans.out"; then
  echo "сеанс: имя модуля без кавычек, а проводник не сказал, что взял его в кавычки"; BEDA=1
elif ! grep -q "вызов самой «Удвоить» внутри её тела" "$RAB/seans.out"; then
  echo "сеанс: вызов «Удвоить» от 21 в теле «Удвоить», а объяснения, где ему место, нет"; BEDA=1
elif ! grep -q "ни одного слова языка" "$RAB/seans.out"; then
  echo "сеанс: вопрос словами, а проводник не сказал, что это не строка языка"; BEDA=1
elif ! grep -q "понял: слова языка: умножить на; числа: 2" "$RAB/seans.out"; then
  echo "сеанс: на верную строку тела нет реакции «понял: …»"; BEDA=1
else
  echo "  зелен  сеанс: ввод владельца — на каждую строку своя реакция, урок пройден"
fi
...
if b'\x1b[1;34m' not in cvet:
    print('цвет: под терминалом слова языка не подсвечены (нет SGR 1;34)'); beda = 1
if b'\x1b[36m' not in cvet:
    print('цвет: под терминалом имена в ёлочках не подсвечены (нет SGR 36)'); beda = 1
if b'\x1b[' in golo:
    print('цвет: под терминалом с NO_COLOR последовательности остались'); beda = 1
if beda == 0:
    print('  зелен  цвет: под терминалом красит слова, имена, числа и строки; NO_COLOR снимает цвет')
sys.exit(beda)
PY
  [ $? = 0 ] || BEDA=1
else
