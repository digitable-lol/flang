...
- `a62e0215` — все четыре: README и справка двоичного говорят «thirteen commands»/«все 13 команд»; `docs/site/cli.md` описывает `--timeout N` как существующий ключ (строки 382, 433–436); `flang/test/jargon-guard.test.mjs` больше не читает `flang/src/parser.mjs`; SPDX дописан в `examples/host-boundary/host.c` и `flang/conc/scheduler.js`;
- `fb29de4d` — перечень команд сверяется сторожем по пяти местам.

Проверено 8 сентября 2026: `bootstrap/flang --help` — «Здесь все 13 команд»; `license-guard.flang` — код 0; все четыре файла из пункта 4 несут `SPDX-License-Identifier`. `npm test` целиком не гонялся.
