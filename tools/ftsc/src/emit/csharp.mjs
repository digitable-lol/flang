...
export function emit(program, options = {}) {
  ...
  /* Последний шаг печати — снять сырые двунаправленные управляющие со всего
     ...
     и в комментарий («правило «…»»), и в строковый литерал (Console.WriteLine
     в прогонщике примеров, имя свойства в исключении), а комментарий читают первым
     и проверить исполнением не могут. Как ведёт себя Roslyn, здесь не проверено:
     тулчейна .NET на машине нет, — и его молчание всё равно не было бы
     доказательством. Форма C# — `\uXXXX`: ровно четыре цифры, и цифра, стоящая в
     строке следом, к экранированию не приклеится. */
