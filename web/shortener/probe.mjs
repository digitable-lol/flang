...
/*
 * ВЕС ВКЛАДКИ — замер, а не оценка, и мерится он тем же способом, каким мерился
 * до печати плана: каждый ответ, который вкладка получила, с его размером.
 * Число это было главной уликой всей работы: страница везла 19 ответов и
 * 1 752 748 байт, из них 16 файлов и 1 692 153 байта — вторая реализация языка,
 * приезжавшая за тремя именами, которые ввозил хозяин.
 *
 * Считаются ВСЕ ответы, включая ответы службы: вкладка везёт и их тоже. Файлы
 * второй реализации считаются отдельно — по каталогу `/flang/src/`, минус
 * хозяин вкладки, у которого ввозов ноль.
 */
const ответы = []
page.on("response", async (ответ) => {
  const адрес = new URL(ответ.url()).pathname
  let байт = 0
  try {
    байт = (await ответ.body()).length
  } catch {
    байт = 0
  }
  ответы.push({ адрес, байт })
})
const вычислитель = (адрес) =>
  адрес.startsWith("/flang/") && !адрес.endsWith("/flang_host_browser.js")
...
try {
  ...
  const всего = ответы.reduce((сумма, о) => сумма + о.байт, 0)
  const вычислителя = ответы.filter((о) => вычислитель(о.адрес))
  console.log(`\nвкладка везёт: ${ответы.length} ответов, ${всего} байт`)
  console.log(
    `из них вторая реализация языка: ${вычислителя.length} файлов, ` +
      `${вычислителя.reduce((сумма, о) => сумма + о.байт, 0)} байт`,
  )
  for (const о of ответы) console.log(`   ${String(о.байт).padStart(8)}  ${о.адрес}`)
