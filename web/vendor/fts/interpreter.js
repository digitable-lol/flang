...
export function prove(input, context) {
...
function evaluate(document, proposition, context) {
        ...
        case "witness": {
            ...
            if (context !== undefined && proposition.value !== undefined && path.length > 0) {
                ...
                if (!resolved.found || !deepEqual(resolved.value, proposition.value)) {
                    throw diagnosticError("FTS_WITNESS_MISMATCH", `witness does not match context at ${formatPath(path)}: expected ${JSON.stringify(proposition.value)}, got ${resolved.found ? JSON.stringify(resolved.value) : "<missing>"}`, { path: "$.proposition" });
                }
            }
            return {
                path,
...
function bundle(document, proposition) {
    ...
    for (const segment of path) {
        if (typeof segment === "string") {
            ...
            if (!Array.isArray(current))
                return { found: false };
            const matches = current.filter((item) => isRecord(item) && Object.entries(segment).every(([key, value]) => deepEqual(item[key], value)));
            if (matches.length !== 1)
                return { found: false };
            current = matches[0];
        }
