...
export function looksLikeNaturalSurface(source) {
    const first = source
        .replace(/^\uFEFF/u, "")
        .split(/\r?\n/u)
...
export function parseNaturalSurface(source) {
    ...
    while (index < lines.length) {
        ...
        const body = [];
        index += 1;
        while (index < lines.length && lines[index].indent > header.indent)
 ...
 */
function normalizeEnglishNaturalSurface(source) {
    const first = source
        .replace(/^\uFEFF/u, "")
        .split(/\r?\n/u)
        .map((line) => stripLineComment(line).trim())
        .find((line) => line.length > 0);
    if (!first || !/^category(?:\s|$)/u.test(first) || first.includes("{"))
        return source;
...
function translateEnglishLine(line) {
    const declarations = [
    ...
    if (returns?.[1] !== undefined)
        return `возвращает ${returnTypes[returns[1]] ?? returns[1]}`;
    ...
    if (stateField?.[1] && stateField[2])
        return `${stateField[1]} является состоянием ${stateField[2]}`;
...
function parseUtility(header, body) {
    const name = readWholeName(header.text.slice("утилита".length), header);
    if (body.length === 0)
        throw naturalError("FTS_NATURAL_UTILITY", `утилита «${name}» не содержит определения`, header);
    ...
    while (index < body.length) {
        ...
        const nested = [];
        index += 1;
        while (index < body.length && body[index].indent > directIndent)
            nested.push(body[index++]);
        if (line.text.startsWith("правило "))
            rules.push(parseUtilityRule(line, nested));
        else if (line.text.startsWith("свойство "))
            properties.push(parseUtilityProperty(line, nested));
        else if (line.text.startsWith("пример "))
            examples.push(parseUtilityExample(line, nested));
        else
            throw naturalError("FTS_NATURAL_UTILITY", "ожидались принимает, возвращает, начинает с, правило, свойство или пример", line);
    }
    if (!input)
        throw naturalError("FTS_UTILITY_INPUT", `утилита «${name}» требует строку 'принимает'`, header);
    if (!output)
        throw naturalError("FTS_UTILITY_OUTPUT", `утилита «${name}» требует строку 'возвращает'`, header);
    if (initial === undefined)
        throw naturalError("FTS_UTILITY_INITIAL", `утилита «${name}» требует строку 'начинает с'`, header);
    return { name, input, output, initial, rules, properties, examples };
}
function parseUtilityRule(header, body) {
    const name = readWholeName(header.text.slice("правило".length), header);
    const when = [];
    let action;
    for (const line of body) {
        if (line.text.startsWith("если "))
            when.push(parseUtilityCondition(line.text.slice("если".length), line));
        else if (line.text.startsWith("и "))
            when.push(parseUtilityCondition(line.text.slice(1), line));
            ...
            if (comparison.operator !== "eq")
                throw naturalError("FTS_UTILITY_ACTION", "результат правила задаётся через 'равен'", line);
    ...
    if (when.length === 0)
        throw naturalError("FTS_UTILITY_RULE", `правило «${name}» требует условие 'если'`, header);
    if (!action)
        throw naturalError("FTS_UTILITY_RULE", `правило «${name}» требует действие 'то'`, header);
    return { name, when, action };
}
function parseUtilityCondition(text, line) {
...
function parseUtilityProperty(header, body) {
...
function parseUtilityExample(header, body) {
    ...
    if (expected === undefined)
        throw naturalError("FTS_UTILITY_EXAMPLE", `пример «${name}» требует строку 'ожидается'`, header);
    return { name, input, expected };
}
function parseUtilityComparison(text, line) {
    const comparisons = [
        ["не меньше ", "gte"],
        ["не больше ", "lte"],
        ["не равен ", "neq"],
...
function parseUtilityOperand(text, line) {
    const source = text.trim();
    if (source === "результат")
        return { kind: "result" };
    ...
    if (source.startsWith("поле "))
        return { kind: "field", field: readWholeName(source.slice("поле".length), line) };
    return { kind: "value", value: parseScalar(source, line) };
}
function normalizeNaturalType(text, line) {
    const source = text.trim();
    return returnedTypes[source] ?? builtinTypes[source] ?? readWholeName(source, line);
}
function parseObject(header, body) {
    ...
    if (body.length === 0)
        throw naturalError("FTS_NATURAL_OBJECT", `объект «${name}» не содержит полей`, header);
    const fields = body.map((line) => parseField(line));
    return { name, fields };
}
...
function parseMorphism(header, body) {
    ...
    for (const line of body) {
        if (line.text.startsWith("если "))
            domain = readWholeName(line.text.slice("если".length), line);
        else if (line.text.startsWith("то "))
            codomain = readWholeName(line.text.slice("то".length), line);
        else if (line.text.startsWith("из "))
            domain = readWholeName(line.text.slice(2), line);
        else if (line.text.startsWith("в "))
            codomain = readWholeName(line.text.slice(1), line);
        else if (line.text.startsWith("по закону "))
            law = readWholeName(line.text.slice("по закону".length), line);
        else
            throw naturalError("FTS_NATURAL_MORPHISM", "в морфизме ожидаются строки 'если', 'то' или 'по закону'", line);
    }
    if (!domain || !codomain)
        throw naturalError("FTS_NATURAL_MORPHISM", `морфизм «${name}» требует строки 'если' и 'то'`, header);
    return { name, domain, codomain, law };
}
function parseTheorem(header, body) {
    const theorem = {
    ...
    for (const line of body) {
        if (line.text.startsWith("дано "))
            parseGiven(line.text.slice("дано".length), line, theorem);
        else if (line.text.startsWith("в данных "))
            parseDataLookup(line.text.slice("в данных".length), line, theorem);
...
function parseGiven(text, line, theorem) {
...
function parseDataLookup(text, line, theorem) {
...
function buildProposition(theorem, structures, morphisms) {
    ...
    if (!field)
        throw naturalError("FTS_UNKNOWN_FIELD", `не найдено поле «${theorem.structure}».«${theorem.field}»`);
    const witness = {
        kind: "witness",
    ...
    for (const name of theorem.morphisms) {
        ...
        if (!morphism)
            throw naturalError("FTS_UNKNOWN_FUNCTOR", `не найден морфизм «${name}»`);
        if (morphism.domain !== resultingType) {
            throw naturalError("FTS_PROOF_TYPE_MISMATCH", `морфизм «${name}» ожидает «${morphism.domain}», получено «${resultingType}»`);
        }
        resultingType = morphism.codomain;
    }
    if (theorem.conclusion && theorem.conclusion !== resultingType) {
        throw naturalError("FTS_THEOREM_CONCLUSION", `теорема объявляет «${theorem.conclusion}», но вывод имеет тип «${resultingType}»`);
    }
    if (theorem.morphisms.length === 0)
        return witness;
...
function readWholeName(text, line) {
    const parsed = readName(text.trim(), line);
    if (parsed.rest.length > 0)
        throw naturalError("FTS_NATURAL_NAME", `лишний текст после имени: '${parsed.rest}'`, line);
    return parsed.value;
}
function readName(text, line) {
    const source = text.trim();
    if (source.startsWith("«")) {
        const end = source.indexOf("»", 1);
        if (end < 0)
            throw naturalError("FTS_UNCLOSED_STRING", "не закрыта кавычка-ёлочка", line);
    ...
    if (source.startsWith('"') || source.startsWith("'")) {
        ...
        for (let index = 1; index < source.length; index += 1) {
            ...
            if (character === quote)
                return { value: value.normalize("NFC"), rest: source.slice(index + 1).trim() };
