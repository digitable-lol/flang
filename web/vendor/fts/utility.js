...
export function executeUtility(document, utilityName, input) {
    const utility = document.utilities?.find((item) => item.name === utilityName);
    if (!utility)
        throw diagnosticError("FTS_UNKNOWN_UTILITY", `не найдена утилита «${utilityName}»`);
    const structure = document.structures.find((item) => item.name === utility.input);
    if (!structure)
        throw diagnosticError("FTS_UTILITY_INPUT", `не найдена входная структура «${utility.input}»`);
    for (const field of structure.fields) {
        const optional = field.type.includes("undefined");
        if (!(field.name in input)) {
            if (!optional)
                throw diagnosticError("FTS_UTILITY_INPUT", `во входных данных отсутствует поле «${field.name}»`);
            continue;
        }
...
export function evaluateUtility(utility, input) {
...
export function testUtilities(document) {
    if ((document.utilities ?? []).length === 0)
        throw diagnosticError("FTS_NO_UTILITIES", "документ не содержит утилит");
    const results = [];
    ...
    if (results.length === 0)
        throw diagnosticError("FTS_NO_UTILITY_EXAMPLES", "утилиты не содержат примеров");
    const passed = results.filter((result) => result.passed).length;
    return { valid: passed === results.length, total: results.length, passed, failed: results.length - passed, results };
}
export function generateTypeScript(document) {
...
function resolveField(input, field) {
    if (!(field in input))
        throw diagnosticError("FTS_UTILITY_INPUT", `во входных данных отсутствует поле «${field}»`);
    return input[field];
}
...
function renderImplementation(utilities, structures) {
    const lines = [
...
function renderTests(utilities) {
    const lines = [
        "// Generated by FTS. Do not edit by hand.",
...
function renderCondition(field, operator, operand) {
