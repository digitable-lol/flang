...
export function validate(input) {
    ...
    const diagnostics = [];
    const report = (code, message, path) => {
    ...
    document.structures.forEach((structure, structureIndex) => {
        ...
        if (structures.has(structure.name))
            report("FTS_DUPLICATE_STRUCTURE", `duplicate structure '${structure.name}'`, `${path}.name`);
        ...
        structure.fields.forEach((field, fieldIndex) => {
            ...
            if (fields.has(field.name))
                report("FTS_DUPLICATE_FIELD", `duplicate field '${structure.name}.${field.name}'`, `${fieldPath}.name`);
    ...
    document.functors.forEach((functor, functorIndex) => {
        ...
        if (functors.has(functor.name))
            report("FTS_DUPLICATE_FUNCTOR", `duplicate functor '${functor.name}'`, `${path}.name`);
...
function validateUtilities(document, structures, report) {
    const names = new Set();
    for (const [utilityIndex, utility] of (document.utilities ?? []).entries()) {
        ...
        if (names.has(utility.name))
            report("FTS_DUPLICATE_UTILITY", `duplicate utility '${utility.name}'`, `${path}.name`);
        ...
        if (!isName(utility.output))
            report("FTS_UTILITY_OUTPUT", `utility '${utility.name}' requires an output type`, `${path}.output`);
        ...
        for (const [ruleIndex, rule] of (utility.rules ?? []).entries()) {
            ...
            if (!isName(rule.name))
                report("FTS_UTILITY_RULE_NAME", "utility rule requires a name", `${rulePath}.name`);
            if (!Array.isArray(rule.when) || rule.when.length === 0) {
                ...
                rule.when.forEach((condition, conditionIndex) => {
                    ...
                    if (!isComparison(condition.operator))
                        report("FTS_UTILITY_COMPARISON", `unknown comparison '${String(condition.operator)}'`, `${conditionPath}.operator`);
                    if (!fieldType)
                        report("FTS_UTILITY_FIELD", `unknown utility input field '${condition.field}'`, `${conditionPath}.field`);
        ...
        for (const [propertyIndex, property] of (utility.properties ?? []).entries()) {
            ...
            if (!isName(property.name))
                report("FTS_UTILITY_PROPERTY_NAME", "utility property requires a name", `${propertyPath}.name`);
            if (!isComparison(property.operator))
                report("FTS_UTILITY_COMPARISON", `unknown comparison '${String(property.operator)}'`, `${propertyPath}.operator`);
        ...
        for (const [exampleIndex, example] of (utility.examples ?? []).entries()) {
            ...
            if (!isName(example.name))
                report("FTS_UTILITY_EXAMPLE_NAME", "utility example requires a name", `${examplePath}.name`);
            for (const [fieldName, value] of Object.entries(example.input ?? {})) {
                const fieldType = fields.get(fieldName);
                if (!fieldType)
                    report("FTS_UTILITY_FIELD", `unknown example field '${fieldName}'`, `${examplePath}.input.${fieldName}`);
...
function validateOperand(operand, fields, path, report) {
    ...
    if (operand.kind === "field" || operand.kind === "percent") {
        ...
        if (!type)
            report("FTS_UTILITY_FIELD", `unknown utility input field '${operand.field}'`, `${path}.field`);
        ...
        if (!isScalar(operand.value))
            report("FTS_UTILITY_VALUE", "utility literal must be scalar", `${path}.value`);
    }
    else if (operand.kind !== "result") {
        report("FTS_UTILITY_OPERAND", `unknown utility operand '${String(operand.kind)}'`, `${path}.kind`);
    }
}
function requiredFields(structure) {
...
function typeKind(type) {
    ...
    if (normalized === "Число" || normalized === "Деньги" || normalized === "number")
        return "number";
    if (normalized === "Строка" || normalized === "Дата" || normalized === "string")
        return "string";
    if (normalized === "Признак" || normalized === "boolean")
        return "boolean";
    return "unknown";
}
function operandKind(operand, fields, output) {
    if (!operand || typeof operand !== "object")
        return "unknown";
    if (operand.kind === "percent")
        return "number";
    if (operand.kind === "field")
        return typeKind(fields.get(operand.field) ?? "");
    if (operand.kind === "result")
        return typeKind(output);
...
export function assertValid(input) {
    ...
    if (!result.valid)
        throw new FtsError("invalid FTS document", result.diagnostics);
    return result.document;
}
function validateProposition(proposition, path, structures, functors, document, report) {
            ...
            if (!Array.isArray(proposition.functors) || proposition.functors.length === 0) {
                ...
                proposition.functors.forEach((functor, index) => {
                    if (!functors.has(functor))
                        report("FTS_UNKNOWN_FUNCTOR", `unknown functor '${functor}'`, `${path}.functors[${index}]`);
                });
            }
            validateProposition(proposition.arg, `${path}.arg`, structures, functors, document, report);
            break;
        default:
            report("FTS_PROPOSITION_KIND", `unknown proposition kind '${String(proposition.kind)}'`, `${path}.kind`);
    }
}
