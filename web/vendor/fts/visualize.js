export function visualize(document, proof = null, mode = "all") {
    ...
    const available = [mermaid_category, mermaid_functors, mermaid_proof].filter((item) => Boolean(item));
    const selected = mode === "category"
        ? mermaid_category
...
export function mermaidCategory(document) {
...
export function mermaidFunctors(document) {
    if (document.functors.length === 0)
        return "";
...
function nodeId(name) {
    const encoded = Array.from(name, (character) => /[A-Za-z0-9_]/.test(character) ? character : `u${character.codePointAt(0).toString(16)}_`).join("");
    return `n_${encoded}`;
}
